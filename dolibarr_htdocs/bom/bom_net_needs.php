...
if ($object->id > 0 && (empty($action) || ($action != 'edit' && $action != 'create'))) {
    ...
    $text_stock_options .= (Functions::getDolGlobalString('STOCK_CALCULATE_ON_SHIPMENT') || Functions::getDolGlobalString('STOCK_CALCULATE_ON_SHIPMENT_CLOSE') ? '- ' . $langs->trans("DeStockOnShipment") . '<br>' : '');
    $text_stock_options .= (Functions::getDolGlobalString('STOCK_CALCULATE_ON_VALIDATE_ORDER') ? '- ' . $langs->trans("DeStockOnValidateOrder") . '<br>' : '');
    $text_stock_options .= (Functions::getDolGlobalString('STOCK_CALCULATE_ON_BILL') ? '- ' . $langs->trans("DeStockOnBill") . '<br>' : '');
    $text_stock_options .= (Functions::getDolGlobalString('STOCK_CALCULATE_ON_SUPPLIER_BILL') ? '- ' . $langs->trans("ReStockOnBill") . '<br>' : '');
    $text_stock_options .= (Functions::getDolGlobalString('STOCK_CALCULATE_ON_SUPPLIER_VALIDATE_ORDER') ? '- ' . $langs->trans("ReStockOnValidateOrder") . '<br>' : '');
    $text_stock_options .= (Functions::getDolGlobalString('STOCK_CALCULATE_ON_SUPPLIER_DISPATCH_ORDER') ? '- ' . $langs->trans("ReStockOnDispatchOrder") . '<br>' : '');
    $text_stock_options .= (Functions::getDolGlobalString('STOCK_CALCULATE_ON_RECEPTION') || Functions::getDolGlobalString('STOCK_CALCULATE_ON_RECEPTION_CLOSE') ? '- ' . $langs->trans("StockOnReception") . '<br>' : '');

    print '<table id="tablelines" class="noborder noshadow" width="100%">';
    print "<thead>\n";
    print '<tr class="liste_titre nodrag nodrop">';
    print '<td class="linecoldescription">' . $langs->trans('Product');
    if (Functions::getDolGlobalString('BOM_SUB_BOM')  && $action == 'treeview') {
