...
$langs->loadLangs(['bills', 'deliveries', 'orders', 'sendings']);
    ...
    if ($object->id > 0) {
        ...
        if ($object->id > 0) {
            ...
            */
            ...
            while ($i < $num_prod) {
                $parameters = ['i' => $i, 'line' => $object->lines[$i], 'num' => $num_prod];
                ...
                if (empty($reshook)) {
                    ...
                    print $object->lines[$i]->showOptionals($extrafields, $mode, ['style' => 'class="oddeven"', 'colspan' => $colspan], '');
                    //}
                }
