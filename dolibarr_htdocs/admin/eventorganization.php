    ...
    foreach ($arrayofparameters as $constname => $val) {
        if ($val['enabled'] == 1) {
            ...
            if ($val['type'] == 'textarea') {
                ...
                print $formcompany->selectProspectCustomerType(Functions::getDolGlobalString($constname), $constname, 'customerorprospect', 'form', '', 1);
            } elseif ($val['type'] == 'securekey') {
                print '<input type="text" class="flat" id="' . $constname . '" name="' . $constname . '" value="' . (GETPOST($constname, 'alpha') ? GETPOST($constname, 'alpha') : Functions::getDolGlobalString($constname)) . '" size="40">';
                ...
                if (isModEnabled("product") || isModEnabled("service")) {
                    $selected = Functions::getDolGlobalString($constname);
        ...
        foreach ($arrayofparameters as $constname => $val) {
            if ($val['enabled'] == 1) {
                ...
                if ($val['type'] == 'textarea') {
                    ...
                    if (Functions::getDolGlobalString($constname)) {
                        include_once DOL_DOCUMENT_ROOT . '/core/class/html.formmail.class.php';
                        $formmail = new FormMail($db);

                        $tmp = explode(':', $val['type']);
                        $labelemailtemplate = Functions::getDolGlobalString($constname);
                    ...
                    if (Functions::getDolGlobalString($constname)) {
                        $c = new Categorie($db);
                        $result = $c->fetch(Functions::getDolGlobalString($constname));
                        if ($result < 0) {
                    ...
                    } elseif (Functions::getDolGlobalString($constname) == 1) {
                        print $langs->trans("Customer");
                    } elseif (Functions::getDolGlobalString($constname) == 0) {
                        print $langs->trans("NorProspectNorCustomer");
                    }
                } elseif ($val['type'] == 'product') {
                    $product = new Product($db);
                    $idproduct = Functions::getDolGlobalString($constname);
                    if ($idproduct > 0) {
