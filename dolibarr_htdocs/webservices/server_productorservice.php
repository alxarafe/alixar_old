...
$productorservice_fields = [
    ...
    'status_tobuy' => ['name' => 'status_tobuy', 'type' => 'xsd:string'],
    'status_tosell' => ['name' => 'status_tosell', 'type' => 'xsd:string'],
    'barcode' => ['name' => 'barcode', 'type' => 'xsd:string'],
    'barcode_type' => ['name' => 'barcode_type', 'type' => 'xsd:string'],
    'country_id' => ['name' => 'country_id', 'type' => 'xsd:string'],
    'country_code' => ['name' => 'country_code', 'type' => 'xsd:string'],
    'customcode' => ['name' => 'customcode', 'type' => 'xsd:string'],

    'price_net' => ['name' => 'price_net', 'type' => 'xsd:string'],
    'price' => ['name' => 'price', 'type' => 'xsd:string'],
    'price_min_net' => ['name' => 'price_min_net', 'type' => 'xsd:string'],
    'price_min' => ['name' => 'price_min', 'type' => 'xsd:string'],

    'price_base_type' => ['name' => 'price_base_type', 'type' => 'xsd:string'],

    'vat_rate' => ['name' => 'vat_rate', 'type' => 'xsd:string'],
    'vat_npr' => ['name' => 'vat_npr', 'type' => 'xsd:string'],
    'localtax1_tx' => ['name' => 'localtax1_tx', 'type' => 'xsd:string'],
    'localtax2_tx' => ['name' => 'localtax2_tx', 'type' => 'xsd:string'],

    'stock_alert' => ['name' => 'stock_alert', 'type' => 'xsd:string'],
    'stock_real' => ['name' => 'stock_real', 'type' => 'xsd:string'],
    'stock_pmp' => ['name' => 'stock_pmp', 'type' => 'xsd:string'],
    'warehouse_ref' => ['name' => 'warehouse_ref', 'type' => 'xsd:string'], // Used only for create or update to set which warehouse to use for stock correction if stock_real differs from database

    'canvas' => ['name' => 'canvas', 'type' => 'xsd:string'],
    'import_key' => ['name' => 'import_key', 'type' => 'xsd:string'],

    'dir' => ['name' => 'dir', 'type' => 'xsd:string'],
    'images' => ['name' => 'images', 'type' => 'tns:ImagesArray'],
];
...
$server->register(
    'getProductOrService',
    // Entry values
    ['authentication' => 'tns:authentication', 'id' => 'xsd:string', 'ref' => 'xsd:string', 'ref_ext' => 'xsd:string', 'lang' => 'xsd:string'],
    // Exit values
    ['result' => 'tns:result', 'product' => 'tns:product'],
    $ns,
...
$server->register(
    ...
    // Entry values
    ['authentication' => 'tns:authentication', 'product' => 'tns:product'],
    // Exit values
...
$server->register(
    ...
    // Entry values
    ['authentication' => 'tns:authentication', 'listofid' => 'xsd:string'],
    // Exit values
    ['result' => 'tns:result', 'nbdeleted' => 'xsd:int'],
    $ns,
...
$server->register(
    ...
    // Entry values
    ['authentication' => 'tns:authentication', 'filterproduct' => 'tns:filterproduct'],
    // Exit values
    ['result' => 'tns:result', 'products' => 'tns:ProductsArray2'],
    $ns,
...
$server->register(
    ...
    // Entry values
    ['authentication' => 'tns:authentication', 'id' => 'xsd:string', 'lang' => 'xsd:string'],
    // Exit values
    ['result' => 'tns:result', 'products' => 'tns:ProductsArray2'],
    $ns,
    ...
    if (!$error) {
        ...
        if ($fuser->hasRight('produit', 'lire') || $fuser->hasRight('service', 'lire')) {
            ...
            if ($result > 0) {
                ...
                if (isset($extrafields->attributes[$elementtype]['label']) && is_array($extrafields->attributes[$elementtype]['label']) && count($extrafields->attributes[$elementtype]['label'])) {
                    foreach ($extrafields->attributes[$elementtype]['label'] as $key => $label) {
                        $productorservice_result_fields = array_merge($productorservice_result_fields, ['options_' . $key => $product->array_options['options_' . $key]]);
                    }
                }
...
function deleteProductOrService($authentication, $listofidstring)
{
    ...
    if (!$error) {
        ...
        if (!$error) {
            ...
            $objectresp = ['result' => ['result_code' => 'OK', 'result_label' => ''], 'nbdeleted' => count($listofiddeleted)];
        } else {
            $db->rollback();
            $error++;
            $errorcode = 'KO';
    ...
    if ($error) {
        ...
        $objectresp = ['result' => ['result_code' => $errorcode, 'result_label' => $errorlabel], 'nbdeleted' => 0];
        ...
        $objectresp = ['result' => ['result_code' => 'NOT_FOUND', 'result_label' => 'No product or service with id ' . implode(',', $listofid) . ' found'], 'nbdeleted' => 0];
    }
...
function getListOfProductsOrServices($authentication, $filterproduct)
{
    ...
    if (!$error) {
        ...
        if ($resql) {
            ...
            while ($i < $num) {
                ...
                $arrayproducts[] = ['id' => $obj->rowid, 'ref' => $obj->ref, 'ref_ext' => $obj->ref_ext];
                $i++;
            }
        } else {
            $error++;
    ...
    if ($error) {
        $objectresp = [
            'result' => ['result_code' => $errorcode, 'result_label' => $errorlabel],
                ...
                if ($res) {
                    ...
                    while ($rec = $db->fetch_array($res)) {
                        ...
                        if ($tmpproduct->status > 0) {
                            ...
                            if (isset($extrafields->attributes[$elementtype]['label']) && is_array($extrafields->attributes[$elementtype]['label']) && count($extrafields->attributes[$elementtype]['label'])) {
                                foreach ($extrafields->attributes[$elementtype]['label'] as $key => $label) {
                                    $products[$iProduct] = array_merge($products[$iProduct], ['options_' . $key => $tmpproduct->array_options['options_' . $key]]);
                                }
                            }
