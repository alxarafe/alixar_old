...
$langs->loadLangs(["bills", "compta", "loan"]);
...
$hookmanager->initHooks(['loancard', 'globalcard']);
...
if ($action == 'create') {
    ...
    $doleditor = new DolEditor('note_private', GETPOST('note_private', 'alpha'), '', 160, 'dolibarr_notes', 'In', false, true, !Functions::getDolGlobalString('FCKEDITOR_ENABLE_NOTE_PUBLIC') ? 0 : 1, ROWS_6, '90%');
    ...
    print '<td>';
    $doleditor = new DolEditor('note_public', GETPOST('note_public', 'alpha'), '', 160, 'dolibarr_notes', 'In', false, true, !Functions::getDolGlobalString('FCKEDITOR_ENABLE_NOTE_PRIVATE') ? 0 : 1, ROWS_6, '90%');
    print $doleditor->Create(1);
    print '</td></tr>';
    ...
    if (isModEnabled('accounting')) {
        ...
        print '<td>';
        print $formaccounting->select_account(GETPOST('accountancy_account_capital') ? GETPOST('accountancy_account_capital') : Functions::getDolGlobalString('LOAN_ACCOUNTING_ACCOUNT_CAPITAL'), 'accountancy_account_capital', 1, '', 1, 1);
        print '</td></tr>';

        // Accountancy_account_insurance
        print '<tr><td class="fieldrequired">' . $langs->trans("LoanAccountancyInsuranceCode") . '</td>';
        print '<td>';
        print $formaccounting->select_account(GETPOST('accountancy_account_insurance') ? GETPOST('accountancy_account_insurance') : Functions::getDolGlobalString('LOAN_ACCOUNTING_ACCOUNT_INSURANCE'), 'accountancy_account_insurance', 1, '', 1, 1);
        print '</td></tr>';

        // Accountancy_account_interest
        print '<tr><td class="fieldrequired">' . $langs->trans("LoanAccountancyInterestCode") . '</td>';
        print '<td>';
        print $formaccounting->select_account(GETPOST('accountancy_account_interest') ? GETPOST('accountancy_account_interest') : Functions::getDolGlobalString('LOAN_ACCOUNTING_ACCOUNT_INTEREST'), 'accountancy_account_interest', 1, '', 1, 1);
        print '</td></tr>';
    } else {
