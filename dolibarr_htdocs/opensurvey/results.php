...
if (GETPOST("ajoutercolonne") && GETPOST('nouvellecolonne') && $object->format == "A") {
    ...
    $nouveauxsujets .= ',';
    $nouveauxsujets .= str_replace([",", "@"], " ", GETPOST("nouvellecolonne")) . (!GETPOST("typecolonne") ? '' : '@' . GETPOST("typecolonne"));
...
foreach ($toutsujet as $value) {
    $tmp = explode('@', $value);
    $listofanswers[] = ['label' => $tmp[0], 'format' => (!empty($tmp[1]) ? $tmp[1] : 'checkbox')];
}
...
while ($compteur < $num) {
    ...
    if (!$testligneamodifier) {
        for ($i = 0; $i < $nbcolonnes; $i++) {
            ...
            if (empty($listofanswers[$i]['format']) || !in_array($listofanswers[$i]['format'], ['yesno', 'foragainst'])) {
                if (((string) $car) == "1") {
        ...
        if ($compteur == $ligneamodifier) {
            for ($i = 0; $i < $nbcolonnes; $i++) {
                ...
                if (!empty($listofanswers[$i]['format']) && $listofanswers[$i]['format'] == 'yesno') {
                    $arraychoice = ['2' => '&nbsp;', '0' => $langs->trans("No"), '1' => $langs->trans("Yes")];
                    print $form->selectarray("choix" . $i, $arraychoice, $car);
                }
                if (!empty($listofanswers[$i]['format']) && $listofanswers[$i]['format'] == 'foragainst') {
                    $arraychoice = ['2' => '&nbsp;', '0' => $langs->trans("Against"), '1' => $langs->trans("For")];
                    print $form->selectarray("choix" . $i, $arraychoice, $car);
                }
                print '</td>' . "\n";
            }
...
if ($nbofcheckbox >= 2) {
    ...
    print '<td></td>' . "\n";
    for ($i = 0; $i < $nbcolonnes; $i++) {
        if (empty($listofanswers[$i]['format']) || !in_array($listofanswers[$i]['format'], ['yesno', 'foragainst']) && isset($sumfor[$i]) && isset($meilleurecolonne) && $sumfor[$i] == $meilleurecolonne) {
