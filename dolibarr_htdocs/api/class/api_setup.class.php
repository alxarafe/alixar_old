...
class Setup extends DolibarrApi
{
    ...
    public function getListOfContactTypes($sortfield = "code", $sortorder = 'ASC', $limit = 100, $page = 0, $type = '', $module = '', $active = 1, $lang = '', $sqlfilters = '')
    {
        ...
        if ($result) {
            ...
            for ($i = 0; $i < $min; $i++) {
                ...
                $this->translateLabel($contact_type, $lang, 'TypeContact_' . $contact_type->type . '_' . $contact_type->source . '_', ["eventorganization", "resource", "projects", "contracts", "bills", "orders", "agenda", "propal", "stocks", "supplier_proposal", "interventions", "sendings", "ticket"]);
                $list[] = $contact_type;
            }
        } else {
    ...
    public function getShippingModes($limit = 100, $page = 0, $active = 1, $lang = '', $sqlfilters = '')
    {
        ...
        if ($result) {
            ...
            for ($i = 0; $i < $min; $i++) {
                ...
                $this->translateLabel($method, $lang, '', ['dict']);
                $list[] = $method;
            }
        } else {
            throw new RestException(503, $this->db->lasterror());
    ...
    public function getCheckIntegrity($target)
    {
        ...
        if ($target == 'local') {
            ...
            $xmlarray = getURLContent($xmlremote, 'GET', '', 1, [], ['http', 'https'], 0);    // Accept http or https links on external remote server only. Same is used into filecheck.php.
            ...
            if (is_object($xml->dolibarr_htdocs_dir[0])) {
                ...
                foreach ($scanfiles as $keyfile => $valfile) {
                    ...
                    if (!in_array($tmprelativefilename, $file_list['insignature'])) {
                        ...
                        $file_list['added'][] = ['filename' => $tmprelativefilename, 'md5' => $md5newfile];
                    }
                }
        ...
        return ['resultcode' => $resultcode, 'resultcomment' => $resultcomment, 'expectedchecksum' => $outexpectedchecksum, 'currentchecksum' => $outcurrentchecksum, 'out' => $out];
    }
