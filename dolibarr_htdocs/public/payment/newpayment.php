...
$langs->loadLangs(["main", "other", "dict", "bills", "companies", "errors", "paybox", "paypal", "stripe"]); // File with generic data
    ...
    if (!getDolGlobalInt('STRIPE_USE_INTENT_WITH_AUTOMATIC_CONFIRMATION')) {
        try {
            ...
            if ($thirdparty_id > 0) {
                ...
                if (!$error) {
                    if ($savesource) {
                        $card = $customer->sources->create(["source" => $stripeToken, "metadata" => $metadata]);
                    } else {
                        $card = $stripeToken;
                    }
                    ...
                    if (empty($card)) {
                        ...
                        ], ["idempotency_key" => "$FULLTAG", "stripe_account" => "$stripeacc"]);
    ...
    if (getDolGlobalInt('STRIPE_USE_INTENT_WITH_AUTOMATIC_CONFIRMATION')) {
        ...
        try {
            if (empty($stripeacc)) {                // If the Stripe connect account not set, we use common API usage
                ...
                $paymentintent = \Stripe\PaymentIntent::retrieve($paymentintent_id, ["stripe_account" => $stripeacc]);
            }
        } catch (Exception $e) {
            $error++;
...
// Check link validity
if ($source && in_array($ref, ['member_ref', 'contractline_ref', 'invoice_ref', 'order_ref', 'donation_ref', ''])) {
    $langs->load("errors");
    ...
    if ($contractline->fk_product) {
        if ($contractline->product->isService() && $contractline->product->duration_value > 0) {
            ...
            if ($contractline->product->duration_value > 1) {
                $dur = ["h" => $langs->trans("Hours"), "d" => $langs->trans("DurationDays"), "w" => $langs->trans("DurationWeeks"), "m" => $langs->trans("DurationMonths"), "y" => $langs->trans("DurationYears")];
            } else {
                $dur = ["h" => $langs->trans("Hour"), "d" => $langs->trans("DurationDay"), "w" => $langs->trans("DurationWeek"), "m" => $langs->trans("DurationMonth"), "y" => $langs->trans("DurationYear")];
            }
...
if ($source == 'organizedeventregistration') {
    $found = true;
    $langs->loadLangs(["members", "eventorganization"]);
