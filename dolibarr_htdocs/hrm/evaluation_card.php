...
$langs->loadLangs(['hrm', 'other', 'products']);  // why products?
...
$hookmanager->initHooks(['evaluationcard', 'globalcard']); // Note that conf->hooks_modules contains array
...
$permissiontovalidate = (Functions::getDolGlobalString('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('hrm', 'evaluation_advance', 'validate')) || (!Functions::getDolGlobalString('MAIN_USE_ADVANCED_PERMS') && $permissiontoadd);
...
if (empty($reshook)) {
    ...
    if ($action == 'close') {
        ...
        $sk = new SkillRank($db);
        $SkillrecordsForActiveUser = $sk->fetchAll('ASC', 'fk_skill', 0, 0, ["customsql" => "fk_object = " . $object->fk_user . " AND objecttype ='" . SkillRank::SKILLRANK_TYPE_USER . "'"], 'AND');
...
if ($object->id > 0 && (empty($action) || ($action != 'edit' && $action != 'create'))) {
    ...
    if ($action != 'presend') {
        ...
        $linktoelem = $form->showLinkToObjectBlock($object, null, ['evaluation']);
