...
$hookmanager->initHooks(['positionlist']); // Note that conf->hooks_modules contains array
...
$sql .= ',';
$sql .= $userstatic->getFieldList('u', ['rowid']);
...
while ($i < $imaxinloop) {
    ...
    if ($mode == 'kanban') {
        ...
        print $object->getKanbanView('', ['user' => $userstatic->getNomUrl(-1), 'job' => $jobstatic->getNomUrl(1), 'selected' => $selected]);
        if ($i == ($imaxinloop - 1)) {
            print '</div>';
            print '</td></tr>';
        }
