...
$langs->loadLangs(['resource', 'companies', 'other', 'main']);
...
$hookmanager->initHooks(['resource', 'resource_card', 'globalcard']);
...
if ($action == 'create' || $object->fetch($id, $ref) > 0) {
    ...
    if ($action == 'create' || $action == 'edit') {
        ...
        print $formresource->select_ziptown($object->zip, 'zipcode', ['town', 'selectcountry_id', 'state_id'], 0, 0, '', 'maxwidth100');
        print '</td>';
        if ($conf->browser->layout == 'phone') {
            print '</tr><tr>';
        }
        print '<td>' . $form->editfieldkey('Town', 'town', '', $object, 0) . '</td><td' . ($conf->browser->layout == 'phone' ? ' colspan="3"' : '') . '>';
        print $formresource->select_ziptown($object->town, 'town', ['zipcode', 'selectcountry_id', 'state_id']);
