...
$hookmanager->initHooks(['molist']); // Note that conf->hooks_modules contains array
...
while ($i < $imaxinloop) {
    ...
    if ($mode == 'kanban') {
        ...
        print $object->getKanbanView('', ['bom' => ($obj->fk_bom > 0 ? $bom : null), 'product' => ($obj->fk_product > 0 ? $product : null), 'selected' => $selected]);
        if ($i == ($imaxinloop - 1)) {
            print '</div>';
            print '</td></tr>';
        }
