...
if (empty($reshook)) {
    ...
    if ($action == 'add' && empty($id) && !empty($TBomLineId)) {
        ...
        foreach ($TBomLineId as $id_bom_line) {
            ...
            $TMoLines = $moline->fetchAll('DESC', 'rowid', '1', '', ['origin_id' => $id_bom_line]);
...
if ($object->id > 0 && (empty($action) || ($action != 'edit' && $action != 'create'))) {
    ...
    if ($action == 'delete') {
        ...
        $formquestion = [
            ['type' => 'checkbox', 'name' => 'deletechilds', 'label' => $label, 'value' => 0],
            [
    ...
    if ($action != 'presend') {
        ...
        $linktoelem = $form->showLinkToObjectBlock($object, null, ['mo']);
