...
class InterfaceActionsBlockedLog extends DolibarrTriggers
{
    ...
    public function runTrigger(string $action, $object, User $user, Lang $langs, Conf $conf)
    {
        ...
        $amounts = 0;
        if (
            $action === 'BILL_VALIDATE' || (($action === 'BILL_DELETE' || $action === 'BILL_SENTBYMAIL') && $object->statut != 0)
        ...
        if (
            $action === 'PAYMENT_CUSTOMER_CREATE' || $action === 'PAYMENT_SUPPLIER_CREATE' || $action === 'DONATION_PAYMENT_CREATE'
