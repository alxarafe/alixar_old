...
if ($action == 'update') {
    if (GETPOST("button") != $langs->trans("Cancel")) {
        ...
        if (!$error) {
            if (strlen(GETPOST('attrname', 'aZ09')) < 3 && !Functions::getDolGlobalString('MAIN_DISABLE_EXTRAFIELDS_CHECK_FOR_UPDATE')) {
                $error++;
                $langs->load("errors");
                $mesg[] = $langs->trans("ErrorValueLength", $langs->transnoentitiesnoconv("AttributeCode"), 3);
                $action = 'edit';
        ...
        // Check reserved keyword with more than 3 characters
        if (!$error) {
            if (in_array(strtoupper(GETPOST('attrname', 'aZ09')), $listofreservedwords) && !Functions::getDolGlobalString('MAIN_DISABLE_EXTRAFIELDS_CHECK_FOR_UPDATE')) {
                $error++;
                $langs->load("errors");
                $mesg[] = $langs->trans("ErrorReservedKeyword", GETPOST('attrname', 'aZ09'));
                $action = 'edit';
