...
$langs->loadLangs(['bills', 'companies', 'donations', 'users']);
...
$hookmanager->initHooks([$object->element . 'card', 'globalcard']);
...
if (empty($reshook)) {
    ...
    if ($action == 'add') {
        ...
        if (isModEnabled("societe") && Functions::getDolGlobalString('DONATION_USE_THIRDPARTIES') && !(GETPOST("socid", 'int') > 0)) {
...
if ($action == 'create') {
    ...
    // Company
    if (isModEnabled("societe") && Functions::getDolGlobalString('DONATION_USE_THIRDPARTIES')) {
        // Thirdparty
        if ($soc->id > 0) {
            ...
            if (Functions::getDolGlobalString('RELOAD_PAGE_ON_CUSTOMER_CHANGE_DISABLED')) {
    ...
    if (!isModEnabled('societe') || !Functions::getDolGlobalString('DONATION_USE_THIRDPARTIES')) {
        ...
        print $formcompany->select_ziptown((GETPOSTISSET("zipcode") ? GETPOST("zipcode") : $object->zip), 'zipcode', ['town', 'selectcountry_id', 'state_id'], 6);
        print ' ';
        print $formcompany->select_ziptown((GETPOSTISSET("town") ? GETPOST("town") : $object->town), 'town', ['zipcode', 'selectcountry_id', 'state_id']);
        print '</tr>';
    ...
    $doleditor = new DolEditor('note_public', $note_public, '', 80, 'dolibarr_notes', 'In', 0, false, !Functions::getDolGlobalString('FCKEDITOR_ENABLE_NOTE_PUBLIC') ? 0 : 1, ROWS_3, '90%');
    ...
    if (empty($user->socid)) {
        ...
        $doleditor = new DolEditor('note_private', $note_private, '', 80, 'dolibarr_notes', 'In', 0, false, !Functions::getDolGlobalString('FCKEDITOR_ENABLE_NOTE_PRIVATE') ? 0 : 1, ROWS_3, '90%');
        print $doleditor->Create(1);
        print '</td></tr>';
    }
...
if (!empty($id) && $action == 'edit') {
    ...
    if (isModEnabled("societe") && Functions::getDolGlobalString('DONATION_USE_THIRDPARTIES')) {
        ...
        // Zip / Town
        print '<tr><td>' . $langs->trans("Zip") . ' / ' . $langs->trans("Town") . '</td><td>';
        print $formcompany->select_ziptown((GETPOSTISSET("zipcode") ? GETPOSTISSET("zipcode") : $object->zip), 'zipcode', ['town', 'selectcountry_id', 'state_id'], 6);
        print ' ';
        print $formcompany->select_ziptown((GETPOSTISSET("town") ? GETPOST("town") : $object->town), 'town', ['zipcode', 'selectcountry_id', 'state_id']);
        print '</tr>';
...
if (!empty($id) && $action != 'edit') {
    ...
    $linktoelem = $form->showLinkToObjectBlock($object, null, ['don']);
