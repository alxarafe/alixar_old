...
$hookmanager->initHooks(['donationindex']);
...
if (Functions::getDolGlobalString('MAIN_SEARCH_FORM_ON_HOME_AREAS')) {     // TODO Add a search into global search combo so we can remove this
    if (isModEnabled('don') && $user->hasRight('don', 'lire')) {
        $listofsearchfields['search_donation'] = ['text' => 'Donation'];
    }
...
foreach ($listofstatus as $status) {
    $dataseries[] = [$donstatic->LibStatut($status, 1), (isset($nb[$status]) ? (int) $nb[$status] : 0)];
