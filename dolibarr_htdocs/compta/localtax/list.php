...
$newcardbutton = '';
$newcardbutton .= dolGetButtonTitle($langs->trans('ViewList'), '', 'fa fa-bars imgforviewmode', $_SERVER["PHP_SELF"] . '?localTaxType=' . $ltt . '&mode=common' . preg_replace('/(&|\?)*mode=[^&]+/', '', $param), '', ((empty($mode) || $mode == 'common') ? 2 : 1), ['morecss' => 'reposition']);
$newcardbutton .= dolGetButtonTitle($langs->trans('ViewKanban'), '', 'fa fa-th-list imgforviewmode', $_SERVER["PHP_SELF"] . '?localTaxType=' . $ltt . '&mode=kanban' . preg_replace('/(&|\?)*mode=[^&]+/', '', $param), '', ($mode == 'kanban' ? 2 : 1), ['morecss' => 'reposition']);
...
if ($result) {
    ...
    while ($i < $imaxinloop) {
        ...
        if ($mode == 'kanban') {
            ...
            // Output Kanban
            print $localtax_static->getKanbanView('', ['selected' => in_array($object->id, $arrayofselected)]);
            if ($i == ($imaxinloop - 1)) {
                print '</div>';
                print '</td></tr>';
            }
