...
$langs->loadLangs(['compta', 'banks', 'bills', 'hrm', 'projects']);
...
$arrayfields = [
    'cs.rowid' => ['label' => "Ref", 'checked' => 1, 'position' => 10],
    'cs.libelle' => ['label' => "Label", 'checked' => 1, 'position' => 20],
    'cs.fk_type' => ['label' => "Type", 'checked' => 1, 'position' => 30],
    'cs.date_ech' => ['label' => "Date", 'checked' => 1, 'position' => 40],
    'cs.periode' => ['label' => "PeriodEndDate", 'checked' => 1, 'position' => 50],
    'p.ref' => ['label' => "ProjectRef", 'checked' => 1, 'position' => 60, 'enable' => (isModEnabled('project'))],
    'cs.fk_user' => ['label' => "Employee", 'checked' => 1, 'position' => 70],
    'cs.fk_mode_reglement' => ['checked' => -1, 'position' => 80, 'label' => "DefaultPaymentMode"],
    'cs.amount' => ['label' => "Amount", 'checked' => 1, 'position' => 100],
    'cs.paye' => ['label' => "Status", 'checked' => 1, 'position' => 110],
];

if (isModEnabled("banque")) {
    $arrayfields['cs.fk_account'] = ['checked' => -1, 'position' => 90, 'label' => "DefaultBankAccount"];
}

$arrayfields = dol_sort_array($arrayfields, 'position');

// Initialize technical object to manage hooks of page. Note that conf->hooks_modules contains array of hook context
$hookmanager->initHooks(['sclist']);
...
if (!empty($arrayfields['cs.paye']['checked'])) {
    ...
    $liststatus = ['0' => $langs->trans("Unpaid"), '1' => $langs->trans("Paid")];
...
while ($i < $imaxinloop) {
    ...
    if ($mode == 'kanban') {
        ...
        // Output Kanban
        print $chargesociale_static->getKanbanView('', ['project' => $projectstatic, 'selected' => in_array($chargesociale_static->id, $arrayofselected)]);
        if ($i == ($imaxinloop - 1)) {
            print '</div>';
            print '</td></tr>';
        }
