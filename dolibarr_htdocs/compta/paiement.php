...
$langs->loadLangs(['companies', 'bills', 'banks', 'multicurrency']);
...
$hookmanager->initHooks(['paiementcard', 'globalcard']);
    ...
    if (($action == 'add_paiement' || ($action == 'confirm_paiement' && $confirm == 'yes')) && $usercanissuepayment) {
        ...
        foreach ($_POST as $key => $value) {
            if (substr($key, 0, 7) == 'amount_' && GETPOST($key) != '') {
                ...
                $formquestion[$i++] = ['type' => 'hidden', 'name' => $key, 'value' => GETPOST($key)];
                ...
                $formquestion[$i++] = ['type' => 'hidden', 'name' => $key, 'value' => GETPOST($key, 'int')];
            }
        }
...
if ($action == 'create' || $action == 'confirm_paiement' || $action == 'add_paiement') {
    ...
    if ($result >= 0) {
        ...
        if ($action == 'add_paiement') {
            ...
            $formquestion[$i++] = ['type' => 'hidden', 'name' => 'facid', 'value' => $facture->id];
            $formquestion[$i++] = ['type' => 'hidden', 'name' => 'socid', 'value' => $facture->socid];
            $formquestion[$i++] = ['type' => 'hidden', 'name' => 'type', 'value' => $facture->type];
        }
