...
$hookmanager->initHooks(['bankannualreport', 'globalcard']);
...
if ($result < 0) {
    ...
    $graph_datas = [];
    for ($i = 0; $i < 12; $i++) {
        $graph_datas[$i] = [$labels[$i], $data_year_0[$i], $data_year_1[$i], $data_year_2[$i]];
    }

    $px1 = new DolGraph();
    $px1->SetData($graph_datas);
    $px1->SetLegend([($year), ($year - 1), ($year - 2)]);
    ...
    $px2->SetLegend([($year), ($year - 1), ($year - 2)]);
