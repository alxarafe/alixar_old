...
$langs->loadLangs(['compta', 'bills', 'donation', 'salaries', 'accountancy', 'loan']);
...
if ($modecompta == "CREANCES-DETTES") {
    ...
    $name = $langs->trans("ReportInOut") . ', ' . $langs->trans("ByPredefinedAccountGroups");
    $period = $form->selectDate($date_start, 'date_start', 0, 0, 0, '', 1, 0) . ' - ' . $form->selectDate($date_end, 'date_end', 0, 0, 0, '', 1, 0);
    $arraylist = ['no' => $langs->trans("CustomerCode"), 'yes' => $langs->trans("AccountWithNonZeroValues"), 'all' => $langs->trans("All")];
...
report_header($name, '', $period, $periodlink, $description, $builddate, $exportlink, ['modecompta' => $modecompta, 'showaccountdetail' => $showaccountdetail], $calcmode);
...
$object = [&$total_ht, &$total_ttc];
$parameters["mode"] = $modecompta;
$parameters["date_start"] = $date_start;
$parameters["date_end"] = $date_end;
// Initialize technical object to manage hooks of expenses. Note that conf->hooks_modules contains array array
$hookmanager->initHooks(['externalbalance']);
