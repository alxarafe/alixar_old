        ...
        for ($i = 0; $i < $nbline; $i++) {
            ...
            if (($mode == "barcode" && $barcode == $obj->barcode) || ($mode == "lotserial" && $barcode == $obj->batch)) {
                if (!empty($obj->fk_entrepot) && $fk_entrepot == $obj->fk_entrepot) {
                    ...
                    $objectreturn = ['fk_warehouse' => $warehouseid, 'fk_product' => $fk_product, 'reelqty' => $reelqty];
                }
            }
        }
        if ($warehousefound < 1) {
            $response = ['status' => 'error', 'errorcode' => 'NotFound', 'message' => 'No warehouse found for barcode' . $barcode];
        } elseif ($warehousefound > 1) {
            $response = ['status' => 'error', 'errorcode' => 'TooManyWarehouse', 'message' => 'Too many warehouse found'];
        } else {
            $response = ['status' => 'success', 'message' => 'Warehouse found', 'object' => $objectreturn];
        }
    } else {
        $response = ['status' => 'error', 'errorcode' => 'NotFound', 'message' => "No results found for barcode"];
    }
} else {
    $response = ['status' => 'error', 'errorcode' => 'ActionError', 'message' => "Error on action"];
}
