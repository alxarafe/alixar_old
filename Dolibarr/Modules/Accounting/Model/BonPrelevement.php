...
class BonPrelevement extends GenericDocument
{
     ...
     *      Note: Filter must be a Dolibarr filter syntax string. Example: "(t.ref:like:'SO-%') or
    ...
    public $fields = [
        ...
        'ref' => ['type' => 'varchar(12)', 'label' => 'Ref', 'enabled' => 1, 'position' => 15, 'notnull' => 0, 'visible' => -1, 'csslist' => 'tdoverflowmax150', 'showoncombobox' => 1,],
        'datec' => ['type' => 'datetime', 'label' => 'DateCreation', 'enabled' => 1, 'position' => 25, 'notnull' => 0, 'visible' => -1,],
        'amount' => ['type' => 'double(24,8)', 'label' => 'Amount', 'enabled' => 1, 'position' => 30, 'notnull' => 0, 'visible' => -1,],
        'statut' => ['type' => 'smallint(6)', 'label' => 'Statut', 'enabled' => 1, 'position' => 500, 'notnull' => 0, 'visible' => -1, 'arrayofkeyval' => [0 => 'Wait', 1 => 'Transfered', 2 => 'Credited']],
        'credite' => ['type' => 'smallint(6)', 'label' => 'Credite', 'enabled' => 1, 'position' => 40, 'notnull' => 0, 'visible' => -1,],
        'note' => ['type' => 'text', 'label' => 'Note', 'enabled' => 1, 'position' => 45, 'notnull' => 0, 'visible' => -1,],
        'date_trans' => ['type' => 'datetime', 'label' => 'Datetrans', 'enabled' => 1, 'position' => 50, 'notnull' => 0, 'visible' => -1,],
        'method_trans' => ['type' => 'smallint(6)', 'label' => 'Methodtrans', 'enabled' => 1, 'position' => 55, 'notnull' => 0, 'visible' => -1,],
        'fk_user_trans' => ['type' => 'integer:User:user/class/user.class.php', 'label' => 'Fkusertrans', 'enabled' => 1, 'position' => 60, 'notnull' => 0, 'visible' => -1, 'css' => 'maxwidth500 widthcentpercentminusxx', 'csslist' => 'tdoverflowmax150',],
        'date_credit' => ['type' => 'datetime', 'label' => 'Datecredit', 'enabled' => 1, 'position' => 65, 'notnull' => 0, 'visible' => -1,],
        'fk_user_credit' => ['type' => 'integer:User:user/class/user.class.php', 'label' => 'Fkusercredit', 'enabled' => 1, 'position' => 70, 'notnull' => 0, 'visible' => -1, 'css' => 'maxwidth500 widthcentpercentminusxx', 'csslist' => 'tdoverflowmax150',],
        'type' => ['type' => 'varchar(16)', 'label' => 'Type', 'enabled' => 1, 'position' => 75, 'notnull' => 0, 'visible' => -1,],
        'fk_bank_account' => ['type' => 'integer', 'label' => 'Fkbankaccount', 'enabled' => 1, 'position' => 80, 'notnull' => 0, 'visible' => -1, 'css' => 'maxwidth500 widthcentpercentminusxx',],
    ];
    public $rowid;
    public $ref;
    public $datec;
    ...
    public function __construct($db)
    {
        ...
        $this->methodes_trans = [0 => 'Internet', 2 => 'Email', 3 => 'Api'];
     ...
     * @param string $type            'direct-debit' or 'bank-transfer'
     * @param int    $did             ID of an existing payment request. If $did is defined, we use the existing
     *                                payment request.
     * @param int    $fk_bank_account Bank account ID the receipt is generated for. Will use the ID into the setup of
     *                                module Direct Debit or Credit Transfer if 0.
    ...
    public function EnregDestinataire($rowid, $client_nom, $rib_banque, $rib_guichet, $rib_number, $amount, $ref, $facid, $rib_dom = '', $type = 'direct-debit')
    {
        ...
        // Address optional D1
        $address = strtr($rib_dom, [" " => "-", chr(13) => " ", chr(10) => ""]);
    ...
    public function EnregDestinataireSEPA($row_code_client, $row_nom, $row_address, $row_zip, $row_town, $row_country_code, $row_cb, $row_cg, $row_cc, $row_somme, $row_ref, $row_idfac, $row_iban, $row_bic, $row_datec, $row_drum, $row_rum, $type = 'direct-debit', $row_comment = '')
    {
        ...
        if ($type != 'bank-transfer') {
            ...
            $addressline1 = strtr($row_address, [chr(13) => ", ", chr(10) => ""]);
            $addressline2 = strtr($row_zip . (($row_zip && $row_town) ? ' ' : (string) $row_town), [chr(13) => ", ", chr(10) => ""]);
    ...
    public function EnregEmetteurSEPA($configuration, $ladate, $nombre, $total, $CrLf = '\n', $format = 'FRST', $type = 'direct-debit', $fk_bank_account = 0)
    {
        ...
        if ($resql) {
            ...
            if ($type != 'bank-transfer') {
                ...
                $addressline1 = strtr($configuration->global->MAIN_INFO_SOCIETE_ADDRESS, [chr(13) => ", ", chr(10) => ""]);
                $addressline2 = strtr($configuration->global->MAIN_INFO_SOCIETE_ZIP . (($configuration->global->MAIN_INFO_SOCIETE_ZIP || ' ' . $configuration->global->MAIN_INFO_SOCIETE_TOWN) ? ' ' : '') . $configuration->global->MAIN_INFO_SOCIETE_TOWN, [chr(13) => ", ", chr(10) => ""]);
                if ($addressline1) {
