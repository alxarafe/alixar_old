    ...
    if ($result) {
        ...
        while ($i < $imaxinloop) {
            ...
            if ($mode == 'kanban') {
                ...
                print $membertype->getKanbanView('', ['selected' => in_array($object->id, $arrayofselected)]);
                if ($i == ($imaxinloop - 1)) {
                    print '</div>';
                    print '</td></tr>';
                }
...
if ($action == 'create') {
    ...
    print $form->selectarray('status', ['0' => $langs->trans('ActivityCeased'), '1' => $langs->trans('InActivity')], 1, 0, 0, 0, '', 0, 0, 0, '', 'minwidth100');
    print '</td></tr>';
...
if ($rowid > 0) {
    if ($action != 'edit') {
        ...
        if ($sall) {
            $sql .= natural_search(["d.firstname", "d.lastname", "d.societe", "d.email", "d.login", "d.address", "d.town", "d.note_public", "d.note_private"], $sall);
        }
        if ($status != '') {
        ...
        if ($action == 'search') {
            if (GETPOST('search', 'alpha')) {
                $sql .= natural_search(["d.firstname", "d.lastname"], GETPOST('search', 'alpha'));
            }
        }
        if (!empty($search_ref)) {
        ...
        if (!empty($search_lastname)) {
            $sql .= natural_search(["d.firstname", "d.lastname"], $search_lastname);
        }
    ...
    if ($action == 'edit') {
        ...
        print '<tr><td>' . $langs->trans("Status") . '</td><td>';
        print $form->selectarray('status', ['0' => $langs->trans('ActivityCeased'), '1' => $langs->trans('InActivity')], $object->status, 0, 0, 0, '', 0, 0, 0, '', 'minwidth100');
        print '</td></tr>';
