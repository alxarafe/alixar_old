...
public $regeximgext = '\.gif|\.jpg|\.jpeg|\.png|\.bmp|\.webp|\.xpm|\.xbm'; // See also into images.lib.php
    ...
    public $fields = [
        ...
        'ref' => ['type' => 'varchar(128)', 'label' => 'Ref', 'enabled' => 1, 'visible' => 1, 'notnull' => 1, 'showoncombobox' => 1, 'index' => 1, 'position' => 10, 'searchall' => 1, 'comment' => 'Reference of object'],
        'entity' => ['type' => 'integer', 'label' => 'Entity', 'enabled' => 1, 'visible' => 0, 'default' => '1', 'notnull' => 1, 'index' => 1, 'position' => 5],
        'label' => ['type' => 'varchar(255)', 'label' => 'Label', 'enabled' => 1, 'visible' => 1, 'notnull' => 1, 'showoncombobox' => 2, 'position' => 15, 'csslist' => 'tdoverflowmax250'],
        'barcode' => ['type' => 'varchar(255)', 'label' => 'Barcode', 'enabled' => 'isModEnabled("barcode")', 'position' => 20, 'visible' => -1, 'showoncombobox' => 3],
        'fk_barcode_type' => ['type' => 'integer', 'label' => 'BarcodeType', 'enabled' => 1, 'position' => 21, 'notnull' => 0, 'visible' => -1,],
        'note_public' => ['type' => 'html', 'label' => 'NotePublic', 'enabled' => 1, 'visible' => 0, 'position' => 61],
        'note' => ['type' => 'html', 'label' => 'NotePrivate', 'enabled' => 1, 'visible' => 0, 'position' => 62],
        ...
        'fk_user_author' => ['type' => 'integer', 'label' => 'UserAuthor', 'enabled' => 1, 'visible' => -2, 'notnull' => 1, 'position' => 510, 'foreignkey' => 'llx_user.rowid'],
        'fk_user_modif' => ['type' => 'integer', 'label' => 'UserModif', 'enabled' => 1, 'visible' => -2, 'notnull' => -1, 'position' => 511],
        //'fk_user_valid' =>array('type'=>'integer',      'label'=>'UserValidation',        'enabled'=>1, 'visible'=>-1, 'position'=>512),
        'localtax1_tx' => ['type' => 'double(6,3)', 'label' => 'Localtax1tx', 'enabled' => 1, 'position' => 150, 'notnull' => 0, 'visible' => -1,],
        'localtax1_type' => ['type' => 'varchar(10)', 'label' => 'Localtax1type', 'enabled' => 1, 'position' => 155, 'notnull' => 1, 'visible' => -1,],
        'localtax2_tx' => ['type' => 'double(6,3)', 'label' => 'Localtax2tx', 'enabled' => 1, 'position' => 160, 'notnull' => 0, 'visible' => -1,],
        'localtax2_type' => ['type' => 'varchar(10)', 'label' => 'Localtax2type', 'enabled' => 1, 'position' => 165, 'notnull' => 1, 'visible' => -1,],
        'last_main_doc' => ['type' => 'varchar(255)', 'label' => 'LastMainDoc', 'enabled' => 1, 'visible' => -1, 'position' => 170],
        'import_key' => ['type' => 'varchar(14)', 'label' => 'ImportId', 'enabled' => 1, 'visible' => -2, 'notnull' => -1, 'index' => 0, 'position' => 1000],
        //'tosell'       =>array('type'=>'integer',      'label'=>'Status',           'enabled'=>1, 'visible'=>1,  'notnull'=>1, 'default'=>'0', 'index'=>1,  'position'=>1000, 'arrayofkeyval'=>array(0=>'Draft', 1=>'Active', -1=>'Cancel')),
        //'tobuy'        =>array('type'=>'integer',      'label'=>'Status',           'enabled'=>1, 'visible'=>1,  'notnull'=>1, 'default'=>'0', 'index'=>1,  'position'=>1000, 'arrayofkeyval'=>array(0=>'Draft', 1=>'Active', -1=>'Cancel')),
        'mandatory_period' => ['type' => 'integer', 'label' => 'mandatoryperiod', 'enabled' => 1, 'visible' => 1, 'notnull' => 1, 'default' => '0', 'index' => 1, 'position' => 1000],
    ];
    /**
     * @var array<string, array<string>>    List of child tables. To test if we can delete object.
     */
    protected $childtables = [
        'supplier_proposaldet' => ['name' => 'SupplierProposal', 'parent' => 'supplier_proposal', 'parentkey' => 'fk_supplier_proposal'],
        'propaldet' => ['name' => 'Proposal', 'parent' => 'propal', 'parentkey' => 'fk_propal'],
        'commandedet' => ['name' => 'Order', 'parent' => 'commande', 'parentkey' => 'fk_commande'],
        'facturedet' => ['name' => 'Invoice', 'parent' => 'facture', 'parentkey' => 'fk_facture'],
        'contratdet' => ['name' => 'Contract', 'parent' => 'contrat', 'parentkey' => 'fk_contrat'],
        'facture_fourn_det' => ['name' => 'SupplierInvoice', 'parent' => 'facture_fourn', 'parentkey' => 'fk_facture_fourn'],
        'commande_fournisseurdet' => ['name' => 'SupplierOrder', 'parent' => 'commande_fournisseur', 'parentkey' => 'fk_commande'],
    ];
    /**
     * {@inheritdoc}
     */
    ...
    public function verify()
    {
        ...
        $arrayofnonnegativevalue = ['weight' => 'Weight', 'width' => 'Width', 'height' => 'Height', 'length' => 'Length', 'surface' => 'Surface', 'volume' => 'Volume'];
    ...
    /**
     * Load information about stock of a product into ->stock_reel, ->stock_warehouse[] (including
     * stock_warehouse[idwarehouse]->detail_batch for batch products) This function need a lot of load. If you use it
     * on list, use a cache to execute it once for each product id. If ENTREPOT_EXTRA_STATUS is set, filtering on
     * warehouse status is possible.
     *
     * @param string $option                        '' = Load all stock info, also from closed and internal warehouses,
     *                                              'nobatch' = do not load batch detail, 'novirtual' = do no load
     *                                              virtual detail You can also filter on 'warehouseclosed',
     ...
     *
     * @param int $includedraftpoforvirtual Include draft status and not yet approved Purchase Orders for virtual stock
     *                                      calculation
     * @param int $dateofvirtualstock       Date of virtual stock
     *
     * @return int                                 Return integer < 0 if KO, > 0 if OK
     ...
     * @param int    $forVirtualStock Ignore rights filter for virtual stock calculation. Set when load_stats_commande
     *                                is used for virtual stock calculation.
     *
     * @return integer                 Array of stats in $this->stats_commande (nb=nb of order, qty=qty ordered), <0 if
     *                                 ko or >0 if ok
     */
     ...
     * @param int    $forVirtualStock    Ignore rights filter for virtual stock calculation.
     * @param int    $dateofvirtualstock Date of virtual stock
     * @param int    $warehouseid        Filter by a warehouse. Warning: When a filter on a warehous is set, it is not
     *                                   possible to calculate an accurate virtual stock because we can't knnow in
     *                                   which warehouse will be done virtual stock changes.
     *
     * @return  integer                     Array of stats in $this->stats_mrptoproduce (nb=nb of order, qty=qty
     *                                      ordered), <0 if ko or >0 if ok
     */
     ...
     * @param int    $ignore_expression When module dynamicprices is on, ignores the math expression for calculating
     *                                  price and uses the db value instead
     * @param int    $ignore_price_load Load product without loading $this->multiprices... array (when we are sure we
     *                                  don't need them)
     * @param int    $ignore_lang_load  Load product without loading $this->multilangs language arrays (when we are
     ...
     * @param double      $nbpiece                         nb of units (should be always positive, use $movement to
     *                                                     decide if we add or remove)
     ...
     * @param int         $disablestockchangeforsubproduct Disable stock change for sub-products of kit (useful only if
     *                                                     product is a subproduct)
     ...
     * @param int         $movement                        0 = add, 1 = remove
     * @param string      $label                           Label of stock movement
     * @param double      $price                           Unit price HT of product, used to calculate average weighted
     *                                                     price (PMP in french). If 0, average weighted price is not
     *                                                     changed.
     * @param string      $inventorycode                   Inventory code
     * @param string      $origin_element                  Origin element type
     * @param int         $origin_id                       Origin id of element
    ...
    public function delete(User $user, $notrigger = 0)
    {
        ...
        if (empty($objectisused)) {
            ...
            if (!$error) {
                $elements = ['product_fournisseur_price', 'product_price', 'product_lang', 'categorie_product', 'product_stock', 'product_customer_price', 'product_lot']; // product_batch is done before
     ...
     *
     * @return  array                               Array of price information array('pu_ht'=> , 'pu_ttc'=> ,
     *                                              'tva_tx'=>'X.Y (code)', ...), 'tva_npr'=>0, ...)
    ...
    public function getSellPrice($thirdparty_seller, $thirdparty_buyer, $pqp = 0)
    {
        ...
        if (is_object($hookmanager)) {
            $parameters = ['thirdparty_seller' => $thirdparty_seller, 'thirdparty_buyer' => $thirdparty_buyer, 'pqp' => $pqp];
            // Note that $action and $object may have been modified by some hooks
        ...
        if (getDolGlobalString('PRODUIT_MULTIPRICES') && !empty($thirdparty_buyer->price_level)) {
            ...
            $filter = ['t.fk_product' => $this->id, 't.fk_soc' => $thirdparty_buyer->id];
        ...
        return ['pu_ht' => $pu_ht, 'pu_ttc' => $pu_ttc, 'price_min' => $price_min, 'price_base_type' => $price_base_type, 'tva_tx' => $tva_tx, 'tva_npr' => $tva_npr];
    }
     ...
     *
     * @return int|string             Return integer <-1 if KO, -1 if qty not enough, 0 if OK but nothing found,
     *                                id_product if OK and found. May also initialize some properties like
     *                                (->ref_supplier, buyprice, fourn_pu, vatrate_supplier...), or printable result of
     *                                hook
     ...
     * @param array    $localtaxes_array  Array with localtaxes info array('0'=>type1,'1'=>rate1,'2'=>type2,'3'=>rate2)
     *                                    (loaded by getLocalTaxesFromRate(vatrate, 0, ...) function).
    ...
    public function load_stats_mo($socid = 0)
    {
        ...
        foreach (['toconsume', 'consumed', 'toproduce', 'produced'] as $role) {
     ...
     *
     * @return array                       Return integer <0 if KO, result[month]=array(valuex,valuey) where month is 0
     *                                     to 11
     */
    ...
    private function _get_stats($sql, $mode, $year = 0)
    {
        ...
        for ($j = 0; $j < 12; $j++) {
            ...
            $result[$j] = [$idx, isset($tab[$year . $month]) ? $tab[$year . $month] : 0];
    ...
    /**
     *     Return tree of all subproducts for product. Tree contains array of array(0=prodid, 1=>qty, 2=>product type,
     *     3=>label, 4=>incdec, 5=>product ref) Set this->sousprods
     *
     * @return void
     */
     ...
     *
     * @return array|int                    Return array(prodid=>array(0=prodid, 1=>qty, 2=>product type, 3=>label,
     *                                      4=>incdec, 5=>product ref)
     */
    public function getChildsArbo($id, $firstlevelonly = 0, $level = 1, $parents = [])
    {
        ...
        if ($level == 1) {
            $alreadyfound = [$id => 1]; // We init array of found object to start of tree, so if we found it later (should not happened), we stop immediately
        }
    ...
    public function loadBatchInfo($batch)
    {
        ...
        if ($resql) {
            ...
            while ($i < $num) {
                $obj = $this->db->fetch_object($resql);
                $result[] = ['batch' => $batch, 'eatby' => $this->db->jdate($obj->eatby), 'sellby' => $this->db->jdate($obj->sellby), 'qty' => $obj->qty];
                $i++;
            }
            return $result;
        } else {
     ...
     *
     * @return int            Minimum recommended price that is higher price among all suppliers *
     ...
     *                                      lastsearch_values when clicking
    ...
    public function getNomUrl($withpicto = 0, $option = '', $maxlength = 0, $save_lastsearch_value = -1, $notooltip = 0, $morecss = '', $add_label = 0, $sep = ' - ')
    {
        ...
        $parameters = ['id' => $this->id, 'getnomurl' => &$result, 'label' => &$label];
