...
while ($i < $imaxinloop) {
    ...
    if ($mode == 'kanban') {
        ...
        print $object->getKanbanView('', ['prod' => $prod, 'selected' => $selected]);
        if ($i == ($imaxinloop - 1)) {
            print '</div>';
            print '</td></tr>';
        }
        ...
        foreach ($object->fields as $key => $val) {
            ...
            if (in_array($val['type'], ['double(24,8)', 'double(6,3)', 'integer', 'real', 'price']) && !in_array($key, ['id', 'rowid', 'ref', 'status']) && empty($val['arrayofkeyval'])) {
                $cssforfield .= ($cssforfield ? ' ' : '') . 'right';
            }

            if (!empty($arrayfields['t.' . $key]['checked'])) {
                print '<td' . ($cssforfield ? ' class="' . $cssforfield . (preg_match('/tdoverflow/', $cssforfield) ? ' classfortooltip' : '') . '"' : '');
                if (preg_match('/tdoverflow/', $cssforfield) && !in_array($val['type'], ['ip', 'url']) && !is_numeric($object->$key)) {
