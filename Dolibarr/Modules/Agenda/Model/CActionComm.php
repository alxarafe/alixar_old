...
class CActionComm
{
     ...
     * @param int        $onlyautoornot 1=Group all type AC_XXX into 1 line AC_MANUAL. 0=Keep details of type, -1 or
     *                                  -2=Keep details and add a combined line per calendar (Default, Auto, BoothConf,
     *                                  ...)
     * @param string     $morefilter    Add more SQL filter
     * @param int        $shortlabel    1=Get short label instead of long label
     *
     * @return array|int                   Array of all event types if OK, <0 if KO. Key of array is id or code
     *                                     depending on parameter $idorcode.
     */
            ...
            if ($nump) {
                ...
                while ($i < $nump) {
                    ...
                    if ($qualified && !empty($obj->module)) {
                        ...
                        if (!in_array($obj->type, ['system', 'systemauto', 'module', 'moduleauto'])) {
                    ...
                    if ($qualified) {
                        ...
                        if ($typecalendar == 'system' || $typecalendar == 'user') {
                            ...
                            $TSystem['all'][$obj->code] = ['id' => $label, 'label' => $label, 'type' => $typecalendar, 'color' => $obj->color, 'picto' => $obj->picto];
                        } elseif ($typecalendar == 'systemauto') {
                            $TSystemAuto['id'][$obj->id] = $label;
                            $TSystemAuto['code'][$obj->code] = $label;
                            $TSystemAuto['all'][$obj->code] = ['id' => $label, 'label' => $label, 'type' => $typecalendar, 'color' => $obj->color, 'picto' => $obj->picto];
                        }
