...
class ActionComm extends GenericDocument
{
     ...
     *              1=View may be restricted to sales representative only if no permission to see all or to company of
    ...
    public $type_id;
    /**
     * @var string Calendar of event (Type of type of event). 'system'=Default calendar, 'systemauto'=Auto calendar,
     *      'birthdate', 'holiday', 'module'=Calendar specific to a module This field contains the type into table
     *      llx_ac_actioncomm ('system', 'systemauto', ...). It should be named 'type_type'.
     */
    public $type;
    /**
     * @var string Code into parent table llx_c_actioncomm (used only if option to use type is set). With default
     *      setup, should be AC_OTH_AUTO or AC_OTH. This field contains the code into table llx_ac_actioncomm.
     */
    public $type_code;
    /**
    ...
    public $type_picto;
    /**
     * @var string Free code to identify action. Ie: Agenda trigger add here AC_TRIGGERNAME ('AC_COMPANY_CREATE',
     *      'AC_PROPAL_VALIDATE', ...) This field is stored into field 'code' into llx_actioncomm.
     */
    public $code;
    /**
    ...
    public $location;
    /**
     * @var int Transparency (ical standard). Used to say if people assigned to event are busy or not by event.
     *      0=available, 1=busy, 2=busy (refused events)
     */
    public $transparency;
    /**
...
/**
     ...
     */
    public $num_vote;        /* A string YYYYMMDDHHMMSS shared by allevent of same series */
        /**
     * @var int if event is paid
     */
    public $event_paid;      /* Rule of recurring */
        /**
     * @var int status use but Event organisation module
     */
    public $status;   /* Repeat until this date */
/**
...
public $recurdateend;
    ...
    /**
     *    Initialize $this->userassigned & this->socpeopleassigned array with list of id of user and contact assigned
     *    to event
     *
    ...
    public function create(User $user, $notrigger = 0)
    {
        ...
        if (!is_array($this->userassigned) && !empty($this->userassigned)) {    // For backward compatibility when userassigned was an int instead of an array
            ...
            $this->userassigned = [];
            $this->userassigned[$tmpid] = ['id' => $tmpid, 'transparency' => $this->transparency];
        }
        ...
        if (empty($this->userassigned) || count($this->userassigned) == 0 || !is_array($this->userassigned)) {
            $this->userassigned = [$userownerid => ['id' => $userownerid, 'transparency' => $this->transparency]];
        }
    ...
    public function getActions($socid = 0, $fk_element = 0, $elementtype = '', $filter = '', $sortfield = 'a.datep', $sortorder = 'DESC', $limit = 0)
    {
        ...
        $hookmanager->initHooks(['agendadao']);

        $sql = "SELECT a.id";
        $sql .= " FROM " . MAIN_DB_PREFIX . "actioncomm as a";
        // Fields from hook
        $parameters = ['sql' => &$sql, 'socid' => $socid, 'fk_element' => $fk_element, 'elementtype' => $elementtype];
     ...
     *
     * @param int $mode         0=Long label, 1=Short label, 2=Picto+Short label, 3=Picto, 4=Picto+Short label, 5=Short
     *                          label+Picto, 6=Picto+Long label, 7=Very short label+Picto
     ...
     * @param int        $mode         0=Long label, 1=Short label, 2=Picto+Short label, 3=Picto, 4=Picto+Short label,
     *                                 5=Short label+Picto, 6=Picto+Long label, 7=Very short label+Picto
     * @param int        $hidenastatus 1=Show nothing if status is "Not applicable"
    ...
    public function getTooltipContentArray($params)
    {
        ...
        if (!empty($this->email_msgid)) {
            ...
            $datas['mailfrom'] = '<br><b>' . $langs->trans('MailFrom') . ':</b> ' . str_replace(['<', '>'], ['&amp;lt', '&amp;gt'], $this->email_from);
            $datas['mailto'] = '<br><b>' . $langs->trans('MailTo') . ':</b> ' . str_replace(['<', '>'], ['&amp;lt', '&amp;gt'], $this->email_to);
        ...
        if (!empty($this->note_private)) {
            ...
            $datas['note'] .= (dol_textishtml($texttoshow) ? str_replace(["\r", "\n"], "", $texttoshow) : str_replace(["\r", "\n"], '<br>', $texttoshow));
            $datas['note'] .= '</div>';
        }
    ...
    public function getNomUrl($withpicto = 0, $maxlength = 0, $classname = '', $option = '', $overwritepicto = 0, $notooltip = 0, $save_lastsearch_value = -1)
    {
        ...
        if (!empty($this->email_msgid)) {
            ...
            $tooltip .= '<br><b>' . $langs->trans('MailFrom') . ':</b> ' . str_replace(['<', '>'], ['&amp;lt', '&amp;gt'], !empty($this->email_from) ? $this->email_from : '');
            $tooltip .= '<br><b>' . $langs->trans('MailTo') . ':</b> ' . str_replace(['<', '>'], ['&amp;lt', '&amp;gt'], !empty($this->email_to) ? $this->email_to : '');
            if (!empty($this->email_tocc)) {
        ...
        if (!empty($this->note_private)) {
            ...
            $tooltip .= (dol_textishtml($texttoshow) ? str_replace(["\r", "\n"], "", $texttoshow) : str_replace(["\r", "\n"], '<br>', $texttoshow));
            $tooltip .= '</div>';
        }
        $linkclose = '';
        ...
        global $action;
        $hookmanager->initHooks(['actiondao']);
     ...
     * @param array   $filters       Array of filters. Example array('notolderthan'=>99, 'year'=>..., 'idfrom'=>...,
     *                               'notactiontype'=>'systemauto', 'project'=>123, ...)
     * @param integer $exportholiday 0 = don't integrate holidays into the export, 1 = integrate holidays into the
     *                               export
     *
    ...
    public function build_exportfile($format, $type, $cachedelay, $filename, $filters, $exportholiday = 0)
    {
        ...
        if ($buildfile) {
            ...
            if ($resql) {
                ...
                $parameters = ['filters' => $filters, 'eventarray' => &$eventarray];
     ...
     *
     * @param bool $override Override $this->userownerid when empty. TODO This should be false by default. True is here
