...
namespace DoliModules\Holiday\Model;
...
class Holiday extends GenericDocument
{
    ...
    public $date_fin_gmt = '';
    /**
     * @var int|string 0:Full days, 2:Start afternoon end morning, -1:Start afternoon end afternoon, 1:Start morning
     *      end morning
     */
    public $halfday = '';
    /**
    ...
    public $statut = 0;
    /**
     * @var int     ID of user that must approve. Real user for approval is fk_user_valid (old version) or
     *      fk_user_approve (new versions)
     */
    public $fk_validator;
    /**
     ...
     * @param string $createifnotfound 'stringvalue'=Create entry with string value if not found. For example
     *                                 'YYYYMMDDHHMMSS'.
     *
        ...
        if ($result) {
            ...
            if ($num) {
                ...
                while ($obj = $this->db->fetch_object($result)) {
                    $types[$obj->rowid] = ['id' => $obj->rowid, 'rowid' => $obj->rowid, 'code' => $obj->code, 'label' => $obj->label, 'affect' => $obj->affect, 'delay' => $obj->delay, 'newbymonth' => $obj->newbymonth];
                }
     ...
     *                                  0:Full days, 2:Start afternoon end morning, -1:Start afternoon end afternoon,
     *                                  1:Start morning end morning
     *
     * @return boolean                 False = New range overlap an existing holiday, True = no overlapping (is never
     *                                 on holiday during checked period).
    ...
    public function verifDateHolidayCP($fk_user, $dateStart, $dateEnd, $halfday = 0)
    {
        ...
        foreach ($this->holiday as $infos_CP) {
            ...
            if ($halfday == 0) {
                ...
                if ($dateStart >= $infos_CP['date_debut'] && $dateStart <= $infos_CP['date_fin']) {
                    if ($dateStart < $infos_CP['date_fin'] || in_array($infos_CP['halfday'], [0, -1])) {
                        return false;
                    }
                }
                if ($dateEnd <= $infos_CP['date_fin'] && $dateEnd >= $infos_CP['date_debut']) {
                    ...
                    if ($dateEnd < $infos_CP['date_fin'] || in_array($infos_CP['halfday'], [0, -1])) {
                        return false;
                    }
                }
            } elseif ($halfday == 1) {
                ...
                if ($dateStart >= $infos_CP['date_debut'] && $dateStart <= $infos_CP['date_fin']) {
                    ...
                    if ($dateStart > $infos_CP['date_debut'] || in_array($infos_CP['halfday'], [0, 1])) {
                        return false;
                    }
                }
                if ($dateEnd <= $infos_CP['date_fin'] && $dateEnd >= $infos_CP['date_debut']) {
                    if ($dateEnd > $infos_CP['date_debut'] || in_array($infos_CP['halfday'], [0, 1])) {
                        return false;
                    }
                }
            } elseif ($halfday == 2) {
    ...
    /**
     *  Check that a user is not on holiday for a particular timestamp. Can check approved leave requests and not into
     *  public holidays of company.
     *
     * @param int     $fk_user   Id user
     * @param integer $timestamp Time stamp date for a day (YYYY-MM-DD) without hours  (= 12:00AM in english and not
     *                           12:00PM that is 12:00)
     * @param string  $status    Filter on holiday status. '-1' = no filter.
     *
     * @return array                               array('morning'=> ,'afternoon'=> ), Boolean is true if user is
     *                                             available for day timestamp.
    ...
    public function verifDateHolidayForTimestamp($fk_user, $timestamp, $status = '-1')
    {
        ...
        if ($resql) {
            ...
            if ($num_rows > 0) {
                ...
                while ($i < $num_rows) {
                    ...
                    $arrayofrecord[$obj->rowid] = ['date_start' => $this->db->jdate($obj->date_start), 'date_end' => $this->db->jdate($obj->date_end), 'halfday' => $obj->halfday, 'status' => $obj->status];
                    $i++;
                }
        ...
        $result = ['morning' => $isavailablemorning, 'afternoon' => $isavailableafternoon];
    ...
    public function selectStatutCP($selected = 0, $htmlname = 'select_statut', $morecss = 'minwidth125')
    {
        ...
        $name = ['DraftCP', 'ToReviewCP', 'ApprovedCP', 'CancelCP', 'RefuseCP'];
        $nb = count($name) + 1;
    ...
    public function LibStatut($status, $mode = 0, $startdate = '')
    {
        ...
        if (!empty($startdate) && $startdate >= dol_now()) {        // If not yet passed, we use a green "in live" color
            $statusType = 'status4';
            $params = ['tooltip' => $this->labelStatus[$status] . ' - ' . $langs->trans("Forthcoming")];
        }
