 ...
 */
class SupplierProposal extends GenericDocument
{
    use CommonIncoterm;
     ...
     */
    public $fk_element = 'fk_supplier_proposal'; // Id client
    /**
...
public $cond_reglement_doc;
    ...
    public function create($user, $notrigger = 0)
    {
        ...
        if (!empty($this->multicurrency_code)) {
            [$this->fk_multicurrency, $this->multicurrency_tx] = MultiCurrency::getIdAndTxFromCode($this->db, $this->multicurrency_code, $now);
        }
    ...
    public function updateOrCreatePriceFournisseur($user)
    {
        ...
        foreach ($this->lines as $product) {
            ...
            if (isModEnabled("multicurrency") && !empty($product->multicurrency_code)) {
                [$fk_multicurrency, $multicurrency_tx] = MultiCurrency::getIdAndTxFromCode($this->db, $product->multicurrency_code);
            }
            $productsupplier->id = $product->fk_product;

            $productsupplier->update_buyprice($product->qty, $product->total_ht, $user, 'HT', $this->thirdparty, '', $ref_fourn, $product->tva_tx, 0, 0, 0, $product->info_bits, '', '', [], '', $product->multicurrency_total_ht, 'HT', $multicurrency_tx, $product->multicurrency_code, '', '', '');
        }
