...
/**
 * Generic function that return javascript to add to a page to transform a common input field into an autocomplete
 * field by calling an Ajax page (ex: /societe/ajax/ajaxcompanies.php). The HTML field must be an input text with
 * id=search_$htmlname. This use the jQuery "autocomplete" function. If we want to use the select2, we must also
 * convert the input into select on funcntions that call this method.
 *
 * @param string $selected              Preselected value
 * @param string $htmlname              HTML name of input field
 * @param string $url                   Ajax Url to call for request: /path/page.php. Must return a json array
 *                                      ('key'=>id, 'value'=>String shown into input field once selected,
 *                                      'label'=>String shown into combo list)
 * @param string $urloption             More parameters on URL request
 * @param int    $minLength             Minimum number of chars to trigger that Ajax search
 * @param int    $autoselect            Automatic selection if just one value (trigger("change") on field is done if
 *                                      search return only 1 result)
 * @param array  $ajaxoptions           Multiple options array
 *                                      - Ex: array('update'=>array('field1','field2'...)) will reset field1 and field2
 *                                      once select done
 *                                      - Ex: array('disabled'=> )
 *                                      - Ex: array('show'=> )
 *                                      - Ex: array('update_textarea'=> )
 *                                      - Ex: array('option_disabled'=> id to disable and warning to show if we select
 *                                      a disabled value (this is possible when using autocomplete ajax)
...
/**
 *  Generic function that return javascript to add to a page to transform a common input field into an autocomplete
 *  field by calling an Ajax page (ex: core/ajax/ziptown.php). The Ajax page can also returns several values (json
 *  format) to fill several input fields. The HTML field must be an input text with id=$htmlname. This use the jQuery
 ...
 * TODO: It is used when COMPANY_USE_SEARCH_TO_SELECT and CONTACT_USE_SEARCH_TO_SELECT are set by
 * html.formcompany.class.php. Should use ajax_autocompleter instead like done by html.form.class.php for
 * select_produits.
 *
 * @param string $htmlname                Name of html select field ('myid' or '.myclass')
 * @param array  $events                  More events option. Example: array(array('method'=>'getContacts',
 ...
 *
 * @return  string                              Return html string to convert a select field into a combo, or '' if
 *                                              feature has been disabled for some reason.
 ...
 *
 * @param string $htmlname                      Name of html select field ('myid' or '.myclass')
 * @param array  $events                        Add some Ajax events option on change of $htmlname component to call
 *                                              ajax to autofill a HTML element (select#htmlname and
 *                                              #inputautocompletehtmlname) Example:
 ...
 * @param array    $input               Array of complementary actions to do if success
 *                                      ("disabled"|"enabled'|'set'|'del') => CSS element to switch, 'alert' => message
 *                                      to show, ... Example: array('disabled'=>array(0=>'cssid'))
 ...
 * @param int      $forcereload         Force to reload page if we click/change value (this is supported only when
 *                                      there is no 'alert' option in input)
 * @param int      $marginleftonlyshort 1 = Add a short left margin on picto, 2 = Add a larger left margin on picto, 0
 *                                      = No left margin.
 ...
 * @return string
 */
function ajax_constantonoff($code, $input = [], $entity = null, $revertonoff = 0, $strict = 0, $forcereload = 0, $marginleftonlyshort = 2, $forcenoajax = 0, $setzeroinsteadofdel = 0, $suffix = '', $mode = '', $morecss = 'inline-block')
{
 ...
 * @param string $morecss   More CSS
 * @param string $htmlname  Name of HTML component. Keep '' or use a different value if you need to use this component
 *                          several time on the same page for the same field.
 ...
 */
function ajax_object_onoff($object, $code, $field, $text_on, $text_off, $input = [], $morecss = '', $htmlname = '', $forcenojs = 0)
{
    global $conf, $langs;
