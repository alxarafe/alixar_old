 ...
 *                                             by doing preg_quote($var,'/'), since this char is used for preg_match
 *                                             function, but must not contains the start and end '/'. Filter is checked
 *                                             into basename only.
 * @param string|string[] $excludefilter       Array of Regex for exclude filter (example:
 *                                             array('(\.meta|_preview.*\.png)$','^\.')). Exclude is checked both into
 *                                             fullpath and into basename (So '^xxx' may exclude 'xxx/dirscanned/...'
 *                                             and dirscanned/xxx').
 ...
 * @param int             $mode                0=Return array minimum keys loaded (faster), 1=Force all keys like date
 *                                             and size to be loaded (slower), 2=Force load of date only, 3=Force load
 *                                             of size only, 4=Force load of perm
...
function dol_dir_list($utf8_path, $types = "all", $recursive = 0, $filter = "", $excludefilter = null, $sortcriteria = "name", $sortorder = SORT_ASC, $mode = 0, $nohook = 0, $relativename = "", $donotfollowsymlinks = 0, $nbsecondsold = 0)
{
    ...
    if ($recursive <= 1) {  // Avoid too verbose log
        ...
        $exclude_array = ($excludefilter === null || $excludefilter === '') ? [] : (is_array($excludefilter) ? $excludefilter : [$excludefilter]);
    ...
    $excludefilterarray = array_merge(['^\.'], $exclude_array);
        ...
        if (($dir = opendir($os_path)) === false) {
            ...
            while (false !== ($os_file = readdir($dir))) {        // $utf8_file is always a basename (in directory $os_path)
                ...
                if ($qualified) {
                    ...
                    } elseif (in_array($types, ["files", "all"])) {
 ...
 * @param string        $filter        Regex filter to restrict list. This regex value must be escaped for '/', since
 *                                     this char is used for preg_match function
 ...
 *
 * @return  array<array{rowid:string,label:string,name:string,path:string,level1name:string,fullname:string,fullpath_orig:string,date_c:string,date_m:string,type:string,keywords:string,cover:string,position:int,acl:string,share:string,description:string}> Array
 *                                                                                                                                                                                                                                                              of
 *                                                                                                                                                                                                                                                              array('name'=>'xxx','fullname'=>'/abc/xxx','date'=>'yyy','size'=>99,'type'=>'dir|file',...)
 * @see dol_dir_list()
 */
 ...
 * @param string               $destfile                Destination file (can't be a directory). If empty, will be same
 *                                                      than source file.
 ...
 * @param int                  $arrayreplacementisregex 1=Array of replacement is already an array with key that is a
 *                                                      regex. Warning: the key must be escaped with preg_quote for '/'
 *
 * @return  int                                                Return integer <0 if error, 0 if nothing done (dest file
 *                                                             already exists), >0 if OK
 * @see     dol_copy()
 */
 ...
 * @param int                  $overwriteifexists Overwrite file if exists (1 by default)
 * @param array<string,string> $arrayreplacement  Array to use to replace filenames with another one during the copy
 *                                                (works only on file names, not on directory names).
 * @param int                  $excludesubdir     0=Do not exclude subdirectories, 1=Exclude subdirectories, 2=Exclude
 *                                                subdirectories if name is not a 2 chars (used for country codes
 *                                                subdirectories).
 * @param string[]             $excludefileext    Exclude some file extensions
 *
 * @return  int                                         Return integer <0 if error, 0 if nothing done (all files
 *                                                      already exists and overwriteifexists=0), >0 if OK
 * @see     dol_copy()
 */
 ...
 *  - Extension .noexe may be added if file is executable and MAIN_DOCUMENT_IS_OUTSIDE_WEBROOT_SO_NOEXE_NOT_REQUIRED is
 *  not set.
 *
 ...
 *
 * @return boolean                 True if no error (file is deleted or if glob is used and there's nothing to delete),
 *                                 False if error
 * @see dol_delete_dir()
 */
 ...
 *
 * @return int                     Number of files and directory we try to remove. NB really removed is returned into
 *                                 var by reference $countdeleted.
 */
...
/**
 * Get and save an upload file (for example after submitting a new file a mail form). Database index of file is also
 ...
 * @param int    $donotupdatesession 1=Do no edit _SESSION variable but update database index. 0=Update _SESSION and
 ...
 * @param int<0,3> $mode          0=Return array minimum keys loaded (faster), 1=Force all keys like date and size to
 *                                be loaded (slower), 2=Force load of date only, 3=Force load of size only
 *
 * @return  null|array{name:string,path:string,level1name:string,relativename:string,fullname:string,date:string,size:int,perm:int,type:string} null
 *                                                                                                                                              if
 *                                                                                                                                              none
 *                                                                                                                                              or
 *                                                                                                                                              Array
 ...
 */
function dol_most_recent_file($dir, $regexfilter = '', $excludefilter = ['(\.meta|_preview.*\.png)$', '^\.'], $nohook = 0, $mode = 0)
{
 ...
 *                                 Example: 'medias', 'invoice', 'logs', 'tax-vat', ...
 * @param string    $original_file Relative path with filename, relative to modulepart.
 * @param string    $entity        Restrict onto entity (0=no restriction)
 * @param User|null $fuser         User object (forced)
 * @param string    $refname       Ref of object to check permission for external users (autodetect if not provided by
 *                                 taking the dirname of $original_file) or for hierarchy
