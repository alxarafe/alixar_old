...
class FormMargin
{
    ...
    public function displayMarginInfos($object, $force_price = false)
    {
        ...
        $parameters = ['marginInfo' => &$marginInfo];
    ...
    public function getMarginInfosArray($object, $force_price = false)
    {
        ...
        foreach ($object->lines as $line) {
            ...
            $pa_ht = (($pv < 0 || ($pv == 0 && in_array($object->element, ['facture', 'facture_fourn']) && $object->type == $object::TYPE_CREDIT_NOTE)) ? -$line->pa_ht : $line->pa_ht);
            if (getDolGlobalInt('INVOICE_USE_SITUATION') == 1) {    // Special case for old situation mode
                // @phan-suppress-next-line PhanUndeclaredConstantOfClass
                if (
                    ($object->element == 'facture' && $object->type == $object::TYPE_SITUATION)
                    // @phan-suppress-next-line PhanUndeclaredConstantOfClass
