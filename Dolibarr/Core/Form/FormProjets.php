...
class FormProjets extends Form
{
     ...
     *
     * @param int            $socid          Id third party (-1=all, 0=only projects not linked to a third party,
     *                                       id=projects not linked or linked to third party id)
     * @param string|Project $selected       Id of preselected project or Project (or ''). Note: If you know the ref,
     *                                       you can also provide it into $selected_input_value to save one request in
     *                                       some cases.
     * @param string         $htmlname       Name of HTML field
     * @param int            $maxlength      Maximum length of label
     * @param int            $option_only    Return only html options lines without the select tag
     * @param int|string     $show_empty     Add an empty line
     * @param int            $discard_closed Discard closed projects (0=Keep, 1=hide completely, 2=Disable). Use a
     *                                       negative value to not show the "discarded" tooltip.
     ...
     * @param string $showmore       'all' = Show project info, 'progress' = Show task progression, ''=Show nothing
     *                               more
     ...
     * @param string     $morecss       More CSS
     * @param int        $limitonstatus Add filters to limit length of list to opened status (for example to avoid
     *                                  ERR_RESPONSE_HEADERS_TOO_BIG on project/element.php page). TODO To implement
    ...
    public function select_element($table_element, $socid = 0, $morecss = '', $limitonstatus = -2, $projectkey = "fk_projet", $placeholder = '')
    {
        ...
        if (!in_array($table_element, ['expensereport_det', 'stock_mouvement'])) {
    ...
    public function selectInvoiceAndLine($selectedInvoiceId = 0, $selectedLineId = 0, $htmlNameInvoice = 'invoiceid', $htmlNameInvoiceLine = 'invoicelineid', $morecss = 'maxwidth500', $filters = [], $lineOnly = 0)
    {
        ...
        if (empty($lineOnly)) {
            ...
            if ($resql) {
                ...
                if (!empty($conf->use_javascript_ajax)) {
                    include_once BASE_PATH . '/../Dolibarr/Lib/Ajax.php';
                    $comboenhancement = ajax_combobox($htmlNameInvoice, [['method' => 'getLines', 'url' => dol_buildpath('/core/ajax/ajaxinvoiceline.php', 1), 'htmlname' => $htmlNameInvoiceLine]], 0, 0);
                    $out .= $comboenhancement;
