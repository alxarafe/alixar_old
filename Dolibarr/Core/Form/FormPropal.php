...
class FormPropal
{
    ...
    public function selectProposalStatus($selected = '', $short = 0, $excludedraft = 0, $showempty = 1, $mode = 'customer', $htmlname = 'propal_statut', $morecss = '')
    {
        ...
        if ($mode == 'supplier') {
            ...
            $listofstatus = [
                0 => ['id' => 0, 'code' => 'PR_DRAFT'],
                1 => ['id' => 1, 'code' => 'PR_OPEN'],
                2 => ['id' => 2, 'code' => 'PR_SIGNED'],
                3 => ['id' => 3, 'code' => 'PR_NOTSIGNED'],
                4 => ['id' => 4, 'code' => 'PR_CLOSED'],
            ];
        } else {
            $prefix = "PropalStatus";
            ...
            if ($resql) {
                ...
                if ($num) {
                    while ($i < $num) {
                        ...
                        $listofstatus[$obj->id] = ['id' => $obj->id, 'code' => $obj->code, 'label' => $obj->label];
                        $i++;
                    }
                }
            } else {
        ...
        foreach ($listofstatus as $key => $obj) {
            ...
            if ($langs->trans($prefix . $key . ($short ? 'Short' : '')) != $prefix . $key . ($short ? 'Short' : '')) {
                ...
                $conv_to_new_code = ['PR_DRAFT' => 'Draft', 'PR_OPEN' => 'Validated', 'PR_CLOSED' => 'Closed', 'PR_SIGNED' => 'Signed', 'PR_NOTSIGNED' => 'NotSigned', 'PR_FAC' => 'Billed'];
        ...
        print ajax_combobox($htmlname, [], 0, 0, 'resolve', ($showempty < 0 ? (string) $showempty : '-1'), $morecss);
    }
}
