 ...
 *
 * TODO Merge all function load_cache_* and loadCache* (except load_cache_vatrates) into one generic function
 * loadCacheTable
 */
class Form
{
    ...
    // Cache arrays
    public $cache_types_paiements = [];
    public $cache_conditions_paiements = [];
    public $cache_transport_mode = [];
    public $cache_availability = [];
    public $cache_demand_reason = [];
    public $cache_types_fees = [];
    public $cache_vatrates = [];
    public $cache_invoice_subtype = [];
    ...
    /**
     *    Return a HTML select string, built from an array of key+value, but content returned into select come from an
     *    Ajax call of an URL. Note: Do not apply langs->trans function on returned content of Ajax service, content
     *    may be entity encoded twice.
     *
     * @param string  $htmlname           Name of html select area
     * @param string  $url                Url. Must return a json_encode of array(key=>array('text'=>'A text',
     *                                    'url'=>'An url'), ...)
     ...
     * @param int     $callurlonselect    If set to 1, some code is added so an url return by the ajax is called when
     *                                    value is selected.
     * @param string  $placeholder        String to use as placeholder
     * @param integer $acceptdelayedhtml  1 = caller is requesting to have html js content not returned but saved into
     *                                    global $delayedhtmlcontent (so caller can show it at end of page to avoid
     *                                    flash FOUC effect)
     *
    ...
    /**
     *  Return a HTML select string, built from an array of key+value, but content returned into select is defined into
     *  $array parameter. Note: Do not apply langs->trans function on returned content of Ajax service, content may be
    ...
    /**
     * Show a multiselect dropbox from an array. If a saved selection of fields exists for user (into
     * $user->conf->MAIN_SELECTEDFIELDS_contextofpage), we use this one instead of default.
     *
     * @param string $htmlname Name of HTML field
     * @param array  $array    Array with array of fields we could show. This array may be modified according to setup
     *                         of user.
     * @param string $varpage  Id of context for page. Can be set by caller with
     ...
     * @param string $forcecapture            '', 'user' or 'environment'. Force parameter capture on HTML input file
     *                                        element to ask a smartphone to allow to open camera to take photo. Auto
     *                                        if ''.
    ...
    public static function showphoto($modulepart, $object, $width = 100, $height = 0, $caneditfield = 0, $cssclass = 'photowithmargin', $imagesize = '', $addlinktofullsize = 1, $cache = 0, $forcecapture = '', $noexternsourceoverwrite = 0)
    {
        ...
        if ($dir) {
            if ($file && file_exists($dir . "/" . $file)) {
                ...
                if (in_array($modulepart, ['societe', 'userphoto', 'contact', 'memberphoto'])) {    // For modules that need a special image when photo not found
     ...
     * @param boolean $perm          Permission to allow button to edit parameter. Set it to 0 to have a not edited
     *                               field.
     * @param string  $typeofdata    Type of data ('string' by default, 'email', 'amount:99', 'numeric:99', 'text' or
     *                               'textarea:rows:cols', 'datepicker' ('day' do not work, don't know why), 'dayhour'
     *                               or 'datehourpicker'
     *                               'checkbox:ckeditor:dolibarr_zzz:width:height:savemethod:1:rows:cols',
     *                               'select;xxx[:class]'...)
     ...
     *                               ':', 3=Do not output table tags but output a ' '
     ...
     * @param string $type             Type of picto ('info', 'infoclickable', 'help', 'helpclickable', 'warning',
     *                                 'superadmin', 'mypicto@mymodule', ...) or image filepath or 'none'
     * @param string $extracss         Add a CSS style to td, div or span tag
     * @param int    $noencodehtmltext Do not encode into html entity the htmltext
     * @param int    $notabs           0=Include table and tr tags, 1=Do not include table and tr tags, 2=use div,
     *                                 3=use span
     * @param string $tooltiptrigger   ''=Tooltip on hover and hidden on smartphone, 'abconsmartphone'=Tooltip on hover
     *                                 and on click on smartphone, 'abc'=Tooltip on click (abc is a unique key,
     *                                 clickable link is on image or on link if param $type='none' or on both if
     *                                 $type='xxxclickable')
     ...
     * @param string        $typeofdata  Type of data ('string' by default, 'checkbox', 'email', 'phone', 'amount:99',
     *                                   'numeric:99',
     *                                   'text' or 'textarea:rows:cols%', 'safehtmlstring', 'restricthtml',
     *                                   'datepicker' ('day' do not work, don't know why), 'dayhour' or
     *                                   'datehourpicker',
     *                                   'ckeditor:dolibarr_zzz:width:height:savemethod:toolbarstartexpanded:rows:cols',
     *                                   'select;xkey:xval,ykey:yval,...')
     * @param string        $editvalue   When in edit mode, use this value as $value instead of value (for example, you
     *                                   can provide here a formatted price instead of numeric value, or a select
     *                                   combo). Use '' to use same than $value
     * @param ?CommonObject $extObject   External object ???
     * @param mixed         $custommsg   String or Array of custom messages : eg array('success' => 'MyMessage',
     *                                   'error' => 'MyMessage')
     * @param string        $moreparam   More param to add on the form on action href URL parameter
     * @param int           $notabletag  Do no output table tags
     * @param string        $formatfunc  Call a specific method of $object->$formatfunc to output field in view mode
     *                                   (For example: 'dol_print_email')
     * @param string        $paramid     Key of parameter for id ('id', 'socid')
     ...
     *                                   array('valuealreadyhtmlescaped'=>1)
     * @param string        $editaction  [=''] use GETPOST default action or set action to edit mode
     *
     * @return string                   HTML edit field
     */
    public function editfieldval($text, $htmlname, $value, $object, $perm, $typeofdata = 'string', $editvalue = '', $extObject = null, $custommsg = null, $moreparam = '', $notabletag = 1, $formatfunc = '', $paramid = 'id', $gm = 'auto', $moreoptions = [], $editaction = '')
    {
        global $conf, $langs;
     ...
     *                                   know why), 'textarea:rows:cols',
     *                                   'ckeditor:dolibarr_zzz:width:height:?:1:rows:cols',
     *                                   'select:loadmethod:savemethod:buttononly')
     ...
     *              - set_time date (must be a local PHP server timestamp or string date with format 'YYYY-MM-DD' or
     *              'YYYY-MM-DD HH:MM')
     ...
     *
     * @param integer|string   $set_time       Pre-selected date (must be a local PHP server timestamp), -1 to keep
     *                                         date not preselected, '' to use current date with 00:00 hour (Parameter
     *                                         'empty' must be 0 or 2).
     * @param string           $prefix         Prefix for fields name
     * @param int              $h              1 or 2=Show also hours (2=hours on a new line), -1 has same effect but
     *                                         hour and minutes are prefilled with 23:59 if date is empty, 3 show hour
     *                                         always empty
     * @param int              $m              1=Show also minutes, -1 has same effect but hour and minutes are
     *                                         prefilled with 23:59 if date is empty, 3 show minutes always empty
     * @param int              $empty          0=Fields required, 1=Empty inputs are allowed, 2=Empty inputs are
     *                                         allowed for hours only
     * @param string           $form_name      Not used
     * @param int              $d              1=Show days, month, years
     * @param int              $addnowlink     Add a link "Now", 1 with server time, 2 with local computer time
     * @param int              $disabled       Disable input fields
     * @param int|string       $fullday        When a checkbox with id #fullday is checked, hours are set with 00:00
     *                                         (if value if 'fulldaystart') or 23:59 (if value is 'fulldayend')
     * @param string           $addplusone     Add a link "+1 hour". Value must be name of another selectDate field.
     * @param int|string|array $adddateof      Add a link "Date of ..." using the following date. Must be
     *                                         array(array('adddateof'=>..., 'labeladddateof'=>...))
     * @param string           $openinghours   Specify hour start and hour end for the select ex 8,20
     * @param int              $stepminutes    Specify step for minutes between 1 and 30
     * @param string           $labeladddateof Label to use for the $adddateof parameter. Deprecated. Used only when
     *                                         $adddateof is not an array.
     * @param string           $placeholder    Placeholder
     * @param mixed            $gm             'auto' (for backward compatibility, avoid this), 'gmt' or 'tzserver' or
     *                                         'tzuserrel'
     *
    ...
    public function selectDate($set_time = '', $prefix = 're', $h = 0, $m = 0, $empty = 0, $form_name = "", $d = 1, $addnowlink = 0, $disabled = 0, $fullday = '', $addplusone = '', $adddateof = '', $openinghours = '', $stepminutes = 1, $labeladddateof = '', $placeholder = '', $gm = 'auto')
    {
        ...
        if ($conf->use_javascript_ajax && !empty($adddateof)) {
            if (!is_array($adddateof)) {
                $arrayofdateof = [['adddateof' => $adddateof, 'labeladddateof' => $labeladddateof]];
            } else {
     ...
     *
     * @param string          $htmlname         Name of html select area. Try to start name with "multi" or
     *                                          "search_multi" if this is a multiselect
     * @param array           $array            Array like array(key => value) or array(key=>array('label'=>...,
     *                                          'data-...'=>..., 'disabled'=>..., 'css'=>...))
     * @param string|string[] $id               Preselected key or preselected keys for multiselect. Use 'ifone' to
     *                                          autoselect record if there is only one record.
     * @param int|string      $show_empty       0 no empty value allowed, 1 or string to add an empty value into list
     *                                          (If 1: key is -1 and value is '' or '&nbsp;', If placeholder string:
     *                                          key is -1 and value is the string), <0 to add an empty value with key
     *                                          that is this value.
     * @param int             $key_in_label     1 to show key into label with format "[key] value"
     * @param int             $value_as_key     1 to use value as key
     * @param string          $moreparam        Add more parameters onto the select tag. For example 'style="width:
     *                                          95%"' to avoid select2 component to go over parent container
     * @param int             $translate        1=Translate and encode value
     * @param int             $maxlen           Length maximum for labels
     * @param int             $disabled         Html select box is disabled
     * @param string          $sort             'ASC' or 'DESC' = Sort on label, '' or 'NONE' or 'POS' = Do not sort,
     *                                          we keep original order
     * @param string          $morecss          Add more class to css styles
     * @param int             $addjscombo       Add js combo
     * @param string          $moreparamonempty Add more param on the empty option line. Not used if show_empty not set
     * @param int             $disablebademail  1=Check if a not valid email, 2=Check string '---', and if found into
     *                                          value, disable and colorize entry
     * @param int             $nohtmlescape     No html escaping (not recommended, use 'data-html' if you need to use
     ...
     * @param string  $typeofdata Type of data ('string' by default, 'email', 'amount:99', 'numeric:99', 'text' or
     *                            'textarea:rows:cols', 'datepicker' ('day' do not work, don't know why),
     *                            'ckeditor:dolibarr_zzz:width:height:savemethod:1:rows:cols', 'select;xxx[:class]'...)
     ...
     *
     * @param string $selected      Value auto selected when at least one record is selected. Not a preselected value.
     *                              Use '0' by default.
     * @param array  $arrayofaction array('code'=>'label', ...). The code is the key stored into the
     *                              GETPOST('massaction') when submitting action.
     ...
     * @param string     $usecodeaskey         ''=Use id as key (default), 'code3'=Use code on 3 alpha as key,
     *                                         'code2"=Use code on 2 alpha as key
     * @param int|string $showempty            Show empty choice
     * @param int        $disablefavorites     1=Disable favorites,
     * @param int        $addspecialentries    1=Add dedicated entries for group of countries (like 'European Economic
     *                                         Community', ...)
     ...
     */
    public function select_country($selected = '', $htmlname = 'country_id', $htmloption = '', $maxlength = 0, $morecss = 'minwidth300', $usecodeaskey = '', $showempty = 1, $disablefavorites = 0, $addspecialentries = 0, $exclude_country_code = [], $hideflags = 0)
    {
        // phpcs:enable
     ...
     * @param int    $forcecombo          Force to load all values and output a standard combobox (with no
     *                                    beautification)
     * @param array  $events              Event options to run on change. Example: array(array('method'=>'getContacts',
     *                                    'url'=>dol_buildpath('/core/ajax/contacts.php',1), 'htmlname'=>'contactid',
     *                                    'params'=>array('add-customer-contact'=>'disabled')))
     * @param int    $disableautocomplete Disable autocomplete
     *
     * @return  string                          HTML string with select and input
     */
    public function select_incoterms($selected = '', $location_incoterms = '', $page = '', $htmlname = 'incoterm_id', $htmloption = '', $forcecombo = 1, $events = [], $disableautocomplete = 0)
    {
        // phpcs:enable
        global $conf, $langs;
     ...
     * @param int     $hidetext  Do not show label 'Type' before combo box (used only if there is at least 2 choices to
     *                           select)
     * @param integer $forceall  1=Force to show products and services in combo list, whatever are activated modules,
     *                           0=No force, 2=Force to show only Products, 3=Force to show only services, -1=Force
     *                           none (and set hidden field to 'service')
     *
     * @return  void
     */
     ...
     *                              3=add an empty value only if more than one record into list
     ...
     * @param array   $events       Event options. Example: array(array('method'=>'getContacts',
     *                              'url'=>dol_buildpath('/core/ajax/contacts.php',1), 'htmlname'=>'contactid',
     *                              'params'=>array('add-customer-contact'=>'disabled')))
     * @param bool    $options_only Return options only (for ajax treatment)
     * @param string  $moreparam    Add more parameters onto the select tag. For example 'style="width: 95%"' to avoid
     *                              select2 component to go over parent container
     ...
     */
    public function select_contacts($socid, $selected = '', $htmlname = 'contactid', $showempty = 0, $exclude = '', $limitto = '', $showfunction = 0, $morecss = '', $showsoc = 0, $forcecombo = 0, $events = [], $options_only = false, $moreparam = '', $htmlid = '')
    {
        // phpcs:enable
     ...
     * @param string     $htmlname       Name of HTML field ('none' for a not editable field)
     * @param int|string $showempty      0=no empty value, 1=add an empty value, 2=add line 'Internal' (used by user
     *                                   edit), 3=add an empty value only if more than one record into list
     * @param string     $exclude        List of contacts id to exclude
     * @param string     $limitto        Disable answers that are not id in this array list
     * @param integer    $showfunction   Add function into label
     * @param string     $morecss        Add more class to class style
     ...
     *                                   'url'=>dol_buildpath('/core/ajax/contacts.php',1), 'htmlname'=>'contactid',
     *                                   'params'=>array('add-customer-contact'=>'disabled')))
     * @param string     $moreparam      Add more parameters onto the select tag. For example 'style="width: 95%"' to
     *                                   avoid select2 component to go over parent container
     * @param string     $htmlid         Html id to use instead of htmlname
     ...
     *
     */
    public function selectcontacts($socid, $selected = [], $htmlname = 'contactid', $showempty = 0, $exclude = '', $limitto = '', $showfunction = 0, $morecss = '', $options_only = false, $showsoc = 0, $forcecombo = 0, $events = [], $moreparam = '', $htmlid = '', $multiple = false, $disableifempty = 0)
    {
     ...
     * @param array|string $include      Array list of users id to include. User '' for all users or 'hierarchy' to
     *                                   have only supervised users or 'hierarchyme' to have supervised + me
     ...
     *
     * @param string|int   $selected       User id or user object of user preselected. If 0 or < -2, we use id of
     *                                     current user. If -1 or '', keep unselected (if empty is allowed)
     * @param string       $htmlname       Field name in form
     ...
     * @param array|string $include        Array list of users id to include. User '' for all users or 'hierarchy' to
     *                                     have only supervised users or 'hierarchyme' to have supervised + me
     * @param array|string $enableonly     Array list of users id to be enabled. If defined, it means that others will
     *                                     be disabled
     * @param string       $force_entity   '0' or list of Ids of environment to force separated by a coma
     * @param int          $maxlength      Maximum length of string into list (0=no limit)
     * @param int          $showstatus     0=show user status only if status is disabled, 1=always show user status
     *                                     into label, -1=never show user status
     * @param string       $morefilter     Add more filters into sql request (Example: 'employee = 1'). This value must
     *                                     not come from user input.
     * @param integer      $show_every     0=default list, 1=add also a value "Everybody" at beginning of list
     * @param string       $enableonlytext If option $enableonlytext is set, we use this text to explain into label why
     *                                     record is disabled. Not used if enableonly is empty.
     * @param string       $morecss        More css
     * @param int          $notdisabled    Show only active users (this will also happened whatever is this option if
     *                                     USER_HIDE_INACTIVE_IN_COMBOBOX is on).
    ...
    public function select_dolusers($selected = '', $htmlname = 'userid', $show_empty = 0, $exclude = null, $disabled = 0, $include = '', $enableonly = '', $force_entity = '', $maxlength = 0, $showstatus = 0, $morefilter = '', $show_every = 0, $enableonlytext = '', $morecss = '', $notdisabled = 0, $outputmode = 0, $multiple = false, $forcecombo = 0)
    {
        ...
        $reshook = $hookmanager->executeHooks('addSQLWhereFilterOnSelectUsers', [], $this, $action);
        if (!empty($reshook)) {
            $sql .= $hookmanager->resPrint;
        }
     ...
     * @param string    $force_entity    '0' or Ids of environment to force
     * @param int       $maxlength       Maximum length of string into list (0=no limit)
     * @param int       $showstatus      0=show user status only if status is disabled, 1=always show user status into
     *                                   label, -1=never show user status
     ...
     * @see select_dolgroups()
     */
    public function select_dolusers_forevent($action = '', $htmlname = 'userid', $show_empty = 0, $exclude = null, $disabled = 0, $include = [], $enableonly = [], $force_entity = '0', $maxlength = 0, $showstatus = 0, $morefilter = '', $showproperties = 0, $listofuserid = [], $listofcontactid = [], $listofotherid = [])
    {
        // phpcs:enable
        global $langs;
     ...
     * @return    string                    HTML select string
     */
    public function select_dolresources_forevent($action = '', $htmlname = 'userid', $show_empty = 0, $exclude = null, $disabled = 0, $include = [], $enableonly = [], $force_entity = '0', $maxlength = 0, $showstatus = 0, $morefilter = '', $showproperties = 0, $listofresourceid = [])
    {
        // phpcs:enable
        global $langs;
     ...
     * @param int        $status                        Sell status: -1=No filter on sell status, 0=Products not on
     *                                                  sell, 1=Products on sell
     * @param int        $finished                      2=all, 1=finished, 0=raw material
     * @param string     $selected_input_value          Value of preselected input text (for use with ajax)
     * @param int        $hidelabel                     Hide label (0=no, 1=yes, 2=show search icon (before) and
     *                                                  placeholder, 3 search icon after)
     * @param array      $ajaxoptions                   Options for ajax_autocompleter
     * @param int        $socid                         Thirdparty Id (to get also price dedicated to this customer)
     * @param string     $showempty                     '' to not show empty line. Translation key to show an empty
     *                                                  line. '1' show empty line with no text.
     * @param int        $forcecombo                    Force to use combo box
     * @param string     $morecss                       Add more css on select
     * @param int        $hidepriceinlabel              1=Hide prices in label
     * @param string     $warehouseStatus               Warehouse status filter to count the quantity in stock.
     *                                                  Following comma separated filter options can be used
     *                                                  'warehouseopen' = count products from open warehouses,
     *                                                  'warehouseclosed' = count products from closed warehouses,
     *                                                  'warehouseinternal' = count products from warehouses for
     *                                                  internal correct/transfer only
     * @param array      $selected_combinations         Selected combinations. Format: array([attrid] => attrval,
     *                                                  [...])
     * @param int        $nooutput                      No print if 1, return the output into a string
     * @param int        $status_purchase               Purchase status: -1=No filter on purchase status, 0=Products
     *                                                  not on purchase, 1=Products on purchase
     *
     * @return     void|string
     */
    public function select_produits($selected = 0, $htmlname = 'productid', $filtertype = '', $limit = 0, $price_level = 0, $status = 1, $finished = 2, $selected_input_value = '', $hidelabel = 0, $ajaxoptions = [], $socid = 0, $showempty = '1', $forcecombo = 0, $morecss = '', $hidepriceinlabel = 0, $warehouseStatus = '', $selected_combinations = null, $nooutput = 0, $status_purchase = -1)
    {
        // phpcs:enable
        global $langs, $conf;
     ...
     * @param int    $outputmode                    0=HTML select string, 1=Array
     * @param int    $socid                         Thirdparty Id (to get also price dedicated to this customer)
     * @param string $showempty                     '' to not show empty line. Translation key to show an empty line.
     *                                              '1' show empty line with no text.
     * @param int    $forcecombo                    Force to use combo box
     * @param string $morecss                       Add more css on select
     * @param int    $hidepriceinlabel              1=Hide prices in label
     * @param string $warehouseStatus               Warehouse status filter to group/count stock. Following comma
     *                                              separated filter options can be used.
     *                                              'warehouseopen' = count products from open warehouses,
     *                                              'warehouseclosed' = count products from closed warehouses,
     *                                              'warehouseinternal' = count products from warehouses for internal
     *                                              correct/transfer only
     * @param int    $status_purchase               Purchase status -1=Return all products, 0=Products not on purchase,
     *                                              1=Products on purchase
     *
    ...
    public function select_produits_list($selected = 0, $htmlname = 'productid', $filtertype = '', $limit = 20, $price_level = 0, $filterkey = '', $status = 1, $finished = 2, $outputmode = 0, $socid = 0, $showempty = '1', $forcecombo = 0, $morecss = '', $hidepriceinlabel = 0, $warehouseStatus = '', $status_purchase = -1)
    {
        ...
        $parameters = [];  // @phan-suppress-current-line PhanPluginRedundantAssignment
     ...
     * @param int    $novirtualstock   Do not load virtual stock, even if slow option
     *                                 STOCK_SHOW_VIRTUAL_STOCK_IN_PRODUCTS_COMBO is on.
     *
     * @return    void
     */
     ...
     * @param int    $type       type of the BOM (-1=Return all BOM, 0=Return disassemble BOM, 1=Return manufacturing
     *                           BOM)
     * @param string $showempty  '' to not show empty line. Translation key to show an empty line. '1' show empty line
     *                           with no text.
     * @param string $morecss    Add more css on select
     ...
     * @return    void
     */
    public function select_produits_fournisseurs($socid, $selected = '', $htmlname = 'productid', $filtertype = '', $filtre = '', $ajaxoptions = [], $hidelabel = 0, $alsoproductwithnosupplierprice = 0, $morecss = '', $placeholder = '')
    {
        // phpcs:enable
        global $langs, $conf;
     ...
     * @param string $selected                       Product price preselected (must be 'id' in
     *                                               product_fournisseur_price or 'idprod_IDPROD')
     * @param string $htmlname                       Name of HTML select
     * @param string $filtertype                     Filter on product type (''=nofilter, 0=product, 1=service)
            ...
            while ($i < $num) {
                ...
                if (!empty($objp->idprodfournprice)) {
                    ...
                    if ($objp->supplier_reputation) {
                        //TODO dictionary
                        $reputations = ['' => $langs->trans('Standard'), 'FAVORITE' => $langs->trans('Favorite'), 'NOTTHGOOD' => $langs->trans('NotTheGoodQualitySupplier'), 'DONOTORDER' => $langs->trans('DoNotOrderThisProductToThisSupplier')];
     ...
     *    Constant MAIN_DEFAULT_PAYMENT_TERM_ID can be used to set default value but scope is all application, probably
     *    not what you want. See instead to force the default value by the caller.
     *
     * @param int    $selected        Id of payment term to preselect by default
     * @param string $htmlname        Nom de la zone select
     * @param int    $filtertype      If > 0, include payment terms with deposit percentage (for objects other than
     ...
     * @param int    $addempty 1=Add an empty value in list, 2=Add an empty value in list only if there is more than 2
     *                         entries.
     *
     * @return    void
     */
     ...
     * @param int    $useempty    1=Add an empty value in list, 2=Add an empty value in list only if there is more than
     *                            2 entries.
     ...
     * @param string $filtre       To filter list. This parameter must not come from input of users
     * @param int    $useempty     1=Add an empty value in list, 2=Add an empty value in list only if there is more
     *                             than 2 entries.
     * @param string $moreattrib   To add more attribute on select
     ...
     *
     * @return int|string               If noouput=0: Return integer <0 if error, Num of bank account found if OK (0,
     *                                  1, 2, ...), If nooutput=1: Return a HTML select string.
     */
     ...
     * @param int|string $useajax        0=No, 1=Yes use Ajax to show the popup, 2=Yes and also submit page with
     *                                   &confirm=no if choice is No, 'xxx'=Yes and preoutput confirm box with div
     *                                   id=dialog-confirm-xxx
     ...
     *       print 'jQuery(".xxxlink").click(function(e) { jQuery("#aparamid").val(jQuery(this).attr("rel"));
     *       jQuery("#dialog-confirm-xxx").dialog("open"); return false; });'."\n"; print '});'."\n"; print
     *       '</script>'."\n";
     *
     * @param string       $page                Url of page to call if confirmation is OK. Can contains parameters
     *                                          (param 'action' and 'confirm' will be reformatted)
     * @param string       $title               Title
     * @param string       $question            Question
     * @param string       $action              Action
     * @param array|string $formquestion        An array with complementary inputs to add into forms:
     *                                          array(array('label'=> ,'type'=> , 'size'=>, 'morecss'=>,
     *                                          'moreattr'=>'autofocus' or 'style=...'))
     *                                          'type' can be 'text', 'password', 'checkbox', 'radio', 'date',
     *                                          'datetime', 'select', 'multiselect', 'morecss',
     *                                          'other', 'onecolumn' or 'hidden'...
     * @param int|string   $selectedchoice      '' or 'no', or 'yes' or '1', 1, '0' or 0
     * @param int|string   $useajax             0=No, 1=Yes use Ajax to show the popup, 2=Yes and also submit page with
     *                                          &confirm=no if choice is No, 'xxx'=Yes and preoutput confirm box with
     *                                          div id=dialog-confirm-xxx
     * @param int|string   $height              Force height of box (0 = auto)
     * @param int          $width               Force width of box ('999' or '90%'). Ignored and forced to 90% on
     *                                          smartphones.
     * @param int          $disableformtag      1=Disable form tag. Can be used if we are already inside a <form>
     *                                          section.
     * @param string       $labelbuttonyes      Label for Yes
     * @param string       $labelbuttonno       Label for No
     *
     * @return string                           HTML ajax code if a confirm ajax popup is required, Pure HTML code if
     *                                          it's an html form
     */
            ...
            foreach ($formquestion as $key => $input) {
                if (is_array($input) && !empty($input)) {
                    ...
                    if ($input['type'] == 'text') {
                        ...
                        $more .= '</div></div>' . "\n";
                        $formquestion[] = ['name' => $input['name'] . 'day'];
                        $formquestion[] = ['name' => $input['name'] . 'month'];
                        $formquestion[] = ['name' => $input['name'] . 'year'];
                        $formquestion[] = ['name' => $input['name'] . 'hour'];
                        $formquestion[] = ['name' => $input['name'] . 'min'];
     ...
     * @param array      $array        Array(key=>value) or Array(key=>array('id'=>key, 'label'=>value, 'color'=> ,
     *                                 'picto'=> , 'labelhtml'=> ))
     ...
     * @param int|string $width        Force width of select box. May be used only when using jquery couch. Example:
     *                                 250, '95%'
     * @param string     $moreattrib   Add more options on select component. Example: 'disabled'
     * @param string     $elemtype     Type of element we show ('category', ...). Will execute a formatting function on
     *                                 it. To use in readonly mode if js component support HTML formatting.
     * @param string     $placeholder  String to use as placeholder
     * @param int        $addjscombo   Add js combo
     *
     * @return  string                      HTML multiselect string
     ...
     */
    public static function multiselectarray($htmlname, $array, $selected = [], $key_in_label = 0, $value_as_key = 0, $morecss = '', $translate = 0, $width = 0, $moreattrib = '', $elemtype = '', $placeholder = '', $addjscombo = -1)
    {
        global $conf, $langs;
    ...
    public function selectyesno($htmlname, $value = '', $option = 0, $disabled = false, $useempty = 0, $addjscombo = 0, $morecss = '', $labelyes = 'Yes', $labelno = 'No')
    {
        ...
        if ($addjscombo) {
            $resultyesno .= ajax_combobox($htmlname, [], 0, 0, 'resolve', ($useempty < 0 ? (string) $useempty : '-1'), $morecss);
        }
     ...
     *
     * @param int    $page            Page
     * @param int    $socid           Id third party (-1=all, 0=only projects not linked to a third party, id=projects
     *                                not linked or linked to third party id)
     ...
     * @param int    $filtertype            If > 0, include payment terms with deposit percentage (for objects other
     *                                      than invoices and invoice templates)
     * @param int    $deposit_percent       < 0 : deposit_percent input makes no sense (for example, in list filters)
     *                                      0 : use default deposit percentage from entry
     *                                      > 0 : force deposit percentage (for example, from company object)
     * @param int    $nooutput              No print is done. String is returned.
    ...
    /**
     *  Output HTML form to select list of input reason (events that triggered an object creation, like after sending
     *  an emailing, making an advert, ...) List found into table c_input_reason loaded by loadCacheInputReason
     *
     * @param string $page     Page
     * @param string $selected Id condition pre-selectionne
     * @param string $htmlname Name of select html field
    ...
    /**
     *    Return list of input reason (events that triggered an object creation, like after sending an emailing, making
     *    an advert, ...) List found into table c_input_reason loaded by loadCacheInputReason
     *
     ...
     * @param string $htmlname   Name of select html field
     * @param string $filtertype To filter on field type in llx_c_paiement ('CRDT' or 'DBIT' or
     *                           array('code'=>xx,'label'=>zz))
     ...
     * Constant MAIN_DEFAULT_PAYMENT_TYPE_ID can used to set default value but scope is all application, probably not
     * what you want.
     *
     ...
     * @param bool    $excludeConfCurrency false = If company current currency not in table, we add it into list.
     *                                     Should always be available. true = we are in currency_rate update , we don't
     *                                     want to see conf->currency in select
     * @param string  $morecss             More css
     *
     * @return    string
     */
     ...
     * @param string $htmlname           Name of HTML select
     * @param string $filter             Optional filters criteras. WARNING: To avoid SQL injection, only few chars
     *                                   [.a-z0-9 =<>()] are allowed here (example: 's.rowid <> x', 's.client IN
     *                                   (1,3)'). Do not use a filter coming from input of users.
     * @param int    $showempty          Add an empty field
     ...
     * @return    string                        HTML output or ''
     */
    public function form_thirdparty($page, $selected = '', $htmlname = 'socid', $filter = '', $showempty = 0, $showtype = 0, $forcecombo = 0, $events = [], $nooutput = 0, $excludeids = [], $textifnothirdparty = '')
    {
        // phpcs:enable
        global $langs;
        ...
        if ($htmlname != "none") {
            ...
            $out .= '<input type="hidden" name="token" value="' . newToken() . '">';
            $out .= $this->select_company($selected, $htmlname, $filter, $showempty, $showtype, $forcecombo, $events, 0, 'minwidth100', '', '', 1, [], false, $excludeids);
            $out .= '<input type="submit" class="button smallpaddingimp valignmiddle" value="' . $langs->trans("Modify") . '">';
            $out .= '</form>';
        } else {
            if ($selected) {
     ...
     *                                     [.a-z0-9 =<>()] are allowed here (example: 's.rowid <> x', 's.client IN
     *                                     (1,3)'). Do not use a filter coming from input of users.
     * @param string $showempty            Add an empty field (Can be '1' or text key to use on empty line like
     *                                     'SelectThirdParty')
     * @param int    $showtype             Show third party type in combolist (customer, prospect or supplier)
     * @param int    $forcecombo           Force to load all values and output a standard combobox (with no
     *                                     beautification)
     * @param array  $events               Ajax event options to run on change. Example:
     *                                     array(array('method'=>'getContacts',
     *                                     'url'=>dol_buildpath('/core/ajax/contacts.php',1), 'htmlname'=>'contactid',
     *                                     'params'=>array('add-customer-contact'=>'disabled')))
     ...
     *                                     avoid select2 component to go over parent container
     * @param string $selected_input_value Value of preselected input text (for use with ajax)
     * @param int    $hidelabel            Hide label (0=no, 1=yes, 2=show search icon (before) and placeholder, 3
     *                                     search icon after)
     * @param array  $ajaxoptions          Options for ajax_autocompleter
     * @param bool   $multiple             add [] in the name of element and add 'multiple' attribute (not working with
     *                                     ajax_autocompleter)
     * @param array  $excludeids           Exclude IDs from the select combo
     ...
     */
    public function select_company($selected = '', $htmlname = 'socid', $filter = '', $showempty = '', $showtype = 0, $forcecombo = 0, $events = [], $limit = 0, $morecss = 'minwidth100', $moreparam = '', $selected_input_value = '', $hidelabel = 1, $ajaxoptions = [], $multiple = false, $excludeids = [], $showcode = 0)
    {
        // phpcs:enable
    ...
    /**
     *  Output html form to select a third party.
     *  Note, you must use the select_company to get the component to select a third party. This function must only be
     *  called by select_company.
     *
     * @param string $selected          Preselected type
     * @param string $htmlname          Name of field in form
     * @param string $filter            Optional filters criteras. WARNING: To avoid SQL injection, only few chars
     *                                  [.a-z0-9 =<>] are allowed here, example: 's.rowid <> x' If you need
     *                                  parenthesis, use the Universal Filter Syntax, example: '(s.client:in:1,3)' Do
     *                                  not use a filter coming from input of users.
     * @param string $showempty         Add an empty field (Can be '1' or text to use on empty line like
     *                                  'SelectThirdParty')
     * @param int    $showtype          Show third party type in combolist (customer, prospect or supplier)
     ...
     */
    public function select_thirdparty_list($selected = '', $htmlname = 'socid', $filter = '', $showempty = '', $showtype = 0, $forcecombo = 0, $events = [], $filterkey = '', $outputmode = 0, $limit = 0, $morecss = 'minwidth100', $moreparam = '', $multiple = false, $excludeids = [], $showcode = 0)
    {
        // phpcs:enable
        global $conf, $user, $langs;
        global $hookmanager;
            ...
            if ($num) {
                while ($i < $num) {
                    ...
                    if (empty($outputmode)) {
                        ...
                        array_push($outarray, ['key' => $obj->rowid, 'value' => $label, 'label' => $label, 'labelhtml' => $labelhtml]);
                    }
        ...
        $this->result = ['nbofthirdparties' => $num];
     ...
     *
     * @param string       $htmlname              Name of HTML select field
     * @param float|string $selectedrate          Force preselected vat rate. Can be '8.5' or '8.5 (NOO)' for example.
     *                                            Use '' for no forcing.
     ...
     *                                            If (seller country==buyer country), then default VAT=product's VAT.
     *                                            End of rule. If (seller and buyer in EU) and sold product = new means
     *                                            of transportation (car, boat, airplane), default VAT =0 (VAT must be
     *                                            paid by the buyer to his country's tax office and not the seller).
     *                                            End of rule. If (seller and buyer in EU) and buyer=private person,
     *                                            then default VAT=VAT of sold product.  End of rule. If (seller and
     *                                            buyer in EU) and buyer=company then default VAT =0. End of rule.
     *                                            Else, default proposed VAT==0. End of rule.
     * @param bool         $options_only          Return HTML options lines only (for ajax treatment)
     * @param int          $mode                  0=Use vat rate as key in combo list, 1=Add VAT code after vat rate
     *                                            into key, -1=Use id of vat line as key
     ...
     *                - Empty (fields empty), if set_time is -1 (in this case, parameter empty must also have value 1)
     *
     * @param integer|string $set_time   Pre-selected date (must be a local PHP server timestamp), -1 to keep date not
     *                                   preselected, '' to use current date with 00:00 hour (Parameter 'empty' must be
     *                                   0 or 2).
     * @param string         $prefix     Prefix for fields name
     * @param int            $h          1 or 2=Show also hours (2=hours on a new line), -1 has same effect but hour
     *                                   and minutes are prefilled with 23:59 if date is empty, 3 show hour always
     *                                   empty
     * @param int            $m          1=Show also minutes, -1 has same effect but hour and minutes are prefilled
     *                                   with 23:59 if date is empty, 3 show minutes always empty
     * @param int            $empty      0=Fields required, 1=Empty inputs are allowed, 2=Empty inputs are allowed for
     *                                   hours only
     * @param string         $form_name  Not used
     * @param int            $d          1=Show days, month, years
     ...
     * @param int            $disabled   Disable input fields
     * @param int            $fullday    When a checkbox with this html name is on, hour and day are set with 00:00 or
     *                                   23:59
     ...
     *              - Empty (fields empty), if set_time is -1 (in this case, parameter empty must also have value 1)
     *
     * @param integer|string $set_time     Pre-selected date (must be a local PHP server timestamp), -1 to keep date
     *                                     not preselected, '' to use current date with 00:00 hour (Parameter 'empty'
     *                                     must be 0 or 2).
     * @param integer|string $set_time_end Pre-selected date (must be a local PHP server timestamp), -1 to keep date
     *                                     not preselected, '' to use current date with 00:00 hour (Parameter 'empty'
     *                                     must be 0 or 2).
     * @param string         $prefix       Prefix for fields name
     * @param int            $empty        0=Fields required, 1=Empty inputs are allowed, 2=Empty inputs are allowed
     *                                     for hours only
     ...
     * @return  string                    HTML select string
     */
    public function selectTypeDuration($prefix, $selected = 'i', $excludetypes = [])
    {
        global $langs;
     ...
     * @param array  $ajaxoptions           Options for ajax_autocompleter
     * @param int    $socid                 Thirdparty Id (to get also price dedicated to this customer)
     * @param string $showempty             '' to not show empty line. Translation key to show an empty line. '1' show
     ...
     * @return        string
     */
    public function selectTickets($selected = '', $htmlname = 'ticketid', $filtertype = '', $limit = 0, $status = 1, $selected_input_value = '', $hidelabel = 0, $ajaxoptions = [], $socid = 0, $showempty = '1', $forcecombo = 0, $morecss = '', $selected_combinations = null, $nooutput = 0)
    {
        global $langs, $conf;
    ...
    protected function constructTicketListOption(&$objp, &$opt, &$optJson, $selected, $filterkey = '')
    {
        ...
        $opt .= "</option>\n";
        $optJson = ['key' => $outkey, 'value' => $outref, 'type' => $outtype];
    }
     ...
     * @return  string
     */
    public function selectProjects($selected = '', $htmlname = 'projectid', $filtertype = '', $limit = 0, $status = 1, $selected_input_value = '', $hidelabel = 0, $ajaxoptions = [], $socid = 0, $showempty = '1', $forcecombo = 0, $morecss = '', $selected_combinations = null, $nooutput = 0)
    {
        global $langs, $conf;
     ...
     * @return        string
     */
    public function selectMembers($selected = '', $htmlname = 'adherentid', $filtertype = '', $limit = 0, $status = 1, $selected_input_value = '', $hidelabel = 0, $ajaxoptions = [], $socid = 0, $showempty = '1', $forcecombo = 0, $morecss = '', $selected_combinations = null, $nooutput = 0)
    {
        global $langs, $conf;
    ...
    protected function constructMemberListOption(&$objp, &$opt, &$optJson, $selected, $filterkey = '')
    {
        ...
        $optJson = ['key' => $outkey, 'value' => $outlabel, 'type' => $outtype];
    }
     ...
     *
     * @param string $objectdesc           'ObjectClass:PathToClass[:AddCreateButtonOrNot[:Filter[:Sortfield]]]'. For
     *                                     hard coded custom needs. Try to prefer method using $objectfield instead of
     *                                     $objectdesc.
     * @param string $htmlname             Name of HTML select component
     * @param int    $preSelectedValue     Preselected value (ID of element)
     * @param string $showempty            ''=empty values not allowed, 'string'=value show if we allow empty values
     *                                     (for example 'All', ...)
     ...
     * @param string $selected_input_value Value of preselected input text (for use with ajax)
     * @param string $objectfield          Object:Field that contains the definition (in table $fields or
     *                                     $extrafields). Example: 'Object:xxx' or 'Module_Object:xxx' or
     *                                     'Object:options_xxx' or 'Module_Object:options_xxx'
     *
    ...
    public function selectForForms($objectdesc, $htmlname, $preSelectedValue, $showempty = '', $searchkey = '', $placeholder = '', $morecss = '', $moreparams = '', $forcecombo = 0, $disabled = 0, $selected_input_value = '', $objectfield = '')
    {
        ...
        $filter = str_replace(
            ['__ENTITY__', '__SHARED_ENTITIES__', '__USER_ID__'],
     ...
     * @param string $htmlname         Name of HTML select component
     * @param int    $preselectedvalue Preselected value (ID of element)
     * @param string $showempty        ''=empty values not allowed, 'string'=value show if we allow empty values (for
     *                                 example 'All', ...)
    ...
    public function selectForFormsList($objecttmp, $htmlname, $preselectedvalue, $showempty = '', $searchkey = '', $placeholder = '', $morecss = '', $moreparams = '', $forcecombo = 0, $outputmode = 0, $disabled = 0, $sortfield = '', $filter = '')
    {
        ...
        if (!empty($objecttmp->fields)) {    // For object that declare it, it is better to use declared fields (like societe, contact, ...)
            ...
            // For backward compatibility
            $objecttmp->fields['ref'] = ['type' => 'varchar(30)', 'label' => 'Ref', 'showoncombobox' => 1];
        }
            ...
            if ($num) {
                while ($i < $num) {
                    ...
                    if (empty($outputmode)) {
                        ...
                        array_push($outarray, ['key' => $obj->rowid, 'value' => $label, 'label' => $label]);
                    }
     ...
     * @param string $type       Type of category ('member', 'customer', 'supplier', 'product', 'contact'). Old mode
     *                           (0, 1, 2, ...) is deprecated.
     ...
     *
     * @param string|int       $type                Type of category ('customer', 'supplier', 'contact', 'product',
     *                                              'member'). Old mode (0, 1, 2, ...) is deprecated.
     * @param string           $selected            Id of category preselected or 'auto' (autoselect category if there
     *                                              is only one element). Not used if $outputmode = 1.
     * @param string           $htmlname            HTML field name
     * @param int              $maxlength           Maximum length for labels
     * @param int|string|array $markafterid         Keep only or removed all categories including the leaf $markafterid
     *                                              in category tree (exclude) or Keep only of category is inside the
     *                                              leaf starting with this id.
     *                                              $markafterid can be an :
     *                                              - int (id of category)
     *                                              - string (categories ids separated by comma)
     *                                              - array (list of categories ids)
     * @param int              $outputmode          0=HTML select string, 1=Array with full label only, 2=Array
     *                                              extended, 3=Array with full picto + label
        ...
        if ($type === Categorie::TYPE_BANK_LINE) {
            ...
            if ($result) {
                ...
                while ($i < $num) {
                    $objp = $this->db->fetch_object($result);
                    if ($objp) {
                        $cate_arbo[$objp->rowid] = ['id' => $objp->rowid, 'fulllabel' => $objp->label, 'color' => '', 'picto' => 'category'];
                    }
                    $i++;
                }
                $this->db->free($result);
     ...
     */
    public function showLinkedObjectBlock($object, $morehtmlright = '', $compatibleImportElementsList = [], $title = 'RelatedObjects')
    {
        global $conf, $langs, $hookmanager;
        global $bc, $action;
        ...
        $hookmanager->initHooks(['commonobject']);
        $parameters = [
        ...
        if (empty($reshook)) {
            ...
            foreach ($object->linkedObjects as $objecttype => $objects) {
                ...
                $dirtpls = array_merge($conf->modules_parts['tpl'], ['/' . $tplpath . '/tpl']);
     ...
     *
     * @param CommonObject $object          Object we want to show links to
     * @param array        $restrictlinksto Restrict links to some elements, for example array('order') or
     *                                      array('supplier_order'). null or array() if no restriction.
     * @param array        $excludelinksto  Do not show links of this type, for example array('order') or
     *                                      array('supplier_order'). null or array() if no exclusion.
     *
     * @return  string                              HTML block
     */
    public function showLinkToObjectBlock($object, $restrictlinksto = [], $excludelinksto = [])
    {
        global $conf, $langs, $hookmanager;
        global $action;
        ...
        if (!empty($listofidcompanytoscan)) {  // If empty, we don't have criteria to scan the object we can link to
            ...
            $hookmanager->initHooks(['commonobject']);
            $parameters = ['listofidcompanytoscan' => $listofidcompanytoscan, 'possiblelinks' => $possiblelinks];
     ...
     * Note: To complete search with a particular filter on select, you can set $object->next_prev_filter set to define
     * SQL criteria.
     *
     ...
     * @param string $fieldid        Name of field id into database to use for select next and previous (we make the
     *                               select max and min on this field compared to $object->ref). Use 'none' to disable
     *                               next/prev.
    ...
    public function showrefnav($object, $paramid, $morehtml = '', $shownav = 1, $fieldid = 'rowid', $fieldref = 'ref', $morehtmlref = '', $moreparam = '', $nodbprefix = 0, $morehtmlleft = '', $morehtmlstatus = '', $morehtmlright = '')
    {
        ...
        // Status
        $parameters = ['morehtmlstatus' => $morehtmlstatus];
        ...
        } elseif (in_array($object->element, ['contact', 'user'])) {
            $ret .= dol_htmlentities($object->getFullName($langs)) . $addgendertxt;
        } elseif ($object->element == 'usergroup') {
            $ret .= dol_htmlentities($object->name);
        } elseif (in_array($object->element, ['action', 'agenda'])) {
            $ret .= $object->ref . '<br>' . $object->label;
        } elseif (in_array($object->element, ['adherent_type'])) {
            $ret .= $object->label;
     ...
     * @param string              $force_entity '0' or Ids of environment to force
     * @param bool                $multiple     add [] in the name of element and add 'multiple' attribute (not working
     *                                          with ajax_autocompleter)
     * @param string              $morecss      More css to add to html component
     *
     * @return    string                            HTML Componont to select a group
     * @see select_dolusers()
     */
    public function select_dolgroups($selected = 0, $htmlname = 'groupid', $show_empty = 0, $exclude = '', $disabled = 0, $include = '', $enableonly = [], $force_entity = '0', $multiple = false, $morecss = '')
    {
        // phpcs:enable
        global $conf, $user, $langs;
     ...
     *
     * @param int    $addcheckuncheckall Add the check all/uncheck all checkbox (use javascript) and code to manage
     *                                   this
     * @param string $cssclass           CSS class
     * @param int    $calljsfunction     0=default. 1=call function initCheckForSelect() after changing status of
     *                                   checkboxes
     ...
     * @return    string
     */
    public function selectExpenseCategories($selected = '', $htmlname = 'fk_c_exp_tax_cat', $useempty = 0, $excludeid = [], $target = '', $default_selected = 0, $params = [], $info_admin = 1)
    {
        global $langs, $user;
     ...
     *
     * @param array  $arrayofcriterias                Array of available search criteria. Example:
     *                                                array($object->element => $object->fields, 'otherfamily' =>
     *                                                otherarrayoffields, ...)
     * @param array  $search_component_params         Array of selected search criteria
     * @param array  $arrayofinputfieldsalreadyoutput Array of input fields already inform. The component will not
     *                                                generate a hidden input field if it is in this list.
     * @param string $search_component_params_hidden  String with $search_component_params criteria
     *
     * @return  string                                      HTML component for advanced search
     */
    public function searchComponent($arrayofcriterias, $search_component_params, $arrayofinputfieldsalreadyoutput = [], $search_component_params_hidden = '')
    {
        global $langs;
        ...
        foreach ($arrayofcriterias as $criteria) {
            foreach ($criteria as $criteriafamilykey => $criteriafamilyval) {
                ...
                if (in_array($criteriafamilykey, ['rowid', 'ref_ext', 'entity', 'extraparams'])) {
                    continue;
                }
                if (in_array($criteriafamilyval['type'], ['date', 'datetime', 'timestamp'])) {
     ...
     * @param string $morecss         More CSS
     * @param string $dol_openinpopup If the button are shown in a context of a page shown inside a popup, we put here
     *                                the string name of popup.
     *
     * @return  string                      Html code with the buttons
     */
    public function buttonsSaveCancel($save_label = 'Save', $cancel_label = 'Cancel', $morebuttons = [], $withoutdiv = false, $morecss = '', $dol_openinpopup = '')
    {
        global $langs;
