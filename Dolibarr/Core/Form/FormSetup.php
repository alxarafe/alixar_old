...
class FormSetup
{
    ...
    {
        // Sorting
        return uasort($this->items, [$this, 'itemSort']);
    }
    ...
    public function exportItemsAsParamsArray()
    {
        $arrayofparameters = [];
        foreach ($this->items as $item) {
            $arrayofparameters[$item->confKey] = [
                'type' => $item->getType(),
...
class FormSetupItem
{
     ...
     * because this two class will quickly evolve it's important to not set or get directly $this->type (will be
     * protected) so this method exist to be sure we can manage evolution easily
     *
     * @return string
     */
     ...
     * because this two class will quickly evolve it's important to not set directly $this->type (will be protected) so
     * this method exist to be sure we can manage evolution easily
     *
     * @param string $type possible values based on old module builder setup : 'string', 'textarea',
     *                     'category:'.Categorie::TYPE_CUSTOMER', 'emailtemplate', 'thirdparty_type'
     *
     ...
     *
     * @param int $catType Type of category ('customer', 'supplier', 'contact', 'product', 'member'). Old mode (0, 1,
     *                     2, ...) is deprecated.
     *
     * @return self
     */
