...
class FormFile
{
     ...
     * @param int     $sectionid            If upload must be done inside a particular ECM section (is sectionid
     *                                      defined, sectiondir must not be)
     ...
     *                                      Deprecated 2 should never be used and if 1 is used, option should not be
     *                                      enabled.
     ...
     * @param string  $htmlname             Name and id of HTML form ('formuserfile' by default, 'formuserfileecm' when
     *                                      used to upload a file in ECM)
     * @param string  $accept               Specifies the types of files accepted (This is not a security check but an
     *                                      user interface facility. eg '.pdf,image/*' or '.png,.jpg' or 'video/*')
     * @param string  $sectiondir           If upload must be done inside a particular directory (if sectiondir
     *                                      defined, sectionid must not be)
     * @param int     $usewithoutform       0=Default, 1=Disable <form> and <input hidden> to use in existing form
     *                                      area, 2=Disable the tag <form> only
     * @param int     $capture              1=Add tag capture="capture" to force use of micro or video recording to
     *                                      generate file. When setting this to 1, you must also provide a value for
     *                                      $accept.
    ...
    public function form_attach_new_file($url, $title = '', $addcancel = 0, $sectionid = 0, $perm = 1, $size = 50, $object = null, $options = '', $useajax = 1, $savingdocmask = '', $linkfiles = 1, $htmlname = 'formuserfile', $accept = '', $sectiondir = '', $usewithoutform = 0, $capture = 0, $disablemulti = 0, $nooutput = 0)
    {
        ...
        if ((getDolGlobalString('MAIN_USE_JQUERY_FILEUPLOAD') && $useajax) || ($useajax == 2)) {
            ...
            $parameters = ['socid' => (isset($GLOBALS['socid']) ? $GLOBALS['socid'] : ''), 'id' => (isset($GLOBALS['id']) ? $GLOBALS['id'] : ''), 'url' => $url, 'perm' => $perm, 'options' => $options];
     ...
     * @param string  $modulesubdir     Sub-directory to scan (Example: '0/1/10', 'FA/DD/MM/YY/9999'). Use '' if file
     *                                  is not into subdir of module.
     ...
     *
     * @param string       $modulepart             Module the files are related to ('propal', 'facture',
     *                                             'facture_fourn', 'mymodule', 'mymodule:MyObject', 'mymodule_temp',
     *                                             ...)
     * @param string       $modulesubdir           Existing (so sanitized) sub-directory to scan (Example: '0/1/10',
     *                                             'FA/DD/MM/YY/9999'). Use '' if file is not into a subdir of module.
     * @param string       $filedir                Directory to scan (must not end with a /). Example:
     *                                             '/mydolibarrdocuments/facture/FAYYMM-1234'
     * @param string       $urlsource              Url of origin page (for return)
     * @param int|string[] $genallowed             Generation is allowed (1/0 or array list of templates)
     * @param int          $delallowed             Remove is allowed (1/0)
     * @param string       $modelselected          Model to preselect by default
     * @param integer      $allowgenifempty        Allow generation even if list of template ($genallowed) is empty
     *                                             (show however a warning)
     * @param integer      $forcenomultilang       Do not show language option (even if MAIN_MULTILANGS defined)
     * @param int          $iconPDF                Deprecated, see getDocumentsLink
     * @param int          $notused                Not used
     * @param integer      $noform                 Do not output html form tags
     * @param string       $param                  More param on http links
     * @param string       $title                  Title to show on top of form. Example: '' (Default to "Documents")
     *                                             or 'none'
     * @param string       $buttonlabel            Label on submit button
     * @param string       $codelang               Default language code to use on lang combo box if multilang is
     *                                             enabled
     ...
     *
     * @return     string|int                              Output string with HTML array of documents (might be empty
     *                                                     string)
     */
    public function showdocuments($modulepart, $modulesubdir, $filedir, $urlsource, $genallowed, $delallowed = 0, $modelselected = '', $allowgenifempty = 1, $forcenomultilang = 0, $iconPDF = 0, $notused = 0, $noform = 0, $param = '', $title = '', $buttonlabel = '', $codelang = '', $morepicto = '', $object = null, $hideifempty = 0, $removeaction = 'remove_file', $tooltipontemplatecombo = '')
    {
        ...
        if (in_array($modulepart, ['contract', 'facture', 'supplier_proposal', 'propal', 'proposal', 'order', 'commande', 'expedition', 'commande_fournisseur', 'expensereport', 'delivery', 'ticket'])) {
        ...
        if ($genallowed) {
            ...
            $addcolumforpicto = ($delallowed || $printer || $morepicto);
            $colspan = (4 + ($addcolumforpicto ? 1 : 0));
            $colspanmore = 0;  // @phan-suppress-current-line PhanPluginRedundantAssignment
            ...
            // Execute hooks
            $parameters = ['colspan' => ($colspan + $colspanmore), 'socid' => (isset($GLOBALS['socid']) ? $GLOBALS['socid'] : ''), 'id' => (isset($GLOBALS['id']) ? $GLOBALS['id'] : ''), 'modulepart' => $modulepart];
            if (is_object($hookmanager)) {
            ...
            if (is_array($file_list)) {
                ...
                foreach ($file_list as $file) {
                    ...
                    if (is_object($hookmanager)) {
                        ...
                        $colspan = (4 + ($addcolumforpicto ? 1 : 0));
                        $colspanmore = 0;
                        $parameters = ['colspan' => ($colspan + $colspanmore), 'socid' => (isset($GLOBALS['socid']) ? $GLOBALS['socid'] : ''), 'id' => (isset($GLOBALS['id']) ? $GLOBALS['id'] : ''), 'modulepart' => $modulepart, 'relativepath' => $relativepath];
     ...
     *
     * @param string $modulepart   'propal', 'facture', 'facture_fourn', ...
     * @param string $modulesubdir Sub-directory to scan (Example: '0/1/10', 'FA/DD/MM/YY/9999'). Use '' if file is not
     *                             into subdir of module.
     ...
     *
     * @return string                  Output string with HTML link of documents (might be empty string). This also
     *                                 fill the array ->infofiles
     */
     ...
     * @param integer $ruleforpicto Rule for picto: 0=Use the generic preview picto, 1=Use the picto of mime type of
     *                              file). Use a negative value to show a generic picto even if preview not available.
     * @param string  $param        More param on http links
     *
     ...
     *
     * @param array       $filearray                Array of files loaded by dol_dir_list('files') function before
     *                                              calling this.
     * @param Object|null $object                   Object on which document is linked to.
     * @param string      $modulepart               Value for modulepart used by download or viewimage wrapper.
     * @param string      $param                    Parameters on sort links (param must start with &, example
     *                                              &aaa=bbb&ccc=ddd)
     * @param int         $forcedownload            Force to open dialog box "Save As" when clicking on file.
     * @param string      $relativepath             Relative path of docs (autodefined if not provided), relative to
     *                                              module dir, not to MAIN_DATA_ROOT.
     * @param int         $permonobject             Permission on object (so permission to delete or crop document)
     * @param int         $useinecm                 Change output for use in ecm module:
     *                                              0 or 6: Add a preview column. Show also a rename button. Show also
     *                                              a crop button for some values of $modulepart (must be supported
     *                                              into hard coded list in this function + photos_resize.php +
     *                                              restrictedArea + checkUserAccessToObject)
     ...
     * @param int         $permtoeditline           Permission to edit document line (You must provide a value, -1 is
     *                                              deprecated and must not be used any more)
     * @param string      $upload_dir               Full path directory so we can know dir relative to MAIN_DATA_ROOT.
     *                                              Fill this to complete file data with database indexes.
     ...
     * @param int         $disablecrop              Disable crop feature on images (-1 = auto, prefer to set it
     *                                              explicitly to 0 or 1)
     * @param string      $moreattrondiv            More attributes on the div for responsive. Example
    ...
    public function list_of_documents($filearray, $object, $modulepart, $param = '', $forcedownload = 0, $relativepath = '', $permonobject = 1, $useinecm = 0, $textifempty = '', $maxlength = 0, $title = '', $url = '', $showrelpart = 0, $permtoeditline = -1, $upload_dir = '', $sortfield = '', $sortorder = 'ASC', $disablemove = 1, $addfilterfields = 0, $disablecrop = -1, $moreattrondiv = '')
    {
        ...
        if ($disablecrop == -1) {
            ...
            if (in_array($modulepart, ['bank', 'bom', 'expensereport', 'facture', 'facture_fournisseur', 'holiday', 'medias', 'member', 'mrp', 'project', 'product', 'produit', 'propal', 'service', 'societe', 'tax', 'tax-vat', 'ticket', 'user'])) {
                $disablecrop = 0;
            }
        }
        ...
        if (!empty($reshook)) { // null or '' for bypass
            ...
            if ($permtoeditline < 0) {  // Old behaviour for backward compatibility. New feature should call method with value 0 or 1
                $permtoeditline = 0;
                if (in_array($modulepart, ['product', 'produit', 'service'])) {
                ...
                if (
                    ...
                    if (!$editline) {
                        ...
                        if (empty($useinecm) || $useinecm == 2 || $useinecm == 6) { // 6=Media file manager
                            ...
                            if ($permtoeditline) {
                                $paramsectiondir = (in_array($modulepart, ['medias', 'ecm']) ? '&section_dir=' . urlencode($relativepath) : '');
     ...
     *
     * @param string $upload_dir         Directory that was scanned. This directory will contains files into subdirs
     *                                   REF/files
     * @param array  $filearray          Array of files loaded by dol_dir_list function before calling this function
     * @param string $modulepart         Value for modulepart used by download wrapper. Value can be
     *                                   $object->table_name (that is 'myobject' or 'mymodule_myobject') or
     *                                   $object->element.'-'.$module (for compatibility purpose)
        ...
        foreach ($filearray as $key => $file) {
            if (
                ...
                if ($modulepart == 'company' || $modulepart == 'tax' || $modulepart == 'tax-vat' || $modulepart == 'salaries') {
                    ...
                    $parameters = ['modulepart' => $modulepart, 'fileinfo' => $file];
                    $reshook = $hookmanager->executeHooks('addSectionECMAuto', $parameters);
                    if ($reshook > 0 && is_array($hookmanager->resArray) && count($hookmanager->resArray) > 0) {
