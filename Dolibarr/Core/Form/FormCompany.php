...
class FormCompany extends Form
{
     ...
     *   The key of the list is the code (there can be several entries for a given code but in this case, the country
     *   field differs). Thus the links with the departments are done on a department independently of its name.
     *
     ...
     * @param integer $country_codeid Country code or id: 0=list for all countries, otherwise country code or country
     *                                rowid to show
            ...
            if ($num) {
                ...
                while ($i < $num) {
                    ...
                    if ($obj->code) {       // We exclude empty line, we will add it later
                        ...
                        $arraydata[$obj->code] = ['code' => $obj->code, 'label' => $labeljs, 'label_sort' => $labelcountry . '_' . $labeljs, 'country_code' => $obj->country_code, 'country' => $labelcountry];
                    }
                    $i++;
                }
                ...
                if (empty($country_codeid)) {   // Introduce empty value (if $country_codeid not empty, empty value was already added)
                    $arraydata[0] = ['code' => 0, 'label' => '', 'label_sort' => '_', 'country_code' => '', 'country' => ''];
                }
    ...
    public function selectCompaniesForNewContact($object, $var_id, $selected = 0, $htmlname = 'newcompany', $limitto = [], $forceid = 0, $moreparam = '', $morecss = '')
    {
        ...
        if (!empty($conf->use_javascript_ajax) && getDolGlobalString('COMPANY_USE_SEARCH_TO_SELECT')) {
            ...
            $events[] = ['method' => 'getContacts', 'url' => dol_buildpath('/core/ajax/contacts.php', 1), 'htmlname' => 'contactid', 'params' => ['add-customer-contact' => 'disabled']];
     ...
     *
     * @return string|void                     Depending on $output param, return the HTML select list (recommended
     *                                         method) or nothing
     */
     ...
     */
    public function showRoles($htmlname, Contact $contact, $rendermode = 'view', $selected = [], $morecss = 'minwidth500')
    {
        if ($rendermode === 'view') {
            $toprint = [];
     ...
     * @param int    $disableautocomplete 1 To disable ajax autocomplete features (browser autocomplete may still
     *                                    occurs)
    ...
    public function get_input_id_prof($idprof, $htmlname, $preselected, $country_code, $morecss = 'maxwidth200')
    {
        ...
        $parameters = ['formlength' => $formlength, 'selected' => $preselected, 'idprof' => $idprof, 'htmlname' => $htmlname, 'country_code' => $country_code];
    ...
    public function selectProspectStatus($htmlname, $prospectstatic, $statusprospect, $idprospect, $mode = "html")
    {
        ...
        if ($mode === "html") {
            ...
            foreach ($prospectstatic->cacheprospectstatus as $key => $val) {
                ...
                $label = $val['label'];
                if (!empty($val['code']) && !in_array($val['code'], ['ST_NO', 'ST_NEVER', 'ST_TODO', 'ST_PEND', 'ST_DONE'])) {
                    //$titlealt = $val['label'];
