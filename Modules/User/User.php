...
use Alxarafe\LibClass\CommonObject;
use Alxarafe\LibClass\CommonPeople;
use stdClass;
...
class User extends CommonObject
{
    ...
    public function fetch($id = 0, $login = '', $sid = '', $loadpersonalconf = 0, $entity = -1, $email = '', $fk_socpeople = 0, $use_email_oauth2 = 0)
    {
        ...
        if ($entity < 0) {
            if ((!Functions::isModEnabled('multicompany') || !Functions::getDolGlobalString('MULTICOMPANY_TRANSVERSE_MODE')) && (!empty($user->entity))) {
            ...
            if (Functions::isModEnabled('multicompany') && Functions::getDolGlobalString('MULTICOMPANY_TRANSVERSE_MODE')) {
                $sql .= " WHERE u.entity IS NOT NULL"; // multicompany is on in transverse mode or user making fetch is on entity 0, so user is allowed to fetch anywhere into database
            } else {
        ...
        if ($resql) {
            ...
            if ($num > 1) {
                ...
                Functions::dol_syslog(get_class($this) . "::fetch more than 1 user found", LOG_WARNING);
            ...
            if ($obj) {
                ...
                $this->api_key = Security::dolDecrypt($obj->api_key);
                ...
                if (!Functions::isModEnabled('multicompany') && $this->admin && $this->entity == 1) {
                    $this->entity = 0;
                }
                ...
                Functions::dol_syslog(get_class($this) . "::fetch user not found", LOG_DEBUG);
    ...
    public function loadPersonalConf()
    {
        ...
        //Functions::dol_syslog(get_class($this).'::fetch load personalized conf', LOG_DEBUG);
        $resql = $this->db->query($sql);
        if ($resql) {
            $num = $this->db->num_rows($resql);
            $i = 0;
    ...
    public function hasRight($module, $permlevel1, $permlevel2 = '')
    {
        ...
        //var_dump($module.' '.Functions::isModEnabled($module).' '.$rightsPath.' '.$permlevel1.' '.$permlevel2);
        if (!Functions::isModEnabled($module)) {
            return 0;
        }
    ...
    public function addrights($rid, $allmodule = '', $allperms = '', $entity = 0, $notrigger = 0)
    {
        ...
        Functions::dol_syslog(get_class($this) . "::addrights $rid, $allmodule, $allperms, $entity, $notrigger for user id=" . $this->id);
    ...
    {
        Functions::dol_syslog(get_class($this) . "::clearrights reset user->rights");
        $this->rights = new stdClass();
        $this->nb_rights = 0;
        $this->all_permissions_are_loaded = 0;
        $this->_tab_loaded = [];
    ...
    public function getrights($moduletag = '', $forcereload = 0)
    {
        ...
        if (Functions::getDolGlobalString('MULTICOMPANY_BACKWARD_COMPATIBILITY')) {
            // on old version, we use entity defined into table r only
            $sql .= " AND r.entity IN (0," . (Functions::isModEnabled('multicompany') && Functions::getDolGlobalString('MULTICOMPANY_TRANSVERSE_MODE') ? "1," : "") . $conf->entity . ")";
        } else {
    ...
    public function setstatus($status)
    {
        ...
        Functions::dol_syslog(get_class($this) . "::setstatus", LOG_DEBUG);
        if ($result) {
            if ($status == 0) {
    ...
    public function delete(User $user)
    {
        ...
        Functions::dol_syslog(get_class($this) . "::delete", LOG_DEBUG);
        ...
        if (!$error) {
            ...
            if ($result < 0) {
                $error++;
                Functions::dol_syslog(get_class($this) . "::delete error -4 " . $this->error, LOG_ERR);
            }
        }
    ...
    public function create($user, $notrigger = 0)
    {
        ...
        Functions::dol_syslog(get_class($this) . "::create login=" . $this->login . ", user=" . (is_object($user) ? $user->id : ''), LOG_DEBUG);

        $badCharUnauthorizedIntoLoginName = Functions::getDolGlobalString('MAIN_LOGIN_BADCHARUNAUTHORIZED', ',@<>"\'');

        // Check parameters
        if (Functions::getDolGlobalString('USER_MAIL_REQUIRED') && !isValidEmail($this->email)) {
            $langs->load("errors");
        ...
        if ($this->login) {
            ...
            if ($resqltochecklogin) {
                ...
                if ($objtochecklogin && $objtochecklogin->nb > 0) {
                    ...
                    Functions::dol_syslog(get_class($this) . "::create " . $this->error, LOG_DEBUG);
                    $this->db->rollback();
                    return -6;
                }
        ...
        Functions::dol_syslog(get_class($this) . "::create", LOG_DEBUG);
        if ($result) {
            ...
            if (Functions::getDolGlobalString('MAIN_DEFAULT_WAREHOUSE_USER') && Functions::getDolGlobalString('STOCK_USERSTOCK_AUTOCREATE')) {
            ...
            if (!$error) {
                ...
                Functions::dol_syslog(get_class($this) . "::create " . $this->error, LOG_ERR);
                $this->db->rollback();
                return -3;
            }
        } else {
    ...
    public function create_from_contact($contact, $login = '', $password = '')
    {
        ...
        if ($result > 0) {
            ...
            Functions::dol_syslog(get_class($this) . "::create_from_contact", LOG_DEBUG);
            if ($resql) {
            ...
            Functions::dol_syslog(get_class($this) . "::create_from_contact - 0");
    ...
    public function create_from_member($member, $login = '')
    {
        ...
        if ($result > 0) {
            ...
            if ($result > 0 && $member->socid) {    // If member is linked to a thirdparty
                ...
                Functions::dol_syslog(get_class($this) . "::create_from_member", LOG_DEBUG);
                $resql = $this->db->query($sql);
                if ($resql) {
                    $this->db->commit();
                    return $this->id;
    ...
    public function update($user, $notrigger = 0, $nosyncmember = 0, $nosyncmemberpass = 0, $nosynccontact = 0)
    {
        ...
        Functions::dol_syslog(get_class($this) . "::update notrigger=" . $notrigger . ", nosyncmember=" . $nosyncmember . ", nosyncmemberpass=" . $nosyncmemberpass);
        ...
        Functions::dol_syslog(get_class($this) . "::update", LOG_DEBUG);
        $resql = $this->db->query($sql);
        if ($resql) {
            ...
            // Update password
            if (!empty($this->pass)) {
                if ($this->pass != $this->pass_indatabase && !Security::dol_verifyHash($this->pass, $this->pass_indatabase_crypted)) {
            ...
            if ($this->fk_member > 0) {
                Functions::dol_syslog(get_class($this) . "::update remove link with member. We will recreate it later", LOG_DEBUG);
            ...
            // Set link to user
            Functions::dol_syslog(get_class($this) . "::update set link with member", LOG_DEBUG);
            ...
            if ($nbrowsaffected) {  // If something has changed in data
                if ($this->fk_member > 0 && !$nosyncmember) {
                    Functions::dol_syslog(get_class($this) . "::update user is linked with a member. We try to update member too.", LOG_DEBUG);
                    ...
                    if ($result > 0) {
                        ...
                        if ($result < 0) {
                            ...
                            Functions::dol_syslog(get_class($this) . "::update error after calling adh->update to sync it with user: " . $this->error, LOG_ERR);
                            $error++;
                        }
                    } elseif ($result < 0) {
                        $this->error = $adh->error;
                ...
                if ($this->contact_id > 0 && !$nosynccontact) {
                    Functions::dol_syslog(get_class($this) . "::update user is linked with a contact. We try to update contact too.", LOG_DEBUG);
            ...
            if (!$error) {
                ...
                Functions::dol_syslog(get_class($this) . "::update error=" . $this->error, LOG_ERR);
                $this->db->rollback();
                return -1;
            }
        } else {
    ...
    public function update_last_login_date()
    {
        ...
        Functions::dol_syslog(get_class($this) . "::update_last_login_date user->id=" . $this->id . " " . $sql, LOG_DEBUG);
        $resql = $this->db->query($sql);
        if ($resql) {
    ...
    public function setPassword($user, $password = '', $changelater = 0, $notrigger = 0, $nosyncmember = 0, $passwordalreadycrypted = 0, $flagdelsessionsbefore = 1)
    {
        ...
        Functions::dol_syslog(get_class($this) . "::setPassword user=" . $user->id . " password=" . preg_replace('/./i', '*', $password) . " changelater=" . $changelater . " notrigger=" . $notrigger . " nosyncmember=" . $nosyncmember, LOG_DEBUG);
        ...
        if (empty($passwordalreadycrypted)) {
            if (Functions::getDolGlobalString('USER_PASSWORD_GENERATED')) {
            ...
            $password_crypted = Security::dol_hash($password);
        }
        ...
        if (!$changelater) {
            ...
            if (!empty($flagdelsessionsbefore)) {
                $sql .= ", flagdelsessionsbefore = '" . $this->db->idate(Functions::dol_now() - 5, 'gmt') . "'";
            }
            ...
            Functions::dol_syslog(get_class($this) . "::setPassword", LOG_DEBUG);
            $result = $this->db->query($sql);
            if ($result) {
                if ($this->db->affected_rows($result)) {
                    ...
                    if ($this->fk_member && !$nosyncmember) {
                        ...
                        if ($result >= 0) {
                            $result = $adh->setPassword($user, $this->pass, (!Functions::getDolGlobalString('DATABASE_PWD_ENCRYPTED') ? 0 : 1), 1); // The encryption is not managed in the 'adherent' module
                            if (is_int($result) && $result < 0) {
                                $this->error = $adh->error;
                                Functions::dol_syslog(get_class($this) . "::setPassword " . $this->error, LOG_ERR);
                                $error++;
                            }
                        } else {
                            $this->error = $adh->error;
                    ...
                    Functions::dol_syslog(get_class($this) . "::setPassword notrigger=" . $notrigger . " error=" . $error, LOG_DEBUG);
            ...
            Functions::dol_syslog(get_class($this) . "::setPassword", LOG_DEBUG); // No log
            $result = $this->db->query($sql);
            if ($result) {
                return $password;
            } else {
    ...
    public function send_password($user, $password = '', $changelater = 0)
    {
        ...
        $appli = Functions::getDolGlobalString('MAIN_APPLICATION_TITLE', constant('DOL_APPLICATION_TITLE'));
        ...
        if (!$changelater) {
            $url = $urlwithroot . '/';
            if (Functions::getDolGlobalString('URL_REDIRECTION_AFTER_CHANGEPASSWORD')) {
                $url = Functions::getDolGlobalString('URL_REDIRECTION_AFTER_CHANGEPASSWORD');
            }

            Functions::dol_syslog(get_class($this) . "::send_password changelater is off, url=" . $url);
            ...
            $url .= '&username=' . urlencode($this->login) . "&passworduidhash=" . urlencode(Security::dol_hash($password . '-' . $this->id . '-' . $conf->file->instance_unique_id));
            if (Functions::isModEnabled('multicompany')) {
                $url .= '&entity=' . (!empty($this->entity) ? $this->entity : 1);
            }

            Functions::dol_syslog(get_class($this) . "::send_password changelater is on, url=" . $url);
    ...
    public function update_clicktodial()
    {
        ...
        Functions::dol_syslog(get_class($this) . '::update_clicktodial', LOG_DEBUG);
        $result = $this->db->query($sql);
    ...
    public function SetInGroup($group, $entity, $notrigger = 0)
    {
        ...
        if ($result) {
            ...
            if (!$error) {
                ...
                Functions::dol_syslog(get_class($this) . "::SetInGroup " . $this->error, LOG_ERR);
                $this->db->rollback();
                return -2;
            }
        } else {
    ...
    public function RemoveFromGroup($group, $entity, $notrigger = 0)
    {
        ...
        if ($result) {
            ...
            if (!$error) {
                ...
                Functions::dol_syslog(get_class($this) . "::RemoveFromGroup " . $this->error, LOG_ERR);
                $this->db->rollback();
                return -2;
            }
        } else {
    ...
    public function isNotIntoValidityDateRange()
    {
        ...
        //Functions::dol_syslog("isNotIntoValidityDateRange ".$this->datestartvalidity);
    ...
    public function getTooltipContentArray($params)
    {
        ...
        if ($infologin > 0) {
            ...
            if (Functions::getDolGlobalString('MAIN_MODULE_MULTICOMPANY')) {
    ...
    public function getNomUrl($withpictoimg = 0, $option = '', $infologin = 0, $notooltip = 0, $maxlen = 24, $hidethirdpartylogo = 0, $mode = '', $morecss = '', $save_lastsearch_value = -1)
    {
        ...
        if (Functions::getDolGlobalString('MAIN_OPTIMIZEFORTEXTBROWSER') && $withpictoimg) {
            $withpictoimg = 0;
        }
    ...
    public function getKanbanView($option = '', $arraydata = null)
    {
        ...
        if (Functions::isModEnabled('multicompany') && $this->admin && !$this->entity) {
    ...
    public function _load_ldap_dn($info, $mode = 0)
    {
        ...
        $dn = '';
        if ($mode == 0) {
            $dn = Functions::getDolGlobalString('LDAP_KEY_USERS') . "=" . $info[Functions::getDolGlobalString('LDAP_KEY_USERS')] . "," . Functions::getDolGlobalString('LDAP_USER_DN');
        } elseif ($mode == 1) {
            $dn = Functions::getDolGlobalString('LDAP_USER_DN');
        } elseif ($mode == 2) {
            $dn = Functions::getDolGlobalString('LDAP_KEY_USERS') . "=" . $info[Functions::getDolGlobalString('LDAP_KEY_USERS')];
        }
        return $dn;
    }
    ...
    public function _load_ldap_info()
    {
        ...
        // Object classes
        $info["objectclass"] = explode(',', Functions::getDolGlobalString('LDAP_USER_OBJECT_CLASS'));
        ...
        foreach ($ldapkey as $constname => $varname) {
            if (!empty($this->$varname) && Functions::getDolGlobalString($constname)) {
                $info[Functions::getDolGlobalString($constname)] = $this->$varname;

                // Check if it is the LDAP key and if its value has been changed
                if (Functions::getDolGlobalString('LDAP_KEY_USERS') && $conf->global->LDAP_KEY_USERS == Functions::getDolGlobalString($constname)) {
        ...
        foreach ($socialnetworks as $key => $value) {
            if (!empty($this->socialnetworks[$value['label']]) && Functions::getDolGlobalString('LDAP_FIELD_' . strtoupper($value['label']))) {
                $info[Functions::getDolGlobalString('LDAP_FIELD_' . strtoupper($value['label']))] = $this->socialnetworks[$value['label']];
            }
        }
        if ($this->address && Functions::getDolGlobalString('LDAP_FIELD_ADDRESS')) {
            $info[Functions::getDolGlobalString('LDAP_FIELD_ADDRESS')] = $this->address;
        }
        if ($this->zip && Functions::getDolGlobalString('LDAP_FIELD_ZIP')) {
            $info[Functions::getDolGlobalString('LDAP_FIELD_ZIP')] = $this->zip;
        }
        if ($this->town && Functions::getDolGlobalString('LDAP_FIELD_TOWN')) {
            $info[Functions::getDolGlobalString('LDAP_FIELD_TOWN')] = $this->town;
        }
        if ($this->note_public && Functions::getDolGlobalString('LDAP_FIELD_DESCRIPTION')) {
            $info[Functions::getDolGlobalString('LDAP_FIELD_DESCRIPTION')] = dol_string_nohtmltag($this->note_public, 2);
        }
        if ($this->socid > 0) {
            $soc = new Societe($this->db);
            $soc->fetch($this->socid);

            $info[Functions::getDolGlobalString('LDAP_FIELD_COMPANY')] = $soc->name;
            if ($soc->client == 1) {
        ...
        if (!empty($this->pass)) {
            if (Functions::getDolGlobalString('LDAP_FIELD_PASSWORD')) {
                $info[Functions::getDolGlobalString('LDAP_FIELD_PASSWORD')] = $this->pass; // this->pass = unencrypted password
            }
            if (Functions::getDolGlobalString('LDAP_FIELD_PASSWORD_CRYPTED')) {
                $info[Functions::getDolGlobalString('LDAP_FIELD_PASSWORD_CRYPTED')] = Security::dol_hash($this->pass, 'openldap'); // Create OpenLDAP password (see LDAP_PASSWORD_HASH_TYPE)
            }
        } elseif (Functions::getDolGlobalString('LDAP_SERVER_PROTOCOLVERSION') !== '3') {
            // Set LDAP password if possible
            // If ldap key is modified and LDAPv3 we use ldap_rename function for avoid lose encrypt password
            if (Functions::getDolGlobalString('DATABASE_PWD_ENCRYPTED')) {
                // Just for the default MD5 !
                if (!Functions::getDolGlobalString('MAIN_SECURITY_HASH_ALGO')) {
                    if ($this->pass_indatabase_crypted && Functions::getDolGlobalString('LDAP_FIELD_PASSWORD_CRYPTED')) {
                        $info[Functions::getDolGlobalString('LDAP_FIELD_PASSWORD_CRYPTED')] = dolGetLdapPasswordHash($this->pass_indatabase_crypted, 'md5frommd5'); // Create OpenLDAP MD5 password from Dolibarr MD5 password
                    }
                }
            } elseif (!empty($this->pass_indatabase)) {
                // Use $this->pass_indatabase value if exists
                if (Functions::getDolGlobalString('LDAP_FIELD_PASSWORD')) {
                    $info[Functions::getDolGlobalString('LDAP_FIELD_PASSWORD')] = $this->pass_indatabase; // $this->pass_indatabase = unencrypted password
                }
                if (Functions::getDolGlobalString('LDAP_FIELD_PASSWORD_CRYPTED')) {
                    $info[Functions::getDolGlobalString('LDAP_FIELD_PASSWORD_CRYPTED')] = Security::dol_hash($this->pass_indatabase, 'openldap'); // Create OpenLDAP password (see LDAP_PASSWORD_HASH_TYPE)
                }
            }
        }

        if (Functions::getDolGlobalString('LDAP_SERVER_TYPE') == 'egroupware') {
        ...
        if (Functions::getDolGlobalString('LDAP_FIELD_USERID')) {
            $info[Functions::getDolGlobalString('LDAP_FIELD_USERID')] = $this->id;
        }
        if (Functions::getDolGlobalString('LDAP_FIELD_GROUPID')) {
            $usergroup = new UserGroup($this->db);
            $groupslist = $usergroup->listGroupsForUser($this->id);
            $info[Functions::getDolGlobalString('LDAP_FIELD_GROUPID')] = '65534';
            if (!empty($groupslist)) {
                foreach ($groupslist as $groupforuser) {
                    $info[Functions::getDolGlobalString('LDAP_FIELD_GROUPID')] = $groupforuser->id; //Select first group in list
                    break;
                }
            }
        }
        if (Functions::getDolGlobalString('LDAP_FIELD_HOMEDIRECTORY') && Functions::getDolGlobalString('LDAP_FIELD_HOMEDIRECTORYPREFIX')) {
            $info[Functions::getDolGlobalString('LDAP_FIELD_HOMEDIRECTORY')] = "{$conf->global->LDAP_FIELD_HOMEDIRECTORYPREFIX}/$this->login";
        }
    ...
    public function update_ldap2dolibarr(&$ldapuser)
    {
        ...
        $tmpvar = Functions::getDolGlobalString('LDAP_FIELD_FIRSTNAME');
        $this->firstname = $ldapuser->$tmpvar;
        $tmpvar = Functions::getDolGlobalString('LDAP_FIELD_NAME');
        $this->lastname = $ldapuser->$tmpvar;
        $tmpvar = Functions::getDolGlobalString('LDAP_FIELD_LOGIN');
        $this->login = $ldapuser->$tmpvar;
        $tmpvar = Functions::getDolGlobalString('LDAP_FIELD_PASSWORD');
        $this->pass = $ldapuser->$tmpvar;
        $tmpvar = Functions::getDolGlobalString('LDAP_FIELD_PASSWORD_CRYPTED');
        $this->pass_indatabase_crypted = $ldapuser->$tmpvar;

        $tmpvar = Functions::getDolGlobalString('LDAP_FIELD_PHONE');
        $this->office_phone = $ldapuser->$tmpvar;
        $tmpvar = Functions::getDolGlobalString('LDAP_FIELD_MOBILE');
        $this->user_mobile = $ldapuser->$tmpvar;
        $tmpvar = Functions::getDolGlobalString('LDAP_FIELD_FAX');
        $this->office_fax = $ldapuser->$tmpvar;
        $tmpvar = Functions::getDolGlobalString('LDAP_FIELD_MAIL');
        $this->email = $ldapuser->$tmpvar;
        foreach ($socialnetworks as $key => $value) {
            $tmpvar = Functions::getDolGlobalString('LDAP_FIELD_' . strtoupper($value['label']));
            $this->socialnetworks[$value['label']] = $ldapuser->$tmpvar;
        }
        $tmpvar = Functions::getDolGlobalString('LDAP_FIELD_SID');
        $this->ldap_sid = $ldapuser->$tmpvar;

        $tmpvar = Functions::getDolGlobalString('LDAP_FIELD_TITLE');
        $this->job = $ldapuser->$tmpvar;
        $tmpvar = Functions::getDolGlobalString('LDAP_FIELD_DESCRIPTION');
        $this->note_public = $ldapuser->$tmpvar;

        $result = $this->update($user);

        Functions::dol_syslog(get_class($this) . "::update_ldap2dolibarr result=" . $result, LOG_DEBUG);
    ...
    public function get_children()
    {
        ...
        Functions::dol_syslog(get_class($this) . "::get_children", LOG_DEBUG);
    ...
    private function loadParentOf()
    {
        ...
        Functions::dol_syslog(get_class($this) . "::loadParentOf", LOG_DEBUG);
        $resql = $this->db->query($sql);
        if ($resql) {
    ...
    public function get_full_tree($deleteafterid = 0, $filter = '')
    {
        ...
        Functions::dol_syslog(get_class($this) . "::get_full_tree get user list", LOG_DEBUG);
        $resql = $this->db->query($sql);
        if ($resql) {
            $i = 0;
            while ($obj = $this->db->fetch_object($resql)) {
        ...
        Functions::dol_syslog(get_class($this) . "::get_full_tree call to build_path_from_id_user", LOG_DEBUG);
        ...
        if ($deleteafterid) {
            ...
            foreach ($this->users as $key => $val) {
                if (
                    preg_match('/' . $keyfilter1 . '/', $val['fullpath']) || preg_match('/' . $keyfilter2 . '/', $val['fullpath'])
        ...
        Functions::dol_syslog(get_class($this) . "::get_full_tree dol_sort_array", LOG_DEBUG);
    ...
    public function getAllChildIds($addcurrentuser = 0)
    {
        ...
        if (isset($this->cache_childids[$this->id])) {
            ...
            Functions::dol_syslog("Build childid for id = " . $idtoscan);
    ...
    {
        // phpcs:enable
        //Functions::dol_syslog(get_class($this)."::build_path_from_id_user id_user=".$id_user." protection=".$protection, LOG_DEBUG);

        if (!empty($this->users[$id_user]['fullpath'])) {
            // Already defined
            Functions::dol_syslog(get_class($this) . "::build_path_from_id_user fullpath and fullname already defined", LOG_WARNING);
            return 0;
        }
        ...
        while (!empty($this->parentof[$cursor_user]) && !empty($this->users[$this->parentof[$cursor_user]])) {
            if (in_array($this->parentof[$cursor_user], $useridfound)) {
                Functions::dol_syslog("The hierarchy of user has a recursive loop", LOG_WARNING);
    ...
    public function generateDocument($modele, $outputlangs, $hidedetails = 0, $hidedesc = 0, $hideref = 0, $moreparams = null)
    {
        ...
        if (!dol_strlen($modele)) {
            if (Functions::getDolGlobalString('USER_ADDON_PDF')) {
                $modele = Functions::getDolGlobalString('USER_ADDON_PDF');
            } else {
                $modele = 'bluesky';
            }
        }
    ...
    public function getOnlineVirtualCardUrl($mode = '', $typeofurl = 'external')
    {
        ...
        $encodedsecurekey = Security::dol_hash($conf->file->instance_unique_id . 'uservirtualcard' . $this->id . '-' . $this->login, 'md5');
        if (Functions::isModEnabled('multicompany')) {
    ...
    public function findUserIdByEmail($email)
    {
        ...
        $sql = 'SELECT rowid';
        $sql .= ' FROM ' . $this->db->prefix() . 'user';
        if (Functions::getDolGlobalString('AGENDA_DISABLE_EXACT_USER_EMAIL_COMPARE_FOR_EXTERNAL_CALENDAR')) {
