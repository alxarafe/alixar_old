...
class UserModule extends Modules
{
     ...
     *
     * @param \Alixar\core\modules\DoliDB $db Database handler
     */
    public function __construct($db)
    {
        ...
        $this->dirs = ["/users/temp"];

        // Config pages
        $this->config_page_url = ["user.php"];
        ...
        $this->langfiles = ["main", "users", "companies", "members", "salaries", "hrm"];
        ...
        // Boxes
        $this->boxes = [
            0 => ['file' => 'box_lastlogin.php', 'enabledbydefaulton' => 'Home'],
            1 => ['file' => 'box_birthdays.php', 'enabledbydefaulton' => 'Home'],
            2 => ['file' => 'box_dolibarr_state_board.php', 'enabledbydefaulton' => 'Home'],
        ];
        ...
        $this->export_permission[$r] = [["user", "user", "export"]];
        ...
        // include BASE_PATH . '/core/extrafieldsinexport.inc.php';
        $this->extrafieldsinexport($keyforselect, $keyforelement, $keyforaliasextra);
        if (!Functions::isModEnabled('adherent')) {
            unset($this->export_fields_array[$r]['u.fk_member']);
            unset($this->export_entities_array[$r]['u.fk_member']);
        }
        $this->export_sql_start[$r] = 'SELECT DISTINCT ';
        $this->export_sql_end[$r] = ' FROM ' . $config->main_db_prefix . 'user as u';
        $this->export_sql_end[$r] .= ' LEFT JOIN ' . $config->main_db_prefix . 'user_extrafields as extra ON u.rowid = extra.fk_object';
        $this->export_sql_end[$r] .= ' LEFT JOIN ' . $config->main_db_prefix . 'usergroup_user as ug ON u.rowid = ug.fk_user';
        $this->export_sql_end[$r] .= ' LEFT JOIN ' . $config->main_db_prefix . 'usergroup as g ON ug.fk_usergroup = g.rowid';
        $this->export_sql_end[$r] .= ' LEFT JOIN ' . $config->main_db_prefix . 'adherent as a ON u.fk_member = a.rowid';
        $this->export_sql_end[$r] .= ' WHERE u.entity IN (' . Functions::getEntity('user') . ')';
        ...
        $this->import_tables_array[$r] = ['u' => $config->main_db_prefix . 'user', 'extra' => $config->main_db_prefix . 'user_extrafields']; // List of tables to insert into (insert done in same order)
        ...
        // Add extra fields
        $sql = "SELECT name, label, fieldrequired FROM " . $config->main_db_prefix . "extrafields WHERE type <> 'separate' AND elementtype = 'user' AND entity IN (0," . $conf->entity . ")";
        ...
        $this->import_fieldshidden_array[$r] = ['u.fk_user_creat' => 'user->id', 'extra.fk_object' => 'lastrowid-' . $config->main_db_prefix . 'user']; // aliastable.field => ('user->id' or 'lastrowid-'.tableparent)
        $this->import_convertvalue_array[$r] = [
            'u.fk_state' => ['rule' => 'fetchidfromcodeid', 'classfile' => '/core/class/cstate.class.php', 'class' => 'Cstate', 'method' => 'fetch', 'dict' => 'DictionaryState'],
            'u.fk_country' => ['rule' => 'fetchidfromcodeid', 'classfile' => '/core/class/ccountry.class.php', 'class' => 'Ccountry', 'method' => 'fetch', 'dict' => 'DictionaryCountry'],
            'u.salary' => ['rule' => 'numeric'],
        ];
        ...
        $this->import_updatekeys_array[$r] = ['u.lastname' => 'Lastname', 'u.firstname' => 'Firstname', 'u.login' => 'Login'];
    }
