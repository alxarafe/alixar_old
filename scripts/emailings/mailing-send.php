...
if (!Functions::getDolGlobalString('MAILING_LIMIT_SENDBYCLI')) {
...
if (Functions::getDolGlobalString('MAILING_LIMIT_SENDBYCLI') == '-1') {
}
    ...
    if ($num) {
        for ($j = 0; $j < $num; $j++) {
            ...
            if (getDolGlobalInt('MAILING_LIMIT_SENDBYCLI') > 0 && empty($max)) {
                $sql2 .= " LIMIT " . Functions::getDolGlobalString('MAILING_LIMIT_SENDBYCLI');
            ...
            if ($resql2) {
                ...
                if ($num2) {
                    ...
                    while ($i < $num2) {
                        ...
                        $signature = ((!empty($user->signature) && !Functions::getDolGlobalString('MAIN_MAIL_DO_NOT_USE_SIGN')) ? $user->signature : '');
                        ...
                        $substitutionarray['__CHECK_READ__'] = '<img src="' . DOL_MAIN_URL_ROOT . '/public/emailing/mailing-read.php?tag=' . urlencode($obj->tag) . '&securitykey=' . dol_hash(Functions::getDolGlobalString('MAILING_EMAIL_UNSUBSCRIBE_KEY') . "-" . $obj->tag . "-" . $obj->email . "-" . $obj->rowid, "md5") . '&email=' . urlencode($obj->email) . '&mtid=' . ((int) $obj->rowid) . '" width="1" height="1" style="width:1px;height:1px" border="0"/>';
                        $substitutionarray['__UNSUBSCRIBE__'] = '<a href="' . DOL_MAIN_URL_ROOT . '/public/emailing/mailing-unsubscribe.php?tag=' . urlencode($obj->tag) . '&unsuscrib=1&securitykey=' . dol_hash(Functions::getDolGlobalString('MAILING_EMAIL_UNSUBSCRIBE_KEY') . "-" . $obj->tag . "-" . $obj->email . "-" . $obj->rowid, "md5") . '&email=' . urlencode($obj->email) . '&mtid=' . ((int) $obj->rowid) . '" target="_blank">' . $langs->trans("MailUnsubcribe") . '</a>';
                        $substitutionarray['__UNSUBSCRIBE_URL__'] = DOL_MAIN_URL_ROOT . '/public/emailing/mailing-unsubscribe.php?tag=' . urlencode($obj->tag) . '&unsuscrib=1&securitykey=' . dol_hash(Functions::getDolGlobalString('MAILING_EMAIL_UNSUBSCRIBE_KEY') . "-" . $obj->tag . "-" . $obj->email . "-" . $obj->rowid, "md5") . '&email=' . urlencode($obj->email) . '&mtid=' . ((int) $obj->rowid);
                        ...
                        if ($onlinepaymentenabled && Functions::getDolGlobalString('PAYMENT_SECURITY_TOKEN')) {
                            $substitutionarray['__SECUREKEYPAYMENT__'] = dol_hash(Functions::getDolGlobalString('PAYMENT_SECURITY_TOKEN'), 2);
                            if (!Functions::getDolGlobalString('PAYMENT_SECURITY_TOKEN_UNIQUE')) {
                                $substitutionarray['__SECUREKEYPAYMENT_MEMBER__'] = dol_hash(Functions::getDolGlobalString('PAYMENT_SECURITY_TOKEN'), 2);
                                $substitutionarray['__SECUREKEYPAYMENT_ORDER__'] = dol_hash(Functions::getDolGlobalString('PAYMENT_SECURITY_TOKEN'), 2);
                                $substitutionarray['__SECUREKEYPAYMENT_INVOICE__'] = dol_hash(Functions::getDolGlobalString('PAYMENT_SECURITY_TOKEN'), 2);
                                $substitutionarray['__SECUREKEYPAYMENT_CONTRACTLINE__'] = dol_hash(Functions::getDolGlobalString('PAYMENT_SECURITY_TOKEN'), 2);
                            } else {
                                $substitutionarray['__SECUREKEYPAYMENT_MEMBER__'] = dol_hash(Functions::getDolGlobalString('PAYMENT_SECURITY_TOKEN') . 'membersubscription' . $obj->source_id, 2);
                                $substitutionarray['__SECUREKEYPAYMENT_ORDER__'] = dol_hash(Functions::getDolGlobalString('PAYMENT_SECURITY_TOKEN') . 'order' . $obj->source_id, 2);
                                $substitutionarray['__SECUREKEYPAYMENT_INVOICE__'] = dol_hash(Functions::getDolGlobalString('PAYMENT_SECURITY_TOKEN') . 'invoice' . $obj->source_id, 2);
                                $substitutionarray['__SECUREKEYPAYMENT_CONTRACTLINE__'] = dol_hash(Functions::getDolGlobalString('PAYMENT_SECURITY_TOKEN') . 'contractline' . $obj->source_id, 2);
                            }
                        }
                        /* For backward compatibility */
                        if (isModEnabled('paypal') && Functions::getDolGlobalString('PAYPAL_SECURITY_TOKEN')) {
                            $substitutionarray['__SECUREKEYPAYPAL__'] = dol_hash(Functions::getDolGlobalString('PAYPAL_SECURITY_TOKEN'), 2);

                            if (!Functions::getDolGlobalString('PAYPAL_SECURITY_TOKEN_UNIQUE')) {
                                $substitutionarray['__SECUREKEYPAYPAL_MEMBER__'] = dol_hash(Functions::getDolGlobalString('PAYPAL_SECURITY_TOKEN'), 2);
                            } else {
                                $substitutionarray['__SECUREKEYPAYPAL_MEMBER__'] = dol_hash(Functions::getDolGlobalString('PAYPAL_SECURITY_TOKEN') . 'membersubscription' . $obj->source_id, 2);
                            }

                            if (!Functions::getDolGlobalString('PAYPAL_SECURITY_TOKEN_UNIQUE')) {
                                $substitutionarray['__SECUREKEYPAYPAL_ORDER__'] = dol_hash(Functions::getDolGlobalString('PAYPAL_SECURITY_TOKEN'), 2);
                            } else {
                                $substitutionarray['__SECUREKEYPAYPAL_ORDER__'] = dol_hash(Functions::getDolGlobalString('PAYPAL_SECURITY_TOKEN') . 'order' . $obj->source_id, 2);
                            }

                            if (!Functions::getDolGlobalString('PAYPAL_SECURITY_TOKEN_UNIQUE')) {
                                $substitutionarray['__SECUREKEYPAYPAL_INVOICE__'] = dol_hash(Functions::getDolGlobalString('PAYPAL_SECURITY_TOKEN'), 2);
                            } else {
                                $substitutionarray['__SECUREKEYPAYPAL_INVOICE__'] = dol_hash(Functions::getDolGlobalString('PAYPAL_SECURITY_TOKEN') . 'invoice' . $obj->source_id, 2);
                            }

                            if (!Functions::getDolGlobalString('PAYPAL_SECURITY_TOKEN_UNIQUE')) {
                                $substitutionarray['__SECUREKEYPAYPAL_CONTRACTLINE__'] = dol_hash(Functions::getDolGlobalString('PAYPAL_SECURITY_TOKEN'), 2);
                            } else {
                                $substitutionarray['__SECUREKEYPAYPAL_CONTRACTLINE__'] = dol_hash(Functions::getDolGlobalString('PAYPAL_SECURITY_TOKEN') . 'contractline' . $obj->source_id, 2);
                            }
                        }
