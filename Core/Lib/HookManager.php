...
class HookManager
{
     ...
     *  First, a hook is declared by a module by adding a constant MAIN_MODULE_MYMODULENAME_HOOKS with value
     *  'nameofcontext1:nameofcontext2:...' into $this->const of module descriptor file. This makes
     *  $conf->hooks_modules loaded with an entry ('modulename'=>array(nameofcontext1,nameofcontext2,...)) When
     *  initHooks function is called, with initHooks(list_of_contexts), an array $this->hooks is defined with instance
     *  of controller class found into file /mymodule/class/actions_mymodule.class.php (if module has declared the
     *  context as a managed context). Then when a hook executeHooks('aMethod'...) is called, the method aMethod found
     *  into class will be executed.
     *
     * @param string[] $arraycontext Array list of searched hooks tab/features. For example: 'thirdpartycard' (for hook
     *                               methods into page card thirdparty), 'thirdpartydao' (for hook methods into
     *                               Societe), ...
     *
     * @return int                             0 or 1
     */
     ...
     *
     * @return     int                     For 'addreplace' hooks (doActions, formConfirm, formObjectOptions,
     *                                     pdf_xxx,...):    Return 0 if we want to keep standard actions, >0 if we want
     *                                     to stop/replace standard actions, <0 if KO. Things to print are returned
     *                                     into ->resprints and set into ->resPrint. Things to return are returned into
     *                                     ->results by hook and set into ->resArray for caller. For 'output' hooks
     *                                     (printLeftBlock, formAddObjectLine, formBuilddocOptions, ...):   Return 0 if
     *                                     we want to keep standard actions, >0 uf we want to stop/replace standard
     *                                     actions (at least one > 0 and replacement will be done), <0 if KO. Things to
     *                                     print are returned into ->resprints and set into ->resPrint. Things to
     *                                     return are returned into ->results by hook and set into ->resArray for
     *                                     caller. All types can also return some values into an array ->results that will be merged into this->resArray for caller.
     *                                      $this->error or this->errors are also defined by class called by this
     *                                      function if error.
     */
