...
abstract class Functions
{
    ...
    /**
     * Return the full path of the directory where a module (or an object of a module) stores its files. Path may
     * depends on the entity if a multicompany module is enabled.
     *
     ...
     * @return string
     */
    public static function getDolGlobalString($key, $default = '')
    {
        $conf = Globals::getConf();
     ...
     */
    public static function isModEnabled($module)
    {
        // Fix special cases
        $arrayconv = [
            'bank' => 'banque',
        ...
        if (!Functions::getDolGlobalString('MAIN_USE_NEW_SUPPLIERMOD')) {
    ...
    {
        if ($timestamp === '') {
            Functions::dol_syslog('Using empty string for a timestamp is deprecated, prefer use of null when calling page ' . $_SERVER["PHP_SELF"], LOG_NOTICE);
            return false;
        }
     ...
     *                                    'propal', 'supplier_proposal', 'invoice', 'supplier_invoice',
     *                                    'payment_various',
    ...
    function getEntity($element, $shared = 1, $currentobject = null)
    {
        ...
        if (is_object($mc)) {
            ...
            $out = '';
            $addzero = ['user', 'usergroup', 'cronjob', 'c_email_templates', 'email_template', 'default_values', 'overwrite_trans'];
     ...
     * Warning:
     * For action=add, use:     $var = GETPOST('var');        // No GETPOSTISSET, so GETPOST always called and default
     * value is retrieved if not a form POST, and value of form is retrieved if it is a form POST. For action=update,
     * use:  $var = GETPOSTISSET('var') ? GETPOST('var') : $object->var;
     *
     ...
     *
     * @return    bool                    True if we have just submit a POST or GET request with the parameter provided
     *                                    (even if param is empty)
     */
     ...
     *  Use the property $user->default_values[path]['createform'] and/or $user->default_values[path]['filters'] and/or
     *  $user->default_values[path]['sortorder'] Note: The property $user->default_values is loaded by main.php when
     *  loading the user.
     *
     ...
     *                               'san_alpha'=Use filter_var with FILTER_SANITIZE_STRING (do not use this for free
     *                               text string)
     *                               'nohtml'=check there is no html content
     *                               'restricthtml'=check html content is restricted to some tags only
     *                               'custom'= custom filter specify $filter and $options)
     * @param int    $method         Type of method (0 = get then post, 1 = only get, 2 = only post, 3 = post then get)
     * @param int    $filter         Filter to apply when $check is set to 'custom'. (See
     *                               http://php.net/manual/en/filter.filters.php for détails)
     * @param mixed  $options        Options to pass to filter_var when $check is set to 'custom'
     * @param string $noreplace      Force disable of replacement of __xxx__ strings.
     *
     * @return string|array         Value found (string or array), or '' if check fails
     */
    public static function GETPOST($paramname, $check = 'alphanohtml', $method = 0, $filter = null, $options = null, $noreplace = 0)
    {
        ...
        if (empty($check)) {
            Functions::dol_syslog("Deprecated use of GETPOST, called with 1st param = " . $paramname . " and a 2nd param that is '', when calling page " . $_SERVER["PHP_SELF"], LOG_WARNING);
        ...
        if (empty($method) || $method == 3 || $method == 4) {
            ...
            // Clean $relativepathstring
            if (DOL_URL_ROOT) {
                $relativepathstring = preg_replace('/^' . preg_quote(DOL_URL_ROOT, '/') . '/', '', $relativepathstring);
            }
            $relativepathstring = preg_replace('/^\//', '', $relativepathstring);
            $relativepathstring = preg_replace('/^custom\//', '', $relativepathstring);
            //var_dump($relativepathstring);
            ...
            if (!empty($_GET['restore_lastsearch_values'])) {        // Use $_GET here and not GETPOST
                ...
                if (Functions::getDolGlobalString('MAIN_ENABLE_DEFAULT_VALUES')) {
                            ...
                            if (isset($user->default_values[$relativepathstring]['createform'])) {
                                foreach ($user->default_values[$relativepathstring]['createform'] as $defkey => $defval) {
                                    ...
                                    if ($defkey != '_noquery_') {
                                        ...
                                        $tmpqueryarraywehave = explode('&', Functions::dol_string_nohtmltag($_SERVER['QUERY_STRING']));
                                        $foundintru = 0;
                            ...
                            if ($paramname == 'sortfield' || $paramname == 'sortorder') {
                                ...
                                if (isset($user->default_values[$relativepathstring]['sortorder'])) {
                                    ...
                                    foreach ($user->default_values[$relativepathstring]['sortorder'] as $defkey => $defval) {
                                        ...
                                        if ($qualified) {
                                            $forbidden_chars_to_replace = [" ", "'", "/", "\\", ":", "*", "?", "\"", "<", ">", "|", "[", "]", ";", "="]; // we accept _, -, . and ,
                                ...
                                foreach ($user->default_values[$relativepathstring]['filters'] as $defkey => $defval) {    // $defkey is a querystring like 'a=b&c=d', $defval is key of user
                                    ...
                                    if ($qualified && isset($user->default_values[$relativepathstring]['filters'][$defkey][$paramname])) {
                                        ...
                                        if (isset($_POST['sall']) || isset($_POST['search_all']) || isset($_GET['sall']) || isset($_GET['search_all'])) {
                                            ...
                                            if (!Functions::getDolGlobalString('MAIN_DISABLE_DEFAULT_FILTER_FOR_QUICK_SEARCH')) {
                                                $forbidden_chars_to_replace = [" ", "'", "/", "\\", ":", "*", "?", "\"", "<", ">", "|", "[", "]", ";", "="]; // we accept _, -, . and ,
        ...
        if (preg_match('/^array/', $check)) {    // If 'array' or 'array:restricthtml' or 'array:aZ09' or 'array:intcomma'
            if (!is_array($out) || empty($out)) {
                ...
                foreach ($out as $outkey => $outval) {
                    $out[$outkey] = Functions::sanitizeVal($outval, $tmpcheck, $filter, $options);
                }
            }
        } else {
            ...
            $out = Functions::sanitizeVal($out, $check, $filter, $options);
        }
        ...
        if ($paramname == 'backtopage' || $paramname == 'backtolist' || $paramname == 'backtourl') {
            ...
            $out = str_replace([':', ';', '@', "\t", ' '], '', $out);        // Can be before the loop because only 1 char is replaced. No risk to retrieve it after other replacements.
            do {
                $oldstringtoclean = $out;
                $out = str_ireplace(['javascript', 'vbscript', '&colon', '&#'], '', $out);
                $out = preg_replace(['/^[^\?]*%/'], '', $out);                // We remove any % chars before the ?. Example in url: '/product/stock/card.php?action=create&backtopage=%2Fdolibarr_dev%2Fhtdocs%2Fpro%25duct%2Fcard.php%3Fid%3Dabc'
                $out = preg_replace(['/^[a-z]*\/\s*\/+/i'], '', $out);        // We remove schema*// to remove external URL
        ...
        if (empty($method) || $method == 3 || $method == 4) {
            if (preg_match('/^search_/', $paramname) || in_array($paramname, ['sortorder', 'sortfield'])) {
     ...
     *  Use the property $user->default_values[path]['creatform'] and/or $user->default_values[path]['filters'] and/or
     *  $user->default_values[path]['sortorder'] Note: The property $user->default_values is loaded by main.php when
     *  loading the user.
     *
     * @param string $paramname Name of the $_GET or $_POST parameter
     * @param int    $method    Type of method (0 = $_GET then $_POST, 1 = only $_GET, 2 = only $_POST, 3 = $_POST then
     *                          $_GET)
     *
    ...
    {
        return Functions::sanitizeVal($out, $check, $filter, $options);
    }
     ...
     * @return string|array                 Value sanitized (string or array). It may be '' if format check fails.
     */
    public static function sanitizeVal($out = '', $check = 'alphanohtml', $filter = null, $options = null)
    {
        ...
        switch ($check) {
            ...
            case 'alphanohtml':    // Recommended for most scalar parameters and search parameters
                if (!is_array($out)) {
                    ...
                    do {
                        $oldstringtoclean = $out;
                        // Remove html tags
                        $out = Functions::dol_string_nohtmltag($out, 0);
            ...
            case 'alphawithlgt':        // No " and no ../ but we keep balanced < > tags with no special chars inside. Can be used for email string like "Name <email>". Less secured than 'alphanohtml'
                if (!is_array($out)) {
                    ...
                    do {
                        $oldstringtoclean = $out;
                        // Remove html tags
                        $out = Functions::dol_html_entity_decode($out, ENT_COMPAT | ENT_HTML5, 'UTF-8');
                        // '"' is dangerous because param in url can close the href= or src= and add javascript functions.
                        // '../' or '..\' is dangerous because it allows dir transversals
                        // Note &#38, '&#0000038', '&#x26'... is a simple char like '&' alone but there is no reason to accept such way to encode input data.
                        $out = str_ireplace(['&#38', '&#0000038', '&#x26', '&quot', '&#34', '&#0000034', '&#x22', '"', '&#47', '&#0000047', '&#92', '&#0000092', '&#x2F', '../', '..\\'], '', $out);
            ...
            case 'custom':
                if (!empty($out)) {
                    if (empty($filter)) {
                        return 'BadParameterForGETPOST - Param 3 of Functions::sanitizeVal()';
                    }
                    /*if (empty($options)) {
                        return 'BadParameterForGETPOST - Param 4 of Functions::sanitizeVal()';
                    }*/
                    if (is_null($options)) {
                        $options = 0;
                    }
                ...
                break;
            default:
                Functions::dol_syslog("Error, you call Functions::sanitizeVal() with a bad value for the check type. Data will be sanitized with alphanohtml.", LOG_ERR);
     ...
     *  The prefix is unique for instance and avoid conflict between multi-instances, even when having two instances
     *  with same root dir or two instances in same virtual servers. This function must not use dol_hash (that is used
     *  for password hash) and need to have all context $conf loaded.
     *
    ...
    function dol_getprefix($mode = '')
    {
        ...
        if ($mode == 'email') {
            ...
            if (Functions::getDolGlobalString('MAIL_PREFIX_FOR_EMAIL_ID')) {    // If MAIL_PREFIX_FOR_EMAIL_ID is set
                if (Functions::getDolGlobalString('MAIL_PREFIX_FOR_EMAIL_ID') != 'SERVER_NAME') {
    ...
    function dol_include_once($relpath, $classname = '')
    {
        ...
        if (!file_exists($fullpath)) {
            Functions::dol_syslog('functions::dol_include_once Tried to load unexisting file: ' . $relpath, LOG_WARNING);
            return false;
        }
    ...
    /**
     *    Return path of url or filesystem. Can check into alternate dir or alternate dir + main dir depending on value
     *    of $returnemptyifnotfound.
     *
     * @param string $path                        Relative path to file (if mode=0) or relative url (if mode=1). Ie:
     *                                            mydir/myfile, ../myfile
     * @param int    $type                        0=Used for a Filesystem path, 1=Used for an URL path (output
     *                                            relative), 2=Used for an URL path (output full path using same host
     *                                            that current url), 3=Used for an URL path (output full path using
     *                                            host defined into $dolibarr_main_url_root of conf file)
     ...
     *
     * @return string                                Full filesystem path (if path=0) or '' if file not found, Full url
     *                                               path (if mode=1)
     */
     ...
     *  With native = 0: Property that are reference are different memory area in the new object (full isolation
     *  clone). This means $this->db of new object may not be valid. With native = 1: Use PHP clone. Property that are
     *  reference are same pointer. This means $this->db of new object is still valid but point to same this->db than
     *  original object. With native = 2: Property that are reference are different memory area in the new object (full
     *  isolation clone). Only scalar and array values are cloned. This means method are not availables and $this->db
     *  of new object is not valid.
     *
     * @param object $object Object to clone
     * @param int    $native 0=Full isolation method, 1=Native PHP method, 2=Full isolation method keeping only scalar
     *                       and array properties (recommended)
     *
    ...
    function dol_sanitizeUrl($stringtoclean, $type = 1)
    {
        ...
        do {
            ...
            $stringtoclean = str_ireplace(['javascript', 'vbscript', '&colon', '&#'], '', $stringtoclean);
        } while ($oldstringtoclean != $stringtoclean);

        if ($type == 1) {
            // removing '//' should disable links to external url like //aaa or http//)
            $stringtoclean = preg_replace(['/^[a-z]*\/\/+/i'], '', $stringtoclean);
        }
    ...
    /**
     *    Clean a string from all non printable ASCII chars (0x00-0x1F and 0x7F). It can also removes also Tab-CR-LF.
     *    UTF8 chars remains. This can be used to sanitize a string and view its real content. Some hacks try to
     *    obfuscate attacks by inserting non printable chars. Note, for information: UTF8 on 1 byte are: \x00-\7F
     *                                 2 bytes are: byte 1 \xc0-\xdf, byte 2 = \x80-\xbf
     *                                 3 bytes are: byte 1 \xe0-\xef, byte 2 = \x80-\xbf, byte 3 = \x80-\xbf
     *                                 4 bytes are: byte 1 \xf0-\xf7, byte 2 = \x80-\xbf, byte 3 = \x80-\xbf, byte 4 =
     *                                 \x80-\xbf
     *
     * @param string $str           String to clean
     ...
     * @param int    $mode               0=Escape also ' and " into ', 1=Escape ' but not " for usage into 'string',
     *                                   2=Escape " but not ' for usage into "string", 3=Escape ' and " with \
     * @param int    $noescapebackslashn 0=Escape also \n. 1=Do not escape \n.
     *
     * @return     string                        Escaped string. Both ' and " are escaped into ' if they are escaped.
     */
    public static function dol_escape_js($stringtoescape, $mode = 0, $noescapebackslashn = 0)
    {
    ...
    /**
     * Return a string ready to be output on a HTML page.
     * To output a text inside an attribute, you can use dolPrintHTMLForAttribute() or dolPrintHTMLForTextArea() inside
     * a textarea
     *
     ...
     *
     * @param string $stringtoescape      String to escape
     * @param int    $keepb               1=Replace b tags with escaped value (except if in $noescapetags), 0=Remove
     *                                    them completely
     * @param int    $keepn               1=Preserve \r\n strings, 0=Replace them with escaped value, -1=Remove them.
     *                                    Set to 1 when escaping for a <textarea>.
    ...
    public static function dol_escape_htmltag($stringtoescape, $keepb = 0, $keepn = 0, $noescapetags = '', $escapeonlyhtmltags = 0, $cleanalsojavascript = 0)
    {
        ...
        if ($cleanalsojavascript) {
            $stringtoescape = dol_string_onlythesehtmltags($stringtoescape, 0, 0, $cleanalsojavascript, 0, [], 0);
        }
        ...
        if (!$keepb) {
            $tmp = strtr($tmp, ["<b>" => '', '</b>' => '', '<strong>' => '', '</strong>' => '']);
        }
        if (!$keepn) {
            $tmp = strtr($tmp, ["\r" => '\\r', "\n" => '\\n']);
        } elseif ($keepn == -1) {
            ...
            if (count($tmparrayoftags)) {
                ...
                foreach ($tmparrayoftags as $tagtoreplace) {
                    ...
                    if (preg_match('/<' . preg_quote($tagtoreplace, '/') . '\s+([^>]+)>/', $tmp, $reg)) {
                        $tmpattributes = str_ireplace(['[', ']'], '_', $reg[1]);    // We must not have [ ] inside the attribute string
    ...
    /**
     * Convert first character of the first word of a string to upper. Never use ucfirst because it does not works with
     * UTF8 strings.
     *
     * @param string $string   String to encode
     ...
     *  Note: You can get log message into html sources by adding parameter &logtohtml=1 (constant MAIN_LOGTOHTML must
     *  be set) This function works only if syslog module is enabled. This must not use any call to other function
     *  calling Functions::dol_syslog (avoid infinite loop).
     *
     * @param string     $message                     Line to log. ''=Show nothing
     * @param int        $level                       Log level
     *                                                On Windows LOG_ERR=4, LOG_WARNING=5, LOG_NOTICE=LOG_INFO=6,
     *                                                LOG_DEBUG=6 si define_syslog_variables ou PHP 5.3+, 7 si dolibarr
     *                                                On Linux   LOG_ERR=3, LOG_WARNING=4, LOG_NOTICE=5, LOG_INFO=6,
     *                                                LOG_DEBUG=7
     * @param int        $ident                       1=Increase ident of 1 (after log), -1=Decrease ident of 1 (before
     *                                                log)
     * @param string     $suffixinfilename            When output is a file, append this suffix into default log
     *                                                filename. Example '_stripe', '_mail'
     * @param string     $restricttologhandler        Force output of log only to this log handler
     * @param array|null $logcontext                  If defined, an array with extra information (can be used by some
     *                                                log handlers)
     *
     * @return    void
     */
    public static function dol_syslog($message, $level = LOG_INFO, $ident = 0, $suffixinfilename = '', $restricttologhandler = '', $logcontext = null)
    {
        ...
        if (!empty($message)) {
            ...
            if (!Functions::getDolGlobalString('MAIN_SHOW_PASSWORD_INTO_LOG')) {
                $message = preg_replace('/password=\'[^\']*\'/', 'password=\'hidden\'', $message); // protection to avoid to have value of password in log
            }

            // If adding log inside HTML page is required
            if (
                (!empty($_REQUEST['logtohtml']) && Functions::getDolGlobalString('MAIN_ENABLE_LOG_TO_HTML'))
     ...
     *                                    Value is 'keyforpopupid:Name_of_html_component_to_set_with
     *                                    id,Name_of_html_component_to_set_with_label'
     ...
     *
     * @param array  $links           Array of tabs (0=>url, 1=>label, 2=>code, 3=>not used, 4=>text after link,
     *                                5=>morecssonlink). Currently initialized by calling a function
     *                                xxx_admin_prepare_head. Note that label into $links[$i][1] must be already HTML
     *                                escaped.
     * @param string $active          Active tab name (document', 'info', 'ldap', ....)
     * @param string $title           Title
     * @param int    $notab           -1 or 0=Add tab header, 1=no tab header (if you set this to 1, using print
     *                                dol_get_fiche_end() to close tab is not required), -2=Add tab header with no
     *                                sepaaration under tab (to start a tab just after), -3=Add tab header but no
     *                                footer separation
     * @param string $picto           Add a picto on tab title
     * @param int    $pictoisfullpath If 1, image path is a full path. If you set this to 1, you can use url returned
     *                                by dol_buildpath('/mymodyle/img/myimg.png',1) for $picto.
     ...
     */
    function dol_fiche_head($links = [], $active = '0', $title = '', $notab = 0, $picto = '', $pictoisfullpath = 0, $morehtmlright = '', $morecss = '', $limittoshow = 0, $moretabssuffix = '')
    {
     ...
     * @param int    $notab           -1 or 0=Add tab header, 1=no tab header (if you set this to 1, using print
     *                                dol_get_fiche_end() to close tab is not required), -2=Add tab header with no
     *                                seaparation under tab (to start a tab just after), -3=-2+'noborderbottom'
     * @param string $picto           Add a picto on tab title
     * @param int    $pictoisfullpath If 1, image path is a full path. If you set this to 1, you can use url returned
     *                                by dol_buildpath('/mymodyle/img/myimg.png',1) for $picto.
     * @param string $morehtmlright   Add more html content on right of tabs title
     ...
     * @return    string
     */
    function dol_get_fiche_head($links = [], $active = '', $title = '', $notab = 0, $picto = '', $pictoisfullpath = 0, $morehtmlright = '', $morecss = '', $limittoshow = 0, $moretabssuffix = '', $dragdropfile = 0)
    {
        ...
        // Show title
        if (!empty($title) && $showtitle && !Functions::getDolGlobalString('MAIN_OPTIMIZEFORTEXTBROWSER')) {
            $limittitle = 30;
        ...
        if (empty($limittoshow)) {
            $limittoshow = (!Functions::getDolGlobalString('MAIN_MAXTABS_IN_CARD') ? 99 : $conf->global->MAIN_MAXTABS_IN_CARD);
        }
        if (!empty($conf->dol_optimize_smallscreen)) {
            $limittoshow = 2;
        }
    ...
    /**
     *  Show tab footer of a card.
     *  Note: $object->next_prev_filter can be set to restrict select to find next or previous record by
     *  $form->showrefnav.
     *
     ...
     * @param string       $fieldid        Name of the field in DB to use to select next et previous (we make the
     *                                     select max and min on this field). Use 'none' for no prev/next search.
     ...
     * @param int          $onlybanner     Put this to 1, if the card will contains only a banner (this add css
     *                                     'arearefnobottom' on div)
    ...
    function dol_banner_tab($object, $paramid, $morehtml = '', $shownav = 1, $fieldid = 'rowid', $fieldref = 'ref', $morehtmlref = '', $moreparam = '', $nodbprefix = 0, $morehtmlleft = '', $morehtmlstatus = '', $onlybanner = 0, $morehtmlright = '')
    {
        ...
        if (Functions::getDolGlobalString('MAIN_USE_ADVANCED_PERMS') && !$user->hasRight('barcode', 'lire_advance')) {
            $showbarcode = 0;
        }
        $modulepart = 'unknown';
        ...
        if ($object->element == 'product') {
            ...
            if ($showimage) {
                ...
                if (Functions::getDolGlobalString('PRODUCT_NODISPLAYIFNOPHOTO')) {
                    $nophoto = '';
            ...
            if ($showimage) {
                ...
                if (Functions::getDolGlobalString('CATEGORY_NODISPLAYIFNOPHOTO')) {
                    $nophoto = '';
                    $morehtmlleft .= '<div class="floatleft inline-block valignmiddle divphotoref"></div>';
                } else {    // Show no photo link
                    $nophoto = '/public/theme/common/nophoto.png';
            ...
            if ($showimage) {
                ...
                if (Functions::getDolGlobalString('BOM_NODISPLAYIFNOPHOTO')) {
                    $nophoto = '';
                    $morehtmlleft .= '<div class="floatleft inline-block valignmiddle divphotoref"></div>';
                } else {    // Show no photo link
                    $nophoto = '/public/theme/common/nophoto.png';
            ...
            if (!$showimage) {
                if (Functions::getDolGlobalString('TICKET_NODISPLAYIFNOPHOTO')) {
                    $nophoto = '';
                    $morehtmlleft .= '<div class="floatleft inline-block valignmiddle divphotoref"></div>';
                } else {    // Show no photo link
            ...
            if ($showimage) {
                if ($modulepart != 'unknown' || method_exists($object, 'getDataToShowPhoto')) {
                    ...
                    if (in_array($modulepart, ['propal', 'commande', 'facture', 'ficheinter', 'contract', 'supplier_order', 'supplier_proposal', 'supplier_invoice', 'expensereport']) && class_exists("Imagick")) {
                        ...
                        if (in_array($modulepart, ['invoice_supplier', 'supplier_invoice'])) {
        ...
        if ($object->element == 'societe') {
            if (!empty($conf->use_javascript_ajax) && $user->hasRight('societe', 'creer') && Functions::getDolGlobalString('MAIN_DIRECT_STATUS_UPDATE')) {
            ...
            if (!empty($conf->use_javascript_ajax) && $user->hasRight('produit', 'creer') && Functions::getDolGlobalString('MAIN_DIRECT_STATUS_UPDATE')) {
        ...
        } elseif (in_array($object->element, ['salary'])) {
            $tmptxt = $object->getLibStatut(6, $object->alreadypaid);
            if (empty($tmptxt) || $tmptxt == $object->getLibStatut(3)) {
                $tmptxt = $object->getLibStatut(5, $object->alreadypaid);
            }
            $morehtmlstatus .= $tmptxt;
        } elseif (in_array($object->element, ['facture', 'invoice', 'invoice_supplier', 'chargesociales', 'loan', 'tva'])) {    // TODO Move this to use ->alreadypaid
        ...
        if (Functions::isModEnabled('accounting') && in_array($object->element, ['bank', 'paiementcharge', 'facture', 'invoice', 'invoice_supplier', 'expensereport', 'payment_various'])) {
        ...
        // Add label
        if (in_array($object->element, ['product', 'bank_account', 'project_task'])) {
        ...
        if (method_exists($object, 'getBannerAddress') && !in_array($object->element, ['product', 'bookmark', 'ecm_directories', 'ecm_files'])) {
        ...
        if (Functions::getDolGlobalString('MAIN_SHOW_TECHNICAL_ID') && (Functions::getDolGlobalString('MAIN_SHOW_TECHNICAL_ID') == '1' || preg_match('/' . preg_quote($object->element, '/') . '/i', $conf->global->MAIN_SHOW_TECHNICAL_ID)) && !empty($object->id)) {
    ...
    function dol_format_address($object, $withcountry = 0, $sep = "\n", $outputlangs = null, $mode = 0, $extralangcode = '')
    {
        ...
        // Zip/Town/State
        if (isset($object->country_code) && in_array($object->country_code, ['AU', 'CA', 'US', 'CN']) || Functions::getDolGlobalString('MAIN_FORCE_STATE_INTO_ADDRESS')) {
        ...
        } elseif (isset($object->country_code) && in_array($object->country_code, ['GB', 'UK'])) {
        ...
        } elseif (isset($object->country_code) && in_array($object->country_code, ['ES', 'TR'])) {
        ...
        } elseif (isset($object->country_code) && in_array($object->country_code, ['JP'])) {
        ...
        } elseif (isset($object->country_code) && in_array($object->country_code, ['IT'])) {
        ...
        if ($hookmanager) {
            $parameters = ['withcountry' => $withcountry, 'sep' => $sep, 'outputlangs' => $outputlangs, 'mode' => $mode, 'extralangcode' => $extralangcode];
     ...
     * @param int    $ts     Timestamp (If is_gmt is true, timestamp is already includes timezone and daylight saving
     *                       offset, if is_gmt is false, timestamp is a GMT timestamp and we must compensate with
     *                       server PHP TZ)
     ...
     *                                        "%d/%m/%Y %H:%M",
     *                                        "%d/%m/%Y %H:%M:%S",
     *                                        "%B"=Long text of month, "%A"=Long text of day, "%b"=Short text of month,
     *                                        "%a"=Short text of day
     *                                        "day", "daytext", "dayhour", "dayhourldap", "dayhourtext", "dayrfc",
     *                                        "dayhourrfc", "...inputnoreduce", "...reduceformat"
     * @param string|bool $tzoutput           true or 'gmt' => string is for Greenwich location
     *                                        false or 'tzserver' => output string is for local PHP server TZ usage
     *                                        'tzuser' => output string is for user TZ (current browser TZ with current
     *                                        dst) => In a future, we should have same behaviour than 'tzuserrel'
     *                                        'tzuserrel' => output string is for user TZ (current browser TZ with dst
     *                                        or not, depending on date position)
     * @param Translate   $outputlangs        Object lang that contains language for text translation.
    ...
    function dol_print_date($time, $format = '', $tzoutput = 'auto', $outputlangs = null, $encodetooutput = false)
    {
        ...
        $reduceformat = (!empty($conf->dol_optimize_smallscreen) && in_array($format, ['day', 'dayhour', 'dayhoursec'])) ? 1 : 0;    // Test on original $format param.
        ...
        if (preg_match('/^([0-9][0-9][0-9][0-9])([0-9][0-9])([0-9][0-9])([0-9][0-9])([0-9][0-9])([0-9][0-9])$/i', $time, $reg)) {    // Deprecated. Ex: 1970-01-01, 1970-01-01 01:00:00, 19700101010000
            ...
            Functions::dol_syslog("Functions.lib::dol_print_date function called with a bad value from page " . (empty($_SERVER["PHP_SELF"]) ? 'unknown' : $_SERVER["PHP_SELF"]), LOG_WARNING);
            ...
            $newformat = str_replace(
                ['%Y', '%y', '%m', '%d', '%H', '%I', '%M', '%S', '%p', 'T', 'Z', '__a__', '__A__', '__b__', '__B__'],
                ['Y', 'y', 'm', 'd', 'H', 'h', 'i', 's', 'A', '__£__', '__$__', '__{__', '__}__', '__[__', '__]__'],
                $format
            );
            $ret = $dtts->format($newformat);
            $ret = str_replace(
                ['__£__', '__$__', '__{__', '__}__', '__[__', '__]__'],
                ['T', 'Z', '__a__', '__A__', '__b__', '__B__'],
                $ret
            );
        } else {
            ...
            if ($time < 100000000000) {    // Protection against bad date values
                ...
                $dtts->setTimezone($tzo);
                $newformat = str_replace(
                    ['%Y', '%y', '%m', '%d', '%H', '%I', '%M', '%S', '%p', '%w', 'T', 'Z', '__a__', '__A__', '__b__', '__B__'],
                    ['Y', 'y', 'm', 'd', 'H', 'h', 'i', 's', 'A', 'w', '__£__', '__$__', '__{__', '__}__', '__[__', '__]__'],
                    $format
                );
                $ret = $dtts->format($newformat);
                $ret = str_replace(
     ...
     * @param string  $forcetimezone '' to use the PHP server timezone. Or use a form like 'gmt', 'Europe/Paris' or
     *                               '+0200' to force timezone.
     *
     ...
     *                                        'auto'
     *                                        'tzuser' = local to user TZ taking dst into account at the current date.
     *                                        Not yet implemented.
     *                                        'tzuserrel' = local to user TZ taking dst into account at the given date.
     *                                        Use this one to convert date input from user into a GMT date.
    ...
    function dol_mktime($hour, $minute, $second, $month, $day, $year, $gm = 'auto', $check = 1)
    {
        ...
        if (empty($gm) || ($gm === 'server' || $gm === 'tzserver')) {
            ...
            try {
                ...
                Functions::dol_syslog("Warning dol_tz_string contains an invalid value " . $_SESSION["dol_tz_string"], LOG_WARNING);
            ...
            try {
                ...
                Functions::dol_syslog("Warning passed timezone contains an invalid value " . $timezone, LOG_WARNING);
            }
        }
     ...
     * @param int        $addlink     0=no link, 1=email has a html email link (+ link to create action if constant
     *                                AGENDA_ADDACTIONFOREMAIL is on)
     * @param int        $max         Max number of characters to show
    ...
    function dol_print_email($email, $cid = 0, $socid = 0, $addlink = 0, $max = 64, $showinvalid = 1, $withpicto = 0)
    {
        ...
        if (!empty($addlink)) {
            ...
            if (($cid || $socid) && Functions::isModEnabled('agenda') && $user->hasRight("agenda", "myactions", "create")) {
                $type = 'AC_EMAIL';
                $linktoaddaction = '';
                if (Functions::getDolGlobalString('AGENDA_ADDACTIONFOREMAIL')) {
        ...
        if ($hookmanager) {
            $parameters = ['cid' => $cid, 'socid' => $socid, 'addlink' => $addlink, 'picto' => $withpicto];
     ...
     * @return    string                            HTML Link
     */
    function dol_print_socialnetworks($value, $cid, $socid, $type, $dictsocialnetworks = [])
    {
        ...
        if (!empty($type)) {
            ...
            if ($type == 'skype') {
                ...
                if (($cid || $socid) && Functions::isModEnabled('agenda') && $user->hasRight('agenda', 'myactions', 'create')) {
                    $addlink = 'AC_SKYPE';
                    $link = '';
                    if (Functions::getDolGlobalString('AGENDA_ADDACTIONFORSKYPE')) {
     ...
     * @param int    $cid         Id of contact if known
     * @param int    $socid       Id of third party if known
     * @param string $addlink     ''=no link to create action, 'AC_TEL'=add link to clicktodial (if module enabled) and
     *                            add link to create event (if conf->global->AGENDA_ADDACTIONFORPHONE set), 'tel'=Force
     *                            "tel:..." link
     * @param string $separ       Separation between numbers for a better visibility example : xx.xx.xx.xx.xx. You can
     *                            also use 'hidenum' to hide the number, keep only the picto.
    ...
    function dol_print_phone($phone, $countrycode = '', $cid = 0, $socid = 0, $addlink = '', $separ = "&nbsp;", $withpicto = '', $titlealt = '', $adddivfloat = 0)
    {
        ...
        if (Functions::getDolGlobalString('MAIN_PHONE_SEPAR')) {
            $separ = Functions::getDolGlobalString('MAIN_PHONE_SEPAR');
        }
        ...
        if (!empty($addlink)) {    // Link on phone number (+ link to add action if conf->global->AGENDA_ADDACTIONFORPHONE set)
            if ($addlink == 'tel' || $conf->browser->layout == 'phone' || (Functions::isModEnabled('clicktodial') && Functions::getDolGlobalString('CLICKTODIAL_USE_TEL_LINK_ON_PHONE_NUMBERS'))) {    // If phone or option for, we use link of phone
                $newphoneastart = '<a href="tel:' . $phone . '">';
                $newphoneaend .= '</a>';
            } elseif (Functions::isModEnabled('clicktodial') && $addlink == 'AC_TEL') {        // If click to dial, we use click to dial url
                if (empty($user->clicktodial_loaded)) {
                    $user->fetch_clicktodial();
                }

                // Define urlmask
                $urlmask = Functions::getDolGlobalString('CLICKTODIAL_URL', 'ErrorClickToDialModuleNotConfigured');
                ...
                if (!Functions::getDolGlobalString('CLICKTODIAL_DO_NOT_USE_AJAX_CALL')) {
                    ...
                    if (Functions::getDolGlobalString('CLICKTODIAL_FORCENEWTARGET')) {
            ...
            //if (($cid || $socid) && Functions::isModEnabled('agenda') && $user->hasRight('agenda', 'myactions', 'create'))
            if (Functions::isModEnabled('agenda') && $user->hasRight("agenda", "myactions", "create")) {
                $type = 'AC_TEL';
                $addlinktoagenda = '';
                if ($addlink == 'AC_FAX') {
                    $type = 'AC_FAX';
                }
                if (Functions::getDolGlobalString('AGENDA_ADDACTIONFORPHONE')) {
        ...
        if ($hookmanager) {
            $parameters = ['countrycode' => $countrycode, 'cid' => $cid, 'socid' => $socid, 'titlealt' => $titlealt, 'picto' => $withpicto];
    ...
    function dol_print_address($address, $htmlid, $element, $id, $noprint = 0, $charfornl = '')
    {
        ...
        if ($address) {
            if ($hookmanager) {
                $parameters = ['element' => $element, 'id' => $id];
            ...
            if (empty($reshook)) {
                ...
                $showgmap = $showomap = 0;
                if (($element == 'thirdparty' || $element == 'societe') && Functions::isModEnabled('google') && Functions::getDolGlobalString('GOOGLE_ENABLE_GMAPS')) {
                    $showgmap = 1;
                }
                if ($element == 'contact' && Functions::isModEnabled('google') && Functions::getDolGlobalString('GOOGLE_ENABLE_GMAPS_CONTACTS')) {
                    $showgmap = 1;
                }
                if ($element == 'member' && Functions::isModEnabled('google') && Functions::getDolGlobalString('GOOGLE_ENABLE_GMAPS_MEMBERS')) {
                    $showgmap = 1;
                }
                if ($element == 'user' && Functions::isModEnabled('google') && Functions::getDolGlobalString('GOOGLE_ENABLE_GMAPS_USERS')) {
                    $showgmap = 1;
                }
                if (($element == 'thirdparty' || $element == 'societe') && Functions::isModEnabled('openstreetmap') && Functions::getDolGlobalString('OPENSTREETMAP_ENABLE_MAPS')) {
                    $showomap = 1;
                }
                if ($element == 'contact' && Functions::isModEnabled('openstreetmap') && Functions::getDolGlobalString('OPENSTREETMAP_ENABLE_MAPS_CONTACTS')) {
                    $showomap = 1;
                }
                if ($element == 'member' && Functions::isModEnabled('openstreetmap') && Functions::getDolGlobalString('OPENSTREETMAP_ENABLE_MAPS_MEMBERS')) {
                    $showomap = 1;
                }
                if ($element == 'user' && Functions::isModEnabled('openstreetmap') && Functions::getDolGlobalString('OPENSTREETMAP_ENABLE_MAPS_USERS')) {
                    $showomap = 1;
                }
                if ($showgmap) {
     ...
     *
     * @param string $address             email (Ex: "toto@example.com". Long form "John Do <johndo@example.com>" will
     *                                    be false)
    ...
    function isValidMXRecord($domain)
    {
        if (function_exists('idn_to_ascii') && function_exists('checkdnsrr')) {
            ...
            if (function_exists('getmxrr')) {
                ...
                if (count($mxhosts) == 1 && !in_array((string) $mxhosts[0], ['', '.'])) {
                    return 1;
                }
     ...
     * @param int    $size           Max string size visible (excluding …). 0 for no limit. WARNING: Final string size
     *                               can have 3 more chars (if we added …, or if size was max+1 so it does not worse to
     *                               replace with ...)
     * @param string $trunc          Where to trunc: 'right', 'left', 'middle' (size must be a 2 power), 'wrap'
     * @param string $stringencoding Tell what is source string encoding
     * @param int    $nodot          Truncation do not add … after truncation. So it's an exact truncation.
     * @param int    $display        Trunc is used to display data and can be changed for small screen. TODO Remove
     *                               this param (must be dealt with CSS)
     *
     * @return string                        Truncated string. WARNING: length is never higher than $size if $nodot is
     *                                       set, but can be 3 chars higher otherwise.
     */
    function dol_trunc($string, $size = 40, $trunc = 'right', $stringencoding = 'UTF-8', $nodot = 0, $display = 0)
    {
        $conf = Globals::getConf();

        if (empty($size) || Functions::getDolGlobalString('MAIN_DISABLE_TRUNC')) {
            return $string;
        }
    ...
    function img_picto($titlealt, $picto, $moreatt = '', $pictoisfullpath = false, $srconly = 0, $notitle = 0, $alt = '', $morecss = '', $marginleftonlyshort = 2)
    {
        ...
        if ($pictoisfullpath) {
            ...
            if (
                ...
                $facolor = '';
                $fasize = '';
                $fa = Functions::getDolGlobalString('MAIN_FONTAWESOME_ICON_STYLE', 'fas');
                if (in_array($pictowithouttext, ['card', 'bell', 'clock', 'establishment', 'file', 'file-o', 'generic', 'minus-square', 'object_generic', 'pdf', 'plus-square', 'timespent', 'note', 'off', 'on', 'object_bookmark', 'bookmark', 'vcard'])) {
                    $fa = 'far';
                }
                if (in_array($pictowithouttext, ['black-tie', 'github', 'google', 'microsoft', 'skype', 'twitter', 'facebook', 'linkedin', 'instagram', 'snapchat', 'stripe', 'stripe-s', 'youtube', 'google-plus-g', 'whatsapp'])) {
                    $fa = 'fab';
                }
                ...
                if ($pictowithouttext == 'off') {
                    ...
                    $marginleftonlyshort = 1;
                } elseif (in_array($pictowithouttext, ['1uparrow', '1downarrow', '1leftarrow', '1rightarrow', '1uparrow_selected', '1downarrow_selected', '1leftarrow_selected', '1rightarrow_selected'])) {
                    $convertarray = ['1uparrow' => 'caret-up', '1downarrow' => 'caret-down', '1leftarrow' => 'caret-left', '1rightarrow' => 'caret-right', '1uparrow_selected' => 'caret-up', '1downarrow_selected' => 'caret-down', '1leftarrow_selected' => 'caret-left', '1rightarrow_selected' => 'caret-right'];
                ...
                if (in_array($pictowithouttext, ['dollyrevert', 'member', 'members', 'contract', 'group', 'resource', 'shipment'])) {
                    $morecss .= ' em092';
                }
                if (in_array($pictowithouttext, ['conferenceorbooth', 'collab', 'eventorganization', 'holiday', 'info', 'project', 'workstation'])) {
                    $morecss .= ' em088';
                }
                if (in_array($pictowithouttext, ['asset', 'intervention', 'payment', 'loan', 'partnership', 'stock', 'technic'])) {
                    $morecss .= ' em080';
                }
            ...
            if (Functions::getDolGlobalString('MAIN_OVERWRITE_THEME_PATH')) {
                $path = Functions::getDolGlobalString('MAIN_OVERWRITE_THEME_PATH') . '/theme/' . $theme; // If the theme does not have the same name as the module
            } elseif (Functions::getDolGlobalString('MAIN_OVERWRITE_THEME_RES')) {
                $path = Functions::getDolGlobalString('MAIN_OVERWRITE_THEME_RES') . '/theme/' . Functions::getDolGlobalString('MAIN_OVERWRITE_THEME_RES'); // To allow an external module to overwrite image resources whatever is activated theme
     ...
     *
     * @param string $titlealt                Text on alt and title of image. Alt only if param notitle is set to 1. If
     *                                        text is "TextA:TextB", use Text A on alt and Text B on title.
     * @param string $picto                   Name of image to show object_picto (example: user, group, action, bill,
     *                                        contract, propal, product, ...) For external modules use
     *                                        imagename@mymodule to search into directory "img" of module.
     * @param string $moreatt                 Add more attribute on img tag (ie: class="datecallink")
     * @param int    $pictoisfullpath         If 1, image path is a full path
     * @param int    $srconly                 Return only content of the src attribute of img.
     * @param int    $notitle                 1=Disable tag title. Use it if you add js tooltip, to avoid duplicate
     *                                        tooltip.
     *
     * @return    string                        Return img tag
     ...
     * @param string     $titlealt        Text on alt and title of image. Alt only if param notitle is set to 1. If
     *                                    text is "TextA:TextB", use Text A on alt and Text B on title.
     * @param string|int $picto           Name of image file to show (If no extension provided, we use '.png'). Image
     *                                    must be stored into htdocs/theme/common directory. Or level of meteo image
     *                                    (0-4).
     ...
     *
     * @param string $titlealt        Text on alt and title of image. Alt only if param notitle is set to 1. If text is
     *                                "TextA:TextB", use Text A on alt and Text B on title.
     * @param string $picto           Name of image file to show (If no extension provided, we use '.png'). Image must
     *                                be stored into htdocs/theme/common directory.
     * @param string $moreatt         Add more attribute on img tag
     * @param int    $pictoisfullpath If 1, image path is a full path
     ...
     * @param string $titlealt Text on alt and title of image. Alt only if param notitle is set to 1. If text is
     *                         "TextA:TextB", use Text A on alt and Text B on title.
     * @param string $moreatt  Add more attribute on img tag (For example 'style="float: right"'). If 1, add float:
     *                         right. Can't be "class" attribute.
    ...
    function img_credit_card($brand, $morecss = null)
    {
        ...
        } elseif (!in_array($brand, ['cc-visa', 'cc-mastercard', 'cc-amex', 'cc-discover', 'cc-jcb', 'cc-diners-club'])) {
            $brand = 'credit-card';
        }
     ...
     * @param integer $infoonimgalt    Info is shown only on alt of star picto, otherwise it is show on output after
     *                                 the star picto
     * @param int     $nodiv           No div
     * @param string  $admin           '1'=Info for admin users. '0'=Info for standard users (change only the look),
     *                                 'error', 'warning', 'xxx'=Other
    ...
    /**
     *  Displays error message system with all the information to facilitate the diagnosis and the escalation of the
     *  bugs. This function must be called when a blocking technical error is encountered. However, one must try to
     *  call it only within php pages, classes must return their error through their property "error".
     *
    ...
    function dol_print_error($db = null, $error = '', $errors = null)
    {
        ...
        Functions::dol_syslog("Error " . $syslog, LOG_ERR);
    }
     ...
     * @param string $prefix                 Prefix for css. Use space after prefix to add your own CSS tag, for
     *                                       example 'mycss '.
     ...
     * @param int    $thead                  0=To use with standard table format, 1=To use inside <thead><tr>, 2=To use
     *                                       with <div>
     * @param string $file                   Url used when we click on sort picto
     * @param string $field                  Field to use for new sorting. Empty if this field is not sortable. Example
     *                                       "t.abc" or "t.abc,t.def"
     * @param string $begin                  ("" by default)
     * @param string $moreparam              Add more parameters on sort url links ("" by default)
     * @param string $moreattrib             Add more attributes on th ("" by default). To add more css class, use
     *                                       param $prefix.
    ...
    function getTitleFieldOfList($name, $thead = 0, $file = "", $field = "", $begin = "", $moreparam = "", $moreattrib = "", $sortfield = "", $sortorder = "", $prefix = "", $disablesortlink = 0, $tooltip = '', $forcenowrapcolumntitle = 0)
    {
        ...
        if (!Functions::getDolGlobalString('MAIN_DISABLE_WRAPPING_ON_COLUMN_TITLE') && empty($forcenowrapcolumntitle)) {
    ...
    {
        Functions::dol_syslog(__FUNCTION__ . " is deprecated", LOG_WARNING);
     ...
     * @param string     $options                  More parameters for links ('' by default, does not include sortfield
     *                                             neither sortorder). Value must be 'urlencoded' before calling
     *                                             function.
     * @param string     $sortfield                Field to sort on ('' by default)
     * @param string     $sortorder                Order to sort ('' by default)
     * @param string     $morehtmlcenter           String in the middle ('' by default). We often find here string
     *                                             $massaction coming from $form->selectMassAction()
     * @param int        $num                      Number of records found by select with limit+1
     * @param int|string $totalnboflines           Total number of records/lines for all pages (if known). Use a
     *                                             negative value of number to not show number. Use '' if unknown.
     * @param string     $picto                    Icon to use before title (should be a 32x32 transparent png file)
     * @param int        $pictoisfullpath          1=Icon name is a full absolute url of image
     ...
     * @param int        $pagenavastextinput       1=Do not suggest list of pages to navigate but suggest the page
     *                                             number into an input field.
     ...
     * @param string  $options         Other url parameters to propagate ("" by default, may include sortfield and
     *                                 sortorder)
     * @param integer $nextpage        Do we show a next page button
     * @param string  $betweenarrows   HTML content to show between arrows. MUST contains '<li> </li>' tags or
     *                                 '<li><span> </span></li>'.
     ...
     * @param int     $hidenavigation  Force to hide the switch mode view and the navigation tool (hide limit section,
     *                                 html in $betweenarrows and $afterarrows but not $beforearrows)
     *
     * @return    void
     */
    function print_fleche_navigation($page, $file, $options = '', $nextpage = 0, $betweenarrows = '', $afterarrows = '', $limit = -1, $totalnboflines = 0, $hideselectlimit = 0, $beforearrows = '', $hidenavigation = 0)
    {
        ...
        if (empty($hidenavigation)) {
            if ((int) $limit > 0 && empty($hideselectlimit)) {
                ...
                //$pagesizechoices.=',2:2';
                if (Functions::getDolGlobalString('MAIN_PAGESIZE_CHOICES')) {
                    $pagesizechoices = Functions::getDolGlobalString('MAIN_PAGESIZE_CHOICES');
                }
     ...
     *
     * @return    string                    String with formatted amounts ('19,6' or '19,6%' or '8.5% (NPR)' or '8.5%
     *                                      *' or '19,6 (CODEX)')
     */
     ...
     * @param Translate|string $outlangs      Object langs for output. '' use default lang. 'none' use international
     *                                        separators.
     * @param int              $trunc         1=Truncate if there is more decimals than MAIN_MAX_DECIMALS_SHOWN
     *                                        (default), 0=Does not truncate. Deprecated because amount are rounded (to
     *                                        unit or total amount accuracy) before being inserted into database or
     *                                        after a computation, so this parameter should be useless.
     * @param int              $rounding      MINIMUM number of decimal to show: 0=no change, -1=we use
     *                                        min($conf->global->MAIN_MAX_DECIMALS_UNIT,$conf->global->MAIN_MAX_DECIMALS_TOT)
     * @param int|string       $forcerounding MAXIMUM number of decimal to forcerounding decimal: -1=no change, 'MU' or
     *                                        'MT' or a numeric to round to MU or MT or to a given number of decimal
     * @param string           $currency_code To add currency symbol (''=add nothing, 'auto'=Use default currency,
     *                                        'XXX'=add currency symbols for XXX currency)
     *
    ...
    function price($amount, $form = 0, $outlangs = '', $trunc = 1, $rounding = -1, $forcerounding = -1, $currency_code = '')
    {
        ...
        if ($rounding == -1) {
            $rounding = min(Functions::getDolGlobalString('MAIN_MAX_DECIMALS_UNIT'), Functions::getDolGlobalString('MAIN_MAX_DECIMALS_TOT'));
        }
        $nbdecimal = $rounding;
        ...
        if ($trunc && $nbdecimal > $conf->global->MAIN_MAX_DECIMALS_SHOWN) {
            $nbdecimal = Functions::getDolGlobalString('MAIN_MAX_DECIMALS_SHOWN');
        ...
        if ((string) $forcerounding != '-1') {
            if ($forcerounding === 'MU') {
                $nbdecimal = Functions::getDolGlobalString('MAIN_MAX_DECIMALS_UNIT');
            } elseif ($forcerounding === 'MT') {
                $nbdecimal = Functions::getDolGlobalString('MAIN_MAX_DECIMALS_TOT');
        ...
        if ($currency_code && is_object($outlangs)) {
            ...
            $listofcurrenciesbefore = ['AUD', 'CAD', 'CNY', 'COP', 'CLP', 'GBP', 'HKD', 'MXN', 'PEN', 'USD', 'CRC'];
            $listoflanguagesbefore = ['nl_NL'];
    ...
    /**
     *    Function that return a number with universal decimal format (decimal separator is '.') from an amount typed
     *    by a user. Function to use on each input amount before any numeric test or database insert. A better name for
     *    this function should be roundtext2num().
     *
     ...
     * @param int          $option                Put 1 if you know that content is already universal format number (so
     *                                            no correction on decimal will be done) Put 2 if you know that number
     *                                            is a user input (so we know we have to fix decimal separator).
     *
     * @return    string                            Amount with universal numeric format (Example: '99.99999').
     *                                            If conversion fails to return a numeric, it returns:
     *                                            - text unchanged or partial if ($rounding = ''): price2num('W9ç', '',
     *                                            0)   => '9ç', price2num('W9ç', '', 1)   => 'W9ç', price2num('W9ç',
     *                                            '', 2)   => '9ç'
     *                                            - '0' if ($rounding is defined):                 price2num('W9ç',
     *                                            'MT', 0) => '9',  price2num('W9ç', 'MT', 1) => '0',
     *                                            price2num('W9ç', 'MT', 2) => '9' Note: The best way to guarantee a
     *                                            numeric value is to add a cast (float) before the price2num(). If
     *                                            amount is null or '', it returns '' if $rounding = '', it returns '0'
     *                                            if $rounding is defined.
     *
    ...
    function price2num($amount, $rounding = '', $option = 0)
    {
        ...
        if ($rounding) {
            $nbofdectoround = '';
            if ($rounding == 'MU') {
                $nbofdectoround = Functions::getDolGlobalString('MAIN_MAX_DECIMALS_UNIT');
            } elseif ($rounding == 'MT') {
                $nbofdectoround = Functions::getDolGlobalString('MAIN_MAX_DECIMALS_TOT');
            } elseif ($rounding == 'MS') {
                $nbofdectoround = isset($conf->global->MAIN_MAX_DECIMALS_STOCK) ? $conf->global->MAIN_MAX_DECIMALS_STOCK : 5;
            } elseif ($rounding == 'CU') {
                $nbofdectoround = max(Functions::getDolGlobalString('MAIN_MAX_DECIMALS_UNIT'), 8);    // TODO Use param of currency
            } elseif ($rounding == 'CT') {
                $nbofdectoround = max(Functions::getDolGlobalString('MAIN_MAX_DECIMALS_TOT'), 8);        // TODO Use param of currency
     ...
     * @param string    $forceunitoutput 'no' or numeric (-3, -6, ...) compared to $unit (In most case, this value is
     *                                   value defined into $conf->global->MAIN_WEIGHT_DEFAULT_UNIT)
    ...
    /**
     *    Return localtax rate for a particular vat, when selling a product with vat $vatrate, from a $thirdparty_buyer
     *    to a $thirdparty_seller Note: This function applies same rules than get_default_tva
     *
        ...
        /*if ($thirdparty_buyer->country_code != $thirdparty_seller->country_code)
    {
        ...
        if (in_array($mysoc->country_code, ['ES'])) {
            $conf->global->MAIN_GET_LOCALTAXES_VALUES_FROM_THIRDPARTY = 1;
        }

        // Search local taxes
        if (Functions::getDolGlobalString('MAIN_GET_LOCALTAXES_VALUES_FROM_THIRDPARTY')) {
            if ($local == 1) {
     ...
     *
     * @param int|string $vatrate        VAT ID or Rate. Value can be value or the string with code into parenthesis or
     *                                   rowid if $firstparamisid is 1. Example: '8.5' or '8.5 (8.5NPR)' or 123.
    ...
    function getTaxesFromId($vatrate, $buyer = null, $seller = null, $firstparamisid = 1)
    {
        ...
        Functions::dol_syslog("getTaxesFromId vat id or rate = " . $vatrate);
     ...
     *  Instead this function must be called when adding a line to get the array of possible values for localtax and
     *  type, and then provide the selected value to the function calcul_price_total.
     *
     * @param int|string $vatrate        VAT ID or Rate+Code. Value can be value or the string with code into
     *                                   parenthesis or rowid if $firstparamisid is 1. Example: '8.5' or '8.5 (8.5NPR)'
     *                                   or 123.
     ...
     *
     * @return    array                            array(localtax_type1(1-6 or 0 if not found), rate localtax1,
     *                                             localtax_type2, rate localtax2, accountancycodecust,
     *                                             accountancycodesupp)
     * @see getTaxesFromId()
     */
    function getLocalTaxesFromRate($vatrate, $local, $buyer, $seller, $firstparamisid = 0)
    {
        ...
        Functions::dol_syslog("getLocalTaxesFromRate vatrate=" . $vatrate . " local=" . $local);
        ...
        if ($resql) {
            ...
            if ($obj) {
                ...
                if ($local == 1) {
                    return [$obj->localtax1_type, get_localtax($vateratestring, $local, $buyer, $seller), $obj->accountancy_code_sell, $obj->accountancy_code_buy];
                } elseif ($local == 2) {
                    return [$obj->localtax2_type, get_localtax($vateratestring, $local, $buyer, $seller), $obj->accountancy_code_sell, $obj->accountancy_code_buy];
                } else {
                    return [$obj->localtax1_type, get_localtax($vateratestring, 1, $buyer, $seller), $obj->localtax2_type, get_localtax($vateratestring, 2, $buyer, $seller), $obj->accountancy_code_sell, $obj->accountancy_code_buy];
                }
            }
        }
    ...
    function get_product_vat_for_country($idprod, $thirdpartytouse, $idprodfournprice = 0)
    {
        ...
        if (!$found) {
            if (!Functions::getDolGlobalString('MAIN_VAT_DEFAULT_IF_AUTODETECT_FAILS')) {
                ...
                // or '1.23 (CODE)'
                $defaulttx = '';
                if (Functions::getDolGlobalString('MAIN_VAT_DEFAULT_IF_AUTODETECT_FAILS') != 'none') {
                    $defaulttx = Functions::getDolGlobalString('MAIN_VAT_DEFAULT_IF_AUTODETECT_FAILS');
                }
        ...
        Functions::dol_syslog("get_product_vat_for_country: ret=" . $ret);
        return $ret;
    }
    ...
    function get_product_localtax_for_country($idprod, $local, $thirdpartytouse)
    {
        ...
        Functions::dol_syslog("get_product_localtax_for_country: ret=" . $ret);
        return $ret;
    }
     ...
     *     VATRULE 2: If the (seller country = buyer country) then the default VAT = VAT of the product sold. End of
     *     rule. VATRULE 3: If (seller and buyer in the European Community) and (property sold = new means of transport
     *     such as car, boat, plane) then VAT by default = 0 (VAT must be paid by the buyer to the tax center of his
     *     country and not to the seller). End of rule. VATRULE 4: If (seller and buyer in the European Community) and
     *     (buyer = individual) then VAT by default = VAT of the product sold. End of rule VATRULE 5: If (seller and
     *     buyer in European Community) and (buyer = company) then VAT by default=0. End of rule VATRULE 6: Otherwise
     *     the VAT proposed by default=0. End of rule.
     *
     ...
     *
     * @return float|string                        Vat rate to use with format 5.0 or '5.0 (XXX)', -1 if we can't guess
     *                                             it
    ...
    function get_default_tva(Societe $thirdparty_seller, Societe $thirdparty_buyer, $idprod = 0, $idprodfournprice = 0)
    {
        ...
        Functions::dol_syslog("get_default_tva: seller use vat=" . $seller_use_vat . ", seller country=" . $seller_country_code . ", seller in cee=" . $seller_in_cee . ", buyer vat number=" . $thirdparty_buyer->tva_intra . " buyer country=" . $buyer_country_code . ", buyer in cee=" . $buyer_in_cee . ", idprod=" . $idprod . ", idprodfournprice=" . $idprodfournprice . ", SERVICE_ARE_ECOMMERCE_200238EC=" . (Functions::getDolGlobalString('SERVICES_ARE_ECOMMERCE_200238EC') ? $conf->global->SERVICES_ARE_ECOMMERCE_200238EC : ''));

        // If services are eServices according to EU Council Directive 2002/38/EC (http://ec.europa.eu/taxation_customs/taxation/vat/traders/e-commerce/article_1610_en.htm)
        // we use the buyer VAT.
        if (Functions::getDolGlobalString('SERVICE_ARE_ECOMMERCE_200238EC')) {
        ...
        if (
            ...
            if ($seller_country_code == 'IN' && Functions::getDolGlobalString('MAIN_SALETAX_AUTOSWITCH_I_CS_FOR_INDIA')) {
        ...
        if (Functions::getDolGlobalString('MAIN_USE_VAT_OF_PRODUCT_FOR_INDIVIDUAL_CUSTOMER_OUT_OF_EEC') && empty($buyer_in_cee)) {
            $isacompany = $thirdparty_buyer->isACompany();
            if (!$isacompany) {
                return get_product_vat_for_country($idprod, $thirdparty_seller, $idprodfournprice);
                //print 'VATRULE extra';
     ...
     * @param int        $color 0=texte only, 1=Text is formatted with a color font style ('ok' or 'error'), 2=Text is
     *                          formatted with 'ok' color.
     *
     ...
     *         or:       $conf->module->dir_output.'/'.get_exdir(0, 0, 0, 0, $object, '')     if multidir_output not
     *         defined. Example out with new usage:       $object is invoice -> 'INYYMM-ABCD' Example out with old
     *         usage:       '015' with level 3->"0/1/5/", '015' with level 1->"5/", 'ABC-1' with level 3 ->"0/0/1/"
     *
     * @param string|int $num          Id of object (deprecated, $object will be used in future)
     * @param int        $level        Level of subdirs to return (1, 2 or 3 levels). (deprecated, global option will
     *                                 be used in future)
     * @param int        $alpha        0=Keep number only to forge path, 1=Use alpha part after the - (By default, use
     *                                 0). (deprecated, global option will be used in future)
     * @param int        $withoutslash 0=With slash at end (except if '/', we return ''), 1=without slash at end
     * @param Object     $object       Object to use to get ref to forge the path.
     * @param string     $modulepart   Type of object ('invoice_supplier, 'donation', 'invoice', ...'). Use '' for
     *                                 autodetect from $object.
     *
    ...
    function get_exdir($num, $level, $alpha, $withoutslash, $object, $modulepart = '')
    {
        ...
        $arrayforoldpath = ['cheque', 'category', 'holiday', 'supplier_invoice', 'invoice_supplier', 'mailing', 'supplier_payment'];
     ...
     * @param string $dataroot Data root directory (To avoid having the data root in the loop. Using this will also
     *                         lost the warning, on first dir, saying PHP has no permission when open_basedir is used)
     * @param string $newmask  Mask for new file (Defaults to $conf->global->MAIN_UMASK or 0755 if unavailable).
     *                         Example: '0444'
     *
        ...
        for ($i = 0; $i < $num; $i++) {
            ...
            if ($ccdir) {
                ...
                if (!@is_dir($ccdir_osencoded)) {
                    ...
                    if (empty($newmask)) {
                        $dirmaskdec = !Functions::getDolGlobalString('MAIN_UMASK') ? octdec('0755') : octdec($conf->global->MAIN_UMASK);
                    }
                    $dirmaskdec |= octdec('0111'); // Set x bit required for directories
                    if (!@mkdir($ccdir_osencoded, $dirmaskdec)) {
                        // Si le is_dir a renvoye une fausse info, alors on passe ici.
                        Functions::dol_syslog("functions.lib::dol_mkdir: Fails to create directory '" . $ccdir . "' or directory already exists.", LOG_WARNING);
                        $nberr++;
                    } else {
                        Functions::dol_syslog("functions.lib::dol_mkdir: Directory '" . $ccdir . "' created", LOG_DEBUG);
    ...
    function dolChmod($filepath, $newmask = '')
    {
        ...
        } elseif (Functions::getDolGlobalString('MAIN_UMASK')) {
     ...
     *
     * @param string  $stringtoclean      String to clean
     * @param integer $removelinefeed     1=Replace all new lines by 1 space, 0=Only ending new lines are removed
     *                                    others are replaced with \n, 2=Ending new lines are removed but others are
     *                                    kept with a same number of \n than nb of <br> when there is both
     *                                    "...<br>\n..."
     * @param string  $pagecodeto         Encoding of input/output string
     * @param integer $strip_tags         0=Use internal strip, 1=Use strip_tags() php function (bugged when text
     *                                    contains a < char that is not for a html tag or when tags is not closed like
     *                                    '<img onload=aaa')
    ...
    public static function dol_string_nohtmltag($stringtoclean, $removelinefeed = 1, $pagecodeto = 'UTF-8', $strip_tags = 0, $removedoublespaces = 1)
    {
        ...
        $temp = Functions::dol_html_entity_decode($temp, ENT_COMPAT | ENT_HTML5, $pagecodeto);
        ...
        $temp = Functions::dol_html_entity_decode($temp, ENT_COMPAT, $pagecodeto);
     ...
     *  This method is used for example by dol_htmlwithnojs() when option MAIN_RESTRICTHTML_REMOVE_ALSO_BAD_ATTRIBUTES
     *  is set to 1.
     *
     * @param string $stringtoclean      String to clean
                ...
                for ($els = $dom->getElementsByTagname('*'), $i = $els->length - 1; $i >= 0; $i--) {
                    for ($attrs = $els->item($i)->attributes, $ii = $attrs->length - 1; $ii >= 0; $ii--) {
                        ...
                        if (!empty($attrs->item($ii)->name)) {
                            ...
                            } elseif (in_array($attrs->item($ii)->name, ['style'])) {
     ...
     *
     * @param string $stringtoencode       String to encode
     * @param int    $nouseofiframesandbox 0=Default, 1=Allow use of option MAIN_SECURITY_USE_SANDBOX_FOR_HTMLWITHNOJS
     *                                     for html sanitizing (not yet working)
     * @param string $check                'restricthtmlnolink' or 'restricthtml' or 'restricthtmlallowclass' or
    ...
    {
        if (empty($nouseofiframesandbox) && Functions::getDolGlobalString('MAIN_SECURITY_USE_SANDBOX_FOR_HTMLWITHNOJS')) {
            ...
            do {
                ...
                if (!empty($out) && Functions::getDolGlobalString('MAIN_RESTRICTHTML_ONLY_VALID_HTML') && $check != 'restricthtmlallowunvalid') {
                    try {
                ...
                if (!empty($out) && Functions::getDolGlobalString('MAIN_RESTRICTHTML_ONLY_VALID_HTML_TIDY') && $check != 'restricthtmlallowunvalid') {
                    try {
                ...
                if (Functions::getDolGlobalString('MAIN_RESTRICTHTML_REMOVE_ALSO_BAD_ATTRIBUTES')) {
     ...
     *    a detection is done before to see if text is already HTML or not. Also, all entities but &,<,>," are
     *    converted. This permits to encode special chars to entities with no double encoding for already encoded HTML
     *    strings. This function also remove last EOL or BR if $removelasteolbr=1 (default). For PDF usage, you can
     *    show text by 2 ways:
    ...
    {
        $ret = Functions::dol_html_entity_decode($stringtodecode, ENT_COMPAT | ENT_HTML5, $pagecodeto);
     ...
     * @return  string                        String decoded
     */
    public static function dol_html_entity_decode($a, $b, $c = 'UTF-8', $keepsomeentities = 0)
    {
        $newstring = $a;
        if ($keepsomeentities) {
    ...
    /**
     *  Concat 2 descriptions with a new line between them (second operand after first one with appropriate new line
     *  separator) text1 html + text2 html => text1 + '<br>' + text2 text1 html + text2 txt  => text1 + '<br>' +
     *  dol_nl2br(text2) text1 txt  + text2 html => dol_nl2br(text1) + '<br>' + text2 text1 txt  + text2 txt  => text1
     *  + '\n' + text2
     *
     * @param string $text1  Text 1
     * @param string $text2  Text 2
     * @param bool   $forxml true=Use <br /> instead of <br> if we have to add a br tag
     * @param bool   $invert invert order of description lines (we often use config
     *                       MAIN_CHANGE_ORDER_CONCAT_DESCRIPTION in this parameter)
     *
    ...
    /**
     * Return array of possible common substitutions. This includes several families like: 'system', 'mycompany',
     * 'object', 'objectamount', 'date', 'user'
     *
     * @param Translate $outputlangs Output language
     * @param int       $onlykey     1=Do not calculate some heavy values of keys (performance enhancement when we need
     *                               only the keys), 2=Values are trunc and html sanitized (to use for help tooltip)
     * @param array     $exclude     Array of family keys we want to exclude. For example array('system', 'mycompany',
     *                               'object', 'objectamount', 'date', 'user', ...)
    ...
    function getCommonSubstitutionArray($outputlangs, $onlykey = 0, $exclude = null, $object = null, $include = null)
    {
        ...
        if ((empty($exclude) || !in_array('user', $exclude)) && (empty($include) || in_array('user', $include))) {
            ...
            $substitutionarray = array_merge($substitutionarray, [
                '__SENDEREMAIL_SIGNATURE__' => (string) ((!Functions::getDolGlobalString('MAIN_MAIL_DO_NOT_USE_SIGN')) ? ($onlykey == 2 ? dol_trunc('SignatureFromTheSelectedSenderProfile', 30) : $emailsendersignature) : ''),
                '__USER_SIGNATURE__' => (string) (($usersignature && !Functions::getDolGlobalString('MAIN_MAIL_DO_NOT_USE_SIGN')) ? ($onlykey == 2 ? dol_trunc(Functions::dol_string_nohtmltag($usersignature), 30) : $usersignature) : ''),
            ]);
        ...
        if (($onlykey || is_object($object)) && (empty($exclude) || !in_array('object', $exclude)) && (empty($include) || in_array('object', $include))) {
            if ($onlykey) {
                ...
                if (Functions::isModEnabled("societe")) {    // Most objects are concerned
                ...
                if (Functions::isModEnabled('adherent') && (!is_object($object) || $object->element == 'adherent') && (empty($exclude) || !in_array('member', $exclude)) && (empty($include) || in_array('member', $include))) {
                ...
                if (Functions::isModEnabled('ticket') && (!is_object($object) || $object->element == 'ticket') && (empty($exclude) || !in_array('ticket', $exclude)) && (empty($include) || in_array('ticket', $include))) {
                ...
                if (Functions::isModEnabled('recruitment') && (!is_object($object) || $object->element == 'recruitmentcandidature') && (empty($exclude) || !in_array('recruitment', $exclude)) && (empty($include) || in_array('recruitment', $include))) {
                ...
                if (Functions::isModEnabled('project') && (empty($exclude) || !in_array('project', $exclude)) && (empty($include) || in_array('project', $include))) {        // Most objects
                ...
                if (Functions::isModEnabled('contrat') && (!is_object($object) || $object->element == 'contract') && (empty($exclude) || !in_array('contract', $exclude)) && (empty($include) || in_array('contract', $include))) {
                    $substitutionarray['__CONTRACT_HIGHEST_PLANNED_START_DATE__'] = 'Highest date planned for a service start';
                    $substitutionarray['__CONTRACT_HIGHEST_PLANNED_START_DATETIME__'] = 'Highest date and hour planned for service start';
                    $substitutionarray['__CONTRACT_LOWEST_EXPIRATION_DATE__'] = 'Lowest data for planned expiration of service';
                    $substitutionarray['__CONTRACT_LOWEST_EXPIRATION_DATETIME__'] = 'Lowest date and hour for planned expiration of service';
                }
                if (Functions::isModEnabled("propal") && (!is_object($object) || $object->element == 'propal') && (empty($exclude) || !in_array('propal', $exclude)) && (empty($include) || in_array('propal', $include))) {
                    $substitutionarray['__ONLINE_SIGN_URL__'] = 'ToOfferALinkForOnlineSignature';
                }
                if (Functions::isModEnabled("ficheinter") && (!is_object($object) || $object->element == 'fichinter') && (empty($exclude) || !in_array('intervention', $exclude)) && (empty($include) || in_array('intervention', $include))) {
                ...
                if (Functions::isModEnabled("expedition") && (!is_object($object) || $object->element == 'shipping')) {
                ...
                if (Functions::isModEnabled("reception") && (!is_object($object) || $object->element == 'reception')) {
                ...
                if ($object->id > 0) {
                    ...
                    if (Functions::getDolGlobalString('PROPOSAL_ALLOW_EXTERNAL_DOWNLOAD') && is_object($object) && $object->element == 'propal') {
                        $substitutionarray['__DIRECTDOWNLOAD_URL_PROPOSAL__'] = $object->getLastMainDocLink($object->element);
                    } else {
                        $substitutionarray['__DIRECTDOWNLOAD_URL_PROPOSAL__'] = '';
                    }
                    if (Functions::getDolGlobalString('ORDER_ALLOW_EXTERNAL_DOWNLOAD') && is_object($object) && $object->element == 'commande') {
                    ...
                    if (Functions::getDolGlobalString('INVOICE_ALLOW_EXTERNAL_DOWNLOAD') && is_object($object) && $object->element == 'facture') {
                        $substitutionarray['__DIRECTDOWNLOAD_URL_INVOICE__'] = $object->getLastMainDocLink($object->element);
                    } else {
                        $substitutionarray['__DIRECTDOWNLOAD_URL_INVOICE__'] = '';
                    }
                    if (Functions::getDolGlobalString('CONTRACT_ALLOW_EXTERNAL_DOWNLOAD') && is_object($object) && $object->element == 'contrat') {
                    ...
                    if (Functions::getDolGlobalString('FICHINTER_ALLOW_EXTERNAL_DOWNLOAD') && is_object($object) && $object->element == 'fichinter') {
                    ...
                    if (Functions::getDolGlobalString('SUPPLIER_PROPOSAL_ALLOW_EXTERNAL_DOWNLOAD') && is_object($object) && $object->element == 'supplier_proposal') {
        ...
        if (Functions::isModEnabled('multicompany')) {
     ...
     * @param array     $substitutionarray                Array with key->val to substitute. Example: array('__MYKEY__'
     *                                                    => 'MyVal', ...)
     * @param Translate $outputlangs                      Output language
     * @param int       $converttextinhtmlifnecessary     0=Convert only value into HTML if text is already in HTML
     *                                                    1=Will also convert initial $text into HTML if we try to
     *                                                    insert one value that is HTML
     *
    ...
    function make_substitutions($text, $substitutionarray, $outputlangs = null, $converttextinhtmlifnecessary = 0)
    {
        ...
        foreach ($substitutionarray as $key => $value) {
            ...
            if (($key == '__USER_SIGNATURE__' || $key == '__SENDEREMAIL_SIGNATURE__') && (Functions::getDolGlobalString('MAIN_MAIL_DO_NOT_USE_SIGN'))) {
                $value = ''; // Protection
            }
    ...
    */
    ...
    /**
     *  Complete the $substitutionarray with more entries coming from external module that had set the
     *  "substitutions=1" into module_part array. In this case, method completesubstitutionarray provided by module is
     *  called.
     *
     ...
     * @param string    $callfunc          What is the name of the custom function that will be called? (default:
     *                                     completesubstitutionarray)
     *
     * @return    void
    ...
    function complete_substitutions_array(&$substitutionarray, $outputlangs, $object = null, $parameters = null, $callfunc = "completesubstitutionarray")
    {
        ...
        foreach ($dirsubstitutions as $reldir) {
            ...
            foreach ($substitfiles as $substitfile) {
                ...
                if (preg_match('/functions_(.*)\.lib\.php/i', $substitfile['name'], $reg)) {
                    ...
                    Functions::dol_syslog("Library " . $substitfile['name'] . " found into " . $dir);
        ...
        if (Functions::getDolGlobalString('ODT_ENABLE_ALL_TAGS_IN_SUBSTITUTIONS')) {
            // to list all tags in odt template
            $tags = '';
            foreach ($substitutionarray as $key => $value) {
                $tags .= '{' . $key . '} => ' . $value . "\n";
            }
            $substitutionarray = array_merge($substitutionarray, ['__ALL_TAGS__' => $tags]);
        }
    }
    ...
    function dolGetFirstLastname($firstname, $lastname, $nameorder = -1)
    {
        ...
        if ($nameorder < 0) {
            $nameorder = (!Functions::getDolGlobalString('MAIN_FIRSTNAME_NAME_POSITION') ? 1 : 0);
        }
        if ($nameorder == 1) {
            $ret .= $firstname;
     ...
     * @param int      $keepembedded Set to 1 in error message must be kept embedded into its html place (this disable
     *                               jnotify)
     *
    ...
    function get_htmloutput_mesg($mesgstring = '', $mesgarray = '', $style = 'ok', $keepembedded = 0)
    {
        ...
        if ((empty($conf->use_javascript_ajax) || Functions::getDolGlobalString('MAIN_DISABLE_JQUERY_JNOTIFY') || $keepembedded) && !preg_match('/<div class=".*">/i', $out)) {
        ...
        if ($out) {
            if (!empty($conf->use_javascript_ajax) && !Functions::getDolGlobalString('MAIN_DISABLE_JQUERY_JNOTIFY') && empty($keepembedded)) {
                $return = '<script nonce="' . getNonce() . '">
					$(document).ready(function() {
						var block = ' . (Functions::getDolGlobalString('MAIN_USE_JQUERY_BLOCKUI') ? "true" : "false") . '
						if (block) {
     ...
     * @param int      $keepembedded Set to 1 if message must be kept embedded into its html place (this disable
     *                               jnotify)
     *
     * @return    void
     *
            ...
            if (is_array($mesgarray)) {
                ...
                foreach ($mesgarray as $val) {
                    if (is_string($val)) {
                        ...
                        Functions::dol_syslog("Error call of dol_htmloutput_mesg with an array with a value that is not a string", LOG_WARNING);
                    }
                }
     ...
     *
     * @param array  $array                       Array to sort (array of array('key1'=>val1,'key2'=>val2,'key3'...) or
     *                                            array of objects)
     * @param string $index                       Key in array to use for sorting criteria
     * @param int    $order                       Sort order ('asc' or 'desc')
     * @param int    $natsort                     If values are strings (I said value not type): 0=Use alphabetical
     *                                            order, 1=use "natural" sort (natsort) If values are numeric (I said
     *                                            value not type): 0=Use numeric order (even if type is string) so use
     *                                            a "natural" sort, 1=use "natural" sort too (same than 0), -1=Force
     *                                            alphabetical order
     * @param int    $case_sensitive              1=sort is case sensitive, 0=not case sensitive
     * @param int    $keepindex                   If 0 and index key of array to sort is a numeric, than index will be
     *                                            rewrote. If 1 or index key is not numeric, key for index is kept
     *                                            after sorting.
     *
     * @return     array                            Sorted array
     */
    public static function dol_sort_array(&$array, $index, $order = 'asc', $natsort = 0, $case_sensitive = 0, $keepindex = 0)
    {
        // Clean parameters
     ...
     *
     * @return    boolean                True if string is UTF8 or ISO compatible with UTF8, False if not (ISO with
    ...
    public static function dol_osencode($str)
    {
        ...
        if (Functions::getDolGlobalString('MAIN_FILESYSTEM_ENCODING')) {
            $tmp = Functions::getDolGlobalString('MAIN_FILESYSTEM_ENCODING');
        }
     ...
     * @param string $filters      Filters to add. WARNING: string must be escaped for SQL and not coming from user
     *                             input.
     *
    ...
    function dol_getIdFromCode($db, $key, $tablename, $fieldkey = 'code', $fieldid = 'id', $entityfilter = 0, $filters = '')
    {
        ...
        Functions::dol_syslog('dol_getIdFromCode (value for field ' . $fieldid . ' from key ' . $key . ' not found into cache)', LOG_DEBUG);
     ...
     * @param string $onlysimplestring        '0' (deprecated, used for computed property of extrafields)=Accept all
     *                                        chars,
     *                                        '1' (most common use)=Accept only simple string with char
     *                                        'a-z0-9\s^$_+-.*>&|=!?():"\',/@';',
     ...
     * @param int    $returnvalue             0=No return (used to execute eval($a=something)). 1=Value of eval is
     *                                        returned (used to eval($something)).
    ...
    function dol_eval($s, $returnvalue = 0, $hideerrors = 1, $onlysimplestring = '1')
    {
        ...
        $conf = Globals::getConf();    // Read of const is done with Functions::getDolGlobalString() but we need $conf->currency for example
        ...
        if (!in_array($onlysimplestring, ['0', '1', '2'])) {
        ...
        try {
            ...
            if ($onlysimplestring == '1') {
                // We must accept: '1 && getDolGlobalInt("doesnotexist1") && Functions::getDolGlobalString("MAIN_FEATURES_LEVEL")'
                // We must accept: '$user->hasRight("cabinetmed", "read") && !$object->canvas=="patient@cabinetmed"'
                if (preg_match('/[^a-z0-9\s' . preg_quote('^$_+-.*>&|=!?():"\',/@', '/') . ']/i', $s)) {
                    if ($returnvalue) {
                        return 'Bad string syntax to evaluate (found chars that are not chars for simplestring): ' . $s;
                    } else {
                        Functions::dol_syslog('Bad string syntax to evaluate (found chars that are not chars for simplestring): ' . $s);
                        return '';
                    }
                }
                ...
                if (strpos($scheck, '(') !== false) {
                    if ($returnvalue) {
                        ...
                        Functions::dol_syslog('Bad string syntax to evaluate (found call of a function or method without using direct name): ' . $s);
                        return '';
                    }
                }
                // TODO
            ...
            if (strpos($s, '::') !== false) {
                if ($returnvalue) {
                    ...
                    Functions::dol_syslog('Bad string syntax to evaluate (double : char is forbidden): ' . $s);
                    return '';
                }
            }
            if (strpos($s, '`') !== false) {
                if ($returnvalue) {
                    ...
                    Functions::dol_syslog('Bad string syntax to evaluate (backtick char is forbidden): ' . $s);
                    return '';
                }
            }
            if (preg_match('/[^0-9]+\.[^0-9]+/', $s)) {    // We refuse . if not between 2 numbers
                if ($returnvalue) {
                    ...
                    Functions::dol_syslog('Bad string syntax to evaluate (dot char is forbidden): ' . $s);
                    return '';
                }
            }
            ...
            $forbiddenphpstrings = ['$$'];
            $forbiddenphpstrings = array_merge($forbiddenphpstrings, ['_ENV', '_SESSION', '_COOKIE', '_GET', '_POST', '_REQUEST']);

            $forbiddenphpfunctions = ["exec", "passthru", "shell_exec", "system", "proc_open", "popen"];
            $forbiddenphpfunctions = array_merge($forbiddenphpfunctions, ["dol_eval", "executeCLI", "verifCond"]);    // native dolibarr functions
            $forbiddenphpfunctions = array_merge($forbiddenphpfunctions, ["base64_decode", "rawurldecode", "urldecode"]); // decode string functions
            $forbiddenphpfunctions = array_merge($forbiddenphpfunctions, ["fopen", "file_put_contents", "fputs", "fputscsv", "fwrite", "fpassthru", "require", "include", "mkdir", "rmdir", "symlink", "touch", "unlink", "umask"]);
            $forbiddenphpfunctions = array_merge($forbiddenphpfunctions, ["get_defined_functions", "get_defined_vars", "get_defined_constants", "get_declared_classes"]);
            $forbiddenphpfunctions = array_merge($forbiddenphpfunctions, ["function", "call_user_func"]);
            $forbiddenphpfunctions = array_merge($forbiddenphpfunctions, ["eval", "create_function", "assert", "mb_ereg_replace"]); // function with eval capabilities
            ...
            if (strpos($s, '__forbiddenstring__') !== false) {
                Functions::dol_syslog('Bad string syntax to evaluate: ' . $s, LOG_WARNING);
                if ($returnvalue) {
                    return 'Bad string syntax to evaluate: ' . $s;
                } else {
                    Functions::dol_syslog('Bad string syntax to evaluate: ' . $s);
                    return '';
                }
            }
            ...
            $error .= $e->getMessage();
            Functions::dol_syslog($error, LOG_WARNING);
            if ($returnvalue) {
     ...
     *
     * @param string $codelang   Language code ('en_IN', 'fr_CA', ...) or ISO Country code on 2 characters in uppercase
     *                           ('IN', 'FR')
     * @param string $moreatt    Add more attribute on img tag (For example 'style="float: right"' or
     *                           'class="saturatemedium"')
     * @param int    $notitlealt No title alt
     *
     * @return    string                HTML img string with flag.
     */
    public static function picto_from_langcode($codelang, $moreatt = '', $notitlealt = 0)
    {
        if (empty($codelang)) {
            return '';
        }
    ...
    function getLanguageCodeFromCountryCode($countrycode)
    {
        ...
        if (function_exists('locale_get_primary_language') && function_exists('locale_get_region')) {    // Need extension php-intl
            ...
            Functions::dol_syslog("Warning Exention php-intl is not available", LOG_WARNING);
        }
     ...
     *                                            'categories_x'       to add a tab in category view ('x': type of
     *                                            category (0=product, 1=supplier, 2=customer, 3=member)
     *                                            'ecm'               to add a tab for another ecm view
     *                                            'stock'            to add a tab for warehouse view
     * @param string      $mode                   'add' to complete head, 'remove' to remove entries
     * @param string      $filterorigmodule       Filter on module origin: 'external' will show only external modules.
     *                                            'core' only core modules. No filter (default) will add both.
     *
     * @return    void
     */
    function complete_head_from_modules($conf, $langs, $object, &$head, &$h, $type, $mode = 'add', $filterorigmodule = '')
    {
            ...
            foreach ($conf->modules_parts['tabs'][$type] as $value) {
                ...
                if ($mode == 'add' && !preg_match('/^\-/', $values[1])) {
                    ...
                    if (count($values) == 6) {
                        ...
                        Functions::dol_syslog('Passing 5 values in tabs module_parts is deprecated. Please update to 6 with permissions.', LOG_WARNING);
                    ...
                    $head = array_merge(array_slice($head, 0, $postab), [$newtab], array_slice($head, $postab));
        ...
        if (!empty($hookmanager)) {
            $parameters = ['object' => $object, 'mode' => $mode, 'head' => &$head, 'filterorigmodule' => $filterorigmodule];
    ...
    function printCommonFooter($zone = 'private')
    {
        ...
        if (empty($reshook)) {
            if (Functions::getDolGlobalString('MAIN_HTML_FOOTER')) {
                print Functions::getDolGlobalString('MAIN_HTML_FOOTER') . "\n";
            }
            ...
            if (!empty($conf->use_javascript_ajax)) {
                ...
                // End of tuning
                if (!empty($_SERVER['MAIN_SHOW_TUNING_INFO']) || Functions::getDolGlobalString('MAIN_SHOW_TUNING_INFO')) {
                    print "\n";
                    print "/* JS CODE TO ENABLE to add memory info */\n";
                    print 'window.console && console.log("';
                    if (Functions::getDolGlobalString('MEMCACHED_SERVER')) {
                        print 'MEMCACHED_SERVER=' . Functions::getDolGlobalString('MEMCACHED_SERVER') . ' - ';
                    }
                    print 'MAIN_OPTIMIZE_SPEED=' . Functions::getDolGlobalString('MAIN_OPTIMIZE_SPEED', 'off');
                ...
                if (Functions::isModEnabled('google') && Functions::getDolGlobalString('MAIN_GOOGLE_AN_ID')) {
                    $tmptagarray = explode(',', Functions::getDolGlobalString('MAIN_GOOGLE_AN_ID'));
     ...
     *
     * @param string $selector Selector ('#id' or 'input[name="ref"]') to use to find the HTML input field that must
     *                         get the autofocus. You must use a CSS selector, so unique id preceding with the '#'
     *                         char.
     *
     * @return    void
     */
     ...
     *
     * @param string|string[] $fields         String or array of strings, filled with the name of all fields in the SQL
     *                                        query we must check (combined with a OR). Example:
     *                                        array("p.field1","p.field2")
     * @param string          $value          The value to look for.
     *                                        If param $mode is 0, can contains several keywords separated with a space
     *                                        or | like "keyword1 keyword2" = We want record field like keyword1 AND
     *                                        field like keyword2 or like "keyword1|keyword2" = We want record field
     *                                        like keyword1 OR field like keyword2 If param $mode is 1, can contains an
     *                                        operator <, > or = like "<10" or ">=100.5 < -1000" If param $mode is 2 or
     *                                        -2, can contains a list of int id separated by comma like "1,3,4" If
     *                                        param $mode is 3 or -3, can contains a list of string separated by comma
     *                                        like "a,b,c".
     * @param integer         $mode           0=value is list of keyword strings,
     *                                        1=value is a numeric test (Example ">5.5 <10"),
     *                                        2=value is a list of ID separated with comma (Example '1,3,4'), -2 is for
     *                                        exclude list,
     *                                        3=value is list of string separated with comma (Example 'text 1,text 2'),
     *                                        -3 if for exclude list,
     *                                        4=value is a list of ID separated with comma (Example '2,7') to be used
     *                                        to search into a multiselect string '1,2,3,4'
     ...
     *                             (default).
     *
     * @return  string                  New file name (full or relative path, including the thumbs/). May be the
     *                                  original path if no thumb can exists.
     */
     ...
     * @param int    $alldata      Return array with all components (1 is recommended, then use a simple a href link
     *                             with the class, target and mime attribute added. 'documentpreview' css class is
     *                             handled by jquery code into main.inc.php)
    ...
    function getAdvancedPreviewUrl($modulepart, $relativepath, $alldata = 0, $param = '')
    {
        ...
        if ($alldata == 1) {
            if ($isAllowedForPreview) {
                return ['target' => '_blank', 'css' => 'documentpreview', 'url' => DOL_URL_ROOT . '/document.php?modulepart=' . urlencode($modulepart) . '&attachment=0&file=' . urlencode($relativepath) . ($param ? '&' . $param : ''), 'mime' => dol_mimetype($relativepath)];
            } else {
                return [];
            }
        }
    ...
    function dolIsAllowedForPreview($file)
    {
        ...
        // Check mime types
        $mime_preview = ['bmp', 'jpeg', 'png', 'gif', 'tiff', 'pdf', 'plain', 'css', 'webp'];
        if (Functions::getDolGlobalString('MAIN_ALLOW_SVG_FILES_AS_IMAGES')) {
     ...
     * @param int    $mode    0=Return full mime, 1=otherwise short mime string, 2=image for mime type, 3=source
     *                        language, 4=css of font fa
     *
     * @return string                Return a mime type family (text/xxx, application/xxx, image/xxx, audio, video,
     *                               archive)
    ...
    function isVisibleToUserType($type_user, &$menuentry, &$listofmodulesforexternal)
    {
        ...
        if (!$menuentry['perms'] && Functions::getDolGlobalString('MAIN_MENU_HIDE_UNAUTHORIZED')) {
     ...
     * @param string $type   type of badge : Primary Secondary Success Danger Warning Info Light Dark status0 status1
     *                       status2 status3 status4 status5 status6 status7 status8 status9
     * @param string $mode   default '' , 'pill', 'dot'
     * @param string $url    the url for link
     * @param array  $params various params for future : recommended rather than adding more function arguments.
     *                       array('attr'=>array('title'=>'abc'))
     *
     * @return  string              Html badge
     */
    function dolGetBadge($label, $html = '', $type = 'primary', $mode = '', $url = '', $params = [])
    {
        $attr = [
     ...
     * @param string $html             Optional : label of badge with html
     * @param string $statusType       status0 status1 status2 status3 status4 status5 status6 status7 status8 status9
     *                                 : image name or badge name
     * @param int    $displayMode      0=Long label, 1=Short label, 2=Picto + Short label, 3=Picto, 4=Picto + Long
     ...
     */
    function dolGetStatus($statusLabel = '', $statusLabelShort = '', $html = '', $statusType = 'status0', $displayMode = 0, $url = '', $params = [])
    {
        $conf = Globals::getConf();
        ...
        } elseif (Functions::getDolGlobalString('MAIN_STATUS_USES_IMAGES')) {
            // Use status with images (for backward compatibility)
            $return = '';
            $htmlLabel = (in_array($displayMode, [1, 2, 5]) ? '<span class="hideonsmartphone">' : '') . (!empty($html) ? $html : $statusLabel) . (in_array($displayMode, [1, 2, 5]) ? '</span>' : '');
            $htmlLabelShort = (in_array($displayMode, [1, 2, 5]) ? '<span class="hideonsmartphone">' : '') . (!empty($html) ? $html : (!empty($statusLabelShort) ? $statusLabelShort : $statusLabel)) . (in_array($displayMode, [1, 2, 5]) ? '</span>' : '');
        ...
        } elseif (!Functions::getDolGlobalString('MAIN_STATUS_USES_IMAGES') && !empty($displayMode)) {
            // Use new badge
     ...
     *
     * @param string       $label             Label or tooltip of button if $text is provided. Also used as tooltip in
     *                                        title attribute. Can be escaped HTML content or full simple text.
     * @param string       $text              Optional : short label on button. Can be escaped HTML content or full
     *                                        simple text.
     * @param string       $actionType        'default', 'danger', 'email', 'clone', 'cancel', 'delete', ...
     * @param string|array $url               Url for link or array of subbutton description ('label'=>, 'url'=>,
     *                                        'lang'=>, 'perm'=> ) Example when an array is used: $arrayforbutaction =
     *                                        array(
     *                                        10 => array('lang'=>'propal',
     *                                        'enabled'=>Functions::isModEnabled("propal"),
     *                                        'perm'=>$user->hasRight('propal', 'creer'), 'label' => 'AddProp',
     *                                        'url'=>'/comm/propal/card.php?action=create&amp;projectid='.$object->id.'&amp;socid='.$object->socid),
     *                                        20 => array('lang'=>'orders',
     *                                        'enabled'=>Functions::isModEnabled("commande"),
     *                                        'perm'=>$user->hasRight('commande', 'creer'), 'label' => 'CreateOrder',
     *                                        'url'=>'/commande/card.php?action=create&amp;projectid='.$object->id.'&amp;socid='.$object->socid),
     *                                        30 => array('lang'=>'bills',
     *                                        'enabled'=>Functions::isModEnabled("facture"),
     *                                        'perm'=>$user->hasRight('facture', 'creer'), 'label' => 'CreateBill',
     *                                        'url'=>'/compta/facture/card.php?action=create&amp;projectid='.$object->id.'&amp;socid='.$object->socid),
     *                                        );
     * @param string       $id                Attribute id of action button. Example 'action-delete'. This can be used
     *                                        for full ajax confirm if this code is reused into the ->formconfirm()
     *                                        method.
     * @param int|boolean  $userRight         User action right
     *                                        // phpcs:disable
     * @param array        $params            = [ // Various params for future : recommended rather than adding more
     *                                        function arguments
     *                                        'attr' => [ // to add or override button attributes
     *                                        'xxxxx' => '', // your xxxxx attribute you want
     *                                        'class' => 'reposition', // to add more css class to the button class
     *                                        attribute
     *                                        'classOverride' => '' // to replace class attribute of the button
     *                                        ],
     *                                        'confirm' => [
     *                                        'url' => 'http://', // Override Url to go when user click on action btn,
     *                                        if empty default url is $url.?confirm=yes, for no js compatibility use
     *                                        $url for fallback confirm.
     *                                        'title' => '', // Override title of modal,  if empty default title use
     *                                        "ConfirmBtnCommonTitle" lang key
     *                                        'action-btn-label' => '', // Override label of action button,  if empty
     *                                        default label use "Confirm" lang key
     *                                        'cancel-btn-label' => '', // Override label of cancel button,  if empty
     *                                        default label use "CloseDialog" lang key
     *                                        'content' => '', // Override text of content,  if empty default content
     *                                        use "ConfirmBtnCommonContent" lang key
     *                                        'modal' => true, // true|false to display dialog as a modal (with dark
     *                                        background)
     *                                        'isDropDrown' => false, // true|false to display dialog as a dropdown
     ...
     */
    function dolGetButtonAction($label, $text = '', $actionType = 'default', $url = '', $id = '', $userRight = 1, $params = [])
    {
        ...
        if (is_array($url)) {
            ...
            if (count($url) > 1) {
                ...
                foreach ($url as $subbutton) {
                    ...
                    $out .= dolGetButtonAction('', $langs->trans($subbutton['label']), 'default', $tmpurl, '', $subbutton['perm'], ['isDropDown' => true]);
                }
                $out .= "</div>";
                $out .= "</div>";
            } else {
     ...
     * @param int    $status    0 no user rights, 1 active, 2 current action or selected, -1 Feature Disabled, -2
     *                          disable Other reason use param $helpText as tooltip help
     * @param array  $params    various params for future : recommended rather than adding more function arguments
     *
     * @return string               html button
     */
    function dolGetButtonTitle($label, $helpText = '', $iconClass = 'fa fa-file', $url = '', $id = '', $status = 1, $params = [])
    {
        ...
        if (Functions::getDolGlobalString('MAIN_BUTTON_HIDE_UNAUTHORIZED') && (!$user->admin) && $status <= 0) {
            return '';
        }

        $class = 'btnTitle';
        if (in_array($iconClass, ['fa fa-plus-circle', 'fa fa-plus-circle size15x', 'fa fa-comment-dots', 'fa fa-paper-plane'])) {
            $class .= ' btnTitlePlus';
        }
     ...
     *
     * @return  array                    array('module'=>, 'classpath'=>, 'element'=>, 'subelement'=>, 'classfile'=>,
     *                                   'classname'=>, 'dir_output'=>)
    ...
    function fetchObjectByElement($element_id, $element_type, $element_ref = '')
    {
        ...
        if ($element_prop['module'] == 'product' || $element_prop['module'] == 'service') {
            ...
            $isModEnabled = (Functions::isModEnabled('product') || Functions::isModEnabled('service'));
        } elseif ($element_prop['module'] == 'societeaccount') {
            $isModEnabled = Functions::isModEnabled('website') || Functions::isModEnabled('webportal');
        } else {
            $isModEnabled = Functions::isModEnabled($element_prop['module']);
        }
     ...
     * For ajax call, you must use this token as a parameter of the call into the js calling script (the called ajax
     * php page must also set constant NOTOKENRENEWAL).
     *
     * @return  string
     * @since Dolibarr v10.0.7
     */
    ...
    /**
     * Start a table with headers and a optional clickable number (don't forget to use "finishSimpleTable()" after the
     * last table row)
     *
     * @param string  $header    The first left header of the table (automatic translated)
     * @param string  $link      (optional) The link to a internal dolibarr page, when click on the number (without the
     *                           first "/")
     ...
     * @param integer $number    (optional) The number that is shown right after the first header, when not set the
     *                           link is shown on the right side of the header as "FullList"
     *
     * @return void
     *
     ...
     * @param integer $num              The count of the rows of the table, when it is zero (0) the "$noneWord" is
     *                                  shown instead
     * @param integer $nbofloop         (optional)    The maximum count of rows thaht the table show (when it is zero
     *                                  (0) no summary line will show, expect "$noneWord" when $num === 0)
     * @param integer $total            (optional)    The total value thaht is shown after when the table has minimum
     *                                  of one entire
    ...
    function readfileLowMemory($fullpath_original_file_osencoded, $method = -1)
    {
        if ($method == -1) {
            $method = 0;
            if (Functions::getDolGlobalString('MAIN_FORCE_READFILE_WITH_FREAD')) {
                $method = 1;
            }
            if (Functions::getDolGlobalString('MAIN_FORCE_READFILE_WITH_STREAM_COPY')) {
                $method = 2;
            }
        }
     ...
     * @param string $texttoshow      Replace the value to show with this text. Use 'none' to show no text (only the
     *                                copy-paste picto)
     *
     ...
     *
     * @param string $filter              String with universal search string. Must be '(aaa:bbb:ccc) OR (ddd:eeee:fff)
     *                                    ...' with aaa is a field name (with alias or not) and bbb is one of this
     *                                    operator '=', '<', '>', '<=', '>=', '!=', 'in', 'notin', 'like', 'notlike',
     *                                    'is', 'isnot'. ccc must not contains ( or ) Example: '((client:=:1) OR
     *                                    ((client:>=:2) AND (client:<=:3))) AND (client:!=:8) AND (nom:like:'a%')'
     ...
     * @param int    $noerror             1=If search criteria is not valid, does not return an error string but
     *                                    invalidate the SQL
     *
    ...
    function forgeSQLFromUniversalSearchCriteria($filter, &$errorstr = '', $noand = 0, $nopar = 0, $noerror = 0)
    {
        ...
        $t = str_replace(['and', 'or', 'AND', 'OR', ' '], '', $t);        // Remove the only strings allowed between each () criteria
     ...
     * @param int    $parenthesislevel Returned level of global parenthesis that we can remove/simplify, 0 if error or
     *                                 we can't simplify.
     *
     * @return    boolean                                True if valid, False if not valid ($error returned parameter
     *                                                   is filled with the reason in such a case)
     * @see forgeSQLFromUniversalSearchCriteria()
     */
     ...
     *
     * @param array $matches Array of found string by regex search. Example: "t.ref:like:'SO-%'" or
    ...
    {
        //Functions::dol_syslog("Convert matches ".$matches[1]);
        if (empty($matches[1])) {
            return '';
        }
     ...
     *                                    Example: "t.ref:like:'SO-%'" or "t.date_creation:<:'20160101'" or
     *                                    "t.date_creation:<:'2016-01-01 12:30:00'" or "t.nature:is:NULL"
     *
    ...
    function getTimelineIcon($actionstatic, &$histo, $key)
    {
        ...
        } elseif (Functions::getDolGlobalString('AGENDA_USE_EVENT_TYPE')) {
     ...
     * @param mixed     $filterobj  Filter on object
     *                              Adherent|Societe|Project|Product|CommandeFournisseur|Dolresource|Ticket|... to list
     *                              events linked to an object
     ...
     */
    function show_actions_messaging($conf, $langs, $db, $filterobj, $objcon = '', $noprint = 0, $actioncode = '', $donetodo = 'done', $filters = [], $sortfield = 'a.datep,a.id', $sortorder = 'DESC')
    {
        global $user, $conf;
        global $form;
        ...
        $sortfield_label_list = ['a.id' => 'id', 'a.datep' => 'dp', 'a.percent' => 'percent'];
        $sortfield_new_list = [];
        ...
        if (Functions::isModEnabled('agenda')) {
            ...
            if ($force_filter_contact === false) {
                if (is_object($filterobj) && in_array(get_class($filterobj), ['Societe', 'Client', 'Fournisseur']) && $filterobj->id) {
            ...
            if (!empty($actioncode)) {
                if (!Functions::getDolGlobalString('AGENDA_USE_EVENT_TYPE')) {
        ...
        if (
            Functions::isModEnabled('mailing') && !empty($objcon->email)
        ...
        if ($sql || $sql2) {    // May not be defined if module Agenda is not enabled and mailing module disabled too
            ...
            Functions::dol_syslog("function.lib::show_actions_messaging", LOG_DEBUG);
            $resql = $db->query($sql);
            if ($resql) {
                $i = 0;
        ...
        if (Functions::isModEnabled('agenda') || (Functions::isModEnabled('mailing') && !empty($objcon->email))) {
            ...
            $out .= $formactions->select_type_actions($actioncode, "actioncode", '', !Functions::getDolGlobalString('AGENDA_USE_EVENT_TYPE') ? 1 : -1, 0, 0, 1, 'minwidth200imp');
            $out .= '</th>';
            ...
            foreach ($histo as $key => $value) {
                ...
                if (!Functions::getDolGlobalString('AGENDA_USE_EVENT_TYPE') && empty($arraylist[$labeltype])) {
                    $labeltype = 'AC_OTH';
                }
                ...
                if (isset($histo[$key]['elementtype']) && !empty($histo[$key]['fk_element'])) {
                    if (isset($conf->cache['elementlinkcache'][$histo[$key]['elementtype']]) && isset($conf->cache['elementlinkcache'][$histo[$key]['elementtype']][$histo[$key]['fk_element']])) {
                        ...
                        if (!isset($conf->cache['elementlinkcache'][$histo[$key]['elementtype']])) {
                            $conf->cache['elementlinkcache'][$histo[$key]['elementtype']] = [];
                        }
                ...
                if (!empty($documents)) {
                    ...
                    foreach ($documents as $doc) {
                        ...
                        $mimeAttr = ' mime="' . $mime . '" ';
                        $class = '';
                        if (in_array($mime, ['image/png', 'image/jpeg', 'application/pdf'])) {
    ...
    /**
     * Helper function that combines values of a dolibarr DatePicker (such as Form::selectDate) for year, month, day
     * (and optionally hour, minute, second) fields to return a timestamp.
     *
     * @param string $prefix    Prefix used to build the date selector (for instance using Form::selectDate)
     * @param string $hourTime  'getpost' to include hour, minute, second values from the HTTP request, 'XX:YY:ZZ' to
     *                          set hour, minute, second respectively (for instance '23:59:59')
    ...
    /**
     * Helper function that combines values of a dolibarr DatePicker (such as Form::selectDate) for year, month, day
     * (and optionally hour, minute, second) fields to return a a portion of URL reproducing the values from the
