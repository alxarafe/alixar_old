...
abstract class Functions
{
    ...
    function sanitizeVal($out = '', $check = 'alphanohtml', $filter = null, $options = null)
    {
        ...
        switch ($check) {
            ...
            case 'custom':
                ...
                break;
            default:
                Functions::dol_syslog("Error, you call sanitizeVal() with a bad value for the check type. Data will be sanitized with alphanohtml.", LOG_ERR);
     ...
     *    This must not use any call to other function calling Functions::dol_syslog (avoid infinite loop).
     *
    ...
    function getCommonSubstitutionArray($outputlangs, $onlykey = 0, $exclude = null, $object = null, $include = null)
    {
        ...
        if ((empty($exclude) || !in_array('user', $exclude)) && (empty($include) || in_array('user', $include))) {
            ...
            $substitutionarray = array_merge($substitutionarray, [
                ...
                '__USER_SIGNATURE__' => (string) (($usersignature && !Functions::getDolGlobalString('MAIN_MAIL_DO_NOT_USE_SIGN')) ? ($onlykey == 2 ? dol_trunc(dol_string_nohtmltag($usersignature), 30) : $usersignature) : ''),
            ]);
    ...
    */
     ...
     *                                        10 => array('lang'=>'propal', 'enabled'=>Functions::isModEnabled("propal"), 'perm'=>$user->hasRight('propal', 'creer'), 'label' => 'AddProp', 'url'=>'/comm/propal/card.php?action=create&amp;projectid='.$object->id.'&amp;socid='.$object->socid),
     *                                        20 => array('lang'=>'orders', 'enabled'=>Functions::isModEnabled("commande"), 'perm'=>$user->hasRight('commande', 'creer'), 'label' => 'CreateOrder', 'url'=>'/commande/card.php?action=create&amp;projectid='.$object->id.'&amp;socid='.$object->socid),
     *                                        30 => array('lang'=>'bills', 'enabled'=>Functions::isModEnabled("facture"), 'perm'=>$user->hasRight('facture', 'creer'), 'label' => 'CreateBill', 'url'=>'/compta/facture/card.php?action=create&amp;projectid='.$object->id.'&amp;socid='.$object->socid),
     *                                        );
