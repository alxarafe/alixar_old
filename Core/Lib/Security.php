...
abstract class Security
{
     ...
     */
    public static function dolDecrypt($chain, $key = '')
    {
     ...
     */
    public static function dol_hash($chain, $type = '0', $nosalt = 0)
    {
        // No need to add salt for password_hash
        if (($type == '0' || $type == 'auto') && Functions::getDolGlobalString('MAIN_SECURITY_HASH_ALGO') && Functions::getDolGlobalString('MAIN_SECURITY_HASH_ALGO') == 'password_hash' && function_exists('password_hash')) {
            return password_hash($chain, PASSWORD_DEFAULT);
        }

        // Salt value
        if (Functions::getDolGlobalString('MAIN_SECURITY_SALT') && $type != '4' && $type !== 'openldap' && empty($nosalt)) {
            $chain = Functions::getDolGlobalString('MAIN_SECURITY_SALT') . $chain;
        }
        ...
        if ($type == '1' || $type == 'sha1') {
            ...
            return dolGetLdapPasswordHash($chain, Functions::getDolGlobalString('LDAP_PASSWORD_HASH_TYPE', 'md5'));
        } elseif ($type == '5' || $type == 'sha256') {
            return hash('sha256', $chain);
        } elseif ($type == '6' || $type == 'password_hash') {
            return password_hash($chain, PASSWORD_DEFAULT);
        } elseif (Functions::getDolGlobalString('MAIN_SECURITY_HASH_ALGO') == 'sha1') {
            return sha1($chain);
        } elseif (Functions::getDolGlobalString('MAIN_SECURITY_HASH_ALGO') == 'sha1md5') {
            return sha1(md5($chain));
        }
    ...
    function restrictedArea(User $user, $features, $object = 0, $tableandshare = '', $feature2 = '', $dbt_keyfield = 'fk_soc', $dbt_select = 'rowid', $isdraft = 0, $mode = 0)
    {
        ...
        */
        ...
        $parameters = ['features' => $features, 'originalfeatures' => $originalfeatures, 'objectid' => $objectid, 'dbt_select' => $dbt_select, 'idtype' => $dbt_select, 'isdraft' => $isdraft];
        ...
        $listofmodules = explode(',', Functions::getDolGlobalString('MAIN_MODULES_FOR_EXTERNAL'));
        ...
        foreach ($featuresarray as $feature) {  // first we check nb of test ko
            ...
            if (!empty($user->socid) && Functions::getDolGlobalString('MAIN_MODULES_FOR_EXTERNAL') && !in_array($featureforlistofmodule, $listofmodules)) {    // If limits on modules for external users, module must be into list of modules for external users
                $readok = 0;
                $nbko++;
                continue;
            }
        ...
        $createok = 1;
        $nbko = 0;
        $wemustcheckpermissionforcreate = (GETPOST('sendit', 'alpha') || GETPOST('linkit', 'alpha') || in_array(GETPOST('action', 'aZ09'), ['create', 'update', 'set', 'upload', 'add_element_resource', 'confirm_deletebank', 'confirm_delete_linked_resource']) || GETPOST('roworder', 'alpha', 2));
        ...
        if (!empty($objectid) && $objectid > 0) {
            ...
            $params = ['objectid' => $objectid, 'features' => implode(',', $featuresarray), 'features2' => $feature2];
    ...
    function checkUserAccessToObject($user, array $featuresarray, $object = 0, $tableandshare = '', $feature2 = '', $dbt_keyfield = '', $dbt_select = 'rowid', $parenttableforentity = '')
    {
        ...
        foreach ($featuresarray as $feature) {
            ...
            $check = ['adherent', 'banque', 'bom', 'don', 'mrp', 'user', 'usergroup', 'payment', 'payment_supplier', 'product', 'produit', 'service', 'produit|service', 'categorie', 'resource', 'expensereport', 'holiday', 'salaries', 'website', 'recruitment', 'chargesociales']; // Test on entity only (Objects with no link to company)
    ...
    {
        $max = Functions::getDolGlobalString('MAIN_UPLOAD_DOC'); // In Kb
