...
abstract class Files
{
     ...
     * @param string   $filter              Regex filter to restrict list. This regex value must be escaped for '/' by
     *                                      doing preg_quote($var,'/'), since this char is used for preg_match
     *                                      function,
     *                                      but must not contains the start and end '/'. Filter is checked into
     *                                      basename
     *                                      only.
     * @param string[] $excludefilter       Array of Regex for exclude filter (example:
     *                                      array('(\.meta|_preview.*\.png)$','^\.')). Exclude is checked both into
     *                                      fullpath and into basename (So '^xxx' may exclude 'xxx/dirscanned/...' and
     *                                      dirscanned/xxx').
     ...
     *                                      size to be loaded (slower), 2=Force load of date only, 3=Force load of size
     *                                      only, 4=Force load of perm
     ...
     *
     * @return  array<array{name:string,path:string,level1name:string,relativename:string,fullname:string,date:string,size:int,perm:int,type:string}> Array
     *                                                                                                                                                of
     *                                                                                                                                                array('name'=>'xxx','fullname'=>'/abc/xxx','date'=>'yyy','size'=>99,'type'=>'dir|file',...)
    ...
    function dol_dir_list($utf8_path, $types = "all", $recursive = 0, $filter = "", $excludefilter = null, $sortcriteria = "name", $sortorder = SORT_ASC, $mode = 0, $nohook = 0, $relativename = "", $donotfollowsymlinks = 0, $nbsecondsold = 0)
    {
        ...
        if ($recursive <= 1) {  // Avoid too verbose log
            ...
            $exclude_array = $excludefilter === null ? [] : (is_array($excludefilter) ? $excludefilter : [$excludefilter]);
            foreach (([$filter] + $exclude_array) as $f) {
        ...
        if (!$nohook && $hookmanager instanceof HookManager) {
            ...
            $hookmanager->initHooks(['fileslib']);
     ...
     * @param string        $filter        Regex filter to restrict list. This regex value must be escaped for '/',
     *                                     since this char is used for preg_match function
     * @param string[]|null $excludefilter Array of Regex for exclude filter (example:
     *                                     array('(\.meta|_preview.*\.png)$','^\.'))
     * @param string        $sortcriteria  Sort criteria ("","fullname","name","date","size")
     * @param int           $sortorder     Sort order (SORT_ASC, SORT_DESC)
     * @param int           $mode          0=Return array minimum keys loaded (faster), 1=Force all keys like
     *                                     description
     *
     * @return  array<array{rowid:string,label:string,name:string,path:string,level1name:string,fullname:string,fullpath_orig:string,date_c:string,datem:string,type:string,keywords:string,cover:string,position:int,acl:string,share:string,description:string}> Array
     *                                                                                                                                                                                                                                                             of
     *                                                                                                                                                                                                                                                             array('name'=>'xxx','fullname'=>'/abc/xxx','date'=>'yyy','size'=>99,'type'=>'dir|file',...)
     * @see dol_dir_list()
     */
     ...
     *
     * @param array<array{name:string,path:string,level1name:string,relativename:string,fullname:string,date:string,size:int,perm:int,type:string}> $filearray   Array
     *                                                                                                                                                           of
     *                                                                                                                                                           array('name'=>'xxx','fullname'=>'/abc/xxx','date'=>'yyy','size'=>99,'type'=>'dir|file',...)
     *                                                                                                                                                           Array
     *                                                                                                                                                           of
     ...
     *                                                                                                                                                           using
     *                                                                                                                                                           dol_dir_list
     * @param string                                                                                                                                $relativedir Relative
     *                                                                                                                                                           dir
     *                                                                                                                                                           from
     *                                                                                                                                                           DOL_DATA_ROOT
     *
     ...
     */
    public static function dol_is_dir($folder)
    {
        $newfolder = Functions::dol_osencode($folder);
        if (is_dir($newfolder)) {
            return true;
        } else {
            return false;
     ...
     * @return  boolean                     True or false
     */
    public static function dol_is_file($pathoffile)
    {
        $newpathoffile = Functions::dol_osencode($pathoffile);
    ...
    {
        $prots = ['file', 'http', 'https', 'ftp', 'zlib', 'data', 'ssh', 'ssh2', 'ogg', 'expect'];
     ...
     * @param array<string,string> $arrayreplacement        Array with strings to replace. Example:
     *                                                      array('valuebefore'=>'valueafter', ...)
     * @param string               $destfile                Destination file (can't be a directory). If empty, will be
     *                                                      same than source file.
     * @param string               $newmask                 Mask for new file (0 by default means
     *                                                      $conf->global->MAIN_UMASK). Example: '0666'
     * @param int                  $indexdatabase           1=index new file into database.
     * @param int                  $arrayreplacementisregex 1=Array of replacement is already an array with key that is
     *                                                      a regex. Warning: the key must be escaped with preg_quote
     *                                                      for '/'
     *
     * @return  int                                Return integer <0 if error, 0 if nothing done (dest file already
     *                                             exists), >0 if OK
     * @see     dol_copy()
     */
     ...
     * @param string $newmask           Mask for new file (0 by default means $conf->global->MAIN_UMASK). Example:
     *                                  '0666'
     * @param int    $overwriteifexists Overwrite file if exists (1 by default)
     * @param int    $testvirus         Do an antivirus test. Move is canceled if a virus is found.
     * @param int    $indexdatabase     Index new file into database.
     *
     * @return  int                         Return integer <0 if error, 0 if nothing done (dest file already exists and
     *                                      overwriteifexists=0), >0 if OK
     * @see     dol_delete_file() dolCopyDir()
     */
    public static function dol_copy($srcfile, $destfile, $newmask = '0', $overwriteifexists = 1, $testvirus = 0, $indexdatabase = 0)
    {
        $db = Globals::getDb();
        //$user = Globals::getUser();

        Functions::dol_syslog("files.lib.php::dol_copy srcfile=" . $srcfile . " destfile=" . $destfile . " newmask=" . $newmask . " overwriteifexists=" . $overwriteifexists);

        if (empty($srcfile) || empty($destfile)) {
            return -1;
        }

        $destexists = static::dol_is_file($destfile);
        if (!$overwriteifexists && $destexists) {
            return 0;
        }

        $newpathofsrcfile = Functions::dol_osencode($srcfile);
        $newpathofdestfile = Functions::dol_osencode($destfile);
        $newdirdestfile = dirname($newpathofdestfile);

        if ($destexists && !is_writable($newpathofdestfile)) {
            Functions::dol_syslog("files.lib.php::dol_copy failed Permission denied to overwrite target file", LOG_WARNING);
            return -1;
        }
        if (!is_writable($newdirdestfile)) {
            Functions::dol_syslog("files.lib.php::dol_copy failed Permission denied to write into target directory " . $newdirdestfile, LOG_WARNING);
            return -2;
        }
        ...
        $testvirusarray = [];
        if ($testvirus) {
            $testvirusarray = Files::dolCheckVirus($srcfile, $destfile);
            if (count($testvirusarray)) {
                Functions::dol_syslog("files.lib.php::dol_copy canceled because a virus was found into source file. we ignore the copy request.", LOG_WARNING);
                return -3;
            }
        }
        ...
        if (!$result) {
            Functions::dol_syslog("files.lib.php::dol_copy failed to copy", LOG_WARNING);
            return -3;
        }
        if (empty($newmask) && Functions::getDolGlobalString('MAIN_UMASK')) {
            $newmask = Functions::getDolGlobalString('MAIN_UMASK');
        }
        if (empty($newmask)) {  // This should no happen
            Functions::dol_syslog("Warning: dol_copy called with empty value for newmask and no default value defined", LOG_WARNING);
            $newmask = '0664';
        }

        Functions::dolChmod($newpathofdestfile, $newmask);
        ...
        if ($result && $indexdatabase) {
            ...
            if (!preg_match('/([\\/]temp[\\/]|[\\/]thumbs|\.meta$)/', $rel_filetocopyafter)) {     // If not a tmp file
                ...
                Functions::dol_syslog("Try to copy also entries in database for: " . $rel_filetocopyafter, LOG_DEBUG);
                include_once DOL_DOCUMENT_ROOT . '/ecm/class/ecmfiles.class.php';
                ...
                if ($resultecmtarget > 0) {   // An entry for target name already exists for target, we delete it, a new one will be created.
                    Functions::dol_syslog("ECM dest file found, remove it", LOG_DEBUG);
                    $ecmfiletarget->delete($user);
                } else {
                    Functions::dol_syslog("ECM dest file not found, create it", LOG_DEBUG);
                }

                $ecmSrcfile = new EcmFiles($db);
                $resultecm = $ecmSrcfile->fetch(0, '', $srcfile);
                if ($resultecm) {
                    Functions::dol_syslog("Fetch src file ok", LOG_DEBUG);
                } else {
                    Functions::dol_syslog("Fetch src file error", LOG_DEBUG);
                }
                ...
                if ($resultecm < 0) {
                    Functions::dol_syslog("Create ECM file ok", LOG_DEBUG);
                    setEventMessages($ecmfile->error, $ecmfile->errors, 'warnings');
                } else {
                    Functions::dol_syslog("Create ECM file error", LOG_DEBUG);
                    setEventMessages($ecmfile->error, $ecmfile->errors, 'warnings');
                }
     ...
     * @param string               $newmask           Mask for new file ('0' by default means
     *                                                getDolGlobalString('MAIN_UMASK')). Example: '0666'
     * @param int                  $overwriteifexists Overwrite file if exists (1 by default)
     * @param array<string,string> $arrayreplacement  Array to use to replace filenames with another one during the
     *                                                copy
     *                                                (works only on file names, not on directory names).
     * @param int                  $excludesubdir     0=Do not exclude subdirectories, 1=Exclude subdirectories,
     *                                                2=Exclude subdirectories if name is not a 2 chars (used for
     *                                                country codes subdirectories).
     * @param string[]             $excludefileext    Exclude some file extensions
     *
     * @return  int                         Return integer <0 if error, 0 if nothing done (all files already exists and
     *                                      overwriteifexists=0), >0 if OK
     * @see     dol_copy()
     */
    public static function dolCopyDir($srcfile, $destfile, $newmask, $overwriteifexists, $arrayreplacement = null, $excludesubdir = 0, $excludefileext = null)
    {
        $result = 0;

        Functions::dol_syslog("files.lib.php::dolCopyDir srcfile=" . $srcfile . " destfile=" . $destfile . " newmask=" . $newmask . " overwriteifexists=" . $overwriteifexists);

        if (empty($srcfile) || empty($destfile)) {
            return -1;
        }

        $destexists = static::dol_is_dir($destfile);
        ...
        if (!$destexists) {
            ...
            if (empty($newmask) && Functions::getDolGlobalString('MAIN_UMASK')) {
                $dirmaskdec = octdec(Functions::getDolGlobalString('MAIN_UMASK'));
            }
            $dirmaskdec |= octdec('0200'); // Set w bit required to be able to create content for recursive subdirs files
            Functions::dol_mkdir($destfile, '', decoct($dirmaskdec));
        }

        $ossrcfile = Functions::dol_osencode($srcfile);
        $osdestfile = Functions::dol_osencode($destfile);
     ...
     * @param string $destfile          Destination file (can't be a directory. use native php @rename() to move a
     *                                  directory)
    ...
    function dolCheckOnFileName($src_file, $dest_file = '')
    {
        if (preg_match('/\.pdf$/i', $dest_file)) {
            if (!getDolGlobalString('MAIN_ANTIVIRUS_ALLOW_JS_IN_PDF')) {
                ...
                if (preg_match('/[\n\s]+\/JavaScript[\n\s]+/m', $tmp)) {
                    return ['File is a PDF with javascript inside'];
                }
            } else {
     ...
     *  - Extension .noexe may be added if file is executable and
     *  MAIN_DOCUMENT_IS_OUTSIDE_WEBROOT_SO_NOEXE_NOT_REQUIRED is not set.
     *
    ...
    function dol_move_uploaded_file($src_file, $dest_file, $allowoverwrite, $disablevirusscan = 0, $uploaderrorcode = 0, $nohook = 0, $varfiles = 'addedfile', $upload_dir = '')
    {
        ...
        if (empty($nohook)) {
            $reshook = $hookmanager->initHooks(['fileslib']);

            $parameters = ['dest_file' => $dest_file, 'src_file' => $src_file, 'file_name' => $file_name, 'varfiles' => $varfiles, 'allowoverwrite' => $allowoverwrite];
     ...
     * @param int         $disableglob   Disable usage of glob like * so function is an exact delete function that will
     *                                   return error if no file found
     * @param int         $nophperrors   Disable all PHP output errors
     * @param int         $nohook        Disable all hooks
     * @param object|null $object        Current object in use
     * @param boolean     $allowdotdot   Allow to delete file path with .. inside. Never use this, it is reserved for
     *                                   migration purpose.
     * @param int         $indexdatabase Try to remove also index entries.
     * @param int         $nolog         Disable log file
     *
     * @return boolean                 True if no error (file is deleted or if glob is used and there's nothing to
     *                                 delete), False if error
     * @see dol_delete_dir()
     */
     ...
     *
     * @return int                     Number of files and directory we try to remove. NB really removed is returned
     *                                 into var by reference $countdeleted.
     */
     ...
     * Note: Only email module seems to use this. Other feature initialize the $_SESSION doing
     * $formmail->clear_attached_files(); $formmail->add_attached_files()
     *
    ...
    /**
     * Get and save an upload file (for example after submitting a new file a mail form). Database index of file is
     * also
     * updated if donotupdatesession is set. All information used are in db, conf, langs, user and _FILES. Note: This
     * function can be used only into a HTML page context.
     *
     * @param string $upload_dir         Directory where to store uploaded file (note: used to forge $destpath =
     *                                   $upload_dir + filename)
     * @param int    $allowoverwrite     1=Allow overwrite existing file
     * @param int    $donotupdatesession 1=Do no edit _SESSION variable but update database index. 0=Update _SESSION
     *                                   and
     *                                   not database index. -1=Do not update SESSION neither db.
     * @param string $varfiles           _FILES var name
     * @param string $savingdocmask      Mask to use to define output filename. For example
     *                                   'XXXXX-__YYYYMMDD__-__file__'
     ...
     * @param string $ext        Format of target file (It is also extension added to file if fileoutput is not
     *                           provided).
        ...
        if ($fileinfo["extension"] == "zip") {
            ...
            if (class_exists('ZipArchive')) {   // Must install php-zip to have it
                ...
                if ($res === true) {
                    ...
                    for ($i = 0; $i < $zip->numFiles; $i++) {
                        ...
                        $zip->extractTo($outputdir . '/', [$zip->getNameIndex($i)]);
                    }

                    $zip->close();
                    return [];
                } else {
                    return ['error' => 'ErrUnzipFails'];
                }
            }

            return ['error' => 'ErrNoZipEngine'];
        } elseif (in_array($fileinfo["extension"], ['gz', 'bz2', 'zst'])) {
            ...
            if ($extension == "tar") {
                ...
                if ($fileinfo["extension"] == "gz") {
                    ...
                    return ['error' => 'ErrorBadFileExtension'];
                }
            ...
            return $resarray["result"] != 0 ? ['error' => $resarray["error"]] : [];
        }
     ...
     * @param string $regexfilter   Regex filter to restrict list. This regex value must be escaped for '/', since this
     *                              char is used for preg_match function
     * @param array  $excludefilter Array of Regex for exclude filter (example:
     *                              array('(\.meta|_preview.*\.png)$','^\.')). This regex value must be escaped for
     *                              '/', since this char is used for preg_match function
     * @param int    $nohook        Disable all hooks
     * @param int    $mode          0=Return array minimum keys loaded (faster), 1=Force all keys like date and size to
     *                              be loaded (slower), 2=Force load of date only, 3=Force load of size only
     *
     * @return  array                       Array with properties (full path, date, ...) of to most recent file
     */
    function dol_most_recent_file($dir, $regexfilter = '', $excludefilter = ['(\.meta|_preview.*\.png)$', '^\.'], $nohook = false, $mode = '')
    {
    ...
    /**
     * Security check when accessing to a document (used by document.php, viewimage.php and webservices to get
     * documents). TODO Replace code that set $accessallowed by a call to restrictedArea()
     *
     * @param string    $modulepart    Module of document ('module', 'module_user_temp', 'module_user' or
     *                                 'module_temp'). Example: 'medias', 'invoice', 'logs', 'tax-vat', ...
     * @param string    $original_file Relative path with filename, relative to modulepart.
     * @param string    $entity        Restrict onto entity (0=no restriction)
     * @param User|null $fuser         User object (forced)
     * @param string    $refname       Ref of object to check permission for external users (autodetect if not
     *                                 provided) or for hierarchy
     * @param string    $mode          Check permission for 'read' or 'write'
     *
     * @return  mixed                           Array with access information : 'accessallowed' &
     *                                          'sqlprotectagainstexternals' & 'original_file' (as a full path name)
     * @see restrictedArea()
     */
    function dol_check_secure_access_document($modulepart, $original_file, $entity, $fuser = null, $refname = '', $mode = 'read')
    {
        ...
        if ($modulepart == 'common') {
            ...
            if (empty($entity) || empty($conf->medias->multidir_output[$entity])) {
                return ['accessallowed' => 0, 'error' => 'Value entity must be provided'];
            }
            $accessallowed = 1;
            ...
            if ($fuser->hasRight('holiday', $read) || $fuser->hasRight('holiday', 'readall') || preg_match('/^specimen/i', $original_file)) {
                ...
                if ($refname && !$fuser->hasRight('holiday', 'readall') && !preg_match('/^specimen/i', $original_file)) {
                    ...
                    $accessallowed = checkUserAccessToObject($user, ['holiday'], $tmpholiday, 'holiday', '', '', 'rowid', '');
                }
            }
            ...
            if ($fuser->hasRight('expensereport', $lire) || $fuser->hasRight('expensereport', 'readall') || preg_match('/^specimen/i', $original_file)) {
                ...
                if ($refname && !$fuser->hasRight('expensereport', 'readall') && !preg_match('/^specimen/i', $original_file)) {
                    ...
                    $accessallowed = checkUserAccessToObject($user, ['expensereport'], $tmpexpensereport, 'expensereport', '', '', 'rowid', '');
                }
            }
        ...
        } elseif (in_array($modulepart, ['tax', 'tax-vat', 'tva']) && !empty($conf->tax->dir_output)) {
            // Wrapping for taxes
            ...
            if ($fuser->hasRight('agenda', 'myactions', $read)) {
                ...
                if ($refname && !preg_match('/^specimen/i', $original_file)) {
                    ...
                    $accessallowed = checkUserAccessToObject($user, ['agenda'], $tmpobject->id, 'actioncomm&societe', 'myactions|allactions', 'fk_soc', 'id', '');
                    if ($user->socid && $tmpobject->socid) {
                        $accessallowed = checkUserAccessToObject($user, ['societe'], $tmpobject->socid);
                    }
                }
            }
            ...
            if ($fuser->hasRight('projet', $lire) || preg_match('/^specimen/i', $original_file)) {
                ...
                if ($refname && !preg_match('/^specimen/i', $original_file)) {
                    ...
                    $accessallowed = checkUserAccessToObject($user, ['projet'], $tmpproject->id, 'projet&project', '', '', 'rowid', '');
                }
            }
            ...
            if ($fuser->hasRight('projet', $lire) || preg_match('/^specimen/i', $original_file)) {
                ...
                if ($refname && !preg_match('/^specimen/i', $original_file)) {
                    ...
                    $accessallowed = checkUserAccessToObject($user, ['projet_task'], $tmptask->id, 'projet_task&project', '', '', 'rowid', '');
                }
            }
            $original_file = $conf->project->multidir_output[$entity] . '/' . $original_file;
            $sqlprotectagainstexternals = "SELECT fk_soc as fk_soc FROM " . MAIN_DB_PREFIX . "projet WHERE ref='" . $db->escape($refname) . "' AND entity IN (" . getEntity('project') . ")";
     ...
     * @param string           $path           Path of files relative to $pathref. We start with ''. Used by recursive
     *                                         calls.
    ...
    function getFilesUpdated(&$file_list, SimpleXMLElement $dir, $path = '', $pathref = '', &$checksumconcat = [])
    {
        ...
        foreach ($dir->md5file as $file) {    // $file is a simpleXMLElement
            ...
            if (!file_exists($pathref . '/' . $filename)) {
                $file_list['missing'][] = ['filename' => $filename, 'expectedmd5' => $expectedmd5, 'expectedsize' => $expectedsize];
            } else {
                ...
                if ($conffile == '/etc/dolibarr/conf.php' && $filename == '/filefunc.inc.php') {    // For install with deb or rpm, we ignore test on filefunc.inc.php that was modified by package
                    ...
                    if ($md5_local != $expectedmd5) {
                        $file_list['updated'][] = ['filename' => $filename, 'expectedmd5' => $expectedmd5, 'expectedsize' => $expectedsize, 'md5' => (string) $md5_local];
                    }
