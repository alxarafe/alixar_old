...
class Ajax
{
	...
	';
        ...
        if (Functions::getDolGlobalString('PRODUIT_CUSTOMER_PRICES_BY_QTY') || Functions::getDolGlobalString('PRODUIT_CUSTOMER_PRICES_BY_QTY_MULTIPRICES')) {
            $script .= '
     ...
     */
    public static function ajax_combobox($htmlname, $events = array(), $minLengthToAutocomplete = 0, $forcefocus = 0, $widthTypeOfAutocomplete = 'resolve', $idforemptyvalue = '-1', $morecss = '')
    {
        $conf = Globals::getConf();

        // select2 can be disabled for smartphones
        if (!empty($conf->browser->layout) && $conf->browser->layout == 'phone' && Functions::getDolGlobalString('MAIN_DISALLOW_SELECT2_WITH_SMARTPHONE')) {
            return '';
        }

        if (Functions::getDolGlobalString('MAIN_DISABLE_AJAX_COMBOX')) {
            return '';
        }
        if (empty($conf->use_javascript_ajax)) {
            return '';
        }
        if (!Functions::getDolGlobalString('MAIN_USE_JQUERY_MULTISELECT') && !defined('REQUIRE_JQUERY_MULTISELECT')) {
            return '';
        }
