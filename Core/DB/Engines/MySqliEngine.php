...
class MySqliEngine extends DB
{
    ...
    public function __construct($host, $user, $pass, $name = '', $port = 0)
    {
        ...
        if (static::$connected && $name) {
            if ($this->select_db($name)) {
                ...
                if (empty($disableforcecharset) && static::$db->character_set_name() != $clientmustbe) {
                    try {
                        //print "You should set the \$dolibarr_main_db_character_set and \$dolibarr_main_db_collation for the PHP to the one of the database ".static::$db->character_set_name();
                        Functions::dol_syslog(get_class($this) . "::DoliDBMysqli You should set the \$dolibarr_main_db_character_set and \$dolibarr_main_db_collation for the PHP to the one of the database " . static::$db->character_set_name(), LOG_WARNING);
                        static::$db->set_charset($clientmustbe); // This set charset, but with a bad collation
                    } catch (Exception $e) {
                    ...
                    if (!preg_match('/general/', $collation)) {
                        static::$db->query("SET collation_connection = " . $collation);
                    }
                }
            } else {
                ...
                Functions::dol_syslog(get_class($this) . "::DoliDBMysqli : Select_db error " . $this->error, LOG_ERR);
            }
        } else {
    ...
    {
        // phpcs:enable
        Functions::dol_syslog(get_class($this) . "::select_db database=" . $database, LOG_DEBUG);
        $result = false;
        try {
            $result = static::$db->select_db($database);
        } catch (Exception $e) {
    ...
    {
        Functions::dol_syslog(get_class($this) . "::connect host=$host, port=$port, login=$login, passwd=--hidden--, name=$name", LOG_DEBUG);
        ...
        try {
            ...
            Functions::dol_syslog(get_class($this) . "::connect failed", LOG_DEBUG);
        }
        return $tmp;
    }
    ...
    public function query($query, $usesavepoint = 0, $type = 'auto', $result_mode = 0)
    {
        ...
        if (!in_array($query, ['BEGIN', 'COMMIT', 'ROLLBACK'])) {
            ...
            Functions::dol_syslog('sql=' . substr($query, 0, $SYSLOG_SQL_LIMIT), LOG_DEBUG);
        }
        if (empty($query)) {
        ...
        try {
            if (!$this->database_name) {
                ...
                $ret = static::$db->query($query, $result_mode);
            } else {
                $ret = static::$db->query($query, $result_mode);
            }
        } catch (Exception $e) {
            Functions::dol_syslog(get_class($this) . "::query Exception in query instead of returning an error: " . $e->getMessage(), LOG_ERR);
            $ret = false;
        }
        ...
        if (!preg_match("/^COMMIT/i", $query) && !preg_match("/^ROLLBACK/i", $query)) {
            ...
            if (!$ret) {
                ...
                if (Functions::getDolGlobalInt('SYSLOG_LEVEL') < LOG_DEBUG) {
                    Functions::dol_syslog(get_class($this) . "::query SQL Error query: " . $query, LOG_ERR); // Log of request was not yet done previously
                }
                Functions::dol_syslog(get_class($this) . "::query SQL Error message: " . $this->lasterrno . " " . $this->lasterror, LOG_ERR);
    ...
    {
        return static::$db->real_escape_string((string) $stringtoencode);
    }
    ...
    public function errno()
    {
        if (!static::$connected) {
            ...
            if (isset($errorcode_map[static::$db->errno])) {
                return $errorcode_map[static::$db->errno];
            }
    ...
    public function DDLCreateDb($database, $charset = '', $collation = '', $owner = '')
    {
        ...
        Functions::dol_syslog($sql, LOG_DEBUG);
        $ret = $this->query($sql);
        if (!$ret) {
    ...
    public function DDLDescTable($table, $field = "")
    {
        ...
        Functions::dol_syslog(get_class($this) . "::DDLDescTable " . $sql, LOG_DEBUG);
    ...
    public function DDLAddField($table, $field_name, $field_desc, $field_position = "")
    {
        ...
        Functions::dol_syslog(get_class($this) . "::DDLAddField " . $sql, LOG_DEBUG);
        if ($this->query($sql)) {
            return 1;
        }
        return -1;
    ...
    public function DDLUpdateField($table, $field_name, $field_desc)
    {
        ...
        Functions::dol_syslog(get_class($this) . "::DDLUpdateField " . $sql, LOG_DEBUG);
        if (!$this->query($sql)) {
            return -1;
        } else {
            return 1;
    ...
    public function DDLCreateUser($dolibarr_main_db_host, $dolibarr_main_db_user, $dolibarr_main_db_pass, $dolibarr_main_db_name)
    {
        ...
        Functions::dol_syslog(get_class($this) . "::DDLCreateUser", LOG_DEBUG); // No sql to avoid password in log
        ...
        if (!$resql) {
            if ($this->lasterrno != 'DB_ERROR_USER_ALREADY_EXISTS') {
                ...
                Functions::dol_syslog(get_class($this) . "::DDLCreateUser sql=" . $sql, LOG_WARNING);
            }
        }
        ...
        Functions::dol_syslog(get_class($this) . "::DDLCreateUser", LOG_DEBUG);
        $resql = $this->query($sql);
        if (!$resql) {
            return -1;
        }
