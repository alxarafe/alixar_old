 ...
 */
class Modules // Can not be abstract, because we need to instantiate it into unActivateModule to be able to disable a module whose files were removed.
{
    /**
    ...
    /**
     * @var string[] List of module class names that must be enabled if this module is enabled. e.g.:
     *      array('modAnotherModule', 'FR'=>'modYetAnotherModule')
     * @see $requiredby
     */
    public $depends;
    ...
    protected function _init($array_sql, $options = '')
    {
        ...
        for ($i = 0; $i < $num; $i++) {
            if (!$err) {
                ...
                Functions::dol_syslog(get_class($this) . "::_init ignoreerror=" . $ignoreerror, LOG_DEBUG);
                ...
                if (!$result) {
                    if (!$ignoreerror) {
                        ...
                        Functions::dol_syslog(get_class($this) . "::_init Warning " . $this->db->lasterror(), LOG_WARNING);
                    }
                }
            }
        }
    ...
    protected function _remove($array_sql, $options = '')
    {
        ...
        for ($i = 0; $i < $num; $i++) {
            if (!$err) {
                Functions::dol_syslog(get_class($this) . "::_remove", LOG_DEBUG);
    ...
    public function getLastActivationDate()
    {
        ...
        $sql = "SELECT tms FROM " . $config->main_db_prefix . "const";
        $sql .= " WHERE " . $this->db->decrypt('name') . " = '" . $this->db->escape($this->const_name) . "'";
        $sql .= " AND entity IN (0, " . ((int) $conf->entity) . ")";

        Functions::dol_syslog(get_class($this) . "::getLastActiveDate", LOG_DEBUG);
    ...
    public function getLastActivationInfo()
    {
        ...
        $sql = "SELECT tms, note FROM " . $config->main_db_prefix . "const";
        $sql .= " WHERE " . $this->db->decrypt('name') . " = '" . $this->db->escape($this->const_name) . "'";
    ...
    protected function _active()
    {
        ...
        Functions::dol_syslog(get_class($this) . "::_active delete activation constant", LOG_DEBUG);
        $resql = $this->db->query($sql);
        if (!$resql) {
            $err++;
        }
        ...
        $sql = "INSERT INTO " . $config->main_db_prefix . "const (name, value, visible, entity, note) VALUES";
        $sql .= " (" . $this->db->encrypt($this->const_name);
        $sql .= ", " . $this->db->encrypt('1');
        $sql .= ", 0, " . ((int) $entity);
        $sql .= ", '" . $this->db->escape($note) . "')";

        Functions::dol_syslog(get_class($this) . "::_active insert activation constant", LOG_DEBUG);
        $resql = $this->db->query($sql);
        if (!$resql) {
            $err++;
        }
    ...
    protected function _unactive()
    {
        ...
        Functions::dol_syslog(get_class($this) . "::_unactive", LOG_DEBUG);
        $this->db->query($sql);
    ...
    protected function _load_tables($reldir, $onlywithsuffix = '')
    {
                    ...
                    if (is_resource($handle)) {
                        ...
                        foreach ($files as $file) {
                            ...
                            if (preg_match('/\.sql$/i', $file) && !preg_match('/\.key\.sql$/i', $file) && substr($file, 0, 4) == 'llx_') {
                                $result = run_sql($dir . $file, !Functions::getDolGlobalString('MAIN_DISPLAY_SQL_INSTALL_LOG') ? 1 : 0, '', 1);
                                if ($result <= 0) {
                                    $error++;
                                }
                            }
        ...
        if (!$dirfound) {
            Functions::dol_syslog("A module ask to load sql files into " . $reldir . " but this directory was not found.", LOG_WARNING);
        }
        return $ok;
    }
    ...
    public function insert_boxes($option = '')
    {
        ...
        if (is_array($this->boxes)) {
            Functions::dol_syslog(get_class($this) . "::insert_boxes", LOG_DEBUG);
            ...
            foreach ($this->boxes as $key => $value) {
                ...
                $sql = "SELECT count(*) as nb FROM " . $config->main_db_prefix . "boxes_def";
                ...
                if ($result) {
                    ...
                    if ($obj->nb == 0) {
                        ...
                        if (!$err) {
                            $sql = "INSERT INTO " . $config->main_db_prefix . "boxes_def (file, entity, note)";
                        ...
                        if (!$err && !preg_match('/newboxdefonly/', $option)) {
                            $lastid = $this->db->last_insert_id($config->main_db_prefix . "boxes_def", "rowid");
                            ...
                            foreach ($pos_name as $key2 => $val2) {
                                ...
                                $sql = "INSERT INTO " . $config->main_db_prefix . "boxes (box_id, position, box_order, fk_user, entity)";
                                $sql .= " VALUES (" . ((int) $lastid) . ", " . ((int) $key2) . ", '0', 0, " . ((int) $conf->entity) . ")";

                                Functions::dol_syslog(get_class($this) . "::insert_boxes onto page " . $key2 . "=" . $val2, LOG_DEBUG);
                                $resql = $this->db->query($sql);
                                if (!$resql) {
                                    $err++;
                                }
            ...
            foreach ($this->boxes as $key => $value) {
                ...
                if ($file == 'box_graph_product_distribution.php') {
                    if (isModEnabled("product") || isModEnabled("service")) {
                        Functions::dol_syslog("We discard deleting module " . $file . " because another module still active requires it.");
                        continue;
                    }
                }
                ...
                if ($this->db->type == 'sqlite3') {
                    ...
                    $sql = "DELETE FROM " . $config->main_db_prefix . "boxes ";
                    $sql .= "WHERE " . $config->main_db_prefix . "boxes.box_id IN (";
                    $sql .= "SELECT " . $config->main_db_prefix . "boxes_def.rowid ";
                    $sql .= "FROM " . $config->main_db_prefix . "boxes_def ";
                    $sql .= "WHERE " . $config->main_db_prefix . "boxes_def.file = '" . $this->db->escape($file) . "') ";
                    $sql .= "AND " . $config->main_db_prefix . "boxes.entity = " . $conf->entity;
                } else {
                    $sql = "DELETE FROM " . $config->main_db_prefix . "boxes";
                    $sql .= " USING " . $config->main_db_prefix . "boxes, " . $config->main_db_prefix . "boxes_def";
                    $sql .= " WHERE " . $config->main_db_prefix . "boxes.box_id = " . $config->main_db_prefix . "boxes_def.rowid";
                    $sql .= " AND " . $config->main_db_prefix . "boxes_def.file = '" . $this->db->escape($file) . "'";
                    $sql .= " AND " . $config->main_db_prefix . "boxes.entity = " . $conf->entity;
                }

                Functions::dol_syslog(get_class($this) . "::delete_boxes", LOG_DEBUG);
                $resql = $this->db->query($sql);
                if (!$resql) {
                    $this->error = $this->db->lasterror();
                    $err++;
                ...
                $sql = "DELETE FROM " . $config->main_db_prefix . "boxes_def";
                $sql .= " WHERE file = '" . $this->db->escape($file) . "'";
    ...
    public function insert_cronjobs()
    {
        ...
        if (is_array($this->cronjobs)) {
            Functions::dol_syslog(get_class($this) . "::insert_cronjobs", LOG_DEBUG);
            ...
            foreach ($this->cronjobs as $key => $value) {
                ...
                $sql = "SELECT count(*) as nb FROM " . $config->main_db_prefix . "cronjob";
                ...
                if ($result) {
                    ...
                    if ($obj->nb == 0) {
                        ...
                        if (!$err) {
                            $sql = "INSERT INTO " . $config->main_db_prefix . "cronjob (module_name, datec, datestart, dateend, label, jobtype, classesname, objectname, methodename, command, params, note,";
                            if (is_int($frequency)) {
                                $sql .= ' frequency,';
                            }
    ...
    public function delete_cronjobs()
    {
        ...
        if (is_array($this->cronjobs)) {
            $sql = "DELETE FROM " . $config->main_db_prefix . "cronjob";
            $sql .= " WHERE module_name = '" . $this->db->escape(empty($this->rights_class) ? strtolower($this->name) : $this->rights_class) . "'";
            $sql .= " AND entity = " . $conf->entity;
            ...
            Functions::dol_syslog(get_class($this) . "::delete_cronjobs", LOG_DEBUG);
            $resql = $this->db->query($sql);
            if (!$resql) {
                $this->error = $this->db->lasterror();
                $err++;
    ...
    public function delete_tabs()
    {
        ...
        Functions::dol_syslog(get_class($this) . "::delete_tabs", LOG_DEBUG);
    ...
    public function insert_tabs()
    {
        ...
        if (!empty($this->tabs)) {
            Functions::dol_syslog(get_class($this) . "::insert_tabs", LOG_DEBUG);
            ...
            foreach ($this->tabs as $key => $value) {
                ...
                if ($newvalue) {
                    $sql = "INSERT INTO " . $config->main_db_prefix . "const (";
                    $sql .= "name";
                    $sql .= ", type";
                    $sql .= ", value";
                    $sql .= ", note";
                    ...
                    $resql = $this->db->query($sql);
                    if (!$resql) {
                        Functions::dol_syslog($this->db->lasterror(), LOG_ERR);
    ...
    public function insert_const()
    {
        ...
        Functions::dol_syslog(get_class($this) . "::insert_const", LOG_DEBUG);
        ...
        foreach ($this->const as $key => $value) {
            ...
            $sql .= " FROM " . $config->main_db_prefix . "const";
            ...
            if ($result) {
                ...
                if ($row[0] == 0) {   // If not found
                    $sql = "INSERT INTO " . $config->main_db_prefix . "const (name,type,value,note,visible,entity)";
                    $sql .= " VALUES (";
                    ...
                    Functions::dol_syslog(get_class($this) . "::insert_const constant '" . $name . "' already exists", LOG_DEBUG);
                }
            } else {
                $err++;
            }
    ...
    public function delete_const()
    {
        ...
        foreach ($this->const as $key => $value) {
            ...
            if ($deleteonunactive) {
                ...
                $sql .= " WHERE " . $this->db->decrypt('name') . " = '" . $this->db->escape($name) . "'";
                $sql .= " AND entity in (0, " . $conf->entity . ")";
                Functions::dol_syslog(get_class($this) . "::delete_const", LOG_DEBUG);
                if (!$this->db->query($sql)) {
                    $this->error = $this->db->lasterror();
                    $err++;
                }
    ...
    public function insert_permissions($reinitadminperms = 0, $force_entity = null, $notrigger = 0)
    {
        ...
        Functions::dol_syslog(get_class($this) . "::insert_permissions", LOG_DEBUG);

        // Test if module is activated
        $sql_del = "SELECT " . $this->db->decrypt('value') . " as value";
        $sql_del .= " FROM " . $config->main_db_prefix . "const";
        ...
        if ($resql) {
            ...
            if ($obj !== null && !empty($obj->value) && !empty($this->rights)) {
                ...
                foreach ($this->rights as $key => $value) {
                    ...
                    $sql = "SELECT count(*) as nb FROM " . $config->main_db_prefix . "rights_def";
                    ...
                    if ($resqlselect) {
                        ...
                        if ($objcount && $objcount->nb == 0) {
                            if (Functions::dol_strlen($r_perms)) {
                                if (Functions::dol_strlen($r_subperms)) {
                                    $sql = "INSERT INTO " . $config->main_db_prefix . "rights_def";
                            ...
                            if (!$resqlinsert) {
                                if ($this->db->errno() != "DB_ERROR_RECORD_ALREADY_EXISTS") {
                                    ...
                                    Functions::dol_syslog(get_class($this) . "::insert_permissions record already exists", LOG_INFO);
                                }
                            }
                    ...
                    if ($reinitadminperms) {
                        $sql = "SELECT rowid FROM " . $config->main_db_prefix . "user WHERE admin = 1";
                        Functions::dol_syslog(get_class($this) . "::insert_permissions Search all admin users", LOG_DEBUG);
                        ...
                        if ($resqlseladmin) {
                            ...
                            while ($i < $num) {
                                ...
                                Functions::dol_syslog(get_class($this) . "::insert_permissions Add permission id " . $r_id . " to user id=" . $obj2->rowid);
                                ...
                                if ($result > 0) {
                                    ...
                                    Functions::dol_syslog(get_class($this) . "::insert_permissions Failed to add the permission to user because fetch return an error", LOG_ERR);
                                }
                                $i++;
                            }
                        } else {
    ...
    public function delete_permissions()
    {
        ...
        $sql = "DELETE FROM " . $config->main_db_prefix . "rights_def";
        $sql .= " WHERE module = '" . $this->db->escape(empty($this->rights_class) ? strtolower($this->name) : $this->rights_class) . "'";
        $sql .= " AND entity = " . $conf->entity;
        Functions::dol_syslog(get_class($this) . "::delete_permissions", LOG_DEBUG);
        if (!$this->db->query($sql)) {
            $this->error = $this->db->lasterror();
            $err++;
        }
    ...
    public function insert_menus()
    {
        ...
        Functions::dol_syslog(get_class($this) . "::insert_menus", LOG_DEBUG);
        ...
        foreach ($this->menu as $key => $value) {
            ...
            if (!$this->menu[$key]['fk_menu']) {
                ...
                if (!$foundparent) {
                    ...
                    Functions::dol_syslog(get_class($this) . "::insert_menus " . $this->error . " " . $this->menu[$key]['fk_menu'], LOG_ERR);
                    $err++;
                }
            }
            ...
            if (!$err) {
                ...
                if ($result > 0) {
                    ...
                    Functions::dol_syslog(get_class($this) . '::insert_menus result=' . $result . " " . $this->error, LOG_ERR);
                    $err++;
                    break;
                }
            }
        ...
        if (!$err) {
            ...
            Functions::dol_syslog(get_class($this) . "::insert_menus " . $this->error, LOG_ERR);
            $this->db->rollback();
        }
    ...
    public function delete_menus()
    {
        ...
        $sql = "DELETE FROM " . $config->main_db_prefix . "menu";
        $sql .= " WHERE module = '" . $this->db->escape($module) . "'";
        $sql .= " AND entity IN (0, " . $conf->entity . ")";

        Functions::dol_syslog(get_class($this) . "::delete_menus", LOG_DEBUG);
        $resql = $this->db->query($sql);
        if (!$resql) {
            $this->error = $this->db->lasterror();
            $err++;
    ...
    public function create_dirs()
    {
        ...
        if (isset($this->dirs) && is_array($this->dirs)) {
            foreach ($this->dirs as $key => $value) {
                ...
                if (!Functions::getDolGlobalString('MAIN_MODULE_MULTICOMPANY') || $conf->entity == 1) {
                    $fulldir = $config->main_data_root . $dir;
                } else {
                    $fulldir = $config->main_data_root . "/" . $conf->entity . $dir;
                }
                // Create dir if it does not exists
                if (!empty($fulldir) && !file_exists($fulldir)) {
                    if (Functions::dol_mkdir($fulldir, $config->main_data_root) < 0) {
                        $this->error = $langs->trans("ErrorCanNotCreateDir", $fulldir);
                        Functions::dol_syslog(get_class($this) . "::_init " . $this->error, LOG_ERR);
                        $err++;
                    }
                }
    ...
    public function insert_dirs($name, $dir)
    {
        ...
        Functions::dol_syslog(get_class($this) . "::insert_dirs", LOG_DEBUG);
        $result = $this->db->query($sql);
        if ($result) {
            $row = $this->db->fetch_row($result);
    ...
    public function delete_dirs()
    {
        ...
        Functions::dol_syslog(get_class($this) . "::delete_dirs", LOG_DEBUG);
        if (!$this->db->query($sql)) {
            $this->error = $this->db->lasterror();
            $err++;
        }
            ...
            foreach ($this->module_parts as $key => $value) {
                ...
                if ($key == 'websitetemplates' && $value == 1) {
                    ...
                    foreach ($docs as $cursorfile) {
                        ...
                        $dest = $config->main_data_root . '/doctemplates/websites/' . $cursorfile['name'];
                ...
                if (!empty($newvalue)) {
                    ...
                    Functions::dol_syslog(get_class($this) . "::insert_module_parts for key=" . $this->const_name . "_" . strtoupper($key), LOG_DEBUG);
                    ...
                    if (!$resql) {
                        if ($this->db->lasterrno() != 'DB_ERROR_RECORD_ALREADY_EXISTS') {
                            ...
                            Functions::dol_syslog(get_class($this) . "::insert_module_parts for " . $this->const_name . "_" . strtoupper($key) . " Record already exists.", LOG_WARNING);
                        }
                    }
                }
            }
    ...
    public function delete_module_parts()
    {
        ...
        if (is_array($this->module_parts)) {
            Functions::dol_syslog(get_class($this) . "::delete_module_parts", LOG_DEBUG);
    ...
    public function getKanbanView($codeenabledisable = '', $codetoconfig = '')
    {
    	...
    	<div class="box-flex-item info-box-module'
            . (Functions::getDolGlobalString($const_name) ? '' : ' --disabled')
            . ($this->isCoreOrExternalModule() == 'external' ? ' --external' : '')
            . ($this->needUpdate ? ' --need-update' : '')
            . '">
	    <div class="info-box info-box-sm info-box-module">
	    <div class="info-box-icon' . (!Functions::getDolGlobalString($const_name) ? '' : ' info-box-icon-module-enabled' . ($versiontrans ? ' info-box-icon-module-warning' : '')) . '">';
        ...
        $return .= '</div>
	    <div class="info-box-content info-box-text-module' . (!Functions::getDolGlobalString($const_name) ? '' : ' info-box-module-enabled' . ($versiontrans ? ' info-box-content-warning' : '')) . '">
    ...
    protected function extrafieldsinimport()
    {
...
// Add extra fields
        $sql = "SELECT name, label, type, param, fieldcomputed, fielddefault, fieldrequired FROM " . $config->main_db_prefix . "extrafields";
...
//print $sql;
        ...
        if ($resql) {    // This can fail when class is used on old database (during migration for example)
            while ($obj = $this->db->fetch_object($resql)) {
                ...
                switch ($typefield) {
                    ...
                    case 'checkbox':
                    case 'select':
                        if (Functions::Functions::getDolGlobalString('EXPORT_LABEL_FOR_SELECT')) {
    ...
    protected function extrafieldsinexport($keyforselect, $keyforelement, $keyforaliasextra)
    {
        $config = Globals::getConfig();
        $db = Globals::getDb();
        $conf = Globals::getConf();
...
// Add extra fields
        $sql = "SELECT name, label, type, param, fieldcomputed, fielddefault FROM " . $config->main_db_prefix . "extrafields";
        $sql .= " WHERE elementtype = '" . $db->escape($keyforselect) . "' AND type <> 'separate' AND entity IN (0, " . ((int) $conf->entity) . ') ORDER BY pos ASC';
//print $sql;
        $resql = $db->query($sql);
        if ($resql) {    // This can fail when class is used on old database (during migration for example)
