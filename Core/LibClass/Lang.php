...
class Lang
{
    ...
    public function setDefaultLang($srclang = 'en_US')
    {
        ...
        if (Functions::getDolGlobalString('MAIN_FORCELANGDIR')) {
            $more = [];
            $i = 0;
            foreach ($conf->file->dol_document_root as $dir) {
                $newdir = $dir . Functions::getDolGlobalString('MAIN_FORCELANGDIR'); // For example $conf->global->MAIN_FORCELANGDIR is '/mymodule' meaning we search files into '/mymodule/langs/xx_XX'
    ...
    public function load($domain, $alt = 0, $stopafterdirection = 0, $forcelangdir = '', $loadfromfileonly = 0, $forceloadifalreadynotfound = 0)
    {
        ...
        if (
            ...
            //Functions::dol_syslog("Translate::Load already loaded for newdomain=".$newdomain);
            return 0;
        }
        ...
        if (empty($langofdir)) {    // This may occurs when load is called without setting the language and without providing a value for forcelangdir
            Functions::dol_syslog("Error: " . get_class($this) . "::load was called for domain=" . $domain . " but language was not set yet with langs->setDefaultLang(). Nothing will be loaded.", LOG_WARNING);
            return -1;
        }
        ...
        foreach ($this->dir as $searchdir) {
            ...
            //Functions::dol_syslog(get_class($this).'::Load Try to read for alt='.$alt.' langofdir='.$langofdir.' domain='.$domain.' newdomain='.$newdomain.' modulename='.$modulename.' file_lang='.$file_lang." => filelangexists=".$filelangexists);
            ...
            if ($filelangexists) {
                ...
                if ($usecachekey) {
                    //Functions::dol_syslog('Translate::Load we will cache result into usecachekey '.$usecachekey);
                    //global $aaa; $aaa+=1;
                    ...
                    if ($fp = @fopen($file_lang, "rt")) {
                        ...
                        if ($usecachekey && count($tabtranslatedomain)) {
                            ...
                            if ($ressetcache < 0) {
                                ...
                                Functions::dol_syslog($error, LOG_ERR);
                            }
                        }
        ...
        if ($alt == 0) {
            ...
            //Functions::dol_syslog("Translate::Load loading alternate translation file (to complete ".$this->defaultlang."/".$newdomain.".lang file)", LOG_DEBUG);
        ...
        if (empty($loadfromfileonly)) {
            ...
            if (!empty($conf->global->$overwritekey)) {    // Overwrite translation with key1:newstring1,key2:newstring2
                ...
                $tmparray = explode(',', Functions::getDolGlobalString($overwritekey));
    ...
    public function loadFromDatabase($db)
    {
        ...
        if (empty($langofdir)) {    // This may occurs when load is called without setting the language and without providing a value for forcelangdir
            Functions::dol_syslog("Error: " . get_class($this) . "::loadFromDatabase was called but language was not set yet with langs->setDefaultLang(). Nothing will be loaded.", LOG_WARNING);
            return -1;
        }
        ...
        if (!$found && Functions::getDolGlobalString('MAIN_ENABLE_OVERWRITE_TRANSLATION')) {
    ...
    public function trans($key, $param1 = '', $param2 = '', $param3 = '', $param4 = '', $maxsize = 0)
    {
        ...
        if (!empty($this->tab_translate[$key])) {    // Translation is available
            ...
            if (!empty($conf->global->$replacekey)) {    // Replacement translation variable with string1:newstring1;string2:newstring2
                $tmparray = explode(';', Functions::getDolGlobalString($replacekey));
                foreach ($tmparray as $tmp) {
                    $tmparray2 = explode(':', $tmp);
     ...
     */
    public function get_available_languages($langdir = BASE_PATH, $maxlength = 0, $usecode = 0, $mainlangonly = 0)
    {
        $this->load("languages");
        ...
        while ($dir = trim(readdir($handle))) {
            ...
            if (preg_match('/^([a-z]+)_([A-Z]+)/i', $dir, $regs)) {
                ...
                if (Functions::getDolGlobalString('MAIN_LANGUAGES_ALLOWED') && !in_array($dir, explode(',', Functions::getDolGlobalString('MAIN_LANGUAGES_ALLOWED')))) {
                    continue;
                }
                ...
                if ($usecode == 1 || Functions::getDolGlobalString('MAIN_SHOW_LANGUAGE_CODE')) {
    ...
    public function getLabelFromKey($db, $key, $tablename, $fieldkey, $fieldlabel, $keyforselect = '', $filteronentity = 0)
    {
        ...
        Functions::dol_syslog(get_class($this) . '::getLabelFromKey', LOG_DEBUG);
        $resql = $db->query($sql);
        if ($resql) {
            $obj = $db->fetch_object($resql);
            if ($obj) {
    ...
    public function loadCacheCurrencies($currency_code)
    {
        ...
        Functions::dol_syslog(get_class($this) . '::loadCacheCurrencies', LOG_DEBUG);
        $resql = $db->query($sql);
        if ($resql) {
            $this->load("dict");
            $label = [];
