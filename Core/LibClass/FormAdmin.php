...
class FormAdmin
{
    ...
    public function select_language($selected = '', $htmlname = 'lang_id', $showauto = 0, $filter = [], $showempty = '', $showwarning = 0, $disabled = 0, $morecss = '', $showcode = 0, $forcecombo = 0, $multiselect = 0, $onlykeys = [], $mainlangonly = 0)
    {
        ...
        if (Functions::getDolGlobalString('MAIN_DEFAULT_LANGUAGE_FILTER')) {
            if (!is_array($filter)) {
                $filter = [];
            }
            $filter[Functions::getDolGlobalString('MAIN_DEFAULT_LANGUAGE_FILTER')] = 1;
        }

        $langs_available = $langs->get_available_languages(BASE_PATH, 12, 0, $mainlangonly);
        ...
        foreach ($langs_available as $key => $value) {
            ...
            $valuetoshow = Functions::picto_from_langcode($key, 'class="saturatemedium"') . ' ' . $valuetoshow;
            if ((is_string($selected) && (string) $selected == (string) $keytouse) || (is_array($selected) && in_array($keytouse, $selected))) {
                $out .= '<option value="' . $keytouse . '" selected data-html="' . Functions::dol_escape_htmltag($valuetoshow) . '">' . $valuetoshow . '</option>';
            } else {
                $out .= '<option value="' . $keytouse . '" data-html="' . Functions::dol_escape_htmltag($valuetoshow) . '">' . $valuetoshow . '</option>';
            }
        }
        $out .= '</select>';
        ...
        // Make select dynamic
        if (!$forcecombo) {
            $out .= Ajax::ajax_combobox($htmlname);
        }
    ...
    public function select_menu($selected, $htmlname, $dirmenuarray, $moreattrib = '')
    {
        ...
        foreach ($menuarray as $key => $val) {
            $tab = explode('_', $key);
            $newprefix = $tab[0];
            if ($newprefix == '1' && (Functions::getDolGlobalInt('MAIN_FEATURES_LEVEL') < 1)) {
                continue;
            }
            if ($newprefix == '2' && (Functions::getDolGlobalInt('MAIN_FEATURES_LEVEL') < 2)) {
                continue;
            }
                    ...
                    if (is_resource($handle)) {
                        while (($file = readdir($handle)) !== false) {
                            if (is_file($dir . "/" . $file) && substr($file, 0, 1) != '.' && substr($file, 0, 3) != 'CVS') {
                                ...
                                if (Functions::getDolGlobalInt('MAIN_FEATURES_LEVEL') == 0 && in_array($file, $expdevmenu)) {
                                    continue;
                                }
