...
trait CommonPeople
{
    ...
    public function getBannerAddress($htmlkey, $object)
    {
        ...
        if ($coords) {
            if (!empty($conf->use_javascript_ajax)) {
                ...
                $namecoords = '';
                if ($this->element == 'contact' && Functions::getDolGlobalString('MAIN_SHOW_COMPANY_NAME_IN_BANNER_ADDRESS')) {
            ...
            $arrayoflangcode = [];
            if (Functions::getDolGlobalString('PDF_USE_ALSO_LANGUAGE_CODE')) {
                $arrayoflangcode[] = Functions::getDolGlobalString('PDF_USE_ALSO_LANGUAGE_CODE');
            }
        ...
        if (
            !in_array($this->country_code, $countriesusingstate) && !Functions::getDolGlobalString('MAIN_FORCE_STATE_INTO_ADDRESS')
            && !Functions::getDolGlobalString('SOCIETE_DISABLE_STATE') && $this->state
        ) {
    ...
    {
        if (Functions::getDolGlobalString('MAIN_FIRST_TO_UPPER')) {
            $this->lastname = Functions::dol_ucwords(Functions::dol_strtolower($this->lastname));
            $this->firstname = Functions::dol_ucwords(Functions::dol_strtolower($this->firstname));
            $this->name = Functions::dol_ucwords(Functions::dol_strtolower($this->name));
            if (property_exists($this, 'name_alias')) {
                $this->name_alias = isset($this->name_alias) ? Functions::dol_ucwords(Functions::dol_strtolower($this->name_alias)) : '';
            }
        }
        if (Functions::getDolGlobalString('MAIN_ALL_TO_UPPER')) {
            $this->lastname = Functions::dol_strtoupper($this->lastname);
            $this->name = Functions::dol_strtoupper($this->name);
            if (property_exists($this, 'name_alias')) {
                $this->name_alias = Functions::dol_strtoupper($this->name_alias);
            }
        }
        if (Functions::getDolGlobalString('MAIN_ALL_TOWN_TO_UPPER')) {
            $this->address = Functions::dol_strtoupper($this->address);
            $this->town = Functions::dol_strtoupper($this->town);
        }
        if (isset($this->email)) {
            $this->email = Functions::dol_strtolower($this->email);
        }
        if (isset($this->personal_email)) {
            $this->personal_email = Functions::dol_strtolower($this->personal_email);
        }
    }
}
