...
use Alxarafe\LibClass\ExtraFields;
...
abstract class CommonObject
{
    ...
    /**
     * @var array        Array with all fields and their property. Do not use it as a static var. It may be modified by
     *      constructor.
     */
    public $fields = [];
    ...
    /**
     * @var mixed        Array of linked objects, set and used when calling ->create() to be able to create links
    ...
    /**
     * @var string        The type of originating object ('commande', 'facture', ...). Note: on some object this field
     *      is called $origin_type
     * @see fetch_origin()
     */
    public $origin;
     ...
     *
     * @return    string|-1                    Returns the link, or an empty string if no link was found, or -1 if
     *                                         error.
     */
    ...
    public function fetch_thirdparty($force_thirdparty_id = 0)
    {
        ...
        if ($idtofetch) {
            ...
            if (Functions::getDolGlobalString('PRODUIT_MULTIPRICES') && empty($this->thirdparty->price_level)) {
     ...
     *
     * @return        int            Return integer <0 if KO, 0 if can't guess type of barcode (ISBN, EAN13...), >0 if
     *                               OK (all barcode properties loaded)
     */
    public function fetch_barcode()
    {
        ...
        if (empty($idtype) && $idtype != '0') {    // If type of barcode no set, we try to guess. If set to '0' it means we forced to have type remain not defined
            if ($this->element == 'product' && Functions::getDolGlobalString('PRODUIT_DEFAULT_BARCODE_TYPE')) {
                $idtype = Functions::getDolGlobalString('PRODUIT_DEFAULT_BARCODE_TYPE');
            } elseif ($this->element == 'societe') {
                $idtype = Functions::getDolGlobalString('GENBARCODE_BARCODETYPE_THIRDPARTY');
            } else {
    ...
    public function update_price($exclspec = 0, $roundingadjust = 'none', $nodatabaseupdate = 0, $seller = null)
    {
        ...
        if (!empty($MODULE)) {
            if (Functions::getDolGlobalString($MODULE)) {
                $modsactivated = explode(',', Functions::getDolGlobalString($MODULE));
        ...
        if ($forcedroundingmode == 'auto' && isset($conf->global->MAIN_ROUNDOFTOTAL_NOT_TOTALOFROUND)) {
            $forcedroundingmode = Functions::getDolGlobalString('MAIN_ROUNDOFTOTAL_NOT_TOTALOFROUND');
     ...
     * @param string $table_element   name of association table
     *
     * @return    int                                Return integer <0 if KO, 0 if nothing done, >0 if OK and something
     *                                               done
     */
    ...
    public function getCanvas($id = 0, $ref = '')
    {
        ...
        if (Functions::getDolGlobalString('MAIN_DISABLE_CANVAS')) {
        ...
        if ($action != 'editline' || $selected != $line->id) {
            ...
            if (!empty($line->fk_product) && $line->fk_product > 0) {
                ...
                if (getDolGlobalInt('MAIN_MULTILANGS')) {
                    ...
                    if (Functions::getDolGlobalString('PRODUIT_TEXTS_IN_THIRDPARTY_LANGUAGE') && empty($newlang) && is_object($this->thirdparty)) {
    ...
    protected function commonGenerateDocument($modelspath, $modele, $outputlangs, $hidedetails, $hidedesc, $hideref, $moreparams = null)
    {
        ...
        if ($obj->type == 'odt' && empty($srctemplatepath)) {
            ...
            if ($varfortemplatedir && Functions::getDolGlobalString($varfortemplatedir)) {
                $dirtoscan = Functions::getDolGlobalString($varfortemplatedir);
    ...
    public function getDefaultCreateValueFor($fieldname, $alternatevalue = null, $type = 'alphanohtml')
    {
        ...
        if (Functions::getDolGlobalString($keyforfieldname)) {
            return Functions::getDolGlobalString($keyforfieldname);
        }
     ...
     *
     * @return    int                        Return integer <0 if error, 0 if no values of alternative languages to
     *                                       find nor found, 1 if a value was found and loaded
     ...
     *                        reset.
     *
     * @return    int                        1 if array_options set, 0 if no value, -1 if error (field required missing
     *                                       for example)
     */
     ...
     *
     * @return    int                        Return integer <0 if error, 0 if no values of extrafield to find nor
     *                                       found, 1 if an attribute is found and value loaded
    ...
    public function deleteExtraFields()
    {
        ...
        if (Functions::getDolGlobalString('MAIN_EXTRAFIELDS_DISABLED')) {
            return 0;
        }
        ...
        if (!empty($this->array_options)) {
            ...
            foreach ($new_array_options as $key => $value) {
                ...
                if (!empty($attrfieldcomputed)) {
                    if (Functions::getDolGlobalString('MAIN_STORE_COMPUTED_EXTRAFIELDS')) {
    ...
    public function insertExtraLanguages($trigger = '', $userused = null)
    {
        ...
        if (Functions::getDolGlobalString('MAIN_EXTRALANGUAGES_DISABLED')) {
    ...
    public function showInputField($val, $key, $value, $moreparam = '', $keysuffix = '', $keyprefix = '', $morecss = 0, $nonewbutton = 0)
    {
        ...
        if (in_array($type, ['date'])) {
            ...
            $out = '';
            if (!empty($conf->use_javascript_ajax) && !Functions::getDolGlobalString('MAIN_EXTRAFIELDS_DISABLE_SELECT2')) {
                    ...
                    if ($extrafields->attributes[$this->table_element]['type'][$key] == 'separate') {
                        ...
                        */
                        ...
                        if ($display_type == 'card') {
                            if (Functions::getDolGlobalString('MAIN_EXTRAFIELDS_USE_TWO_COLUMS') && ($e % 2) == 0) {
                                $colspan = 0;
                            }
                        ...
                        if ($display_type == 'card') {
                            ...
                            if (Functions::getDolGlobalString('MAIN_VIEW_LINE_NUMBER') && ($action == 'view' || $action == 'valid' || $action == 'editline' || $action == 'confirm_valid' || $action == 'confirm_cancel')) {
                                $out .= '<td></td>';
                            }
                        ...
                        if (Functions::getDolGlobalString('MAIN_EXTRAFIELDS_USE_TWO_COLUMS') && (($e % 2) == 1)) {
    ...
    public function defineBuyPrice($unitPrice = 0.0, $discountPercent = 0.0, $fk_product = 0)
    {
        ...
        if (($unitPrice > 0) && (isset($conf->global->ForceBuyingPriceIfNull) && getDolGlobalInt('ForceBuyingPriceIfNull') > 0)) {
            ...
            if (!empty($fk_product) && $fk_product > 0) {
                if (Functions::getDolGlobalString('MARGIN_TYPE') == 'costprice') {
                ...
                } elseif (Functions::getDolGlobalString('MARGIN_TYPE') == 'pmp') {
                    require_once DOL_DOCUMENT_ROOT . '/product/class/product.class.php';
                    $product = new Product($this->db);
                    $result = $product->fetch($fk_product);
                    if ($result <= 0) {
     ...
     *
     * @return    int                            Return integer <0 if KO, 0=Nothing done because object has child, >0
     *                                           if OK
     */
