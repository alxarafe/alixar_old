...
$langs->loadLangs(["compta", "banks", "bills", "users", "salaries", "hrm", "trips"]);
if (isModEnabled('project')) {
    $langs->load("projects");
}
...
$hookmanager->initHooks(['salarycard', 'globalcard']);
...
if ($action == 'add' && empty($cancel)) {
    ...
    if (!$error) {
        ...
        if (!empty($auto_create_paiement) && !$error) {
            ...
            $paiement->amounts = [$object->id => $amount]; // Tableau de montant
...
if ($id > 0) {
    ...
    if ($action === 'clone') {
        ...
        $formquestion[] = ['type' => 'date', 'name' => 'clone_date_start', 'label' => $langs->trans("DateStart"), 'value' => -1];
        $formquestion[] = ['type' => 'date', 'name' => 'clone_date_end', 'label' => $langs->trans("DateEnd"), 'value' => -1];
        $formquestion[] = ['type' => 'text', 'name' => 'amount', 'label' => $langs->trans("Amount"), 'value' => price($object->amount), 'morecss' => 'width100 right'];
