...
$langs->loadLangs(["compta", "salaries", "bills", "hrm"]);
...
$hookmanager->initHooks(['salarieslist']); // Note that conf->hooks_modules contains array
...
if ($search_user) {
    $sql .= natural_search(['u.login', 'u.lastname', 'u.firstname', 'u.email'], $search_user);
}
if ($search_label) {
    $sql .= natural_search(['sal.label'], $search_label);
}
...
if ($search_chq_number) {
    $sql .= natural_search(['s.num_payment'], $search_chq_number);
}
...
while ($i < $imaxinloop) {
    ...
    if ($mode == 'kanban') {
        ...
        // Output Kanban
        print $paymentsalstatic->getKanbanView('', ['selected' => in_array($object->id, $arrayofselected)]);
        if ($i == ($imaxinloop - 1)) {
            print '</div>';
            print '</td></tr>';
        }
