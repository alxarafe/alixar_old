...
if (empty($reshook)) {
    ...
    if (empty($backtopage) || ($cancel && empty($id))) {
        if (empty($backtopage) || ($cancel && strpos($backtopage, '__ID__'))) {
            if (empty($id) && (($action != 'add' && $action != 'create') || $cancel)) {
                ...
                $backtopage = '/user/card.php?id=' . ((!empty($id) && $id > 0) ? $id : '__ID__');
            }
        }
    }
    ...
    if (
        (($action == 'confirm_password' && $confirm == 'yes' && $caneditpasswordandsee)
...
if ($action == 'create' || $action == 'adduserldap') {
    ...
    if (
        (isModEnabled('salaries') && $user->hasRight("salaries", "read") && in_array($id, $childids))
    ...
    if ($id > 0) {
        ...
        if ($action != 'edit') {
            ...
            if (
                (empty($user->socid) && in_array($id, $childids))   // A user can always see salary/value information for its subordinates
                || (isModEnabled('salaries') && $user->hasRight("salaries", "readall"))
                || (isModEnabled('hrm') && $user->hasRight("hrm", "employee", "read"))
            ) {
                $langs->load("salaries");
            ...
            */
            ...
            if (empty($reshook)) {
                ...
                if ($caneditfield && (!isModEnabled('multicompany') || !$user->entity || ($object->entity == $conf->entity) || ($conf->global->MULTICOMPANY_TRANSVERSE_MODE && $object->entity == 1))) {
                    ...
                    $caneditpasswordandsee && !$object->ldap_sid &&
                ...
                if (getDolGlobalString('USER_PASSWORD_GENERATED') != 'none') {
                    if ($object->status == $object::STATUS_DISABLED) {
                        ...
                        ($user->id != $id && $caneditpasswordandsee) && $object->login && !$object->ldap_sid &&
                    ...
                    if ($object->status == $object::STATUS_DISABLED) {
                        ...
                        ($user->id != $id && $caneditpasswordandsend) && $object->login && !$object->ldap_sid &&
                        ((!isModEnabled('multicompany') && $object->entity == $user->entity) || !$user->entity || ($object->entity == $conf->entity) || ($conf->global->MULTICOMPANY_TRANSVERSE_MODE && $object->entity == 1))
                    ) {
                        if ($object->email) {
                            unset($params['attr']['title']);
                ...
                if (
                    $user->id != $id && $candisableuser && $object->statut == 0 &&
                    ((!isModEnabled('multicompany') && $object->entity == $user->entity) || !$user->entity || ($object->entity == $conf->entity) || ($conf->global->MULTICOMPANY_TRANSVERSE_MODE && $object->entity == 1))
                ) {
                    unset($params['attr']['title']);
                ...
                // Disable user
                if (
                    $user->id != $id && $candisableuser && $object->statut == 1 &&
                    ((!isModEnabled('multicompany') && $object->entity == $user->entity) || !$user->entity || ($object->entity == $conf->entity) || ($conf->global->MULTICOMPANY_TRANSVERSE_MODE && $object->entity == 1))
                ) {
                    unset($params['attr']['title']);
