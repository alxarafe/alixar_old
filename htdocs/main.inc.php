...
// Sanity check on URL
if (!defined('NOSCANPHPSELFFORINJECTION') && !empty(DOL_PHP_SELF)) {
	$morevaltochecklikepost = array(DOL_PHP_SELF);
...
if (!defined('NOTOKENRENEWAL') && !defined('NOSESSION')) {
	...
	if (!preg_match('/\.(css|js|json)\.php$/', DOL_PHP_SELF)) {
		...
		if (!isset($_SESSION['newtoken']) || getDolGlobalInt('MAIN_SECURITY_CSRF_TOKEN_RENEWAL_ON_EACH_CALL')) {
			...
			dol_syslog("NEW TOKEN generated by : ".DOL_PHP_SELF, LOG_DEBUG);
		}
	}
}
...
if ((!defined('NOCSRFCHECK') && empty($dolibarr_nocsrfcheck) && getDolGlobalInt('MAIN_SECURITY_CSRF_WITH_TOKEN')) || defined('CSRFCHECK_WITH_TOKEN')) {
	...
	if (
		...
		if (!GETPOST('token', 'alpha') || GETPOST('token', 'alpha') == 'notrequired') {
			...
			if (GETPOSTINT('uploadform')) {
				dol_syslog("--- Access to ".(empty($_SERVER["REQUEST_METHOD"]) ? '' : $_SERVER["REQUEST_METHOD"].' ').DOL_PHP_SELF." refused. File size too large or not provided.");
				...
				if (defined('CSRFCHECK_WITH_TOKEN')) {
					dol_syslog("--- Access to ".(empty($_SERVER["REQUEST_METHOD"]) ? '' : $_SERVER["REQUEST_METHOD"].' ').DOL_PHP_SELF." refused by CSRF protection (CSRFCHECK_WITH_TOKEN protection) in main.inc.php. Token not provided.", LOG_WARNING);
					...
					dol_syslog("--- Access to ".(empty($_SERVER["REQUEST_METHOD"]) ? '' : $_SERVER["REQUEST_METHOD"].' ').DOL_PHP_SELF." refused by CSRF protection (POST method or GET with a sensible value for 'action' parameter) in main.inc.php. Token not provided.", LOG_WARNING);
	...
	if (GETPOSTISSET('token') && GETPOST('token') != 'notrequired' && GETPOST('token', 'alpha') != $sessiontokenforthisurl) {
		dol_syslog("--- Access to ".(empty($_SERVER["REQUEST_METHOD"]) ? '' : $_SERVER["REQUEST_METHOD"].' ').DOL_PHP_SELF." refused by CSRF protection (invalid token), so we disable POST and some GET parameters - referrer=".(empty($_SERVER['HTTP_REFERER']) ? '' : $_SERVER['HTTP_REFERER']).", action=".GETPOST('action', 'aZ09').", _GET|POST['token']=".GETPOST('token', 'alpha'), LOG_WARNING);
...
$modulepart = explode("/", DOL_PHP_SELF);
...
if (!defined('NOLOGIN')) {
	...
	if (!isset($_SESSION["dol_login"])) {
		...
		if (!empty($dolibarr_main_demo) && DOL_PHP_SELF == DOL_URL_ROOT.'/index.php') {  // We ask index page
		...
		if (!$login || (in_array('ldap', $authmode) && empty($passwordtotest))) {	// With LDAP we refused empty password because some LDAP are "opened" for anonymous access so connection is a success.
			...
			dol_syslog("--- Access to ".(empty($_SERVER["REQUEST_METHOD"]) ? '' : $_SERVER["REQUEST_METHOD"].' ').DOL_PHP_SELF." - action=".GETPOST('action', 'aZ09')." - actionlogin=".GETPOST('actionlogin', 'aZ09')." - showing the login form and exit", LOG_NOTICE);
	...
	if (!isset($_SESSION["dol_login"])) {
		...
		if (!empty($landingpage)) {    // Example: /index.php
			...
			if (DOL_PHP_SELF != $newpath) {   // not already on landing page (avoid infinite loop)
...
dol_syslog("--- Access to ".(empty($_SERVER["REQUEST_METHOD"]) ? '' : $_SERVER["REQUEST_METHOD"].' ').DOL_PHP_SELF.' - action='.GETPOST('action', 'aZ09').', massaction='.GETPOST('massaction', 'aZ09').(defined('NOTOKENRENEWAL') ? ' NOTOKENRENEWAL='.constant('NOTOKENRENEWAL') : ''), LOG_NOTICE);
//Another call for easy debug
//dol_syslog("Access to ".DOL_PHP_SELF.' '.$_SERVER["HTTP_REFERER"].' GET='.join(',',array_keys($_GET)).'->'.join(',',$_GET).' POST:'.join(',',array_keys($_POST)).'->'.join(',',$_POST));
...
function top_menu($head, $title = '', $target = '', $disablejs = 0, $disablehead = 0, $arrayofjs = array(), $arrayofcss = array(), $morequerystring = '', $helppagename = '')
{
	...
	if ((empty($conf->dol_hide_topmenu) || GETPOSTINT('dol_invisible_topmenu')) && (!defined('NOREQUIREMENU') || !constant('NOREQUIREMENU'))) {
		...
		if (!getDolGlobalString('MAIN_PRINT_DISABLELINK') && !getDolGlobalString('MAIN_OPTIMIZEFORTEXTBROWSER')) {
			...
			$text = '<a href="'.dol_escape_htmltag(DOL_PHP_SELF).'?'.$qs.($qs ? '&' : '').'optioncss=print" target="_blank" rel="noopener noreferrer">';
...
if (!function_exists("llxFooter")) {
	...
	function llxFooter($comment = '', $zone = 'private', $disabledoutputofmessages = 0)
	{
		...
		if ((DOL_PHP_SELF == DOL_URL_ROOT.'/index.php') || $forceping) {
