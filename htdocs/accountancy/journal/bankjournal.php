...
$langs->loadLangs(["companies", "other", "compta", "banks", "bills", "donations", "loan", "accountancy", "trips", "salaries", "hrm", "members"]);
    ...
    while ($i < $num) {
        ...
        if (is_array($links) && count($links) > 0) {
            ...
            foreach ($links as $key => $val) {
                ...
                if (in_array($links[$key]['type'], ['sc', 'payment_sc', 'payment', 'payment_supplier', 'payment_vat', 'payment_expensereport', 'banktransfert', 'payment_donation', 'member', 'payment_loan', 'payment_salary', 'payment_various'])) {
                    // So we excluded 'company' and 'user' here. We want only payment lines

                    // We save tabtype for a future use, to remember what kind of payment it is
                    $tabpay[$obj->rowid]['type'] = $links[$key]['type'];
                    $tabtype[$obj->rowid] = $links[$key]['type'];
                } elseif (in_array($links[$key]['type'], ['company', 'user'])) {
                ...
                foreach ($tabtp[$key] as $k => $mt) {
                    if ($mt) {
                        ...
                        if ($tabtype[$key] == 'payment') {    // If payment is payment of customer invoice, we get ref of invoice
                            ...
                            $bookkeeping->numero_compte = getDolGlobalString('SALARIES_ACCOUNTING_ACCOUNT_PAYMENT');
                            $bookkeeping->label_compte = $accountingaccountpayment->label;
                        } elseif (in_array($tabtype[$key], ['sc', 'payment_sc'])) {   // If payment is payment of social contribution
                            $bookkeeping->subledger_account = '';
                        ...
                        if ($result < 0) {
                            ...
                            if ($lettering && getDolGlobalInt('ACCOUNTING_ENABLE_LETTERING') && getDolGlobalInt('ACCOUNTING_ENABLE_AUTOLETTERING')) {
                                ...
                                $nb_lettering = $lettering_static->bookkeepingLetteringAll([$bookkeeping->id]);
                            }
                        }
                    }
                }
...
if (empty($action) || $action == 'view') {
    ...
    if (
        getDolGlobalString('ACCOUNTING_ACCOUNT_CUSTOMER') == "" || getDolGlobalString('ACCOUNTING_ACCOUNT_CUSTOMER') == '-1'
