...
$langs->loadLangs(["accountancy", "compta"]);
...
$hookmanager->initHooks(['bookkeepinglist']);

$formaccounting = new FormAccounting($db);
$form = new Form($db);

if (!in_array($action, ['delmouv', 'delmouvconfirm']) && !GETPOSTISSET('begin') && !GETPOSTISSET('formfilteraction') && GETPOST('page', 'int') == '' && !GETPOST('noreset', 'int') && $user->hasRight('accounting', 'mouvements', 'export')) {
...
$arrayfields = [
    't.piece_num' => ['label' => $langs->trans("TransactionNumShort"), 'checked' => 1],
    't.code_journal' => ['label' => $langs->trans("Codejournal"), 'checked' => 1],
    't.doc_date' => ['label' => $langs->trans("Docdate"), 'checked' => 1],
    't.doc_ref' => ['label' => $langs->trans("Piece"), 'checked' => 1],
    't.numero_compte' => ['label' => $langs->trans("AccountAccountingShort"), 'checked' => 1],
    't.subledger_account' => ['label' => $langs->trans("SubledgerAccount"), 'checked' => 1],
    't.label_operation' => ['label' => $langs->trans("Label"), 'checked' => 1],
    't.debit' => ['label' => $langs->trans("AccountingDebit"), 'checked' => 1],
    't.credit' => ['label' => $langs->trans("AccountingCredit"), 'checked' => 1],
    't.lettering_code' => ['label' => $langs->trans("LetteringCode"), 'checked' => 1],
    't.date_creation' => ['label' => $langs->trans("DateCreation"), 'checked' => 0],
    't.tms' => ['label' => $langs->trans("DateModification"), 'checked' => 0],
    't.date_export' => ['label' => $langs->trans("DateExport"), 'checked' => 0],
    't.date_validated' => ['label' => $langs->trans("DateValidationAndLock"), 'checked' => 0, 'enabled' => !getDolGlobalString("ACCOUNTANCY_DISABLE_CLOSURE_LINE_BY_LINE")],
    't.import_key' => ['label' => $langs->trans("ImportId"), 'checked' => 0, 'position' => 1100],
];
...
if (GETPOST('nomassaction', 'int') || in_array($massaction, ['preunletteringauto', 'preunletteringmanual', 'predeletebookkeepingwriting'])) {
    $arrayofmassactions = [];
}
