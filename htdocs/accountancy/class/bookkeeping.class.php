...
class BookKeeping extends CommonObject
{
     ...
     */
    public function fetchAll($sortorder = '', $sortfield = '', $limit = 0, $offset = 0, array $filter = [], $filtermode = 'AND', $showAlreadyExportMovements = 1)
    {
        global $conf;
     ...
     * @return    int                    Return integer <0 if KO, >0 if OK
     */
    public function fetchAllBalance($sortorder = '', $sortfield = '', $limit = 0, $offset = 0, array $filter = [], $filtermode = 'AND', $option = 0)
    {
        global $conf;
    ...
    public function getRootAccount($account = null)
    {
        ...
        if ($resql) {
            ...
            $result = ['id' => $obj->rowid, 'account_number' => $obj->account_number, 'label' => $obj->label];
            return $result;
        } else {
            $this->error = "Error " . $this->db->lasterror();
            dol_syslog(__METHOD__ . " " . $this->error, LOG_ERR);
    ...
    public function closeFiscalPeriod($fiscal_period_id, $new_fiscal_period_id, $separate_auxiliary_account = false, $generate_bookkeeping_records = true)
    {
        ...
        if (!$error && !empty($generate_bookkeeping_records)) {
            ...
            if (empty($journal_id)) {
                $langs->loadLangs(['errors', 'accountancy']);
