...
if ($object->id > 0 && (empty($action) || ($action != 'edit' && $action != 'create'))) {
	...
	if ($action == 'delete') {
		$formconfirm = $form->formconfirm(DOL_PHP_SELF.'?id='.$object->id, $langs->trans('DeleteTarget'), $langs->trans('ConfirmDeleteObject'), 'confirm_delete', '', 0, 1);
	}
	...
	if ($action != 'presend' && $action != 'editline') {
		...
		if (empty($reshook)) {
			...
			if ($object->status == $object::STATUS_VALIDATED) {
				print dolGetButtonAction('', $langs->trans('Disable'), 'default', DOL_PHP_SELF.'?id='.$object->id.'&action=confirm_setdraft&confirm=yes&token='.newToken(), '', $permissiontoadd);
			}

			// Enable
			if ($object->status == $object::STATUS_DRAFT) {
				print dolGetButtonAction('', $langs->trans('Enable'), 'default', DOL_PHP_SELF.'?id='.$object->id.'&action=confirm_validate&confirm=yes&token='.newToken(), '', $permissiontoadd);
			}

			// Clone
			print dolGetButtonAction($langs->trans('ToClone'), '', 'default', DOL_PHP_SELF.'?id='.$object->id.'&action=clone&token='.newToken(), '', $permissiontoadd);
