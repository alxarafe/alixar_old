...
$selectedvariant = !empty($_SESSION['addvariant_' . $object->id]) ? $_SESSION['addvariant_' . $object->id] : [];
$selected = "";
// Security check
...
if (($action == 'add' || $action == 'create') && empty($massaction) && !GETPOST('selectvariant', 'alpha') && empty($subaction)) {    // We click on Create all defined combinations
    ...
    if (!$features) {
        ...
        if (getDolGlobalString('PRODUIT_MULTIPRICES')) {
            ...
            $level_price_impact = [1 => $price_impact];
            $level_price_impact_percent = [1 => $price_impact_percent];
        }
    ...
    if ($action == 'add' || ($action == 'edit')) {
        ...
        if (is_array($productCombination2ValuePairs1)) {
            ...
            if (is_array($productCombination2ValuePairs1) && count($productCombination2ValuePairs1)) {
                ?>
                <tr>
                    <td class="titlefieldcreate tdtop">
                        <label for="features"><?php echo $langs->trans('Attributes') ?></label></td>
                    <td class="tdtop">
            ...
            if (!getDolGlobalString('PRODUIT_MULTIPRICES')) {
                ...
                <tr>
                    ...
                    <td>
                        <input type="text" id="price_impact" name="price_impact" value="<?php echo price($price_impact) ?>">
                        <input type="checkbox" id="price_impact_percent" name="price_impact_percent" <?php echo $price_impact_percent ? ' checked' : '' ?>>
                        <label for="price_impact_percent"><?php echo $langs->trans('PercentageVariation') ?></label>
                    </td>
                </tr>
                <?php
            } else {
        ...
        if ($action === 'delete') {
            if ($prodcomb->fetch($valueid) > 0) {
                ...
                print $form->formconfirm(
                    ...
                    [['label' => $langs->trans('DeleteLinkedProduct'), 'type' => 'checkbox', 'name' => 'delete_product', 'value' => false]],
                    0,
                    1
                );
            }
        } elseif ($action === 'copy') {
            print $form->formconfirm('combinations.php?id=' . $id, $langs->trans('ToClone'), $langs->trans('ConfirmCloneProductCombinations'), 'confirm_copycombination', [['type' => 'text', 'label' => $langs->trans('CloneDestinationReference'), 'name' => 'dest_product']], 0, 1);
        }
