...
foreach ($listofreferent as $key => $label) {
    $project_elements[$key] = ['name' => $key, 'type' => 'tns:elementsArray'];
}
$server->wsdl->addComplexType(
    'elements',
    'complexType',
...
$server->register(
    'createProject',
    // Entry values
    ['authentication' => 'tns:authentication', 'project' => 'tns:project'],
    // Exit values
...
$server->register(
    'getProject',
    // Entry values
    ['authentication' => 'tns:authentication', 'id' => 'xsd:string', 'ref' => 'xsd:string'],
    // Exit values
    ['result' => 'tns:result', 'project' => 'tns:project'],
    $ns,
    $ns . '#getProject',
    $styledoc,
    $styleuse,
        ...
        if ($fuser->hasRight('projet', 'lire')) {
            ...
            if ($result > 0) {
                ...
                if (isset($extrafields->attributes[$elementtype]['label']) && is_array($extrafields->attributes[$elementtype]['label']) && count($extrafields->attributes[$elementtype]['label'])) {
                    ...
                    foreach ($extrafields->attributes[$elementtype]['label'] as $key => $label) {
                        $project_result_fields = array_merge($project_result_fields, ['options_' . $key => $project->array_options['options_' . $key]]);
                    }
                }
                ...
                foreach ($listofreferent as $key => $tablename) {
                    ...
                    if (count($element_array) > 0 && is_array($element_array)) {
                        foreach ($element_array as $element) {
                            ...
                            $elements[$key][] = ['id' => $idofelement, 'user' => $idofelementuser];
                        }
                    }
                }
