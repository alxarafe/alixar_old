...
$langs->loadLangs(["main", "install", "other"]);
...
$res = $hookmanager->initHooks(['demo']);
...
if (empty($reshook)) {
    ...
    // Visible
    $alwayscheckedmodules = ['barcode', 'bookmark', 'categorie', 'externalrss', 'fckeditor', 'geoipmaxmind', 'gravatar', 'memcached', 'syslog', 'user', 'webservices']; // Technical module we always want
    $alwaysuncheckedmodules = ['dav', 'dynamicprices', 'incoterm', 'loan', 'multicurrency', 'paybox', 'paypal', 'stripe', 'google', 'printing', 'scanner', 'socialnetworks', 'website']; // Module we don't want by default
    // Not visible
    $alwayshiddencheckedmodules = [
        'accounting', 'api', 'barcode', 'blockedlog', 'bookmark', 'clicktodial', 'comptabilite', 'cron', 'document', 'domain', 'externalrss', 'externalsite', 'fckeditor', 'geoipmaxmind', 'gravatar', 'label', 'ldap',
        'mailmanspip', 'notification', 'oauth', 'syslog', 'user', 'webservices', 'workflow',
        // Extended modules
        'memcached', 'numberwords', 'zipautofillfr',
    ];
    $alwayshiddenuncheckedmodules = [
        'cashdesk', 'collab', 'dav', 'debugbar', 'emailcollector', 'ftp', 'hrm', 'modulebuilder', 'printing', 'webservicesclient', 'zappier',
        // Extended modules
...
function llxHeaderVierge($title, $head = "")
{
    ...
    top_htmlhead($head, $title, 0, 0, [], ['public/demo/demo.css'], 0, 1);
