...
$langs->loadLangs(["main", "other", "dict", "bills", "companies", "paybox", "paypal"]);
    ...
    if (array_key_exists('MEM', $tmptag) && $tmptag['MEM'] > 0) {
                                ...
                                if ($chpi) {
                                    ...
                                    if (empty($stripecu)) {
                                        ...
                                        if (preg_match('/^pi_/', $TRANSACTIONID) && $customer) {
                                            \Stripe\PaymentIntent::update($chpi->id, ['customer' => $customer->id]);
                                        }
                                    } else {
        ...
        if ($result) {
            ...
            if (!empty($FinalPaymentAmt) && $paymentTypeId > 0) {
                ...
                if ($currencyCodeType == $conf->currency) {
                    $paiement->amounts = [$object->id => $FinalPaymentAmt]; // Array with all payments dispatching with invoice id
                } else {
                    $paiement->multicurrency_amounts = [$object->id => $FinalPaymentAmt]; // Array with all payments dispatching
            ...
            if (isModEnabled('facture')) {
                if (!empty($FinalPaymentAmt) && $paymentTypeId > 0) {
                    ...
                    if ($result > 0) {
                        ...
                        $paiement->datepaye = $now;
                        if ($currencyCodeType == $conf->currency) {
                            $paiement->amounts = [$invoice->id => $FinalPaymentAmt]; // Array with all payments dispatching with invoice id
                        } else {
                            $paiement->multicurrency_amounts = [$invoice->id => $FinalPaymentAmt]; // Array with all payments dispatching
        ...
        if ($result) {
            ...
            if (!empty($FinalPaymentAmt) && $paymentTypeId > 0) {
                ...
                if ($currencyCodeType == $conf->currency) {
                    $paiement->amounts = [$object->id => $totalpaid]; // Array with all payments dispatching with donation
                } else {
                ...
                if ($resultvalidate < 0) {
                    ...
                    if (!$error) {
                        ...
                        if ($resultthirdparty < 0) {
                            ...
                            // Load traductions files required by page
                            $outputlangs->loadLangs(["main", "members", "eventorganization"]);
                            // Get email content from template
                            $arraydefaultmessage = null;
                            ...
                            if ($resultboothupdate < 0) {
                                ...
                                if ($resultinvoice < 0) {
                                    ...
                                    if ($resultthirdparty < 0) {
                                        ...
                                        $mailfile = new CMailFile($subjecttosend, $sendto, $from, $texttosend, [], [], [], '', '', 0, $ishtml);
        ...
        if ($result) {
            ...
            if (isModEnabled('facture')) {
                if (!empty($FinalPaymentAmt) && $paymentTypeId > 0) {
                    include_once DOL_DOCUMENT_ROOT . '/compta/facture/class/facture.class.php';
                    $invoice = new Facture($db);
                    $result = $invoice->createFromContract($object, $user, [(int) $contract_lines]);
                    if ($result > 0) {
                        // $object->classifyBilled($user);
                        $invoice->validate($user);
                        // Creation of payment line
...
if ($ispaymentok) {
    ...
    if ($sendemail) {
        ...
        $companylangs->loadLangs(['main', 'members', 'bills', 'paypal', 'paybox']);
        ...
        require_once DOL_DOCUMENT_ROOT . '/core/class/CMailFile.class.php';
        $mailfile = new CMailFile($topic, $sendto, $from, $content, [], [], [], '', '', 0, $ishtml);
