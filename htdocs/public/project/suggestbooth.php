...
$langs->loadLangs(["main", "companies", "install", "other", "eventorganization"]);
...
if (empty($reshook) && $action == 'add') {
    ...
    if (!$error) {
        ...
        if (!$error) {
            ...
            if ($resultcategory <= 0) {
                ...
                $resultsetcategory = $thirdparty->setCategoriesCommon([$category->id], Categorie::TYPE_CUSTOMER, false);
    ...
    if (!$error) {
        ...
        $mailfile = new CMailFile($subjecttosend, $sendto, $from, $texttosend, [], [], [], '', '', 0, $ishtml, '', '', $trackid);
