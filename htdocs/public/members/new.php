...
$langs->loadLangs(["main", "members", "companies", "install", "other", "errors"]);
...
$hookmanager->initHooks(['publicnewmembercard', 'globalcard']);
...
if (empty($reshook) && $action == 'add') {
    ...
    if (!in_array(GETPOST('morphy'), ['mor', 'phy'])) {
        $error++;
...
if (getDolGlobalString('MEMBER_SKIP_TABLE') || getDolGlobalString('MEMBER_NEWFORM_FORCETYPE') || $action == 'create') {
    ...
    print $formcompany->select_ziptown(GETPOST('zipcode'), 'zipcode', ['town', 'selectcountry_id', 'state_id'], 0, 1, '', 'width75');
    print ' / ';
    ...
    if (getDolGlobalString('MEMBER_NEWFORM_DOLIBARRTURNOVER')) {
        $arraybudget = ['50' => '<= 100 000', '100' => '<= 200 000', '200' => '<= 500 000', '300' => '<= 1 500 000', '600' => '<= 3 000 000', '1000' => '<= 5 000 000', '2000' => '5 000 000+'];
    ...
    $result = $measuringUnits->fetchAll('', '', 0, 0, ['t.active' => 1]);
    $units = [];
