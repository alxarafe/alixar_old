...
$langs->loadLangs(["companies", "other", "ticket"]);
...
$hookmanager->initHooks(['ticketpubliclist', 'globalcard']);
...
$arrayofjs = [];
$arrayofcss = ['/ticket/css/styles.css.php'];
...
if ($action == "view_ticketlist") {
    ...
    if ($display_ticket_list) {
        ...
        $arrayfields = [
            't.datec' => ['label' => $langs->trans("Date"), 'checked' => 1],
            't.date_read' => ['label' => $langs->trans("TicketReadOn"), 'checked' => 0],
            't.date_close' => ['label' => $langs->trans("TicketCloseOn"), 'checked' => 0],
            't.ref' => ['label' => $langs->trans("Ref"), 'checked' => 1],
            //'t.track_id' => array('label' => $langs->trans("IDTracking"), 'checked' => 0),
            't.fk_statut' => ['label' => $langs->trans("Status"), 'checked' => 1],
            't.subject' => ['label' => $langs->trans("Subject"), 'checked' => 1],
            'type.code' => ['label' => $langs->trans("Type"), 'checked' => 1],
            'category.code' => ['label' => $langs->trans("Category"), 'checked' => 1],
            'severity.code' => ['label' => $langs->trans("Severity"), 'checked' => 1],
            't.progress' => ['label' => $langs->trans("Progression"), 'checked' => 0],
            //'t.fk_contract' => array('label' => $langs->trans("Contract"), 'checked' => 0),
            't.fk_user_create' => ['label' => $langs->trans("Author"), 'checked' => 1],
            't.fk_user_assign' => ['label' => $langs->trans("AssignedTo"), 'checked' => 0],
        ...
        if (isset($extrafields->attributes[$object->table_element]['label']) && is_array($extrafields->attributes[$object->table_element]['label']) && count($extrafields->attributes[$object->table_element]['label'])) {
            foreach ($extrafields->attributes[$object->table_element]['label'] as $key => $val) {
                if ($extrafields->attributes[$object->table_element]['type'][$key] != 'separate') {
                    ...
                    $arrayfields["ef." . $key] = ['label' => $extrafields->attributes[$object->table_element]['label'][$key], 'checked' => ($extrafields->attributes[$object->table_element]['list'][$key] < 0) ? 0 : 1, 'position' => $extrafields->attributes[$object->table_element]['pos'][$key], 'enabled' => $enabled && $extrafields->attributes[$object->table_element]['perms'][$key]];
                }
            }
        }
        ...
        if (isset($search_fk_status) && $search_fk_status == 'non_closed') {
            $filter['t.fk_statut'] = [0, 1, 3, 4, 5, 6];
