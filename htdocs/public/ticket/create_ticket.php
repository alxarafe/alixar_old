...
$langs->loadLangs(['companies', 'other', 'mails', 'ticket']);
...
$hookmanager->initHooks(['publicnewticketcard', 'globalcard']);
    ...
    if ($action == 'create_ticket' && GETPOST('save', 'alpha')) {
        ...
        if (!$error) {
            ...
            if ($with_contact && !($with_contact->id > 0)) {
                ...
                if ($result < 0) {
                    $error++;
                    $errors = ($company->error ? [$company->error] : $company->errors);
                ...
                if (!$error) {
                    ...
                    if ($result < 0) {
                        $error++;
                        $errors = ($with_contact->error ? [$with_contact->error] : $with_contact->errors);
                        array_push($object->errors, $errors);
                        $action = 'create_ticket';
                    } else {
            ...
            if (!$error) {
                ...
                if ($id <= 0) {
                    $error++;
                    $errors = ($object->error ? [$object->error] : $object->errors);
                    array_push($object->errors, $object->error ? [$object->error] : $object->errors);
                    $action = 'create_ticket';
                }
            }
            ...
            if (!$error && $id > 0) {
                ...
                if (!$error) {
                    ...
                    $messagetoshow = str_replace(['{s1}', '{s2}'], ['<strong>' . $object->track_id . '</strong>', '<strong>' . $object->ref . '</strong>'], $messagetoshow);
...
$arrayofjs = [];
$arrayofcss = ['/opensurvey/css/style.css', '/ticket/css/styles.css.php'];
...
if ($action != "infos_success") {
    ...
    $formticket->param = ['returnurl' => $_SERVER['PHP_SELF'] . ($conf->entity > 1 ? '?entity=' . $conf->entity : '')];
