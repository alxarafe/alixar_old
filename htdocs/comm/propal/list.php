...
$langs->loadLangs(['companies', 'propal', 'compta', 'bills', 'orders', 'products', 'deliveries', 'categories']);
if (isModEnabled("expedition")) {
    $langs->loadLangs(['sendings']);
}
...
$hookmanager->initHooks(['propallist']);
...
$checkedtypetiers = 0;
$arrayfields = [
    'p.ref' => ['label' => "Ref", 'checked' => 1],
    'p.ref_client' => ['label' => "RefCustomer", 'checked' => -1],
    'pr.ref' => ['label' => "ProjectRef", 'checked' => 1, 'enabled' => (isModEnabled('project') ? 1 : 0)],
    'pr.title' => ['label' => "ProjectLabel", 'checked' => 0, 'enabled' => (isModEnabled('project') ? 1 : 0)],
    's.nom' => ['label' => "ThirdParty", 'checked' => 1],
    's.name_alias' => ['label' => "AliasNameShort", 'checked' => -1],
    's.town' => ['label' => "Town", 'checked' => -1],
    's.zip' => ['label' => "Zip", 'checked' => -1],
    'state.nom' => ['label' => "StateShort", 'checked' => 0],
    'country.code_iso' => ['label' => "Country", 'checked' => 0],
    'typent.code' => ['label' => "ThirdPartyType", 'checked' => $checkedtypetiers],
    'p.date' => ['label' => "DatePropal", 'checked' => 1],
    'p.fin_validite' => ['label' => "DateEnd", 'checked' => 1],
    'p.date_livraison' => ['label' => "DeliveryDate", 'checked' => 0],
    'p.date_signature' => ['label' => "DateSigning", 'checked' => 0],
    'ava.rowid' => ['label' => "AvailabilityPeriod", 'checked' => 0],
    'p.fk_shipping_method' => ['label' => "SendingMethod", 'checked' => 0, 'enabled' => isModEnabled("expedition")],
    'p.fk_input_reason' => ['label' => "Origin", 'checked' => 0, 'enabled' => 1],
    'p.fk_cond_reglement' => ['label' => "PaymentConditionsShort", 'checked' => 0],
    'p.fk_mode_reglement' => ['label' => "PaymentMode", 'checked' => 0],
    'p.total_ht' => ['label' => "AmountHT", 'checked' => 1],
    'p.total_tva' => ['label' => "AmountVAT", 'checked' => 0],
    'p.total_ttc' => ['label' => "AmountTTC", 'checked' => 0],
    'p.total_ht_invoiced' => ['label' => "AmountInvoicedHT", 'checked' => 0, 'enabled' => getDolGlobalString('PROPOSAL_SHOW_INVOICED_AMOUNT')],
    'p.total_invoiced' => ['label' => "AmountInvoicedTTC", 'checked' => 0, 'enabled' => getDolGlobalString('PROPOSAL_SHOW_INVOICED_AMOUNT')],
    'p.multicurrency_code' => ['label' => 'Currency', 'checked' => 0, 'enabled' => (!isModEnabled("multicurrency") ? 0 : 1)],
    'p.multicurrency_tx' => ['label' => 'CurrencyRate', 'checked' => 0, 'enabled' => (!isModEnabled("multicurrency") ? 0 : 1)],
    'p.multicurrency_total_ht' => ['label' => 'MulticurrencyAmountHT', 'checked' => 0, 'enabled' => (!isModEnabled("multicurrency") ? 0 : 1)],
    'p.multicurrency_total_tva' => ['label' => 'MulticurrencyAmountVAT', 'checked' => 0, 'enabled' => (!isModEnabled("multicurrency") ? 0 : 1)],
    'p.multicurrency_total_ttc' => ['label' => 'MulticurrencyAmountTTC', 'checked' => 0, 'enabled' => (!isModEnabled("multicurrency") ? 0 : 1)],
    'p.multicurrency_total_ht_invoiced' => ['label' => 'MulticurrencyAmountInvoicedHT', 'checked' => 0, 'enabled' => isModEnabled("multicurrency") && getDolGlobalString('PROPOSAL_SHOW_INVOICED_AMOUNT')],
    'p.multicurrency_total_invoiced' => ['label' => 'MulticurrencyAmountInvoicedTTC', 'checked' => 0, 'enabled' => isModEnabled("multicurrency") && getDolGlobalString('PROPOSAL_SHOW_INVOICED_AMOUNT')],
    'u.login' => ['label' => "Author", 'checked' => 1, 'position' => 10],
    'sale_representative' => ['label' => "SaleRepresentativesOfThirdParty", 'checked' => -1],
    ...
    'p.datec' => ['label' => "DateCreation", 'checked' => 0, 'position' => 500],
    'p.tms' => ['label' => "DateModificationShort", 'checked' => 0, 'position' => 500],
    'p.date_cloture' => ['label' => "DateClosing", 'checked' => 0, 'position' => 500],
    'p.note_public' => ['label' => 'NotePublic', 'checked' => 0, 'position' => 510, 'enabled' => (!getDolGlobalInt('MAIN_LIST_HIDE_PUBLIC_NOTES'))],
    'p.note_private' => ['label' => 'NotePrivate', 'checked' => 0, 'position' => 511, 'enabled' => (!getDolGlobalInt('MAIN_LIST_HIDE_PRIVATE_NOTES'))],
    'p.fk_statut' => ['label' => "Status", 'checked' => 1, 'position' => 1000],
];
...
if (empty($arrayfields['s.name_alias']['checked']) && $search_societe) {
    $sql .= natural_search(["s.nom", "s.name_alias"], $search_societe);
} else {
    if ($search_societe) {
...
$searchCategoryCustomerList = [$search_categ_cus];
...
$searchCategoryProductList = [$search_product_category];
...
if (isModEnabled('categorie') && $user->hasRight('categorie', 'read') && ($user->hasRight('product', 'read') || $user->hasRight('service', 'read'))) {
    ...
    $moreforfilter .= $formcategory->getFilterBox(Categorie::TYPE_PRODUCT, [$search_product_category], 'maxwidth300', $searchCategoryProductOperator, 0, 0, $tmptitle);
}
...
while ($i < $imaxinloop) {
    ...
    if ($mode == 'kanban') {
        ...
        $arrayofparams = ['selected' => in_array($object->id, $arrayofselected), 'authorlink' => $userstatic->getNomUrl(-2), 'projectlink' => $projectstatic->getNomUrl(2)];
