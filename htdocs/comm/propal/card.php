...
$langs->loadLangs(['companies', 'propal', 'compta', 'bills', 'orders', 'products', 'deliveries', 'sendings', 'other']);
...
$hookmanager->initHooks(['propalcard', 'globalcard']);
        ...
        if (!$error && $propal_qty_requirement && (!empty($product_desc) || (!empty($idprod) && $idprod > 0))) {
            ...
            if (!empty($idprod) && $idprod > 0) {
                ...
                if (getDolGlobalString('PRODUIT_MULTIPRICES') && $object->thirdparty->price_level) {
                    ...
                    $filter = ['t.fk_product' => $prod->id, 't.fk_soc' => $object->thirdparty->id];
...
if ($action == 'create') {
    ...
    if (empty($reshook)) {
        ...
        if (isModEnabled('stock') && getDolGlobalString('WAREHOUSE_ASK_WAREHOUSE_DURING_PROPAL')) {
            ...
            print img_picto('', 'stock', 'class="pictofixedwidth"') . $formproduct->selectWarehouses($warehouse_id, 'warehouse_id', '', 1, 0, 0, '', 0, 0, [], 'maxwidth500 widthcentpercentminusxx');
            print '</td></tr>';
        }
    ...
    if ($action == 'clone') {
        ...
        $formquestion = [
            ...
            ['type' => 'other', 'name' => 'socid', 'label' => $langs->trans("SelectThirdParty"), 'value' => $form->select_company(GETPOST('socid', 'int'), 'socid', $filter, '', 0, 0, null, 0, 'maxwidth300')],
            ['type' => 'checkbox', 'name' => 'update_prices', 'label' => $langs->trans('PuttingPricesUpToDate'), 'value' => 0],
            ['type' => 'checkbox', 'name' => 'update_desc', 'label' => $langs->trans('PuttingDescUpToDate'), 'value' => 0],
        ];
        if (getDolGlobalString('PROPAL_CLONE_DATE_DELIVERY') && !empty($object->delivery_date)) {
            $formquestion[] = ['type' => 'date', 'name' => 'date_delivery', 'label' => $langs->trans("DeliveryDate"), 'value' => $object->delivery_date];
        }
    ...
    if ($action == 'closeas') {
        ...
        if (!getDolGlobalString('PROPAL_SKIP_ACCEPT_REFUSE')) {
            $formquestion[] = ['type' => 'select', 'name' => 'statut', 'label' => '<span class="fieldrequired">' . $langs->trans("CloseAs") . '</span>', 'values' => [$object::STATUS_SIGNED => $object->LibStatut($object::STATUS_SIGNED), $object::STATUS_NOTSIGNED => $object->LibStatut($object::STATUS_NOTSIGNED)]];
        }
        $formquestion[] = ['type' => 'text', 'name' => 'note_private', 'label' => $langs->trans("Note"), 'value' => ''];                // Field to complete private note (not replace)
        ...
        if (isModEnabled('notification')) {
            ...
            $formquestion = array_merge($formquestion, [
                ['type' => 'onecolumn', 'value' => $notify->confirmMessage('PROPAL_CLOSE_SIGNED', $object->socid, $object)],
            ]);
        }
    ...
    if ($action != 'presend') {
        ...
        $linktoelem = $form->showLinkToObjectBlock($object, null, ['propal']);

        $compatibleImportElementsList = false;
        if ($user->hasRight('propal', 'creer') && $object->statut == Propal::STATUS_DRAFT) {
            $compatibleImportElementsList = ['commande', 'propal', 'facture']; // import from linked elements
        }
