...
if (getDolGlobalString('AGENDA_REMINDER_BROWSER')) {
    $TRemindTypes['browser'] = ['label' => $langs->trans('BrowserPush'), 'disabled' => (!getDolGlobalString('AGENDA_REMINDER_BROWSER') ? 1 : 0)];
}
if (getDolGlobalString('AGENDA_REMINDER_EMAIL')) {
    $TRemindTypes['email'] = ['label' => $langs->trans('EMail'), 'disabled' => (!getDolGlobalString('AGENDA_REMINDER_EMAIL') ? 1 : 0)];
}

$TDurationTypes = ['y' => $langs->trans('Years'), 'm' => $langs->trans('Month'), 'w' => $langs->trans('Weeks'), 'd' => $langs->trans('Days'), 'h' => $langs->trans('Hours'), 'i' => $langs->trans('Minutes')];
...
if (empty($reshook) && $action == 'add') {
    ...
    $percentage = in_array(GETPOST('status'), [-1, 100]) ? GETPOST('status') : (in_array($complete, [-1, 100]) ? $complete : GETPOST("percentage", 'int')); // If status is -1 or 100, percentage is not defined and we must use status
...
if ($action == 'create') {
    ...
    if (isModEnabled("societe")) {
        ...
        if (GETPOST('socid', 'int') > 0) {
            ...
            $events = [];
            $events[] = ['method' => 'getContacts', 'url' => dol_buildpath('/core/ajax/contacts.php?showempty=1', 1), 'htmlname' => 'contactid', 'params' => ['add-customer-contact' => 'disabled']];
        ...
        print $form->selectcontacts(GETPOSTISSET('socid') ? GETPOSTINT('socid') : $select_contact_default, $preselectedids, 'socpeopleassigned[]', 1, '', '', 0, 'minwidth300 widthcentpercentminusxx maxwidth500', false, 0, [], false, 'multiple', 'contactid');
        print '</td></tr>';
    }
    ...
    if (getDolGlobalString('AGENDA_REMINDER_EMAIL') || getDolGlobalString('AGENDA_REMINDER_BROWSER')) {
        ...
        print $form->selectTypeDuration('offsetunit', 'i', ['y', 'm']);
        print '</td></tr>';
    ...
    if ($action == 'edit') {
		...
		*/
        ...
        if (isModEnabled("societe")) {
            ...
            print img_picto('', 'contact', 'class="paddingrightonly"') . $form->selectcontacts(!getDolGlobalString('MAIN_ACTIONCOM_CAN_ADD_ANY_CONTACT') ? $object->socid : 0, array_keys($object->socpeopleassigned), 'socpeopleassigned[]', 1, '', '', 1, 'minwidth300 widthcentpercentminusx', false, 0, 0, [], 'multiple', 'contactid');
            print '</div>';
            print '</td>';
            print '</tr>';
        }
        ...
        if (getDolGlobalString('AGENDA_REMINDER_EMAIL') || getDolGlobalString('AGENDA_REMINDER_BROWSER')) {
            ...
            print $form->selectTypeDuration('offsetunit', $actionCommReminder->offsetunit, ['y', 'm']);
            print '</td></tr>';
		...
		*/
        ...
        if (!empty($object->fk_element) && !empty($object->elementtype) && !in_array($object->elementtype, ['societe', 'contact', 'project'])) {
            include_once DOL_DOCUMENT_ROOT . '/core/lib/functions2.lib.php';
