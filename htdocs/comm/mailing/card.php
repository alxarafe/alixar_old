...
$hookmanager->initHooks(['mailingcard', 'globalcard']);
            ...
            if ($resql) {
                ...
                if ($num) {
                    ...
                    while ($iforemailloop < $num && $iforemailloop < $conf->global->MAILING_LIMIT_SENDBYWEB) {
                        ...
                        $parameters = ['mode' => 'emailing'];
                        $substitutionarray = getCommonSubstitutionArray($langs, 0, ['object', 'objectamount'], $targetobject); // Note: On mass emailing, this is null because be don't know object
...
if ($action == 'create') {
    ...
    if (getDolGlobalInt('EMAILINGS_SUPPORT_ALSO_SMS')) {
        $arrayoftypes = ["email" => "Email", "sms" => "SMS"];
        ...
        if ($action != 'edit' && $action != 'edittxt' && $action != 'edithtml') {
            ...
            if ($action == 'clone') {
                ...
                $formquestion = [
                    ...
                    ['type' => 'checkbox', 'name' => 'clone_content', 'label' => $langs->trans("CloneContent"), 'value' => 1],
                    ['type' => 'checkbox', 'name' => 'clone_receivers', 'label' => $langs->trans("CloneReceivers"), 'value' => 0],
                ];
            ...
            // Actions Buttons
            if (GETPOST('cancel', 'alpha') || $confirm == 'no' || $action == '' || in_array($action, ['settodraft', 'valid', 'delete', 'sendall', 'clone', 'test', 'editevenunsubscribe'])) {
