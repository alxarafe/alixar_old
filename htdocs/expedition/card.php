...
$langs->loadLangs(["sendings", "companies", "bills", 'deliveries', 'orders', 'stocks', 'other', 'propal', 'productbatch']);
...
$hookmanager->initHooks(['expeditioncard', 'globalcard']);
...
if ($action == 'create') {
    ...
    if ($origin) {
        ...
        if ($object->fetch($origin_id)) {    // This include the fetch_lines
            ...
            // Other attributes
            $parameters = ['objectsrc' => isset($objectsrc) ? $objectsrc : '', 'colspan' => ' colspan="3"', 'cols' => '3', 'socid' => $socid];
            ...
            while ($indiceAsked < $numAsked) {
                ...
                $parameters = ['i' => $indiceAsked, 'line' => $line, 'num' => $numAsked];
                ...
                if (empty($reshook)) {
                    ...
                    if (!empty($extrafields)) {
                        ...
                        print $expLine->showOptionals($extrafields, 'edit', ['style' => 'class="drag drop oddeven"', 'colspan' => $colspan], $indiceAsked, '', 1);
                    }
                }
        ...
        if ($resql) {
            ...
            while ($i < $num) {
                ...
                if ($obj) {
                    ...
                    $alreadysent[$obj->rowid][$obj->shipmentline_id] = [
    ...
    for ($i = 0; $i < $num_prod; $i++) {
        $parameters = ['i' => $i, 'line' => $lines[$i], 'line_id' => $line_id, 'num' => $num_prod, 'alreadysent' => $alreadysent, 'editColspan' => !empty($editColspan) ? $editColspan : 0, 'outputlangs' => $outputlangs];
        $reshook = $hookmanager->executeHooks('printObjectLine', $parameters, $object, $action);
        if ($reshook < 0) {
            setEventMessages($hookmanager->error, $hookmanager->errors, 'errors');
        }
        ...
        if (empty($reshook)) {
            ...
            if (!empty($extrafields)) {
                ...
                if ($action == 'editline' && $line->id == $line_id) {
                    print $lines[$i]->showOptionals($extrafields, 'edit', ['colspan' => $colspan], !empty($indiceAsked) ? $indiceAsked : '', '', 0, 'card');
                } else {
                    print $lines[$i]->showOptionals($extrafields, 'view', ['colspan' => $colspan], !empty($indiceAsked) ? $indiceAsked : '', '', 0, 'card');
                }
            }
        }
    }
    ...
    if ($action != 'presend' && $action != 'editline') {
        ...
        $linktoelem = $form->showLinkToObjectBlock($object, null, ['shipping']);
