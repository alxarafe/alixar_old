...
$hookmanager->initHooks(['shipmentlist']);
...
$checkedtypetiers = 0;
$arrayfields = [
    'e.ref' => ['label' => $langs->trans("Ref"), 'checked' => 1, 'position' => 1],
    'e.ref_customer' => ['label' => $langs->trans("RefCustomer"), 'checked' => 1, 'position' => 2],
    's.nom' => ['label' => $langs->trans("ThirdParty"), 'checked' => 1, 'position' => 3],
    's.town' => ['label' => $langs->trans("Town"), 'checked' => 1, 'position' => 4],
    's.zip' => ['label' => $langs->trans("Zip"), 'checked' => 1, 'position' => 5],
    'state.nom' => ['label' => $langs->trans("StateShort"), 'checked' => 0, 'position' => 6],
    'country.code_iso' => ['label' => $langs->trans("Country"), 'checked' => 0, 'position' => 7],
    'typent.code' => ['label' => $langs->trans("ThirdPartyType"), 'checked' => $checkedtypetiers, 'position' => 8],
    'e.date_delivery' => ['label' => $langs->trans("DateDeliveryPlanned"), 'checked' => 1, 'position' => 9],
    'e.fk_shipping_method' => ['label' => $langs->trans('SendingMethod'), 'checked' => 1, 'position' => 10],
    'e.tracking_number' => ['label' => $langs->trans("TrackingNumber"), 'checked' => 1, 'position' => 11],
    'e.weight' => ['label' => $langs->trans("Weight"), 'checked' => 0, 'position' => 12],
    'e.datec' => ['label' => $langs->trans("DateCreation"), 'checked' => 0, 'position' => 500],
    'e.tms' => ['label' => $langs->trans("DateModificationShort"), 'checked' => 0, 'position' => 500],
    'e.fk_statut' => ['label' => $langs->trans("Status"), 'checked' => 1, 'position' => 1000],
    'l.ref' => ['label' => $langs->trans("DeliveryRef"), 'checked' => 1, 'enabled' => (getDolGlobalInt('MAIN_SUBMODULE_DELIVERY') ? 1 : 0)],
    'l.date_delivery' => ['label' => $langs->trans("DateReceived"), 'checked' => 1, 'enabled' => (getDolGlobalInt('MAIN_SUBMODULE_DELIVERY') ? 1 : 0)],
    'e.billed' => ['label' => $langs->trans("Billed"), 'checked' => 1, 'position' => 1000, 'enabled' => 'getDolGlobalString("WORKFLOW_BILL_ON_SHIPMENT") !== "0"'],
];
...
if (in_array($massaction, ['presend'])) {
    $arrayofmassactions = [];
}
...
if (!empty($arrayfields['e.fk_statut']['checked'])) {
    ...
    print $form->selectarray('search_status', ['0' => $langs->trans('StatusSendingDraftShort'), '1' => $langs->trans('StatusSendingValidatedShort'), '2' => $langs->trans('StatusSendingProcessedShort')], $search_status, 1, 0, 0, '', 0, 0, 0, '', 'search_status width100 onrightofpage');
    print '</td>';
}
// Status billed
...
while ($i < $imaxinloop) {
    ...
    if ($mode == 'kanban') {
        ...
        print $object->getKanbanView('', ['thirdparty' => $companystatic->getNomUrl(1), 'selected' => $selected]);
