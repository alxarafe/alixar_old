...
$langs->loadLangs(['orders', 'sendings', 'companies', 'bills', 'propal', 'deliveries', 'stocks', 'productbatch', 'incoterm', 'other']);

$id = GETPOST('id', 'int'); // id of order
$ref = GETPOST('ref', 'alpha');
$action = GETPOST('action', 'aZ09');

$hookmanager->initHooks(['ordershipmentcard']);
    ...
    if ($object->fetch($id, $ref) > 0) {
        ...
        if ($resql) {
            ...
            while ($i < $num) {
                ...
                $parameters = ['i' => $i, 'line' => $objp, 'num' => $num];
