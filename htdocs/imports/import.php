...
$langs->loadLangs(['exports', 'compta', 'errors', 'admin']);
...
if ($step == 1 || !$datatoimport) {
    ...
    if (count($objimport->array_import_module)) {
        ...
        foreach ($sortedarrayofmodules as $key => $value) {
            ...
            if (in_array($objimport->array_import_code[$key], ['produit_supplierprices', 'produit_multiprice', 'produit_languages'])) {
...
if ($step == 2 && $datatoimport) {
    ...
    // Special cas for import common to module/services
    if (in_array($objimport->array_import_code[0], ['produit_supplierprices', 'produit_multiprice', 'produit_languages'])) {
        $titleofmodule = $langs->trans("ProductOrService");
    }
    print $titleofmodule;
    print '</td></tr>';
...
if ($step == 4 && $datatoimport) {
    ...
    foreach ($fieldstarget as $key => $label) {
        ...
        if (!empty($isrequired)) {
            ...
            $fieldstarget_tmp[$key] = ["label" => $newlabel, "required" => true];
        } else {
            $fieldstarget_tmp[$key] = ["label" => $label, "required" => false];
        }
    ...
    foreach ($fieldstarget as $code => $line) {
        ...
        $optionsall[$code] = ['labelkey' => $line['label'], 'labelkeyarray' => $tmparray, 'label' => $labeltoshow, 'required' => (empty($line["required"]) ? 0 : 1), 'position' => !empty($line['position']) ? $line['position'] : 0];
    ...
    if (count($array_match_file_to_database)) {
        ...
        print '<td>';
        $arrayvisibility = ['private' => $langs->trans("Private"), 'all' => $langs->trans("Everybody")];
    ...
    if ($action != 'launchsimu') {
        ...
        if (!empty($objimport->array_import_run_sql_after[0]) && is_array($objimport->array_import_run_sql_after[0])) {
            ...
            foreach ($objimport->array_import_run_sql_after[0] as $sqlafterimport) {
                ...
                if (!$resqlafterimport) {
                    $arrayoferrors['none'][] = ['lib' => $langs->trans("Error running final request: " . $sqlafterimport)];
                    $error++;
                }
            }
        }
