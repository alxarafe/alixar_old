...
class FormCardWebPortal
{
    ...
    public function init($elementEn, $id = 0, $permissiontoread = 0, $permissiontoadd = 0, $permissiontodelete = 0, $permissionnote = 0, $permissiondellink = 0)
    {
        ...
        $hookmanager->initHooks(['webportal' . $elementEn . 'card', 'globalcard']); // Note that conf->hooks_modules contains array
    ...
    public function doActions()
    {
        ...
        if ($action == 'update' && !empty($permissiontoadd)) {
            foreach ($object->fields as $key => $val) {
                ...
                if (in_array($key, ['rowid', 'entity', 'import_key'])) {
                    continue;
                }
                if (in_array($key, ['date_creation', 'tms', 'fk_user_creat', 'fk_user_modif'])) {
                    if (!in_array(abs($val['visible']), [1, 3, 4])) {
                ...
                } elseif (in_array($object->fields[$key]['type'], ['date', 'datetime'])) {
                ...
                if (getDolGlobalInt('MAIN_FEATURES_LEVEL') >= 2 || getDolGlobalString('MAIN_ACTIVATE_VALIDATION_RESULT')) {
                    if (!$error && !empty($val['validate']) && is_callable([$object, 'validateField'])) {
    ...
    protected function bodyEdit()
    {
        ...
        foreach ($object->fields as $key => $val) {
            ...
            if (in_array($val['type'], ['int', 'integer'])) {
                ...
                $value = GETPOSTISSET($key) ? GETPOST($key, $check) : $object->$key;
            } elseif (in_array($val['type'], ['date', 'datetime'])) {
