...
class PartnershipCardController extends Controller
{
    ...
    public function checkAccess()
    {
        ...
        $this->accessRight = isModEnabled('partnership') && in_array($cardAccess, ['visible', 'edit']) && $context->logged_partnership && $context->logged_partnership->id > 0;
    ...
    public function action()
    {
        ...
        // set form card
        $cardAccess = getDolGlobalString('WEBPORTAL_PARTNERSHIP_CARD_ACCESS');
        $permissiontoread = (int) isModEnabled('partnership') && in_array($cardAccess, ['visible', 'edit']);
        $permissiontoadd = (int) isModEnabled('partnership') && in_array($cardAccess, ['edit']);
