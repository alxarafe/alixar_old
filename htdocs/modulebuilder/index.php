...
$langs->loadLangs(["admin", "modulebuilder", "other", "cron", "errors"]);
...
foreach ($dirsrootforscan as $tmpdirread) {
        ...
        foreach ($dirsincustom as $dircustomcursor) {
            ...
            if (dol_is_file($fullname . '/' . $FILEFLAG)) {
                ...
                if ($modulenamewithcase) {
                    $listofmodules[$dircustomcursor['name']] = [
    ...
    if ($forceddirread && empty($listofmodules)) {    // $forceddirread is 1 if we forced dir to read with dirins=... or with module=...@mydir
        $listofmodules[strtolower($module)] = [
...
// init API, PHPUnit
if ($dirins && in_array($action, ['initapi', 'initphpunit', 'initpagecontact', 'initpagedocument', 'initpagenote', 'initpageagenda']) && !empty($module)) {
    ...
    if ($result > 0) {
        ...
        if ($varnametoupdate) {
            ...
            $arrayreplacement = ['/\$' . preg_quote($varnametoupdate, '/') . ' = 0;/' => '$' . $varnametoupdate . ' = 1;'];
...
if ($dirins && $action == 'initsqlextrafields' && !empty($module)) {
    ...
    $arrayreplacement = ['/\$isextrafieldmanaged = 0;/' => '$isextrafieldmanaged = 1;'];
    dolReplaceInFile($srcfile, $arrayreplacement, '', 0, 0, 1);
}
...
if ($dirins && $action == 'initcss' && !empty($module)) {
    ...
    if ($result > 0) {
        ...
        $arrayreplacement = ['/\/\/\s*\'' . preg_quote('/' . strtolower($module) . '/css/' . strtolower($module) . '.css.php', '/') . '\'/' => '\'/' . strtolower($module) . '/css/' . strtolower($module) . '.css.php\''];
        dolReplaceInFile($srcfile, $arrayreplacement, '', 0, 0, 1);
    } else {
        $langs->load("errors");
        setEventMessages($langs->trans('ErrorFailToCreateFile', $destfile), null, 'errors');
...
if ($dirins && $action == 'initjs' && !empty($module)) {
    ...
    if ($result > 0) {
        ...
        // Update descriptor file to uncomment file
        $srcfile = $dirins . '/' . strtolower($module) . '/core/modules/mod' . $module . '.class.php';
        $arrayreplacement = ['/\/\/\s*\'' . preg_quote('/' . strtolower($module) . '/js/' . strtolower($module) . '.js.php', '/') . '\'/' => '\'/' . strtolower($module) . '/js/' . strtolower($module) . '.js.php\''];
        dolReplaceInFile($srcfile, $arrayreplacement, '', 0, 0, 1);
    } else {
        $langs->load("errors");
        setEventMessages($langs->trans('ErrorFailToCreateFile', $destfile), null, 'errors');
...
if ($dirins && $action == 'initdoc' && !empty($module)) {
    ...
    if ($result > 0) {
        ...
        if (file_exists($dirins . '/' . strtolower($module) . '/ChangeLog.md')) {
            ...
            $stringLog = implode("\n", $strreplace);
            dolReplaceInFile($destfile, ['//include::ChangeLog.md[]' => '', '__CHANGELOG__' => $stringLog]);
        }
    ...
    if ($newlangcode) {
        ...
        if ($diroflang == $dolibarr_main_document_root . '/' . $modulelowercase) {
            ...
            if (!dol_is_dir($srcfile) || !dol_is_file($srcfile)) {
                ...
                $arrayofreplacement = ['mymodule' => $modulelowercase];
            }
...
if ($dirins && $action == 'confirm_removefile' && !empty($module)) {
    ...
    if ($relativefilename) {
        ...
        if (!$result) {
            ...
            if (in_array($tab, ['css', 'js'])) {
                $srcfile = $dirins . '/' . strtolower($module) . '/core/modules/mod' . $module . '.class.php';
                $arrayreplacement = ['/^\s*\'' . preg_quote('/' . $relativefilename, '/') . '\',*/m' => '                // \'/' . $relativefilename . '\','];
                dolReplaceInFile($srcfile, $arrayreplacement, '', 0, 0, 1);
            }
            ...
            if (preg_match('/_extrafields/', $relativefilename)) {
                ...
                $srcfile = $dirins . '/' . strtolower($module) . '/class/' . strtolower($objectname) . '.class.php';
                $arrayreplacement = ['/\$isextrafieldmanaged = 1;/' => '$isextrafieldmanaged = 0;'];
                dolReplaceInFile($srcfile, $arrayreplacement, '', 0, 0, 1);
            }
            ...
            if ($varnametoupdate) {
                $srcfile = $dirins . '/' . strtolower($module) . '/lib/' . strtolower($module) . '_' . strtolower($objectname) . '.lib.php';
                $arrayreplacement = ['/\$' . preg_quote($varnametoupdate, '/') . ' = 1;/' => '$' . preg_quote($varnametoupdate, '/') . ' = 0;'];
                dolReplaceInFile($srcfile, $arrayreplacement, '', 0, 0, 1);
            }
        }
    }
    ...
    if ($tablename) {
        ...
        if (empty($_results)) {
            ...
            while ($obj = $db->fetch_object($_results)) {
                ...
                if (in_array($fieldname, ['ref_ext', 'model_pdf', 'note_public', 'note_private'])) {
                    $visible = 0;
                }
                // enabled
                $enabled = 1;
                ...
                // position
                $position = $i;
                if (in_array($fieldname, ['status', 'statut', 'fk_status', 'fk_statut'])) {
                    $position = 500;
                }
                if ($fieldname == 'import_key') {
                    $position = 900;
                ...
                if (in_array($fieldname, ['note_public', 'note_private'])) {
                    $cssview = 'wordbreak';
                }
                if (in_array($fieldname, ['ref', 'label']) || preg_match('/integer:/', $type)) {
    ...
    if (!$error) {
        ...
        if (!$counter) {
            ...
            if ($checkComment < 0) {
                ...
                $arrayofreplacement = ['/* END MODULEBUILDER LEFTMENU MYOBJECT */' => '/*LEFTMENU ' . strtoupper($objectname) . '*/' . $stringtoadd . "\n\t\t" . '/*END LEFTMENU ' . strtoupper($objectname) . '*/' . "\n\t\t" . '/* END MODULEBUILDER LEFTMENU MYOBJECT */'];
...
if ($dirins && $action == "update_props_module" && !empty(GETPOST('keydescription', 'alpha')) && empty($cancel)) {
    ...
    if (isset($propertyToUpdate) && !empty(GETPOST('propsmodule'))) {
        ...
        if ($propertyToUpdate === 'version') {
            dolReplaceInFile($modulelogfile, ["## " . $moduleobj->$propertyToUpdate => $newValue]);
        }

        $fileLines = file($moduledescriptorfile);
        foreach ($fileLines as &$line) {
            if (strpos($line, $lineToReplace) === 0) {
                dolReplaceInFile($moduledescriptorfile, [$line => $newLine]);
                break;
            }
        }
...
if ($module == 'initmodule') {
    ...
    if (!$error) {
                            ...
                            if (!empty($properties)) {
                                ...
                                if ($tab == 'specifications') {
                                    if ($action != 'editfile' || empty($file)) {
                                        ...
                                        $specs = dol_dir_list(dol_buildpath($modulelowercase . '/doc', 0), 'files', 1, '(\.md|\.asciidoc)$', ['\/temp\/']);
        ...
        if ($tab == 'menus') {
            ...
            $permissions = $moduleobj->rights;
            $crud = ['read' => 'CRUDRead', 'write' => 'CRUDCreateWrite', 'delete' => 'Delete'];
        ...
        if ($tab == 'permissions') {
            ...
            $crud = ['read' => 'CRUDRead', 'write' => 'CRUDCreateWrite', 'delete' => 'Delete'];
            $labels = ["Read objects of " . $module, "Create/Update objects of " . $module, "Delete objects of " . $module];
                ...
                if (count($perms)) {
                    ...
                    foreach ($perms as $perm) {
                        ...
                        if ($action == 'edit_right' && $perm[0] == (int) GETPOST('permskey', 'int')) {
                            ...
                            print '<td>';
                            if (in_array($perm[5], ['lire', 'read', 'creer', 'write', 'effacer', 'delete'])) {
        ...
        if ($tab == 'triggers') {
            ...
            $triggers = $interfaces->getTriggersList(['/' . strtolower($module) . '/core/triggers']);
        ...
        if ($tab == 'widgets') {
            ...
            $widgets = ModeleBoxes::getWidgetsList(['/' . strtolower($module) . '/core/boxes']);
        ...
        if ($tab == 'emailings') {
            ...
            $emailingselectors = MailingTargets::getEmailingSelectorsList(['/' . strtolower($module) . '/core/modules/mailings']);
        ...
        if ($tab == 'cli') {
            ...
            $dircli = ['/' . strtolower($module) . '/scripts'];
