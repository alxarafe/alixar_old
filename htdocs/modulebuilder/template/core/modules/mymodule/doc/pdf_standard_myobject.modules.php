        ...
        if (getMultidirOutput($object)) {
            ...
            if (file_exists($dir)) {
                ...
                for ($i = 0; $i < $nblines; $i++) {
                    ...
                    $this->tva_array[$vatrate . ($vatcode ? ' (' . $vatcode . ')' : '')] = ['vatrate' => $vatrate, 'vatcode' => $vatcode, 'amount' => $this->tva_array[$vatrate . ($vatcode ? ' (' . $vatcode . ')' : '')]['amount'] + $tvaligne];
