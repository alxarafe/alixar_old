...
$langs->loadLangs(["recruitment", "other", "users"]);
...
$hookmanager->initHooks(['recruitmentcandidaturecard', 'globalcard']); // Note that conf->hooks_modules contains array
...
if ($object->id > 0 && (empty($action) || ($action != 'edit' && $action != 'create'))) {
    ...
    if ($action == 'makeofferordecline') {
        ...
        $formquestion = [
            ['type' => 'select', 'name' => 'status', 'label' => '<span class="fieldrequired">' . $langs->trans("CloseAs") . '</span>', 'values' => [$object::STATUS_CONTRACT_PROPOSED => $object->LibStatut($object::STATUS_CONTRACT_PROPOSED), $object::STATUS_REFUSED => $object->LibStatut($object::STATUS_REFUSED)]],
            ['type' => 'text', 'name' => 'note_private', 'label' => $langs->trans("Note"), 'value' => ''],                // Field to complete private note (not replace)
        ];
    ...
    if ($action == 'closeas') {
        ...
        //Form to close proposal (signed or not)
        $formquestion = [
            ['type' => 'select', 'name' => 'status', 'label' => '<span class="fieldrequired">' . $langs->trans("CloseAs") . '</span>', 'values' => [$object::STATUS_CONTRACT_SIGNED => $object->LibStatut($object::STATUS_CONTRACT_SIGNED), $object::STATUS_CONTRACT_REFUSED => $object->LibStatut($object::STATUS_CONTRACT_REFUSED)]],
            ['type' => 'text', 'name' => 'note_private', 'label' => $langs->trans("Note"), 'value' => ''],                // Field to complete private note (not replace)
        ];
    ...
    if ($action != 'presend') {
        ...
        $linktoelem = $form->showLinkToObjectBlock($object, null, ['recruitmentcandidature']);
