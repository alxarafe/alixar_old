...
$langs->loadLangs(["recruitment", "other"]);
...
$hookmanager->initHooks(['recruitmentjobpositioncard', 'globalcard']); // Note that conf->hooks_modules contains array
...
if ($object->id > 0 && (empty($action) || ($action != 'edit' && $action != 'create'))) {
    ...
    if ($action == 'closeas') {
        ...
        $formquestion = [
            ['type' => 'select', 'name' => 'status', 'label' => '<span class="fieldrequired">' . $langs->trans("CloseAs") . '</span>', 'values' => [3 => $object->LibStatut($object::STATUS_RECRUITED), 9 => $object->LibStatut($object::STATUS_CANCELED)]],
    ...
    */
    ...
    if ($action != 'presend') {
        ...
        $linktoelem = $form->showLinkToObjectBlock($object, null, ['recruitmentjobposition']);
