...
$langs->loadLangs(["recruitment", "boxes"]);
...
if ($conf->use_javascript_ajax) {
    ...
    if ($resql) {
        ...
        $listofstatus = [0, 1, 3, 9];
        foreach ($listofstatus as $status) {
            $dataseries[] = [dol_html_entity_decode($staticrecruitmentjobposition->LibStatut($status, 1), ENT_QUOTES | ENT_HTML5), (isset($vals[$status]) ? (int) $vals[$status] : 0)];
    ...
    if ($resql) {
        ...
        $listofstatus = [0, 1, 3, 5, 8, 9];
        foreach ($listofstatus as $status) {
            $dataseries[] = [dol_html_entity_decode($staticrecruitmentcandidature->LibStatut($status, 1), ENT_QUOTES | ENT_HTML5), (isset($vals[$status]) ? (int) $vals[$status] : 0)];
