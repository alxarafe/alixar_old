...
$langs->loadLangs(['resource', 'other', 'interventions']);
...
$hookmanager->initHooks(['element_resource']);
$object->available_resources = ['dolresource'];
...
if (!$ret) {
        ...
        if (is_object($act)) {
            ...
            if (empty($donotclearsession)) {
                if ($act->userownerid > 0) {
                    $listofuserid[$act->userownerid] = ['id' => $act->userownerid, 'transparency' => $act->transparency]; // Owner first
                }
    ...
    $parameters = ['element' => $element, 'element_id' => $element_id, 'element_ref' => $element_ref];
    ...
    foreach ($object->available_resources as $modresources => $resources) {
        ...
        foreach ($resources as $resource_obj) {
            ...
            $dirtpls = array_merge($conf->modules_parts['tpl'], [$defaulttpldir], [$path . $defaulttpldir]);
