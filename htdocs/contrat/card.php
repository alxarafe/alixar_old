...
$langs->loadLangs(["contracts", "orders", "companies", "bills", "products", 'compta']);
...
$hookmanager->initHooks(['contractcard', 'globalcard']);
                ...
                if ($id > 0) {
                    ...
                    if ($result > 0) {
                        ...
                        for ($i = 0; $i < $num; $i++) {
                            ...
                            if ($product_type == 1 || (getDolGlobalString('CONTRACT_SUPPORT_PRODUCTS') && in_array($product_type, [0, 1]))) {    // TODO Exclude also deee
                                // service prédéfini
...
if ($action == 'create') {
    ...
    // Other attributes
    $parameters = ['objectsrc' => $objectsrc, 'colspan' => ' colspan="3"', 'cols' => '3'];
    ...
    if ($object->id > 0) {
        ...
        if ($action == 'delete') {
            ...
            $formquestion = [
                ['type' => 'date', 'name' => 'd_start', 'label' => $langs->trans("DateServiceActivate"), 'value' => dol_now()],
                [
                    'type' => 'date', 'name' => 'd_end', 'label' => $langs->trans("DateEndPlanned"), /*'value' => $form->selectDate('', "end", $usehm, $usehm, '', "active", 1, 0),*/
                    '', '',
                ],
                ['type' => 'text', 'name' => 'comment', 'label' => $langs->trans("Comment"), 'value' => '', '', '', 'class' => 'minwidth300', 'moreattr' => 'autofocus'],
            ];
            ...
            // Clone confirmation
            $formquestion = [['type' => 'other', 'name' => 'socid', 'label' => $langs->trans("SelectThirdParty"), 'value' => $form->select_company(GETPOST('socid', 'int'), 'socid', $filter)]];
        ...
        $usemargins = 0;
        if (isModEnabled('margin') && !empty($object->element) && in_array($object->element, ['facture', 'propal', 'commande'])) {
            $usemargins = 1;
        }
        ...
        while ($cursorline <= $nbofservices) {
            ...
            if ($result) {
                ...
                if ($action != 'editline' || GETPOST('rowid') != $objp->rowid) {
                    ...
                    if (is_array($extralabelslines) && count($extralabelslines) > 0) {
                        ...
                        print $line->showOptionals($extrafields, 'view', ['class' => 'oddeven', 'style' => $moreparam, 'colspan' => $colspan], '', '', 1);
                    }
                } else {
                    // Line in mode update
                    // Ligne carac
                    ...
                    if (is_array($extralabelslines) && count($extralabelslines) > 0) {
                        ...
                        $line->id = $objp->rowid;
                        $line->fetch_optionals();
                        print $line->showOptionals($extrafields, 'edit', ['style' => 'class="oddeven"', 'colspan' => $colspan], '', '', 1);
                    }
                }
            ...
            if ($action == 'move' && !$_REQUEST["cancel"] && $user->hasRight('contrat', 'creer') && $object->lines[$cursorline - 1]->id == GETPOST('rowid')) {
                ...
                $formquestion = [
                    ...
                    ['type' => 'select', 'name' => 'newcid', 'values' => $arraycontractid],
                ];
        ...
        if ($action != 'presend') {
            ...
            $linktoelem = $form->showLinkToObjectBlock($object, null, ['contrat']);
