...
$langs->loadLangs(['contracts', 'products', 'companies', 'compta']);
...
$hookmanager->initHooks(['contractlist']);
...
$arrayfields = [
    'c.ref' => ['label' => $langs->trans("Ref"), 'checked' => 1, 'position' => 10],
    'c.ref_customer' => ['label' => $langs->trans("RefCustomer"), 'checked' => 1, 'position' => 12],
    'c.ref_supplier' => ['label' => $langs->trans("RefSupplier"), 'checked' => 1, 'position' => 14],
    's.nom' => ['label' => $langs->trans("ThirdParty"), 'checked' => 1, 'position' => 30],
    's.email' => ['label' => $langs->trans("ThirdPartyEmail"), 'checked' => 0, 'position' => 30],
    's.town' => ['label' => $langs->trans("Town"), 'checked' => 0, 'position' => 31],
    's.zip' => ['label' => $langs->trans("Zip"), 'checked' => 1, 'position' => 32],
    'state.nom' => ['label' => $langs->trans("StateShort"), 'checked' => 0, 'position' => 33],
    'country.code_iso' => ['label' => $langs->trans("Country"), 'checked' => 0, 'position' => 34],
    'sale_representative' => ['label' => $langs->trans("SaleRepresentativesOfThirdParty"), 'checked' => -1, 'position' => 80],
    'c.date_contrat' => ['label' => $langs->trans("DateContract"), 'checked' => 1, 'position' => 45],
    'c.datec' => ['label' => $langs->trans("DateCreation"), 'checked' => 0, 'position' => 500],
    'c.tms' => ['label' => $langs->trans("DateModificationShort"), 'checked' => 0, 'position' => 500],
    'lower_planned_end_date' => ['label' => $langs->trans("LowerDateEndPlannedShort"), 'checked' => 1, 'position' => 900, 'help' => $langs->trans("LowerDateEndPlannedShort")],
    'status' => ['label' => $langs->trans("Status"), 'checked' => 1, 'position' => 1000],
];
// Extra fields
...
if ($search_contract) {
    $sql .= natural_search(['c.rowid', 'c.ref'], $search_contract);
}
if (!empty($search_ref_customer)) {
    $sql .= natural_search(['c.ref_customer'], $search_ref_customer);
}
if (!empty($search_ref_supplier)) {
    $sql .= natural_search(['c.ref_supplier'], $search_ref_supplier);
}
if ($search_zip) {
...
// Add where from hooks
$parameters = ['search_dfyear' => $search_dfyear, 'search_op2df' => $search_op2df];
...
while ($i < $imaxinloop) {
    ...
    if ($obj->country_id > 0) {
        if (!isset($cacheCountryIDCode[$obj->country_id]['code'])) {
            ...
            $cacheCountryIDCode[$obj->country_id] = ['code' => empty($tmparray['code']) ? '' : $tmparray['code'], 'label' => empty($tmparray['label']) ? '' : $tmparray['label']];
        }
