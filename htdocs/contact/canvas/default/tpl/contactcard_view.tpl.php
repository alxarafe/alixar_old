...
if (empty($user->socid)) {
	...
	if ($user->hasRight('societe', 'contact', 'creer')) {
		print '<a class="butAction" href="'.DOL_PHP_SELF.'?id='.$this->control->tpl['id'].'&action=edit&token='.newToken().'&canvas='.$canvas.'">'.$langs->trans('Modify').'</a>';
	}

	if (!$this->control->tpl['user_id'] && $user->hasRight('user', 'user', 'creer')) {
		print '<a class="butAction" href="'.DOL_PHP_SELF.'?id='.$this->control->tpl['id'].'&action=create_user&token='.newToken().'&canvas='.$canvas.'">'.$langs->trans("CreateDolibarrLogin").'</a>';
	}

	if ($user->hasRight('societe', 'contact', 'supprimer')) {
		print '<a class="butActionDelete" href="'.DOL_PHP_SELF.'?id='.$this->control->tpl['id'].'&action=delete&token='.newToken().'&canvas='.$canvas.'">'.$langs->trans('Delete').'</a>';
	}
