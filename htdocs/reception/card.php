...
$langs->loadLangs(["receptions", "companies", "bills", 'deliveries', 'orders', 'stocks', 'other', 'propal', 'sendings']);
...
$hookmanager->initHooks(['receptioncard', 'globalcard']);
...
if ($action == 'create') {
    ...
    if ($origin) {
        ...
        if ($objectsrc->fetch($origin_id)) {    // This include the fetch_lines
            ...
            // Other attributes
            $parameters = ['objectsrc' => $objectsrc, 'colspan' => ' colspan="3"', 'cols' => '3', 'socid' => $socid];
            ...
            foreach ($_POST as $key => $value) {
                ...
                if (preg_match('/^product_([0-9]+)_([0-9]+)$/i', $key, $reg)) {
                    ...
                    $dispatchLines[$numAsked] = ['paramSuffix' => $paramSuffix, 'prod' => GETPOST($prod, 'int'), 'qty' => price2num(GETPOST($qty), 'MS'), 'ent' => GETPOST($ent, 'int'), 'pu' => price2num(GETPOST($pu), 'MU'), 'comment' => GETPOST('comment'), 'fk_commandefourndet' => GETPOST($fk_commandefourndet, 'int')];
                }
                ...
                if (preg_match('/^product_batch_([0-9]+)_([0-9]+)$/i', $key, $reg)) {
                    ...
                    $fk_commandefourndet = 'fk_commandefourndet_' . $paramSuffix;
                    $dispatchLines[$numAsked] = ['paramSuffix' => $paramSuffix, 'prod' => GETPOST($prod, 'int'), 'qty' => price2num(GETPOST($qty), 'MS'), 'ent' => GETPOST($ent, 'int'), 'pu' => price2num(GETPOST($pu), 'MU'), 'comment' => GETPOST('comment'), 'fk_commandefourndet' => GETPOST($fk_commandefourndet, 'int'), 'DLC' => $dDLC, 'DLUO' => $dDLUO, 'lot' => GETPOST($lot, 'alpha')];
                }
                ...
                if (preg_match('/^productl([0-9]+)$/i', $key, $reg)) {
                    ...
                    $dispatchLines[$numAsked] = ['prod' => GETPOST($prod, 'int'), 'qty' => price2num(GETPOST($qty), 'MS'), 'ent' => GETPOST($ent, 'int'), 'pu' => price2num(GETPOST($pu), 'MU'), 'comment' => GETPOST($comment), 'fk_commandefourndet' => GETPOST($fk_commandefourndet, 'int'), 'DLC' => $dDLC, 'DLUO' => $dDLUO, 'lot' => GETPOST($lot, 'alpha')];
                }
            }
            ...
            while ($indiceAsked <= $numAsked) {    // Loop on $dispatchLines. Warning: $dispatchLines must be sorted by fk_commandefourndet (it is a regroupment key on output)
                ...
                if (!empty($extrafields)) {
                    ...
                    print $recLine->showOptionals($extrafields, 'edit', ['style' => 'class="oddeven"', 'colspan' => $colspan], $indiceAsked, '', 1);
                }
        ...
        if ($resql) {
            ...
            while ($i < $num) {
                ...
                if ($obj) {
                    ...
                    $alreadysent[$obj->rowid][$obj->receptionline_id] = ['reception_ref' => $obj->reception_ref, 'reception_id' => $obj->reception_id, 'warehouse' => $obj->fk_entrepot, 'qty' => $obj->qty, 'date_valid' => $obj->date_valid, 'date_delivery' => $obj->date_delivery];
                }
                $i++;
            }
        }
    ...
    for ($i = 0; $i < $num_prod; $i++) {
        ...
        if (!empty($extralabelslines) && is_array($extralabelslines) && count($extralabelslines) > 0) {
            ...
            if ($action == 'editline' && $lines[$i]->id == $line_id) {
                print $line->showOptionals($extrafields, 'edit', ['colspan' => $colspan], '');
            } else {
                print $line->showOptionals($extrafields, 'view', ['colspan' => $colspan], '');
            }
        }
    }
    print '</tbody>';
