...
$langs->loadLangs(["partnership", "other"]);
...
$hookmanager->initHooks(['partnershipcard', 'globalcard']); // Note that conf->hooks_modules contains array
...
if ($object->id > 0 && (empty($action) || ($action != 'edit' && $action != 'create'))) {
    ...
    if ($action == 'refuse') {
        ...
        $formquestion = [
            ['type' => 'text', 'name' => 'reason_decline_or_cancel', 'label' => $langs->trans("Note"), 'morecss' => 'reason_decline_or_cancel minwidth400', 'value' => ''],                // Field to complete private note (not replace)
        ];
    ...
    */
    ...
    if ($action != 'presend') {
        ...
        $linktoelem = $form->showLinkToObjectBlock($object, null, ['partnership']);
