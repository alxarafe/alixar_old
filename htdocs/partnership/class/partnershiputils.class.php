...
class PartnershipUtils
{
    ...
    public function doCancelStatusOfMemberPartnership()
    {
        ...
        $langs->loadLangs(["partnership", "member"]);
                    ...
                    if ($expirationdate && $expirationdate < $now) {  // If contract expired (we already had a test into main select, this is a security)
                        ...
                        if ($result < 0) {
                            ...
                            if (!empty($newlang)) {
                                ...
                                $outputlangs->loadLangs(['main', 'member', 'partnership']);
                            }
                            ...
                            $from = dol_string_nospecial($conf->global->MAIN_INFO_SOCIETE_NOM, ' ', [","]) . ' <' . getDolGlobalString('MAIN_INFO_SOCIETE_MAIL') . '>';
    ...
    private function checkDolibarrBacklink($website = null)
    {
        ...
        $tmpgeturl = getURLContent($website, 'GET', '', 1, [], ['http', 'https'], 0);
