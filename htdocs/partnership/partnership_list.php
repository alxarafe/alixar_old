...
$langs->loadLangs(["partnership", "members", "other"]);
...
if ($socid > 0) {
    $hookmanager->initHooks(['thirdpartypartnership', 'globalcard']);
} elseif ($memberid > 0) {
    $hookmanager->initHooks(['memberpartnership', 'globalcard']);
} else {
    $hookmanager->initHooks(['partnershiplist']); // Note that conf->hooks_modules contains array
}
...
if (GETPOST('nomassaction', 'int') || in_array($massaction, ['prevalidate', 'presend', 'predelete'])) {
    $arrayofmassactions = [];
}
...
if ($managedfor == 'member') {
    ...
    $selectarray = ['-1' => '', 'withoutsubscription' => $langs->trans("WithoutSubscription"), 'uptodate' => $langs->trans("UpToDate"), 'outofdate' => $langs->trans("OutOfDate")];
...
while ($i < $imaxinloop) {
    ...
    if ($mode == 'kanban') {
        ...
        print $object->getKanbanView('', ['thirdparty' => $object->thirdparty, 'selected' => $selected]);
        if ($i == ($imaxinloop - 1)) {
            print '</div>';
            print '</td></tr>';
        }
