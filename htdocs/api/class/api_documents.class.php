...
class Documents extends DolibarrApi
{
    ...
    public function builddoc($modulepart, $original_file = '', $doctemplate = '', $langcode = '')
    {
        ...
        $file_content = file_get_contents($original_file_osencoded);
        return ['filename' => $filename, 'content-type' => dol_mimetype($filename), 'filesize' => filesize($original_file), 'content' => base64_encode($file_content), 'langcode' => $outputlangs->defaultlang, 'template' => $templateused, 'encoding' => 'base64'];
    }
    ...
    public function getDocumentsListByElement($modulepart, $id = 0, $ref = '', $sortfield = '', $sortorder = '')
    {
        ...
        if (empty($filearray)) {
            ...
            if (($object->id) > 0 && !empty($modulepart)) {
                ...
                $result = $ecmfile->fetchAll('', '', 0, 0, ['t.src_object_type' => $objectType, 't.src_object_id' => $object->id]);
                if ($result < 0) {
    ...
    public function post($filename, $modulepart, $ref = '', $subdir = '', $filecontent = '', $fileencoding = '', $overwriteifexists = 0, $createdirifnotexists = 1)
    {
        ...
        $moreinfo = ['note_private' => 'File uploaded using API /documents from IP ' . getUserRemoteIP()];
