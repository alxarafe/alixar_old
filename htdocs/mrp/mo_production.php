...
$langs->loadLangs(["mrp", "stocks", "other", "product", "productbatch"]);
...
$hookmanager->initHooks(['moproduction', 'globalcard']); // Note that conf->hooks_modules contains array
...
if (empty($reshook)) {
    ...
    if (in_array($action, ['confirm_consumeorproduce', 'confirm_consumeandproduceall']) && $permissiontoproduce) {
...
$morejs = ['/mrp/js/lib_dispatch.js.php'];
...
if ($object->id > 0 && (empty($action) || ($action != 'edit' && $action != 'create'))) {
    ...
    */
    ...
    if (in_array($action, ['consumeorproduce', 'consumeandproduceall', 'addconsumeline', 'addproduceline', 'editline'])) {
        ...
        if (in_array($action, ['consumeorproduce', 'consumeandproduceall'])) {
            ...
            print '<div class="center' . (in_array($action, ['consumeorproduce', 'consumeandproduceall']) ? ' formconsumeproduce' : '') . '">';
    ...
    if (!empty($object->table_element_line)) {
        ...
        // Warehouse
        print '<td>';
        if ($collapse || in_array($action, ['consumeorproduce', 'consumeandproduceall'])) {
        ...
        if ($action == 'addconsumeline') {
            ...
            // Product
            print '<td>';
            print $form->select_produits('', 'productidtoadd', '', 0, 0, -1, 2, '', 1, [], 0, '1', 0, 'maxwidth300');
            print '</td>';
            // Qty
                    ...
                    if ($action == 'editline' && $lineid == $line->id) {
                        ...
                        */
                        ...
                        if (!empty($extrafields)) {
                            ...
                            $temps = $line->showOptionals($extrafields, 'edit', [], '', '', 1, 'line');
                            if (!empty($temps)) {
    ...
    if (in_array($action, ['consumeorproduce', 'consumeandproduceall', 'addconsumeline'])) {
        print "</form>\n";
    } ?>
