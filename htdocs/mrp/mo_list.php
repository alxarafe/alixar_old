...
$newcardbutton .= dolGetButtonTitle($langs->trans('New'), '', 'fa fa-plus-circle', DOL_URL_ROOT.'/mrp/mo_card.php?action=create&backtopage='.urlencode(DOL_PHP_SELF), '', $permissiontoadd);
...
foreach ($object->fields as $key => $val) {
	...
	if (!empty($arrayfields['t.'.$key]['checked'])) {
		if ($key == "fk_product") {
			print getTitleFieldOfList($arrayfields['t.'.$key]['label'], 0, DOL_PHP_SELF, 'p.ref', '', $param, ($cssforfield ? 'class="'.$cssforfield.'"' : ''), $sortfield, $sortorder, ($cssforfield ? $cssforfield.' ' : ''))."\n";
		} else {
