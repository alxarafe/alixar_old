...
$hookmanager->initHooks(['mrpindex']);

// Load translation files required by the page
$langs->loadLangs(["companies", "mrp"]);
...
if ($conf->use_javascript_ajax) {
    ...
    if ($resql) {
        ...
        $listofstatus = [0, 1, 2, 3, 9];
        foreach ($listofstatus as $status) {
            $dataseries[] = [$staticmo->LibStatut($status, 1), (isset($vals[$status]) ? (int) $vals[$status] : 0)];
