...
foreach ($data as $val) {
	...
	while (!empty($year) && $oldyear > $year + 1) {
		...
		print '<td class="center"><a href="'.DOL_PHP_SELF.'?year='.$oldyear.'&amp;mode='.$mode.($socid > 0 ? '&socid='.$socid : '').($userid > 0 ? '&userid='.$userid : '').'">'.$oldyear.'</a></td>';
	...
	print '<tr class="oddeven" height="24">';
	print '<td class="center"><a href="'.DOL_PHP_SELF.'?year='.$year.'&amp;mode='.$mode.($socid > 0 ? '&socid='.$socid : '').($userid > 0 ? '&userid='.$userid : '').'">'.$year.'</a></td>';
