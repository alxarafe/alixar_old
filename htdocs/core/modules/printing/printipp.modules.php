	...
	public function listAvailablePrinters()
	{
		...
		foreach ($list as $value) {
			...
			if ($conf->global->PRINTIPP_URI_DEFAULT == $value) {
				...
				$html .= '<a href="'.DOL_PHP_SELF.'?action=setvalue&token='.newToken().'&mode=test&varname=PRINTIPP_URI_DEFAULT&driver=printipp&value='.urlencode($value).'" alt="'.$langs->trans("Default").'">'.img_picto($langs->trans("Disabled"), 'off').'</a>';
			}
			$html .= '</td>';
			$html .= '</tr>'."\n";
		}
