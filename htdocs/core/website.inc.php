...
if (empty($pageid) && !empty($websitepagefile)) {
    $pageid = str_replace(['.tpl.php', 'page'], ['', ''], basename($websitepagefile));
    if ($pageid == 'index.php') {
...
if (!is_object($weblangs)) {
    $weblangs = new Lang('', $conf);
}
if (!is_object($pagelangs)) {
    $pagelangs = new Lang('', $conf);
}
if ($pageid > 0) {
    ...
    if (!defined('USEDOLIBARREDITOR') && (in_array($websitepage->type_container, ['menu', 'other']) || empty($websitepage->status) && !defined('USEDOLIBARRSERVER'))) {
