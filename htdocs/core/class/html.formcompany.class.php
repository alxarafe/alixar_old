		...
		if (!empty($conf->use_javascript_ajax) && getDolGlobalString('COMPANY_USE_SEARCH_TO_SELECT')) {
			...
			if ($resql) {
				...
				if ($conf->use_javascript_ajax) {
					$javaScript = "window.location='" . dol_escape_js(DOL_PHP_SELF) . "?" . $var_id . "=" . ($forceid > 0 ? $forceid : $object->id) . $moreparam . "&" . $htmlname . "=' + form." . $htmlname . ".options[form." . $htmlname . ".selectedIndex].value;";
