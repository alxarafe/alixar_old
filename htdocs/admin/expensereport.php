					...
					if (file_exists($dir.'/'.$file)) {
						...
						if ($modulequalified) {
							...
							if ($conf->global->EXPENSEREPORT_ADDON_PDF == "$name") {
								...
								print '<a href="'.DOL_PHP_SELF.'?action=setdoc&token='.newToken().'&value='.$name.'&scan_dir='.$module->scandir.'&label='.urlencode($module->name).'" alt="'.$langs->trans("Default").'">'.img_picto($langs->trans("Disabled"), 'off').'</a>';
							}
							print '</td>';
