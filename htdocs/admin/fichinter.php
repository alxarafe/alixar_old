				...
				if (preg_match('/^(mod_.*)\.php$/i', $file, $reg)) {
					...
					if ($module->isEnabled()) {
						...
						if ($conf->global->FICHEINTER_ADDON == $classname) {
							...
							print '<a class="reposition" href="'.DOL_PHP_SELF.'?action=setmod&token='.newToken().'&value='.urlencode($classname).'" alt="'.$langs->trans("Default").'">'.img_picto($langs->trans("Disabled"), 'switch_off').'</a>';
						}
						print '</td>';
