...
if ($conf->use_javascript_ajax) {
	...
	if (!getDolGlobalString('USER_MAIL_REQUIRED')) {
		print '<a href="'.DOL_PHP_SELF.'?action=set_USER_MAIL_REQUIRED&token='.newToken().'">'.img_picto($langs->trans("Disabled"), 'off').'</a>';
	} else {
		print '<a href="'.DOL_PHP_SELF.'?action=del_USER_MAIL_REQUIRED&token='.newToken().'">'.img_picto($langs->trans("Enabled"), 'on').'</a>';
	}
}
print '</td></tr>';
...
if ($conf->use_javascript_ajax) {
	...
	if (!getDolGlobalString('USER_HIDE_INACTIVE_IN_COMBOBOX')) {
		print '<a href="'.DOL_PHP_SELF.'?action=set_USER_HIDE_INACTIVE_IN_COMBOBOX&token='.newToken().'">'.img_picto($langs->trans("Disabled"), 'off').'</a>';
	} else {
		print '<a href="'.DOL_PHP_SELF.'?action=del_USER_HIDE_INACTIVE_IN_COMBOBOX&token='.newToken().'">'.img_picto($langs->trans("Enabled"), 'on').'</a>';
	}
}
print '</td></tr>';
						...
						if (file_exists($dir.'/'.$file)) {
							...
							if ($modulequalified) {
								...
								if (in_array($name, $def)) {
									...
									print '<a href="'.DOL_PHP_SELF.'?action=del_default&token='.newToken().'&value='.urlencode($name).'">';
									print img_picto($langs->trans("Enabled"), 'switch_on');
									print '</a>';
									print '</td>';
								} else {
									print '<td class="center">'."\n";
									print '<a href="'.DOL_PHP_SELF.'?action=set_default&token='.newToken().'&value='.urlencode($name).'&scandir='.urlencode($module->scandir).'&label='.urlencode($module->name).'">';
								...
								if (getDolGlobalString('USER_ADDON_PDF_ODT') == $name) {
									...
									print '<a href="'.DOL_PHP_SELF.'?action=unsetdoc&token='.newToken().'&value='.urlencode($name).'&scandir='.urlencode($module->scandir).'&label='.urlencode($module->name).'" alt="'.$langs->trans("Default").'">'.img_picto($langs->trans("Default"), 'on').'</a>';
								} else {
									print '<a href="'.DOL_PHP_SELF.'?action=setdoc&token='.newToken().'&value='.urlencode($name).'&scandir='.urlencode($module->scandir).'&label='.urlencode($module->name).'" alt="'.$langs->trans("Default").'">'.img_picto($langs->trans("Disabled"), 'off').'</a>';
								}
								print '</td>';
