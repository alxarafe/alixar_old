 ...
 *		\ingroup    core
 *		\brief      Page to administer email sender profiles
 */
...
if (empty($reshook)) {
	...
	$backurlforlist = DOL_PHP_SELF.'?action=list';
	...
	if ($action == 'delete') {
		$sql = "DELETE FROM ".MAIN_DB_PREFIX."c_email_senderprofile WHERE rowid = ".GETPOSTINT('id');
		$resql = $db->query($sql);
		if ($resql) {
...
$newcardbutton = '';
if ($action != 'create') {
	$newcardbutton .= dolGetButtonTitle($langs->trans('New'), '', 'fa fa-plus-circle', DOL_PHP_SELF.'?action=create', '', $permissiontoadd);
	...
	if ($action == 'edit') {
		...
		print $form->selectarray('active', $object->fields['active']['arrayofkeyval'], (GETPOSTISSET('active') ? GETPOSTINT('active') : $object->active), 0, 0, 0, '', 1);
		print '</td></tr>';
		print '</table>';
	...
	*/
	...
	print '<tr><td>'.$langs->trans("Status").'</td><td>';
	print $form->selectarray('active', $object->fields['active']['arrayofkeyval'], GETPOSTINT('active'), 0);
	print '</td></tr>';
	print '</table>';
...
print_barre_liste('', $page, DOL_PHP_SELF, $param, $sortfield, $sortorder, $massactionbutton, $num, $nbtotalofrecords, 'title_companies', 0, $newcardbutton, '', $limit);
//print_barre_liste($title, $page, DOL_PHP_SELF, $param, $sortfield, $sortorder, $massactionbutton, $num, $nbtotalofrecords, 'title_companies', 0, '', '', $limit);
