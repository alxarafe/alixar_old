...
$langsLoad = ['projects', 'bills', 'orders', 'companies'];
...
$parameters = ['socid' => $socid, 'projectid' => $projectid];
                    ...
                    foreach ($data as $fk_product => $timespent_data) {
                        ...
                        if (!empty($fk_product) && $fk_product !== $idprod) {
                            if (!array_key_exists($fk_product, $product_data_cache)) {
                                ...
                                $product_data_cache[$fk_product] = ['duration' => $prodDurationHours, 'dataforprice' => $dataforprice];
                            } else {
                ...
                foreach ($arrayoftasks as $timespent_id => $value) {
                    ...
                    if (!empty($value['fk_product']) && $value['fk_product'] !== $idprod) {
                        if (!array_key_exists($value['fk_product'], $product_data_cache)) {
                            ...
                            $product_data_cache[$value['fk_product']] = ['duration' => $prodDurationHours, 'dataforprice' => $dataforprice];
                        } else {
...
if (($id > 0 || !empty($ref)) || $projectidforalltimes > 0 || $allprojectforuser > 0) {
    ...
    if ($projectstatic->id > 0) {
        ...
        $paramsbutton = ['morecss' => 'reposition'];
    ...
    if (in_array($massaction, ['presend', 'predelete', 'generateinvoice', 'generateinter'])) {
        $arrayofmassactions = [];
    }
    ...
    if ($projectstatic->id > 0 || $allprojectforuser > 0) {
        ...
        $hookmanager->initHooks(['tasktimelist']);
        ...
        $parameters = ['formConfirm' => $formconfirm, "projectstatic" => $projectstatic, "withproject" => $withproject];
        ...
        $arrayfields = [];
        $arrayfields['t.element_date'] = ['label' => $langs->trans("Date"), 'checked' => 1];
        $arrayfields['p.fk_soc'] = ['label' => $langs->trans("ThirdParty"), 'type' => 'integer:Societe:/societe/class/societe.class.php:1', 'checked' => 1];
        $arrayfields['s.name_alias'] = ['label' => $langs->trans("AliasNameShort"), 'type' => 'integer:Societe:/societe/class/societe.class.php:1'];
        if ((empty($id) && empty($ref)) || !empty($projectidforalltimes)) {    // Not a dedicated task
            ...
            $arrayfields['t.element_ref'] = ['label' => $langs->trans("RefTask"), 'checked' => 1];
            $arrayfields['t.element_label'] = ['label' => $langs->trans("LabelTask"), 'checked' => 1];
        }
        $arrayfields['author'] = ['label' => $langs->trans("By"), 'checked' => 1];
        $arrayfields['t.note'] = ['label' => $langs->trans("Note"), 'checked' => 1];
        if (isModEnabled('service') && !empty($projectstatic->thirdparty) && $projectstatic->thirdparty->id > 0 && $projectstatic->usage_bill_time) {
            $arrayfields['t.fk_product'] = ['label' => $langs->trans("Product"), 'checked' => 1];
        }
        $arrayfields['t.element_duration'] = ['label' => $langs->trans("Duration"), 'checked' => 1];
        $arrayfields['value'] = ['label' => $langs->trans("Value"), 'checked' => 1, 'enabled' => isModEnabled("salaries")];
        $arrayfields['valuebilled'] = ['label' => $langs->trans("Billed"), 'checked' => 1, 'enabled' => (((getDolGlobalInt('PROJECT_HIDE_TASKS') || !getDolGlobalInt('PROJECT_BILL_TIME_SPENT')) ? 0 : 1) && $projectstatic->usage_bill_time)];
        // Extra fields
        ...
        if ($action == 'createtime' && $user->hasRight('projet', 'time')) {
            ...
            if (!getDolGlobalString('PROJECT_HIDE_TASKS') && getDolGlobalString('PROJECT_BILL_TIME_SPENT')) {
                ...
                if (isModEnabled("service") && !empty($projectstatic->thirdparty) && $projectstatic->thirdparty->id > 0 && $projectstatic->usage_bill_time) {
                    ...
                    print $form->select_produits((GETPOSTISSET('fk_product') ? GETPOST("fk_product", 'int') : ''), 'fk_product', '1', 0, $projectstatic->thirdparty->price_level, 1, 2, '', 1, [], $projectstatic->thirdparty->id, 'None', 0, 'maxwidth150', 0, '', null, 1);
                    print '</td>';
                }
            }
        ...
        foreach ($tasks as $task_time) {
                ...
                if (!getDolGlobalString('PROJECT_HIDE_TASKS') && getDolGlobalString('PROJECT_BILL_TIME_SPENT')) {
                    if ($projectstatic->usage_bill_time) {
                        if ($task_time->invoice_id) {
                            ...
                            if ($result > 0) {
                                if ($action == 'editline' && $_GET['lineid'] == $task_time->rowid) {
                                    print $formproject->selectInvoiceAndLine($task_time->invoice_id, $task_time->invoice_line_id, 'invoiceid', 'invoicelineid', 'maxwidth500', ['p.rowid' => $projectstatic->id]);
                                } else {
            ...
            // Fields from hook
            $parameters = ['arrayfields' => $arrayfields, 'obj' => $task_time, 'i' => $i, 'totalarray' => &$totalarray];
            $reshook = $hookmanager->executeHooks('printFieldListValue', $parameters); // Note that $action and $object may have been modified by hook
            print $hookmanager->resPrint;
            ...
            if ($action == 'splitline' && GETPOST('lineid', 'int') == $task_time->rowid) {
                ...
                // Fields from hook
                $parameters = ['arrayfields' => $arrayfields, 'obj' => $task_time, 'mode' => 'split1'];
                $reshook = $hookmanager->executeHooks('printFieldListValue', $parameters); // Note that $action and $object may have been modified by hook
                print $hookmanager->resPrint;
                ...
                // Fields from hook
                $parameters = ['arrayfields' => $arrayfields, 'obj' => $task_time, 'mode' => 'split2'];
                $reshook = $hookmanager->executeHooks('printFieldListValue', $parameters); // Note that $action and $object may have been modified by hook
                print $hookmanager->resPrint;
