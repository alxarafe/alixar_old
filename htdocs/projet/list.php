...
$langs->loadLangs(['projects', 'companies', 'commercial']);
...
$hookmanager->initHooks(['projectlist']);
...
$arrayfields['s.nom'] = ['label' => $langs->trans("ThirdParty"), 'checked' => 1, 'position' => 21, 'enabled' => (!isModEnabled('societe') ? 0 : 1)];
$arrayfields['s.name_alias'] = ['label' => "AliasNameShort", 'checked' => 0, 'position' => 22];
$arrayfields['commercial'] = ['label' => $langs->trans("SaleRepresentativesOfThirdParty"), 'checked' => 0, 'position' => 23];
$arrayfields['c.assigned'] = ['label' => $langs->trans("AssignedTo"), 'checked' => 1, 'position' => 120];
$arrayfields['opp_weighted_amount'] = ['label' => $langs->trans('OpportunityWeightedAmountShort'), 'checked' => 0, 'enabled' => (!getDolGlobalString('PROJECT_USE_OPPORTUNITIES') ? 0 : 1), 'position' => 106];
...
if ((is_numeric($search_opp_status) && $search_opp_status >= 0) || in_array($search_opp_status, ['all', 'openedopp', 'notopenedopp', 'none'])) {
...
if (in_array($massaction, ['presend', 'predelete', 'preaffecttag', 'preaffectuser'])) {
    $arrayofmassactions = [];
}
...
if (!empty($arrayfields['p.public']['checked'])) {
    ...
    $array = ['' => '', 0 => $langs->trans("PrivateProject"), 1 => $langs->trans("SharedProject")];
...
while ($i < $imaxinloop) {
    ...
    if ($mode == 'kanban') {
        ...
        $arrayofdata = ['assignedusers' => $stringassignedusers, 'thirdparty' => $companystatic, 'selected' => $selected];
