...
$langsLoad = ['projects', 'users', 'companies'];
...
$hookmanager->initHooks(['projecttaskscard', 'globalcard']);
...
$arrayfields = [
    't.ref' => ['label' => "RefTask", 'checked' => 1, 'position' => 1],
    't.label' => ['label' => "LabelTask", 'checked' => 1, 'position' => 2],
    't.description' => ['label' => "Description", 'checked' => 0, 'position' => 3],
    't.dateo' => ['label' => "DateStart", 'checked' => 1, 'position' => 4],
    't.datee' => ['label' => "Deadline", 'checked' => 1, 'position' => 5],
    't.planned_workload' => ['label' => "PlannedWorkload", 'checked' => 1, 'position' => 6],
    't.duration_effective' => ['label' => "TimeSpent", 'checked' => 1, 'position' => 7],
    't.progress_calculated' => ['label' => "ProgressCalculated", 'checked' => 1, 'position' => 8],
    't.progress' => ['label' => "ProgressDeclared", 'checked' => 1, 'position' => 9],
    't.progress_summary' => ['label' => "TaskProgressSummary", 'checked' => 1, 'position' => 10],
    't.budget_amount' => ['label' => "Budget", 'checked' => 0, 'position' => 11],
    'c.assigned' => ['label' => "TaskRessourceLinks", 'checked' => 1, 'position' => 12],
];
if ($object->usage_bill_time) {
    $arrayfields['t.tobill'] = ['label' => $langs->trans("TimeToBill"), 'checked' => 0, 'position' => 11];
    $arrayfields['t.billed'] = ['label' => $langs->trans("TimeBilled"), 'checked' => 0, 'position' => 12];
}
...
if ($action == 'create' && $user->hasRight('projet', 'creer') && (empty($object->thirdparty->id) || $userWrite > 0)) {
    ...
    $linktotasks = dolGetButtonTitle($langs->trans('ViewList'), '', 'fa fa-bars imgforviewmode', DOL_URL_ROOT . '/projet/tasks.php?id=' . $object->id, '', 1, ['morecss' => 'reposition btnTitleSelected']);
    $linktotasks .= dolGetButtonTitle($langs->trans('ViewGantt'), '', 'fa fa-stream imgforviewmode', DOL_URL_ROOT . '/projet/ganttview.php?id=' . $object->id . '&withproject=1', '', 1, ['morecss' => 'reposition marginleftonly']);
