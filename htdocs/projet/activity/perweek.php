...
$hookmanager->initHooks(['timesheetperweekcard']);
...
$arrayfields['t.planned_workload'] = ['label' => 'PlannedWorkload', 'checked' => 1, 'enabled' => 1, 'position' => 5];
$arrayfields['t.progress'] = ['label' => 'ProgressDeclared', 'checked' => 1, 'enabled' => 1, 'position' => 10];
...
if (!empty($extrafields->attributes['projet_task']['label']) && is_array($extrafields->attributes['projet_task']['label']) && count($extrafields->attributes['projet_task']['label']) > 0) {
    foreach ($extrafields->attributes['projet_task']['label'] as $key => $val) {
        if (!empty($extrafields->attributes['projet_task']['list'][$key])) {
            $arrayfields["efpt." . $key] = ['label' => $extrafields->attributes['projet_task']['label'][$key], 'checked' => (($extrafields->attributes['projet_task']['list'][$key] < 0) ? 0 : 1), 'position' => $extrafields->attributes['projet_task']['pos'][$key], 'enabled' => (abs((int) $extrafields->attributes['projet_task']['list'][$key]) != 3 && $extrafields->attributes['projet_task']['perms'][$key])];
        }
    }
}
...
if ($search_project_ref) {
    $morewherefilter .= natural_search(["p.ref", "p.title"], $search_project_ref);
}
if ($search_task_ref) {
    $morewherefilter .= natural_search("t.ref", $search_task_ref);
}
if ($search_task_label) {
    $morewherefilter .= natural_search(["t.ref", "t.label"], $search_task_label);
}
...
llxHeader("", $title, "", '', '', '', ['/core/js/timesheet.js']);
...
for ($idw = 0; $idw < 7; $idw++) {
    ...
    if ($test) {
        $isavailable[$dayinloopfromfirstdaytoshow] = ['morning' => false, 'afternoon' => false, 'morning_reason' => 'public_holiday', 'afternoon_reason' => 'public_holiday'];
    }
}
