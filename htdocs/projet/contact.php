...
$hookmanager->initHooks(['projectcontactcard', 'globalcard']);
    ...
    if ($action == 'addcontact') {
        ...
        if ($personToAffect > 0 && (!getDolGlobalString('PROJECT_HIDE_TASKS') || $nbTasks > 0)) {
            ...
            if (empty($task_to_affect)) {
                ...
                $formquestion[] = ['type' => 'other', 'name' => 'tasksavailable', 'label' => '', 'value' => '<input type="hidden" id="tasksavailable" name="tasksavailable" value="' . implode(',', array_keys($task_to_affect)) . '">'];
            }
