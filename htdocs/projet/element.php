...
$langs->loadLangs(['projects', 'companies', 'suppliers', 'compta']);
if (isModEnabled('facture')) {
    $langs->load("bills");
}
if (isModEnabled('commande')) {
...
$hookmanager->initHooks(['projectOverview']);
...
$parameters = ['listofreferent' => $listofreferent];
...
$langs->loadLangs(["suppliers", "bills", "orders", "proposals", "margins"]);
...
foreach ($listofreferent as $key => $value) {
    ...
    $exclude_select_element = ['payment_various'];
    ...
    if ($qualified) {
        ...
        $array_of_element_linkable_with_different_thirdparty = ['facture_fourn', 'commande_fournisseur'];
        ...
        if (in_array($tablename, ['projet_task'])) {
        ...
        // Thirdparty or user
        print '<td>';
        if (in_array($tablename, ['projet_task']) && $key == 'project_task') {
            print ''; // if $key == 'project_task', we don't want details per user
        } elseif (in_array($tablename, ['payment_various'])) {
            print ''; // if $key == 'payment_various', we don't have any thirdparty
        } elseif (in_array($tablename, ['expensereport_det', 'don', 'projet_task', 'stock_mouvement', 'salary'])) {
        ...
        if (is_array($elementarray) && count($elementarray) > 0) {
            ...
            for ($i = 0; $i < $num; $i++) {
                ...
                } elseif (in_array($tablename, ['projet_task'])) {
...
function canApplySubtotalOn($tablename)
{
    ...
    return in_array($tablename, ['facture_fourn', 'commande_fournisseur']);
}
