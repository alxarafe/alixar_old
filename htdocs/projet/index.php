...
$hookmanager->initHooks(['projectsindex']);
...
if (empty($reshook)) {
    if ($action == 'refresh_search_project_user') {
        $search_project_user = GETPOST('search_project_user', 'int');
        $tabparam = ["MAIN_SEARCH_PROJECT_USER_PROJECTSINDEX" => $search_project_user];
...
print_projecttasks_array($db, $form, $socid, $projectsListId, 0, 0, $listofoppstatus, ['projectlabel', 'plannedworkload', 'declaredprogress', 'prospectionstatus', 'projectstatus'], $max);
