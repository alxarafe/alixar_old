...
$langs->loadLangs(["other", "holiday", "mails", "trips"]);
...
$hookmanager->initHooks(['holidaycard', 'globalcard']);
    ...
    if ($action == 'confirm_send') {
        ...
        if ($object->statut == Holiday::STATUS_DRAFT && $cancreate) {
            ...
            if ($verif > 0) {
                ...
                $mail = new CMailFile($subject, $emailTo, $emailFrom, $message, [], [], [], '', '', 0, 1, '', '', $trackid);
            ...
            if ($canread) {
                ...
                if (!$edit && $action != 'editvalidator') {
                    ...
                    if (empty($include_users)) {
                        ...
                        $arrayofvalidatorstoexclude = (($user->admin || ($user->id != $userRequest->id)) ? '' : [$user->id]); // Nobody if we are admin or if we are not the user of the leave.
                ...
                if ($action == 'refuse') {
                    $array_input = [['type' => "text", 'label' => $langs->trans('DetailRefusCP'), 'name' => "detail_refuse", 'size' => "50", 'value' => ""]];
