...
$hookmanager->initHooks(['skilltab', 'globalcard']); // Note that conf->hooks_modules contains array
    ...
    if ($action == 'addSkill') {
        ...
        if (!empty($TNote)) {
            foreach ($TNote as $skillId => $rank) {
                $TSkills = $skill->fetchAll('ASC', 't.rowid', 0, 0, ['customsql' => 'fk_object=' . ((int) $id) . " AND objecttype='" . $db->escape($objecttype) . "' AND fk_skill = " . ((int) $skillId)]);
