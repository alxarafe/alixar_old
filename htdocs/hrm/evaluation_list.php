...
$hookmanager->initHooks(['evaluationlist']); // Note that conf->hooks_modules contains array
...
if ($search_all) {
    ...
    print '<!-- Search done like if HRM_EVALUATION_QUICKSEARCH_ON_FIELDS = ' . $setupstring . ' -->' . "\n";
...
while ($i < $imaxinloop) {
    ...
    if ($mode == 'kanban') {
        ...
        print $object->getKanbanView('', ['user' => $userstatic->getNomUrl(-1), 'job' => $job->getNomUrl(1), 'selected' => $selected]);
        if ($i == ($imaxinloop - 1)) {
            print '</div>';
            print '</td></tr>';
        }
