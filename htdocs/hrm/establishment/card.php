...
if ($action == 'create') {
    ...
    print '<td>';
    print $formcompany->select_ziptown(GETPOSTISSET('town') ? GETPOST('town', 'alpha') : $object->town, 'town', [
        'zipcode',
        'selectcountry_id',
    ]);
    print '</td>';
...
if ((!empty($id) || !empty($ref)) && $action == 'edit') {
    ...
    if ($result > 0) {
        ...
        if ($action == 'edit') {
            ...
            print $formcompany->select_ziptown($object->zip, 'zipcode', [
                    'town',
                    'selectcountry_id',
                ], 6) . '</tr>';
            print '<tr><td>' . $form->editfieldkey('Town', 'town', '', $object, 0) . '</td><td>';
            print $formcompany->select_ziptown($object->town, 'town', [
                    'zipcode',
                    'selectcountry_id',
                ]) . '</td></tr>';
