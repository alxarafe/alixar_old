...
$hookmanager->initHooks(['takeposfrontend']);
...
$langs->loadLangs(["bills", "orders", "commercial", "cashdesk", "receiptprinter", "banks"]);
...
$disablejs = 0;
$disablehead = 0;
$arrayofjs = ['/takepos/js/jquery.colorbox-min.js']; // TODO It seems we don't need this
$arrayofcss = ['/takepos/css/pos.css.php', '/takepos/css/colorbox.css'];
    ...
    <script>
        ...
        */
        ...
        function FreeZone() {
            ...
            $.colorbox({
                href: "freezone.php?action=freezone&token=<?php echo newToken(); ?>&place=" + place,
                width: "80%",
                height: "40%",
                transition: "none",
                iframe: "true",
    ...
    <div class="container">
        ...
        <div class="row1<?php if (!getDolGlobalString('TAKEPOS_HIDE_HEAD_BAR')) {
            ...
            <div class="div2">
                ...
                <button type="button" class="calcbutton2 poscolordelete" id="delete" onclick="deleteline()">
            ...
            if (!getDolGlobalString('TAKEPOS_BAR_RESTAURANT')) {
                $menus[$r++] = ['title' => '<span class="fa fa-layer-group paddingrightonly"></span><div class="trunc">' . $langs->trans("New") . '</div>', 'action' => 'New();'];
            } else {
                // BAR RESTAURANT specific menu
                $menus[$r++] = ['title' => '<span class="fa fa-layer-group paddingrightonly"></span><div class="trunc">' . $langs->trans("Place") . '</div>', 'action' => 'Floors();'];
            }

            if (getDolGlobalString('TAKEPOS_HIDE_HEAD_BAR')) {
                if (getDolGlobalString('TAKEPOS_CHOOSE_CONTACT')) {
                    $menus[$r++] = ['title' => '<span class="far fa-building paddingrightonly"></span><div class="trunc">' . $langs->trans("Contact") . '</div>', 'action' => 'Contact();'];
                } else {
                    $menus[$r++] = ['title' => '<span class="far fa-building paddingrightonly"></span><div class="trunc">' . $langs->trans("Customer") . '</div>', 'action' => 'Customer();'];
                }
            }
            if (!getDolGlobalString('TAKEPOS_HIDE_HISTORY')) {
                $menus[$r++] = ['title' => '<span class="fa fa-history paddingrightonly"></span><div class="trunc">' . $langs->trans("History") . '</div>', 'action' => 'History();'];
            }
            $menus[$r++] = ['title' => '<span class="fa fa-cube paddingrightonly"></span><div class="trunc">' . $langs->trans("FreeZone") . '</div>', 'action' => 'FreeZone();'];
            $menus[$r++] = ['title' => '<span class="fa fa-percent paddingrightonly"></span><div class="trunc">' . $langs->trans("InvoiceDiscountShort") . '</div>', 'action' => 'Reduction();'];

            $menus[$r++] = ['title' => '<span class="fas fa-cut paddingrightonly"></span><div class="trunc">' . $langs->trans("SplitSale") . '</div>', 'action' => 'Split();'];

            // BAR RESTAURANT specific menu
            if (getDolGlobalString('TAKEPOS_BAR_RESTAURANT')) {
                if (getDolGlobalString('TAKEPOS_ORDER_PRINTERS')) {
                    $menus[$r++] = ['title' => '<span class="fa fa-blender-phone paddingrightonly"></span><div class="trunc">' . $langs->trans("Order") . '</span>', 'action' => 'TakeposPrintingOrder();'];
                }
            }

            // Last action that close the sell (payments)
            $menus[$r++] = ['title' => '<span class="far fa-money-bill-alt paddingrightonly"></span><div class="trunc">' . $langs->trans("Payment") . '</div>', 'action' => 'CloseBill();'];
            if (getDolGlobalString('TAKEPOS_DIRECT_PAYMENT')) {
                $menus[$r++] = ['title' => '<span class="far fa-money-bill-alt paddingrightonly"></span><div class="trunc">' . $langs->trans("DirectPayment") . ' <span class="opacitymedium">(' . $langs->trans("Cash") . ')</span></div>', 'action' => 'DirectPayment();'];
            }
            ...
            if (getDolGlobalString('TAKEPOS_BAR_RESTAURANT')) {
                ...
                if (getDolGlobalString('TAKEPOS_BAR_RESTAURANT')) {
                    if (getDolGlobalString('TAKEPOS_PRINT_METHOD') == "takeposconnector") {
                        if (getDolGlobalString('TAKEPOS_PRINT_SERVER') && filter_var($conf->global->TAKEPOS_PRINT_SERVER, FILTER_VALIDATE_URL) == true) {
                            $menus[$r++] = ['title' => '<span class="fa fa-receipt paddingrightonly"></span><div class="trunc">' . $langs->trans("Receipt") . '</div>', 'action' => 'TakeposConnector(placeid);'];
                        } else {
                            $menus[$r++] = ['title' => '<span class="fa fa-receipt paddingrightonly"></span><div class="trunc">' . $langs->trans("Receipt") . '</div>', 'action' => 'TakeposPrinting(placeid);'];
                        }
                        ...
                        $menus[$r++] = ['title' => '<span class="fa fa-receipt paddingrightonly"></span><div class="trunc">' . $langs->trans("Receipt") . '</div>', 'action' => 'DolibarrTakeposPrinting(placeid);'];
                    } else {
                        $menus[$r++] = ['title' => '<span class="fa fa-receipt paddingrightonly"></span><div class="trunc">' . $langs->trans("Receipt") . '</div>', 'action' => 'Print(placeid);'];
                    }
                }
                if (getDolGlobalString('TAKEPOS_PRINT_METHOD') == "takeposconnector" && getDolGlobalString('TAKEPOS_ORDER_NOTES') == 1) {
                    $menus[$r++] = ['title' => '<span class="fa fa-sticky-note paddingrightonly"></span><div class="trunc">' . $langs->trans("OrderNotes") . '</div>', 'action' => 'TakeposOrderNotes();'];
                }
                if (getDolGlobalString('TAKEPOS_SUPPLEMENTS')) {
                    $menus[$r++] = ['title' => '<span class="fa fa-receipt paddingrightonly"></span><div class="trunc">' . $langs->trans("ProductSupplements") . '</div>', 'action' => 'LoadProducts(\'supplements\');'];
                }
            }

            if (getDolGlobalString('TAKEPOS_PRINT_METHOD') == "takeposconnector") {
                $menus[$r++] = ['title' => '<span class="fa fa-receipt paddingrightonly"></span><div class="trunc">' . $langs->trans("DOL_OPEN_DRAWER") . '</div>', 'action' => 'OpenDrawer();'];
            }
            ...
            if ($resql) {
                ...
                if ($num) {
                    $obj = $db->fetch_object($resql);
                    $menus[$r++] = ['title' => '<span class="fas fa-file-invoice-dollar paddingrightonly"></span><div class="trunc">' . $langs->trans("CashReport") . '</div>', 'action' => 'CashReport(' . $obj->rowid . ');'];
                    if ($obj->status == 0) {
                        $menus[$r++] = ['title' => '<span class="fas fa-cash-register paddingrightonly"></span><div class="trunc">' . $langs->trans("CloseCashFence") . '</div>', 'action' => 'CloseCashFence(' . $obj->rowid . ');'];
                    }
                }
            }
            ...
            if ($r % 3 == 2) {
                $menus[$r++] = ['title' => '', 'style' => 'visibility: hidden;'];
            }

            if (getDolGlobalString('TAKEPOS_HIDE_HEAD_BAR')) {
                $menus[$r++] = ['title' => '<span class="fa fa-sign-out-alt paddingrightonly"></span><div class="trunc">' . $langs->trans("Logout") . '</div>', 'action' => 'window.location.href=\'' . DOL_URL_ROOT . '/user/logout.php?token=' . newToken() . '\';'];
            }

            if (getDolGlobalString('TAKEPOS_WEIGHING_SCALE')) {
                $menus[$r++] = ['title' => '<span class="fa fa-balance-scale paddingrightonly"></span><div class="trunc">' . $langs->trans("WeighingScale") . '</div>', 'action' => 'WeighingScale();'];
            }
