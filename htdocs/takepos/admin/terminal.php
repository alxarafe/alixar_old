...
$langs->loadLangs(["admin", "cashdesk", "printing", "receiptprinter"]);
...
if (GETPOST('action', 'alpha') == 'set') {
    ...
    foreach ($paiements as $modep) {
        if (in_array($modep->code, ['LIQ', 'CB', 'CHQ'])) {
            continue;
        }
...
print '<td>';
print ajax_constantonoff("TAKEPOS_FORBID_SALES_TO_DEFAULT_CUSTOMER", [], $conf->entity, 0, 0, 1, 0);
print '</td></tr>';
...
if (isModEnabled("banque")) {
    ...
    if (isModEnabled('stripe') && getDolGlobalString('STRIPE_CARD_PRESENT')) {
        ...
        if ($stripeacc) {
            $readers = \Stripe\Terminal\Reader::all('', ["location" => getDolGlobalString('STRIPE_LOCATION'), "stripe_account" => $stripeacc]);
        } else {
            $readers = \Stripe\Terminal\Reader::all('', ["location" => getDolGlobalString('STRIPE_LOCATION')]);
        }
