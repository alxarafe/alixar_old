...
$hookmanager->initHooks(['takeposinvoice']);

$langs->loadLangs(["companies", "commercial", "bills", "cashdesk", "stocks", "banks"]);
...
if (empty($reshook)) {
    ...
    if ($action == "addline" && ($user->hasRight('takepos', 'run') || defined('INCLUDE_PHONEPAGE_FROM_PUBLIC_PAGE'))) {
        ...
        if ($idoflineadded <= 0 && empty($err)) {
            ...
            $line = ['description' => $prod->description, 'price' => $price, 'tva_tx' => $tva_tx, 'localtax1_tx' => $localtax1_tx, 'localtax2_tx' => $localtax2_tx, 'remise_percent' => $customer->remise_percent, 'price_ttc' => $price_ttc, 'array_options' => $array_options];
            ...
            $parameters = ['prod' => $prod, 'line' => $line];
...
if ((getDolGlobalString('TAKEPOS_PHONE_BASIC_LAYOUT') == 1 && $conf->browser->layout == 'phone') || defined('INCLUDE_PHONEPAGE_FROM_PUBLIC_PAGE')) {
    ...
    $arrayofjs = ['/takepos/js/jquery.colorbox-min.js'];
    $disablejs = 0;
    $disablehead = 0;
...
?>
    ...
    <script type="text/javascript">
        ...
        function SendTicket(id) {
            ...
            $.colorbox({
                href: "send.php?facid=" + id,
                width: "70%",
                height: "30%",
                transition: "none",
                iframe: "true",
                title: '<?php echo dol_escape_js($langs->trans("SendTicket")); ?>'
            });
            return true;
        }
        ...
        function PrintBox(id, action) {
            ...
            $.colorbox({
                href: "printbox.php?facid=" + id + "&action=" + action + "&token=<?php echo newToken(); ?>",
                width: "80%",
                height: "200px",
                transition: "none",
                iframe: "true",
                title: "<?php echo $langs->trans("PrintWithoutDetails"); ?>"
            });
            return true;
        }
        ...
        function Print(id, gift) {
            ...
            $.colorbox({
                href: "receipt.php?facid=" + id + "&gift=" + gift,
                width: "40%",
                height: "90%",
                transition: "none",
                iframe: "true",
