...
$hookmanager->initHooks(['takeposproductsearch']); // new context for product search hooks
...
if ($action == 'getProducts') {
    ...
    if ($result > 0) {
        $filter = [];
        if ($tosell != '') {
            $filter = ['customsql' => 'o.tosell = ' . ((int) $tosell)];
        }
    ...
    if (isModEnabled('barcode') && !empty($barcode_rules)) {
        ...
        foreach ($barcode_rules_arr as $barcode_rules_values) {
            ...
            if (count($barcode_rules_values_arr) == 2) {
                ...
                $barcode_rules_list[] = ['code' => $barcode_rules_values_arr[0], 'char_nb' => $char_nb];
    ...
    $sql .= natural_search(['ref', 'label', 'barcode'], $term);
