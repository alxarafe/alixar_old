...
$langs->loadLangs(['companies', 'propal', 'supplier_proposal', 'compta', 'bills', 'orders', 'products']);
...
$hookmanager->initHooks(['supplier_proposallist']);
...
$checkedtypetiers = 0;
$arrayfields = [
    'sp.ref' => ['label' => $langs->trans("Ref"), 'checked' => 1],
    's.nom' => ['label' => $langs->trans("Supplier"), 'checked' => 1],
    's.name_alias' => ['label' => "AliasNameShort", 'checked' => 0],
    's.town' => ['label' => $langs->trans("Town"), 'checked' => 1],
    's.zip' => ['label' => $langs->trans("Zip"), 'checked' => 1],
    'state.nom' => ['label' => $langs->trans("StateShort"), 'checked' => 0],
    'country.code_iso' => ['label' => $langs->trans("Country"), 'checked' => 0],
    'typent.code' => ['label' => $langs->trans("ThirdPartyType"), 'checked' => $checkedtypetiers],
    'sp.date_valid' => ['label' => $langs->trans("DateValidation"), 'checked' => 1],
    'sp.date_livraison' => ['label' => $langs->trans("DateEnd"), 'checked' => 1],
    'sp.total_ht' => ['label' => $langs->trans("AmountHT"), 'checked' => 1],
    'sp.total_tva' => ['label' => $langs->trans("AmountVAT"), 'checked' => 0],
    'sp.total_ttc' => ['label' => $langs->trans("AmountTTC"), 'checked' => 0],
    'sp.multicurrency_code' => ['label' => 'Currency', 'checked' => 0, 'enabled' => (!isModEnabled("multicurrency") ? 0 : 1)],
    'sp.multicurrency_tx' => ['label' => 'CurrencyRate', 'checked' => 0, 'enabled' => (!isModEnabled("multicurrency") ? 0 : 1)],
    'sp.multicurrency_total_ht' => ['label' => 'MulticurrencyAmountHT', 'checked' => 0, 'enabled' => (!isModEnabled("multicurrency") ? 0 : 1)],
    'sp.multicurrency_total_vat' => ['label' => 'MulticurrencyAmountVAT', 'checked' => 0, 'enabled' => (!isModEnabled("multicurrency") ? 0 : 1)],
    'sp.multicurrency_total_ttc' => ['label' => 'MulticurrencyAmountTTC', 'checked' => 0, 'enabled' => (!isModEnabled("multicurrency") ? 0 : 1)],
    'u.login' => ['label' => $langs->trans("Author"), 'checked' => 1, 'position' => 10],
    'sp.datec' => ['label' => $langs->trans("DateCreation"), 'checked' => 0, 'position' => 500],
    'sp.tms' => ['label' => $langs->trans("DateModificationShort"), 'checked' => 0, 'position' => 500],
    'sp.fk_statut' => ['label' => $langs->trans("Status"), 'checked' => 1, 'position' => 1000],
];
// Extra fields
...
if ($search_login) {
    $sql .= natural_search(['u.lastname', 'u.firstname', 'u.login'], $search_login);
}
...
if ($resql) {
    ...
    while ($i < $imaxinloop) {
        ...
        if ($mode == 'kanban') {
            ...
            print $objectstatic->getKanbanView('', ['thirdparty' => $companystatic, 'userauthor' => $userstatic, 'selected' => in_array($obj->id, $arrayofselected)]);
            if ($i == ($imaxinloop - 1)) {
                print '</div>';
                print '</td></tr>';
            }
