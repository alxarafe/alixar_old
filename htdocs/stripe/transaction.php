...
$langs->loadLangs(['compta', 'salaries', 'bills', 'hrm', 'stripe']);
...
if (!$rowid) {
    ...
    try {
        if ($stripeacc) {
            $txn = \Stripe\BalanceTransaction::all(["limit" => $limit], ["stripe_account" => $stripeacc]);
        } else {
            $txn = \Stripe\BalanceTransaction::all(["limit" => $limit]);
        }
