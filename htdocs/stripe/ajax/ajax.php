...
require_once DOL_DOCUMENT_ROOT . '/vendor/stripe/stripe-php/init.php';
...
if ($action == 'getConnexionToken') {
    try {
        ...
        if (empty($stripeacc)) {                // If the Stripe connect account not set, we use common API usage
            ...
            $connectionToken = \Stripe\Terminal\ConnectionToken::create($array, ["stripe_account" => $stripeacc]);
        }
    ...
    try {
        ...
        if (empty($stripeacc)) {                // If the Stripe connect account not set, we use common API usage
            ...
            $intent = \Stripe\PaymentIntent::retrieve($json_obj->id, ["stripe_account" => $stripeacc]);
        }
