...
if ($result < 0) {
    ...
    if ($mode == 'standard') {
        ...
        foreach ($datas as $i => $val) {
            $graph_datas[$i] = [isset($labels[$i]) ? $labels[$i] : '', $datas[$i]];
        ...
        $arraylegends = [$langs->transnoentities("Balance")];
        if ($object->min_desired) {
        ...
        $px1->SetType(['lines', 'lines', 'lines']);
        $px1->setBgColor('onglet');
        $px1->setBgColorGrid([255, 255, 255]);
    ...
    if ($mode == 'standard') {
        ...
        $px2->SetType(['linesnopoint', 'linesnopoint', 'linesnopoint']);
    ...
    if ($mode == 'showalltime') {
        ...
        $px3->SetType(['linesnopoint', 'linesnopoint', 'linesnopoint']);
        $px3->setBgColor('onglet');
        $px3->setBgColorGrid([255, 255, 255]);
    ...
    if ($mode == 'standard') {
        ...
        foreach ($data_credit as $i => $val) {
            $graph_datas[$i] = [$labels[$i], $data_credit[$i], $data_debit[$i]];
        }
        $px4 = new DolGraph();
        $px4->SetData($graph_datas);
        $px4->SetLegend([$langs->transnoentities("Credit"), $langs->transnoentities("Debit")]);
        ...
        $px4->SetType(['bars', 'bars']);
        $px4->SetShading(3);
        $px4->setBgColor('onglet');
        $px4->setBgColorGrid([255, 255, 255]);
    ...
    if ($mode == 'standard') {
        ...
        $px5->SetLegend([$langs->transnoentities("Credit"), $langs->transnoentities("Debit")]);
        ...
        $px5->SetType(['bars', 'bars']);
        $px5->SetShading(3);
        $px5->setBgColor('onglet');
        $px5->setBgColorGrid([255, 255, 255]);
