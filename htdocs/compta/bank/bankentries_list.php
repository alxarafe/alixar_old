...
$langs->loadLangs(["banks", "bills", "categories", "companies", "margins", "salaries", "loan", "donations", "trips", "members", "compta", "accountancy"]);
...
$hookmanager->initHooks(['banktransactionlist', $contextpage]);
...
$arrayfields = [
    'b.rowid' => ['label' => $langs->trans("Ref"), 'checked' => 1, 'position' => 10],
    'b.label' => ['label' => $langs->trans("Description"), 'checked' => 1, 'position' => 20],
    'b.dateo' => ['label' => $langs->trans("DateOperationShort"), 'checked' => 1, 'position' => 30],
    'b.datev' => ['label' => $langs->trans("DateValueShort"), 'checked' => 1, 'position' => 40],
    'type' => ['label' => $langs->trans("Type"), 'checked' => 1, 'position' => 50],
    'b.num_chq' => ['label' => $langs->trans("Numero"), 'checked' => 1, 'position' => 60],
    'bu.label' => ['label' => $langs->trans("ThirdParty") . '/' . $langs->trans("User"), 'checked' => 1, 'position' => 70],
    'ba.ref' => ['label' => $langs->trans("BankAccount"), 'checked' => (($id > 0 || !empty($ref)) ? 0 : 1), 'position' => 80],
    'b.debit' => ['label' => $langs->trans("Debit"), 'checked' => 1, 'position' => 90],
    'b.credit' => ['label' => $langs->trans("Credit"), 'checked' => 1, 'position' => 100],
    'balancebefore' => ['label' => $langs->trans("BalanceBefore"), 'checked' => 0, 'position' => 110],
    'balance' => ['label' => $langs->trans("Balance"), 'checked' => 1, 'position' => 120],
    'b.num_releve' => ['label' => $langs->trans("AccountStatement"), 'checked' => 1, 'position' => 130],
    'b.conciliated' => ['label' => $langs->trans("BankLineReconciled"), 'enabled' => $object->rappro, 'checked' => ($action == 'reconcile' ? 1 : 0), 'position' => 140],
    'b.fk_bordereau' => ['label' => $langs->trans("ChequeNumber"), 'checked' => 0, 'position' => 150],
];
// Extra fields
...
if ($search_thirdparty_user) {
    ...
    $sql .= "	  WHERE " . natural_search(["subUser.firstname", "subUser.lastname"], $search_thirdparty_user, '', 1) . ")";

    $sql .= " OR b.rowid IN ";
    $sql .= " 	( SELECT bu.fk_bank FROM " . MAIN_DB_PREFIX . "bank_url AS bu";
    $sql .= "	 JOIN " . MAIN_DB_PREFIX . "bank AS b2 ON b2.rowid = bu.fk_bank";
    $sql .= "	 JOIN " . MAIN_DB_PREFIX . "societe AS subSoc ON (bu.type = 'company' AND bu.url_id = subSoc.rowid)";
    $sql .= "	  WHERE " . natural_search(["subSoc.nom"], $search_thirdparty_user, '', 1);
    $sql .= "))";
}
if ($search_description) {
...
if ($resql) {
    ...
    */
    ...
    while ($i < $imaxinloop) {
        ...
        // Fields from hook
        $parameters = ['arrayfields' => $arrayfields, 'obj' => $objp, 'i' => $i, 'totalarray' => &$totalarray];
