...
$arrayfields = [
    ...
    'b.account_number' => ['label' => $langs->trans("AccountAccounting"), 'checked' => (isModEnabled('accounting')), 'position' => 18],
    'b.fk_accountancy_journal' => ['label' => $langs->trans("AccountancyJournal"), 'checked' => (isModEnabled('accounting')), 'position' => 20],
...
foreach ($accounts as $key => $type) {
    ...
    if ($mode == 'kanban') {
        ...
        print $objecttmp->getKanbanView('', ['selected' => $selected]);
        if ($i == ($imaxinloop - 1)) {
            print '</div>';
            print '</td></tr>';
        }
