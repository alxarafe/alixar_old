...
$langs->loadLangs(["compta", "banks", "bills", "accountancy"]);
...
$arrayfields = [
    'ref' => ['label' => "Ref", 'checked' => 1, 'position' => 100],
    'label' => ['label' => "Label", 'checked' => 1, 'position' => 110],
    'datep' => ['label' => "DatePayment", 'checked' => 1, 'position' => 120],
    'datev' => ['label' => "DateValue", 'checked' => -1, 'position' => 130],
    'type' => ['label' => "PaymentMode", 'checked' => 1, 'position' => 140],
    'project' => ['label' => "Project", 'checked' => 1, 'position' => 200, "enabled" => isModEnabled('project')],
    'bank' => ['label' => "BankAccount", 'checked' => 1, 'position' => 300, "enabled" => isModEnabled("banque")],
    'entry' => ['label' => "BankTransactionLine", 'checked' => 1, 'position' => 310, "enabled" => isModEnabled("banque")],
    'account' => ['label' => "AccountAccountingShort", 'checked' => 1, 'position' => 400, "enabled" => isModEnabled('accounting')],
    'subledger' => ['label' => "SubledgerAccount", 'checked' => 1, 'position' => 410, "enabled" => isModEnabled('accounting')],
    'debit' => ['label' => "Debit", 'checked' => 1, 'position' => 500],
    'credit' => ['label' => "Credit", 'checked' => 1, 'position' => 510],
];
...
if ($search_label) {
    $sql .= natural_search(['v.label'], $search_label);
}
...
if (!empty($arrayfields['account']['checked'])) {
    ...
    print $formaccounting->select_account($search_accountancy_account, 'search_accountancy_account', 1, [], 1, 1, 'maxwidth200');
    print '</div>';
    print '</td>';
}
...
while ($i < $imaxinloop) {
    ...
    if ($mode == 'kanban') {
        ...
        // Output Kanban
        print $variousstatic->getKanbanView('', ['selected' => in_array($object->id, $arrayofselected)]);
        if ($i == ($imaxinloop) - 1) {
            print '</div>';
            print '</td></tr>';
        }
