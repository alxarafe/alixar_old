...
$langs->loadLangs(["compta", "banks", "bills", "users", "accountancy", "categories"]);
...
$hookmanager->initHooks(['variouscard', 'globalcard']);
...
if ($action == 'create') {
    ...
    $sensarray = ['0' => $langs->trans("Debit"), '1' => $langs->trans("Credit")];
...
if ($id) {
    ...
    if ($action === 'clone') {
        ...
        $formquestion = [
            ['type' => 'text', 'name' => 'clone_label', 'label' => $langs->trans("Label"), 'value' => $langs->trans("CopyOf") . ' ' . $object->label],
            ['type' => 'date', 'tdclass' => 'fieldrequired', 'name' => 'clone_date_payment', 'label' => $langs->trans("DatePayment"), 'value' => -1],
            ['type' => 'date', 'name' => 'clone_date_value', 'label' => $langs->trans("DateValue"), 'value' => -1],
            ['type' => 'other', 'tdclass' => 'fieldrequired', 'name' => 'clone_accountid', 'label' => $langs->trans("BankAccount"), 'value' => $form->select_comptes($object->fk_account, "accountid", 0, '', 1, '', 0, 'minwidth200', 1)],
            ['type' => 'text', 'name' => 'clone_amount', 'label' => $langs->trans("Amount"), 'value' => price($object->amount)],
            ['type' => 'select', 'name' => 'clone_sens', 'label' => $langs->trans("Sens") . ' ' . $set_value_help, 'values' => $sensarray, 'default' => $object->sens],
        ];
