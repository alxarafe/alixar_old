...
$langs->loadLangs(['bills', 'companies', 'compta', 'products', 'banks', 'main', 'withdrawals']);
...
$hookmanager->initHooks(['invoicecard', 'globalcard']);
...
if (empty($reshook)) {
    ...
    if ($action == 'confirm_clone' && $confirm == 'yes' && $permissiontoadd) {
        ...
        if (
            (preg_match('/^[\(]?PROV/i', $object->ref) || empty($object->ref)) &&    // empty should not happened, but when it occurs, the test save life
            ...
            if (!$error) {
                ...
                if (!empty($origin) && !empty($originid)) {
                    ...
                    if ($id > 0) {
                        ...
                        if (GETPOST('type') == Facture::TYPE_DEPOSIT && in_array($typeamount, ['amount', 'variable'])) {
        ...
        if (
            in_array($object->status, [Facture::STATUS_CLOSED, Facture::STATUS_VALIDATED])
            && $object->type == Facture::TYPE_SITUATION
            && $usercancreate
            && !$objectidnext
            ...
            if ($newCycle > 1) {
                ...
                if (count($object->tab_next_situation_invoice) > 0) {
                    foreach ($object->tab_next_situation_invoice as $next_invoice) {
                        if (
                            $next_invoice->type == Facture::TYPE_CREDIT_NOTE
...
if ($action == 'create') {
    ...
    if (empty($reshook)) {
        ...
        if (getDolGlobalString('INVOICE_USE_DEFAULT_DOCUMENT')) { // Hidden conf
            ...
            $listtType = [Facture::TYPE_STANDARD, Facture::TYPE_REPLACEMENT, Facture::TYPE_CREDIT_NOTE, Facture::TYPE_DEPOSIT, Facture::TYPE_SITUATION];
            $jsListType = '';
        ...
        // Other attributes
        $parameters = ['objectsrc' => !empty($objectsrc) ? $objectsrc : 0, 'colspan' => ' colspan="2"', 'cols' => '2', 'socid' => $socid];
        ...
        if ($object->type != Facture::TYPE_DEPOSIT && getDolGlobalString('STOCK_CALCULATE_ON_BILL') && $object->status >= 1) {
            ...
            if ($qualified_for_stock_change) {
                ...
                $formquestion = [
                    ...
                    ['type' => 'other', 'name' => 'idwarehouse', 'label' => $label, 'value' => $formproduct->selectWarehouses(GETPOST('idwarehouse') ? GETPOST('idwarehouse') : 'ifone', 'idwarehouse', '', 1, 0, 0, $langs->trans("NoStockAction"), 0, $forcecombo)],
                ];
        ...
        if ($object->type != Facture::TYPE_DEPOSIT && getDolGlobalString('STOCK_CALCULATE_ON_BILL')) {
            ...
            if ($qualified_for_stock_change) {
                ...
                $formquestion = [
                    ...
                    // => 1),
                    ['type' => 'other', 'name' => 'idwarehouse', 'label' => $label, 'value' => $value],
                ];
            }
        }
    ...
    if ($action == 'paid' && $resteapayer > 0 && (!getDolGlobalString('INVOICE_CAN_SET_PAID_EVEN_IF_PARTIALLY_PAID') || $resteapayer != $object->total_ttc)) {
        ...
        // Create a form table
        $formquestion = ['text' => $langs->trans("ConfirmClassifyPaidPartiallyQuestion"), ['type' => 'radio', 'name' => 'close_code', 'label' => $langs->trans("Reason"), 'values' => $arrayreasons], ['type' => 'text', 'name' => 'close_note', 'label' => $langs->trans("Comment"), 'value' => '', 'morecss' => 'minwidth300']];
    ...
    if ($action == 'canceled') {
        ...
        if ($objectidnext && $statusreplacement == 0) {
            ...
            // Create a form table
            $formquestion = ['text' => $langs->trans("ConfirmCancelBillQuestion"), ['type' => 'radio', 'name' => 'close_code', 'label' => $langs->trans("Reason"), 'values' => $arrayreasons], ['type' => 'text', 'name' => 'close_note', 'label' => $langs->trans("Comment"), 'value' => '', 'morecss' => 'minwidth300']];
    ...
    if ($action == 'clone') {
        ...
        $formquestion = [
            ['type' => 'other', 'name' => 'socid', 'label' => $langs->trans("SelectThirdParty"), 'value' => $form->select_company($object->socid, 'socid', $filter, 1)],
            ['type' => 'date', 'name' => 'newdate', 'label' => $langs->trans("Date"), 'value' => dol_now()],
        ];
    ...
    $parameters = ['formConfirm' => $formconfirm, 'lineid' => $lineid, 'remainingtopay' => &$resteapayer];
    ...
    if ($action != 'prerelance' && $action != 'presend' && $action != 'valid' && $action != 'editline') {
        ...
        if (empty($reshook)) {
            ...
            // Reopen an invoice
            if (
                (($object->type == Facture::TYPE_STANDARD || $object->type == Facture::TYPE_REPLACEMENT)
            ...
            if ($object->type == Facture::TYPE_CREDIT_NOTE || $object->type == Facture::TYPE_DEPOSIT || $object->type == Facture::TYPE_STANDARD || $object->type == Facture::TYPE_SITUATION) {
                ...
                // For credit note
                if (
                    $object->type == Facture::TYPE_CREDIT_NOTE && $object->status == Facture::STATUS_VALIDATED && $object->paye == 0 && $usercancreate
                ...
                if ($object->type == Facture::TYPE_DEPOSIT && $usercancreate && $object->status > Facture::STATUS_DRAFT && empty($discount->id)) {
                    ...
                    if (price2num($object->total_ttc, 'MT') == price2num($sumofpaymentall, 'MT') || getDolGlobalInt('DEPOSIT_AS_CREDIT_AVAILABLE_EVEN_UNPAID') || ($object->type == Facture::STATUS_ABANDONED && in_array($object->close_code, ['bankcharge', 'discount_vat', 'other']))) {
            ...
            // Classify paid
            if (
                $object->status == Facture::STATUS_VALIDATED && $object->paye == 0 && $usercanissuepayment && (
    ...
    if ($action != 'prerelance' && $action != 'presend') {
        ...
        $linktoelem = $form->showLinkToObjectBlock($object, null, ['invoice']);
        ...
        if (
            ...
            $compatibleImportElementsList = ['commande', 'propal']; // import from linked elements
        }
