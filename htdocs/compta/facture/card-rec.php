...
$langs->loadLangs(['bills', 'companies', 'compta', 'admin', 'other', 'products', 'banks']);
...
$hookmanager->initHooks(['invoicereccard', 'globalcard']);
...
if ($action == 'create') {
    ...
    if ($object->fetch($id, $ref) > 0) {
        ...
        print $form->selectarray('unit_frequency', ['d' => $langs->trans('Day'), 'm' => $langs->trans('Month'), 'y' => $langs->trans('Year')], (GETPOST('unit_frequency') ? GETPOST('unit_frequency') : 'm'));
        print "</td></tr>";
        ...
        $select = ['0' => $langs->trans('BillStatusDraft'), '1' => $langs->trans('BillStatusValidated')];
        ...
        if (getDolGlobalString('INVOICE_REC_CAN_DISABLE_DOCUMENT_FILE_GENERATION')) {
            ...
            $select = ['0' => $langs->trans('DoNotGenerateDoc'), '1' => $langs->trans('AutoGenerateDoc')];
    ...
    if ($object->id > 0) {
        ...
        if ($action == 'editfrequency') {
            ...
            print $form->selectarray('unit_frequency', ['d' => $langs->trans('Day'), 'm' => $langs->trans('Month'), 'y' => $langs->trans('Year')], ($object->unit_frequency ? $object->unit_frequency : 'm'));
            print '</td>';
