...
$langs->loadLangs(['bills', 'companies', 'products', 'categories']);
...
$checkedtypetiers = 0;
$arrayfields = [
    'f.ref' => ['label' => "Ref", 'checked' => 1, 'position' => 5],
    'f.ref_client' => ['label' => "RefCustomer", 'checked' => -1, 'position' => 10],
    'f.type' => ['label' => "Type", 'checked' => 0, 'position' => 15],
    'f.datef' => ['label' => "DateInvoice", 'checked' => 1, 'position' => 20],
    'f.date_valid' => ['label' => "DateValidation", 'checked' => 0, 'position' => 22],
    'f.subtype' => ['label' => "InvoiceSubtype", 'checked' => 0, 'position' => 17],
    'f.date_lim_reglement' => ['label' => "DateDue", 'checked' => 1, 'position' => 25],
    'f.date_closing' => ['label' => "DateClosing", 'checked' => 0, 'position' => 30],
    'p.ref' => ['label' => "ProjectRef", 'langs' => 'projects', 'checked' => 1, 'enabled' => (!isModEnabled('project') ? 0 : 1), 'position' => 40],
    'p.title' => ['label' => "ProjectLabel", 'checked' => 0, 'enabled' => (!isModEnabled('project') ? 0 : 1), 'position' => 41],
    's.nom' => ['label' => "ThirdParty", 'checked' => 1, 'position' => 50],
    's.name_alias' => ['label' => "AliasNameShort", 'checked' => 1, 'position' => 51],
    's.code_client' => ['label' => "CustomerCodeShort", 'checked' => -1, 'position' => 52],
    's2.nom' => ['label' => 'ParentCompany', 'position' => 32, 'checked' => 0],
    's.town' => ['label' => "Town", 'checked' => -1, 'position' => 55],
    's.zip' => ['label' => "Zip", 'checked' => 1, 'position' => 60],
    'state.nom' => ['label' => "StateShort", 'checked' => 0, 'position' => 65],
    'country.code_iso' => ['label' => "Country", 'checked' => 0, 'position' => 70],
    'typent.code' => ['label' => "ThirdPartyType", 'checked' => $checkedtypetiers, 'position' => 75],
    'f.fk_mode_reglement' => ['label' => "PaymentMode", 'checked' => 1, 'position' => 80],
    'f.fk_cond_reglement' => ['label' => "PaymentConditionsShort", 'checked' => 1, 'position' => 85],
    'f.module_source' => ['label' => "POSModule", 'langs' => 'cashdesk', 'checked' => ($contextpage == 'poslist' ? 1 : 0), 'enabled' => "(isModEnabled('cashdesk') || isModEnabled('takepos') || getDolGlobalInt('INVOICE_SHOW_POS'))", 'position' => 90],
    'f.pos_source' => ['label' => "POSTerminal", 'langs' => 'cashdesk', 'checked' => ($contextpage == 'poslist' ? 1 : 0), 'enabled' => "(isModEnabled('cashdesk') || isModEnabled('takepos') || getDolGlobalInt('INVOICE_SHOW_POS'))", 'position' => 91],
    'f.total_ht' => ['label' => "AmountHT", 'checked' => 1, 'position' => 95],
    'f.total_tva' => ['label' => "AmountVAT", 'checked' => 0, 'position' => 100],
    'f.total_localtax1' => ['label' => $langs->transcountry("AmountLT1", $mysoc->country_code), 'checked' => 0, 'enabled' => ($mysoc->localtax1_assuj == "1"), 'position' => 110],
    'f.total_localtax2' => ['label' => $langs->transcountry("AmountLT2", $mysoc->country_code), 'checked' => 0, 'enabled' => ($mysoc->localtax2_assuj == "1"), 'position' => 120],
    'f.total_ttc' => ['label' => "AmountTTC", 'checked' => 0, 'position' => 130],
    'dynamount_payed' => ['label' => "Received", 'checked' => 0, 'position' => 140],
    'rtp' => ['label' => "Rest", 'checked' => 0, 'position' => 150], // Not enabled by default because slow
    'f.multicurrency_code' => ['label' => 'Currency', 'checked' => 0, 'enabled' => (!isModEnabled('multicurrency') ? 0 : 1), 'position' => 280],
    'f.multicurrency_tx' => ['label' => 'CurrencyRate', 'checked' => 0, 'enabled' => (!isModEnabled('multicurrency') ? 0 : 1), 'position' => 285],
    'f.multicurrency_total_ht' => ['label' => 'MulticurrencyAmountHT', 'checked' => 0, 'enabled' => (!isModEnabled('multicurrency') ? 0 : 1), 'position' => 290],
    'f.multicurrency_total_vat' => ['label' => 'MulticurrencyAmountVAT', 'checked' => 0, 'enabled' => (!isModEnabled('multicurrency') ? 0 : 1), 'position' => 291],
    'f.multicurrency_total_ttc' => ['label' => 'MulticurrencyAmountTTC', 'checked' => 0, 'enabled' => (!isModEnabled('multicurrency') ? 0 : 1), 'position' => 292],
    'multicurrency_dynamount_payed' => ['label' => 'MulticurrencyAlreadyPaid', 'checked' => 0, 'enabled' => (!isModEnabled('multicurrency') ? 0 : 1), 'position' => 295],
    'multicurrency_rtp' => ['label' => 'MulticurrencyRemainderToPay', 'checked' => 0, 'enabled' => (!isModEnabled('multicurrency') ? 0 : 1), 'position' => 296], // Not enabled by default because slow
    'total_pa' => ['label' => ((getDolGlobalString('MARGIN_TYPE') == '1') ? 'BuyingPrice' : 'CostPrice'), 'checked' => 0, 'position' => 300, 'enabled' => (!isModEnabled('margin') || !$user->hasRight('margins', 'liretous') ? 0 : 1)],
    'total_margin' => ['label' => 'Margin', 'checked' => 0, 'position' => 301, 'enabled' => (!isModEnabled('margin') || !$user->hasRight('margins', 'liretous') ? 0 : 1)],
    'total_margin_rate' => ['label' => 'MarginRate', 'checked' => 0, 'position' => 302, 'enabled' => (!isModEnabled('margin') || !$user->hasRight('margins', 'liretous') || !getDolGlobalString('DISPLAY_MARGIN_RATES') ? 0 : 1)],
    'total_mark_rate' => ['label' => 'MarkRate', 'checked' => 0, 'position' => 303, 'enabled' => (!isModEnabled('margin') || !$user->hasRight('margins', 'liretous') || !getDolGlobalString('DISPLAY_MARK_RATES') ? 0 : 1)],
    'f.datec' => ['label' => "DateCreation", 'checked' => 0, 'position' => 500],
    'f.tms' => ['type' => 'timestamp', 'label' => 'DateModificationShort', 'enabled' => 1, 'visible' => -1, 'notnull' => 1, 'position' => 502],
    'u.login' => ['label' => "UserAuthor", 'checked' => 1, 'position' => 504],
    'sale_representative' => ['label' => "SaleRepresentativesOfThirdParty", 'checked' => 0, 'position' => 506],
    ...
    'f.note_public' => ['label' => 'NotePublic', 'checked' => 0, 'position' => 520, 'enabled' => (!getDolGlobalInt('MAIN_LIST_HIDE_PUBLIC_NOTES'))],
    'f.note_private' => ['label' => 'NotePrivate', 'checked' => 0, 'position' => 521, 'enabled' => (!getDolGlobalInt('MAIN_LIST_HIDE_PRIVATE_NOTES'))],
    'f.fk_fac_rec_source' => ['label' => 'GeneratedFromTemplate', 'checked' => 0, 'position' => 530, 'enabled' => '1'],
    'f.fk_statut' => ['label' => "Status", 'checked' => 1, 'position' => 1000],
];

if (getDolGlobalString("INVOICE_USE_SITUATION") && getDolGlobalString('INVOICE_USE_RETAINED_WARRANTY')) {
    $arrayfields['f.retained_warranty'] = ['label' => $langs->trans("RetainedWarranty"), 'checked' => 0, 'position' => 86];
}
...
$parameters = ['socid' => $socid, 'arrayfields' => &$arrayfields];
            ...
            foreach ($arrayofselected as $toselectid) {
                ...
                if ($result < 0) {
                    ...
                    if ($facture->type != Facture::TYPE_CREDIT_NOTE && $facture->status == Facture::STATUS_VALIDATED && $facture->paye == 0) {
                        ...
                        $parameters = ['remaintopay' => $remaintopay];
    ...
    if (!$user->hasRight('prelevement', 'bons', 'creer')) {
        ...
        foreach ($arrayofselected as $toselectid) {
            ...
            if ($result > 0) {
                ...
                // hook to finalize the remaining amount, considering e.g. cash discount agreements
                $parameters = ['remaintopay' => $objecttmp->resteapayer];
...
if (empty($arrayfields['s.name_alias']['checked']) && $search_company) {
    $sql .= natural_search(["s.nom", "s.name_alias"], $search_company);
} else {
    if ($search_company) {
...
if ($search_login) {
    $sql .= natural_search(['u.login', 'u.firstname', 'u.lastname'], $search_login);
}
...
$searchCategoryProductList = $search_product_category ? [$search_product_category] : [];
...
$searchCategoryCustomerList = $search_categ_cus ? [$search_categ_cus] : [];
...
if (in_array($massaction, ['presend', 'predelete', 'makepayment'])) {
    $arrayofmassactions = [];
}
...
if (!in_array($massaction, ['makepayment'])) {
...
if ($massaction == 'makepayment') {
    ...
    $formquestion = [
        ...
        ['type' => 'date', 'name' => 'datepaiment', 'label' => $langs->trans("Date"), 'datenow' => 1],
        ['type' => 'other', 'name' => 'paiementid', 'label' => $langs->trans("PaymentMode"), 'value' => $form->select_types_paiements(GETPOST('search_paymentmode'), 'paiementid', '', 0, 0, 1, 0, 1, '', 1)],
        ['type' => 'other', 'name' => 'bankid', 'label' => $langs->trans("BankAccount"), 'value' => $form->select_comptes('', 'bankid', 0, '', 0, '', 0, '', 1)],
...
if (!empty($arrayfields['f.fk_statut']['checked'])) {
    ...
    $liststatus = ['0' => $langs->trans("BillShortStatusDraft"), '0,1' => $langs->trans("BillShortStatusDraft") . '+' . $langs->trans("BillShortStatusNotPaid"), '1' => $langs->trans("BillShortStatusNotPaid"), '1,2' => $langs->trans("BillShortStatusNotPaid") . '+' . $langs->trans("BillShortStatusPaid"), '2' => $langs->trans("BillShortStatusPaid"), '3' => $langs->trans("BillShortStatusCanceled")];
...
if ($num > 0) {
    ...
    while ($i < $imaxinloop) {
        ...
        if ($mode == 'kanban') {
            ...
            $arraydata = ['alreadypaid' => $paiement, 'thirdparty' => $companystatic->getNomUrl(1, '', 12), 'userauthor' => $userstatic->getNomUrl(1), 'selected' => in_array($object->id, $arrayofselected)];
