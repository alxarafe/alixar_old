...
$hookmanager->initHooks(['recapcomptacard', 'globalcard']);
...
$arrayfields = [
    'f.datef' => ['label' => "Date", 'checked' => 1],
    //...
];

// Initialize technical object to manage hooks of page. Note that conf->hooks_modules contains array of hook context
$hookmanager->initHooks(['supplierbalencelist', 'globalcard']);
    ...
    if (isModEnabled('facture') && $user->hasRight('facture', 'lire')) {
        ...
        if ($resql) {
            ...
            for ($i = 0; $i < $num; $i++) {
                ...
                $parameters = ['socid' => $id, 'values' => &$values, 'fac' => $fac, 'userstatic' => $userstatic];
                ...
                if ($resqlp) {
                    ...
                    while ($j < $nump) {
                        ...
                        $parameters = ['socid' => $id, 'values' => &$values, 'fac' => $fac, 'userstatic' => $userstatic, 'paymentstatic' => $paymentstatic];
