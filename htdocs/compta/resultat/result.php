...
$langs->loadLangs(['compta', 'bills', 'donation', 'salaries', 'accountancy']);
...
if ($modecompta == "CREANCES-DETTES") {
    ...
    $period = $form->selectDate($date_start, 'date_start', 0, 0, 0, '', 1, 0) . ' - ' . $form->selectDate($date_end, 'date_end', 0, 0, 0, '', 1, 0);
    $arraylist = ['no' => $langs->trans("None"), 'yes' => $langs->trans("AccountWithNonZeroValues"), 'all' => $langs->trans("All")];
...
report_header($name, '', $period, $periodlink ?? '', $description, $builddate, $exportlink ?? '', ['modecompta' => $modecompta, 'action' => ''], $calcmode);
