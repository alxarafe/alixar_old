...
$langs->loadLangs(['compta', 'bills', 'banks', 'hrm']);
...
$hookmanager->initHooks(['taxcard', 'globalcard']);

// Initialize technical objects
$object = new ChargeSociales($db);
$extrafields = new ExtraFields($db);
$diroutputmassaction = $conf->tax->dir_output . '/temp/massgeneration/' . $user->id;
$hookmanager->initHooks(['taxsocialcontributioncard', 'globalcard']);
...
if ($id > 0) {
    ...
    if ($result > 0) {
        ...
        if ($action === 'clone') {
            $formquestion = [
                ['type' => 'text', 'name' => 'clone_label', 'label' => $langs->trans("Label"), 'value' => $langs->trans("CopyOf") . ' ' . $object->label, 'tdclass' => 'fieldrequired'],
            ];
            if (getDolGlobalString('TAX_ADD_CLONE_FOR_NEXT_MONTH_CHECKBOX')) {
                $formquestion[] = ['type' => 'checkbox', 'name' => 'clone_for_next_month', 'label' => $langs->trans("CloneTaxForNextMonth"), 'value' => 1];
            } else {
                $formquestion[] = ['type' => 'date', 'datenow' => 1, 'name' => 'clone_date_ech', 'label' => $langs->trans("Date"), 'value' => -1];
                $formquestion[] = ['type' => 'date', 'name' => 'clone_period', 'label' => $langs->trans("PeriodEndDate"), 'value' => -1];
                $formquestion[] = ['type' => 'text', 'name' => 'amount', 'label' => $langs->trans("Amount"), 'value' => price($object->amount), 'morecss' => 'width100'];
            }
