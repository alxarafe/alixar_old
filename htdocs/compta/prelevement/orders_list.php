...
$langs->loadLangs(['banks', 'categories', 'withdrawals']);
...
$hookmanager->initHooks(['withdrawalsreceiptslist']);
...
while ($i < $imaxinloop) {
    ...
    if ($mode == 'kanban') {
        ...
        print $directdebitorder->getKanbanView('', ['selected' => in_array($obj->id, $arrayofselected)]);
        if ($i == ($imaxinloop - 1)) {
            print '</div>';
            print '</td></tr>';
        }
