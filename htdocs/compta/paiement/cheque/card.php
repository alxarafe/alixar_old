...
if ($action == 'builddoc' && $user->hasRight('banque', 'cheque')) {
	...
	if ($result <= 0) {
		...
		header('Location: '.DOL_PHP_SELF.'?id='.$object->id.(!getDolGlobalString('MAIN_JUMP_TAG') ? '' : '#builddoc'));
		exit;
	}
...
if ($action == 'new') {
	$head = array();
	$h = 0;
	$head[$h][0] = DOL_PHP_SELF.'?action=new';
	...
	$h = 0;
	$head = array();
	$head[$h][0] = DOL_PHP_SELF.'?id='.$object->id;
	 ...
	 */
	if ($action == 'delete') {
		print $form->formconfirm(DOL_PHP_SELF.'?id='.$object->id, $langs->trans("DeleteCheckReceipt"), $langs->trans("ConfirmDeleteCheckReceipt"), 'confirm_delete', '', '', 1);
	}
	 ...
	 */
	if ($action == 'valide') {
		print $form->formconfirm(DOL_PHP_SELF.'?id='.$object->id, $langs->trans("ValidateCheckReceipt"), $langs->trans("ConfirmValidateCheckReceipt"), 'confirm_validate', '', '', 1);
	}
	...
	if ($action == 'reject_check') {
		...
		print $form->formconfirm(DOL_PHP_SELF.'?id='.$object->id, $langs->trans("RejectCheck"), $langs->trans("ConfirmRejectCheck"), 'confirm_reject_check', $formquestion, '', 1);
	}
}
...
if ($action == 'new') {
	...
	if (getDolGlobalString('MAIN_SHOW_EXTREF')) {
		...
		if ($action != 'editrefext') {
			print '<td class="right"><a class="editfielda" href="'.DOL_PHP_SELF.'?action=editrefext&token='.newToken().'&id='.((int) $object->id).'">'.img_edit($langs->trans('SetRefExt'), 1).'</a></td>';
		}
		print '</tr></table>';
		print '</td><td>';
		if ($action == 'editrefext') {
			print '<form name="setrefext" action="'.DOL_PHP_SELF.'?id='.$object->id.'" method="post">';
			print '<input type="hidden" name="token" value="'.newToken().'">';
	...
	if ($resql) {
		...
		print_liste_field_titre("DateChequeReceived", DOL_PHP_SELF, "b.dateo,b.rowid", "", $param, 'align="center"', $sortfield, $sortorder);
		print_liste_field_titre("Numero", DOL_PHP_SELF, "b.num_chq", "", $param, 'align="center"', $sortfield, $sortorder);
		print_liste_field_titre("CheckTransmitter", DOL_PHP_SELF, "b.emetteur", "", $param, "", $sortfield, $sortorder);
		print_liste_field_titre("Bank", DOL_PHP_SELF, "b.banque", "", $param, "", $sortfield, $sortorder);
		print_liste_field_titre("Amount", DOL_PHP_SELF, "b.amount", "", $param, 'class="right"', $sortfield, $sortorder);
		print_liste_field_titre("Payment", DOL_PHP_SELF, "p.rowid", "", $param, 'align="center"', $sortfield, $sortorder);
		print_liste_field_titre("LineRecord", DOL_PHP_SELF, "b.rowid", "", $param, 'align="center"', $sortfield, $sortorder);
		...
		if ($num > 0) {
			while ($objp = $db->fetch_object($resql)) {
				...
				if ($object->statut == 0) {
					print '<a href="'.DOL_PHP_SELF.'?id='.$object->id.'&action=remove&token='.newToken().'&lineid='.$objp->rowid.'">'.img_delete().'</a>';
				}
				if ($object->statut == 1 && $objp->statut != 2) {
					print '<a href="'.DOL_PHP_SELF.'?id='.$object->id.'&action=reject_check&token='.newToken().'&lineid='.$objp->rowid.'">'.img_picto($langs->trans("RejectCheck"), 'disable').'</a>';
				}
				if ($objp->statut == 2) {
...
if ($user->socid == 0 && !empty($object->id) && $object->statut == 0 && $user->hasRight('banque', 'cheque')) {
	print '<a class="butAction" href="'.DOL_PHP_SELF.'?id='.$object->id.'&action=valide&token='.newToken().'&sortfield='.$sortfield.'&sortorder='.$sortorder.'">'.$langs->trans('Validate').'</a>';
}
