...
$langs->loadLangs(['bills', 'banks', 'companies']);
...
$hookmanager->initHooks(['paymentcard', 'globalcard']);
...
if (!empty($object->ext_payment_id)) {
    ...
    if (isModEnabled('stripe') && in_array($object->ext_payment_site, ['Stripe', 'StripeLive'])) {
...
if ($resql) {
    ...
    if ($num > 0) {
        while ($i < $num) {
            ...
            $parameters = ['fk_paiement' => (int) $object->id];
