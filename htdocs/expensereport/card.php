...
$langs->loadLangs(["trips", "bills", "mails"]);
...
$hookmanager->initHooks(['expensereportcard', 'globalcard']);
    ...
    if ($result > 0) {
        ...
        if ($action == 'edit' && ($object->status < 3 || $object->status == 99)) {
            ...
            if ($action == 'clone') {
                ...
                $formquestion = [
                    'text' => '',
                    ['type' => 'other', 'name' => 'fk_user_author', 'label' => $langs->trans("SelectTargetUser"), 'value' => $form->select_dolusers((GETPOST('fk_user_author', 'int') > 0 ? GETPOST('fk_user_author', 'int') : $user->id), 'fk_user_author', 0, null, 0, $criteriaforfilter, '', '0', 0, 0, '', 0, '', 'maxwidth150')],
                ];
            ...
            if ($action == 'cancel') {
                $array_input = ['text' => $langs->trans("ConfirmCancelTrip"), ['type' => "text", 'label' => '<strong>' . $langs->trans("Comment") . '</strong>', 'name' => "detail_cancel", 'value' => ""]];
            ...
            if ($action == 'refuse') {        // Deny
                $array_input = ['text' => $langs->trans("ConfirmRefuseTrip"), ['type' => "text", 'label' => $langs->trans("Comment"), 'name' => "detail_refuse", 'value' => ""]];
                ...
                foreach ($object->lines as &$line) {
                    ...
                    if ($action == 'editline' && $line->id == GETPOST('rowid', 'int')) {
                        ...
                        if (getDolGlobalString('MAIN_USE_EXPENSE_IK')) {
                            ...
                            $params = ['fk_expense' => $object->id, 'fk_expense_det' => $line->id, 'date' => $line->date];
