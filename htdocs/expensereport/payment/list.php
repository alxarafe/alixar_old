...
$langs->loadLangs(['expensereports', 'bills', 'banks', 'compta']);
...
$arrayfields = [
    'pndf.rowid' => ['label' => "RefPayment", 'checked' => 1, 'position' => 10],
    'pndf.datep' => ['label' => "Date", 'checked' => 1, 'position' => 20],
    'u.login' => ['label' => "User", 'checked' => 1, 'position' => 30],
    'c.libelle' => ['label' => "Type", 'checked' => 1, 'position' => 40],
    'pndf.num_payment' => ['label' => "Numero", 'checked' => 1, 'position' => 50, 'tooltip' => "ChequeOrTransferNumber"],
    'ba.label' => ['label' => "BankAccount", 'checked' => 1, 'position' => 60, 'enable' => (isModEnabled("banque"))],
    'pndf.amount' => ['label' => "Amount", 'checked' => 1, 'position' => 70],
];
$arrayfields = dol_sort_array($arrayfields, 'position');

// Initialize technical object to manage hooks of page. Note that conf->hooks_modules contains array of hook context
$hookmanager->initHooks(['paymentexpensereportlist']);
...
if ($search_user) {
    $sql .= natural_search(['u.login', 'u.lastname', 'u.firstname'], $search_user);
}
...
while ($i < $imaxinloop) {
    ...
    if ($mode == 'kanban') {
        ...
        // Output Kanban
        print $paymentexpensereportstatic->getKanbanView('', ['selected' => in_array($objp->id, $arrayofselected)]);
        if ($i == ($imaxinloop - 1)) {
            print '</div>';
            print '</td></tr>';
        }
