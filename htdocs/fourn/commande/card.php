...
if (empty($reshook)) {
    $backurlforlist = '/fourn/commande/list.php' . ($socid > 0 ? '?socid=' . ((int) $socid) : '');
    ...
    if (empty($backtopage) || ($cancel && empty($id))) {
        if (empty($backtopage) || ($cancel && strpos($backtopage, '__ID__'))) {
            if (empty($id) && (($action != 'add' && $action != 'create') || $cancel)) {
                ...
                $backtopage = '/fourn/commande/card.php?id=' . ((!empty($id) && $id > 0) ? $id : '__ID__');
            }
        }
    }
            ...
            if (empty($reshook)) {
                ...
                if (in_array($object->statut, array(CommandeFournisseur::STATUS_ACCEPTED))) {
                    ...
                    if (!$buttonshown && $usercanapprove) {
                        if (
                            !getDolGlobalString('SUPPLIER_ORDER_REOPEN_BY_APPROVER_ONLY')
                    ...
                    if (!$buttonshown && $usercanapprovesecond && getDolGlobalString('SUPPLIER_ORDER_3_STEPS_TO_BE_APPROVED')) {
                        if (
                            !getDolGlobalString('SUPPLIER_ORDER_REOPEN_BY_APPROVER2_ONLY')
