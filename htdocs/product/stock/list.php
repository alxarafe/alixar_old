...
print '<form action="'.DOL_PHP_SELF.'" id="searchFormList" method="POST" name="formulaire">';
if ($optioncss != '') {
...
$newcardbutton .= dolGetButtonTitle($langs->trans('MenuNewWarehouse'), '', 'fa fa-plus-circle', DOL_URL_ROOT.'/product/stock/card.php?action=create&backtopage='.urlencode(DOL_PHP_SELF), '', $user->hasRight('stock', 'creer'));

print_barre_liste($title, $page, DOL_PHP_SELF, $param, $sortfield, $sortorder, $massactionbutton, $num, $nbtotalofrecords, 'stock', 0, $newcardbutton, '', $limit, 0, 0, 1);
...
if (!empty($arrayfields["stockqty"]['checked'])) {
	print_liste_field_titre("PhysicalStock", DOL_PHP_SELF, "stockqty", '', $param, '', $sortfield, $sortorder, 'right ');
	$totalarray['nbfield']++;
	$totalarray['type'][$totalarray['nbfield']] = 'stock';
}

if (!empty($arrayfields["estimatedvalue"]['checked'])) {
	print_liste_field_titre("EstimatedStockValue", DOL_PHP_SELF, "estimatedvalue", '', $param, '', $sortfield, $sortorder, 'right ');
	$totalarray['nbfield']++;
	$totalarray['type'][$totalarray['nbfield']] = 'price';
}

if (!empty($arrayfields["estimatedstockvaluesell"]['checked'])) {
	print_liste_field_titre("EstimatedStockValueSell", DOL_PHP_SELF, "", '', $param, '', $sortfield, $sortorder, 'right ');
	$totalarray['nbfield']++;
	$totalarray['type'][$totalarray['nbfield']] = 'price';
}
...
if (!empty($arrayfields['t.statut']['checked'])) {
	print_liste_field_titre($arrayfields['t.statut']['label'], DOL_PHP_SELF, "t.statut", '', $param, '', $sortfield, $sortorder, 'center ');
	$totalarray['nbfield']++;
}
