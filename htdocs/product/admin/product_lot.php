					...
					if (substr($file, 0, 8) == 'mod_lot_' && substr($file, dol_strlen($file) - 3, 3) == 'php') {
						...
						if ($module->isEnabled()) {
							...
							if ($conf->global->PRODUCTBATCH_LOT_ADDON == $file) {
								...
								print '<a class="reposition" href="'.DOL_PHP_SELF.'?action=setmodlot&token='.newToken().'&value='.urlencode($file).'">';
					...
					if (substr($file, 0, 7) == 'mod_sn_' && substr($file, dol_strlen($file) - 3, 3) == 'php') {
						...
						if ($module->isEnabled()) {
							...
							if ($conf->global->PRODUCTBATCH_SN_ADDON == $file) {
								...
								print '<a class="reposition" href="'.DOL_PHP_SELF.'?action=setmodsn&token='.newToken().'&value='.urlencode($file).'">';
								print img_picto($langs->trans("Disabled"), 'switch_off');
								print '</a>';
							}
							print '</td>';
