...
if (empty($reshook)) {
    $backurlforlist = '/product/list.php?type=' . $type;
    ...
    if (empty($backtopage) || ($cancel && empty($id))) {
        if (empty($backtopage) || ($cancel && strpos($backtopage, '__ID__'))) {
            if (empty($id) && (($action != 'add' && $action != 'create') || $cancel)) {
                ...
                $backtopage = '/product/card.php?id=' . ((!empty($id) && $id > 0) ? $id : '__ID__');
            }
        }
    }
        ...
        if (empty($reshook)) {
            ...
            if (isModEnabled('productbatch')) {
                ...
                if ($status_batch !== '0') {
                    ...
                    if (
                        (getDolGlobalString('PRODUCTBATCH_LOT_ADDON') == 'mod_lot_advanced')
            ...
            if (empty($reshook)) {
                ...
                if (isModEnabled('productbatch')) {
                    if ($object->isProduct() || getDolGlobalString('STOCK_SUPPORTS_SERVICES')) {
                        ...
                        print '</td></tr>';
                        if (
                            (($object->status_batch == '1' && getDolGlobalString('PRODUCTBATCH_LOT_USE_PRODUCT_MASKS') && getDolGlobalString('PRODUCTBATCH_LOT_ADDON') == 'mod_lot_advanced')
...
if (
    ($action == 'delete' && (empty($conf->use_javascript_ajax) || !empty($conf->dol_use_jmobile)))  // Output when action = clone if jmobile or no js
...
// Clone confirmation
if (
    ($action == 'clone' && (empty($conf->use_javascript_ajax) || !empty($conf->dol_use_jmobile)))       // Output when action = clone if jmobile or no js
    || (!empty($conf->use_javascript_ajax) && empty($conf->dol_use_jmobile))
) {                         // Always output when not jmobile nor js
