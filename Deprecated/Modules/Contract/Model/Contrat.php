...
namespace DoliModules\Contract\Model;
 ...
 */
class Contrat extends GenericDocument
{
...
const STATUS_SIGNED_ALL = 9;
     ...
     * @var string
     */
    public $ref_customer; // Object societe
    public $from;
    /**
...
public $societe;
    ...
    public $fields = [
        ...
        'signed_status' => ['type' => 'smallint(6)', 'label' => 'SignedStatus', 'enabled' => 1, 'visible' => -1, 'position' => 50, 'arrayofkeyval' => [0 => 'NoSignature', 1 => 'SignedSender', 2 => 'SignedReceiver', 9 => 'SignedAll']],
     ...
     *
     * @param int $only_services       0=Default for all, 1=Force only services (depending on setup, we may also have
     *                                 physical products in a contract)
     * @param int $loadalsotranslation 0=Default to not load translations, 1=Load also translations of product
     *                                 descriptions
     ...
     *
     * @return int                     Return integer <0 if KO, 0 if not found or if two records found for same ref, Id
     *                                 of contract if OK
     */
     ...
     * CAN BE A CRON TASK
     * Loop on each contract lines and update the end of date. Do not execute the update if there is one pending
     * invoice linked to contract.
     *
     * @param int $thirdparty_id    Thirdparty id
     * @param int $delayindaysshort To renew the resources x day before (positive value) or after (negative value) the
     *                              end of date (default is 0)
     *
     ...
     *                  label + Picto, 6=Long label + Picto, 7=Same than 6 with fixed length
     *
     * @return string                  Label
     */
     ...
     * @param int $mode   0=long label, 1=short label, 2=Picto + short label, 3=Picto, 4=Picto + long label (status of
     *                    services), 5=Short label + Picto, 6=Long label + Picto (status of services), 7=Same than 6
     *                    with fixed length (status of services)
     *
     * @return string                  Label
     */
