...
if ($search_type) {
    $sql .= natural_search(['c.fk_type'], $search_type);
}
if ($search_lastname) {
    $sql .= natural_search(['d.lastname', 'd.societe'], $search_lastname);
}
if ($search_firstname) {
...
while ($i < $imaxinloop) {
    ...
    if ($mode == 'kanban') {
        ...
        // Output Kanban
        print $subscription->getKanbanView('', ['selected' => in_array($object->id, $arrayofselected), 'adherent_type' => $adht, 'member' => $adherent, 'bank' => ($obj->fk_account > 0 ? $accountstatic : null)]);
        if ($i == ($imaxinloop - 1)) {
            print '</div>';
            print '</td></tr>';
        }
