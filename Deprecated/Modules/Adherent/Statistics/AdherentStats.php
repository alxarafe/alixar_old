...
namespace DoliModules\Adherent\Statistics;
...
class AdherentStats extends Stats
{
     ...
     *
     * @return     array                   Array with total of draft, pending, uptodate, expired, resiliated for each
     *                                     member type
     */
    public function countMembersByTypeAndStatus($numberYears = 0)
    {
        ...
        if ($result) {
            ...
            while ($i < $num) {
                ...
                $MembersCountArray[$objp->fk_adherent_type] = [
                    'label' => $objp->label,
    ...
    public function countMembersByTagAndStatus($numberYears = 0)
    {
        ...
        if ($result) {
            ...
            while ($i < $num) {
                $objp = $this->db->fetch_object($result);
                $MembersCountArray[$objp->fk_categorie] = [
                    'label' => $objp->label,
                    'members_draft' => (int) $objp->members_draft,
                    'members_pending' => (int) $objp->members_pending,
                    'members_uptodate' => (int) $objp->members_uptodate,
