...
class Categories extends DolibarrApi
{
    ...
    public function post($request_data = null)
    {
        ...
        if ($this->category->create(DolibarrApiAccess::$user) < 0) {
            throw new RestException(500, 'Error when creating category', array_merge([$this->category->error], $this->category->errors));
        }
    ...
    public function getListForObject($id, $type, $sortfield = "s.rowid", $sortorder = 'ASC', $limit = 0, $page = 0)
    {
        ...
        if (!is_array($categories)) {
            throw new RestException(600, 'Error when fetching object categories', array_merge([$this->category->error], $this->category->errors));
        }
        return $categories;
    }
            ...
            if ($result > 0) {
                ...
                if ($result < 0) {
                    if ($this->category->error != 'DB_ERROR_RECORD_ALREADY_EXISTS') {
                        throw new RestException(500, 'Error when linking object', array_merge([$this->category->error], $this->category->errors));
                    }
                }
            } else {
                throw new RestException(500, 'Error when fetching object', array_merge([$object->error], $object->errors));
            }
    ...
    public function unlinkObjectById($id, $type, $object_id)
    {
        ...
        if (!empty($object)) {
            ...
            if ($result > 0) {
                ...
                if ($result < 0) {
                    throw new RestException(500, 'Error when unlinking object', array_merge([$this->category->error], $this->category->errors));
                }
            } else {
                throw new RestException(500, 'Error when fetching object', array_merge([$object->error], $object->errors));
            }
