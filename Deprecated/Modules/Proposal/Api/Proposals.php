...
namespace DoliModules\Proposal\Api;
...
class Proposals extends DolibarrApi
{
     ...
     * @param string $thirdparty_ids Thirdparty ids to filter commercial proposals (example '1' or '1,2,3')
     *                               {@pattern /^[0-9,]*$/i}
     * @param string $sqlfilters     Other criteria to filter answers separated by a comma. Syntax example
     *                               "(t.ref:like:'SO-%') and (t.datec:<:'20160101')"
    ...
    public function post($request_data = null)
    {
        ...
        if ($this->propal->create(DolibarrApiAccess::$user) < 0) {
            throw new RestException(500, "Error creating order", array_merge([$this->propal->error], $this->propal->errors));
        }
     ...
     * @param string $sqlfilters Other criteria to filter answers separated by a comma. d is the alias for proposal
     *                           lines table, p is the alias for product table. "Syntax example "(p.ref:like:'SO-%')
     *                           AND (d.date_start:<:'20220101')"
     *
     * @url GET {id}/lines
     *
     * @return array
