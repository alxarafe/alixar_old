...
namespace DoliModules\Product\Api;

use DoliCore\Api\DolibarrApi;
use DoliModules\Product\Model\Categorie;
use DoliModules\Product\Model\DolibarrApiAccess;
use DoliModules\Product\Model\Product;
use DoliModules\Product\Model\ProductAttribute;
use DoliModules\Product\Model\ProductAttributeValue;
use DoliModules\Product\Model\ProductCombination;
use DoliModules\Product\Model\ProductCombination2ValuePair;
use DoliModules\Product\Model\ProductCustomerPrice;
use DoliModules\Product\Model\ProductFournisseur;
...
class Products extends DolibarrApi
{
     ...
     * @param bool   $includeparentid       Load also ID of parent product (if product is a variant of a parent
     *                                      product)
     * @param bool   $includeifobjectisused Check if product object is used and set property 'is_object_used' with
     *                                      result.
     * @param bool   $includetrans          Load also the translations of product label and description
     *
     * @return array|mixed                      Data without useless information
     ...
     * @param int    $mode             Use this param to filter list (0 for all, 1 for only product, 2 for only
     *                                 service)
     ...
     * @param bool   $ids_only         Return only IDs of product instead of all properties (faster, above all if list
     *                                 is long)
     * @param int    $variant_filter   Use this param to filter list (0 = all, 1=products without variants, 2=parent of
     *                                 variants, 3=variants only)
     * @param bool   $pagination_data  If this parameter is set to true the response will include pagination data.
     *                                 Default value is false. Page starts from 0
     * @param int    $includestockdata Load also information about stock (slower)
     * @param string $properties       Restrict the data returned to these properties. Ignored if empty. Comma
     *                                 separated list of properties names
     *
    ...
    public function post($request_data = null)
    {
        ...
        if ($this->product->create(DolibarrApiAccess::$user) < 0) {
            throw new RestException(500, "Error creating product", array_merge([$this->product->error], $this->product->errors));
        }
    ...
    /**
     * Update product.
     * Price will be updated by this API only if option is set on "One price per product". See other APIs for other
     * price modes.
     *
    ...
    public function put($id, $request_data = null)
    {
        ...
        if ($result <= 0) {
            throw new RestException(500, "Error updating product", array_merge([$this->product->error], $this->product->errors));
        }
    ...
    public function getCategories($id, $sortfield = "s.rowid", $sortorder = 'ASC', $limit = 0, $page = 0)
    {
        ...
        if ($result < 0) {
            throw new RestException(503, 'Error when retrieve category list : ' . implode(',', array_merge([$categories->error], $categories->errors)));
        }
    ...
    public function getCustomerPricesPerSegment($id)
    {
        ...
        if ($result < 0) {
            throw new RestException(503, 'Error when retrieve prices list : ' . implode(',', array_merge([$this->product->error], $this->product->errors)));
        }
     ...
     * @param string       $supplier_reputation           Reputation with this product to the defined supplier (empty,
     *                                                    FAVORITE, DONOTORDER)
     * @param array        $localtaxes_array              Array with localtaxes info
     *                                                    array('0'=>type1,'1'=>rate1,'2'=>type2,'3'=>rate2) (loaded by
     *                                                    getLocalTaxesFromRate(vatrate, 0, ...) function).
     ...
     * @param string $sqlfilters Other criteria to filter answers separated by a comma. Syntax example "(t.tobuy:=:0)
     *                           and (t.tosell:=:1)"
     *
     * @return array              Array of product objects
     *
     ...
     * @param string $sqlfilters Other criteria to filter answers separated by a comma. Syntax example
     *                           "(t.ref:like:color)"
     * @param string $properties Restrict the data returned to these properties. Ignored if empty. Comma separated list
     *                           of properties names
     *
     * @return array
     *
     ...
     *
     * @param int $id           ID of Product
     * @param int $includestock Default value 0. If parameter is set to 1 the response will contain stock data of each
     *                          variant
     *
     * @return array
     *
     ...
     * id_size=>id_small, id_option=>id_val_a, ...)
     *
     * @param int    $id                      ID of Product
     ...
     * @param array  $features                List of attributes pairs id_attribute->id_value. Example:
     *                                        array(id_color=>id_Blue, id_size=>id_small, id_option=>id_val_a, ...)
