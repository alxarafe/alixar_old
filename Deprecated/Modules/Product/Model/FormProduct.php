...
class FormProduct
{
     ...
     *
     * @param string|int $selected     Id of preselected warehouse ('' or '-1' for no value, 'ifone' and
     *                                 'ifonenodefault' = select value if one value otherwise no value, '-2' to use the
     *                                 default value from setup)
     ...
     * @param int        $showfullpath 1=Show full path of name (parent ref into label), 0=Show only ref of current
     *                                 warehouse
     ...
     * @param int        $addempty 1=Add an empty value in list, 2=Add an empty value in list only if there is more
     *                             than 2 entries.
     *
     * @return   void
     */
     ...
     *
     * @param string|int|array $selected        Id of preselected warehouse ('' or '-1' for no value, 'ifone' and
     *                                          'ifonenodefault' = select value if one value otherwise no value, '-2'
     *                                          to use the default value from setup)
     * @param string           $htmlname        Name of html select html
     * @param string           $filterstatus    warehouse status filter, following comma separated filter options can
     *                                          be used
     *                                          'warehouseopen' = select products from open warehouses,
     *                                          'warehouseclosed' = select products from closed warehouses,
     *                                          'warehouseinternal' = select products from warehouses for internal
     *                                          correct/transfer only
     * @param int              $empty           1=Can be empty, 0 if not
     * @param int              $disabled        1=Select is disabled
     * @param int              $fk_product      Add quantity of stock in label for product with id fk_product. Nothing
     *                                          if 0.
     * @param string           $empty_label     Empty label if needed (only if $empty=1)
     ...
     * @param string           $morecss         Add more css classes to HTML select
     * @param array            $exclude         Warehouses ids to exclude
     * @param int              $showfullpath    1=Show full path of name (parent ref into label), 0=Show only ref of
     *                                          current warehouse
     * @param bool|int         $stockMin        [=false] Value of minimum stock to filter (only warehouse with stock >
     *                                          stockMin are loaded) or false not not filter by minimum stock
     * @param string           $orderBy         [='e.ref'] Order by
     ...
     * @param int      $fk_product              Add quantity of stock in label for product with id fk_product. Nothing
     *                                          if 0.
     * @param string   $batch                   Add quantity of batch stock in label for product with batch name batch,
     *                                          batch name precedes batch_id. Nothing if ''.
     * @param string   $status                  warehouse status filter, following comma separated filter options can
     *                                          be used
     *                                          'warehouseopen' = select products from open warehouses,
     *                                          'warehouseclosed' = select products from closed warehouses,
     *                                          'warehouseinternal' = select products from warehouses for internal
     ...
     *
     * @param string|int $selected     Id of preselected lot stock id ('' for no value, 'ifone'=select value if one
     ...
     * @param array      $objectLines  Only cache lot numbers for products in lines of object. If no lines only for
     *                                 fk_product. If no fk_product, all.
     * @param string     $empty_label  Empty label if needed (only if $empty=1)
     * @param int        $forcecombo   1=Force combo iso ajax select2
     * @param array      $events       Events to add to select2
     * @param string     $morecss      Add more css classes to HTML select
    ...
    public function selectLotStock($selected = '', $htmlname = 'batch_id', $filterstatus = '', $empty = 0, $disabled = 0, $fk_product = 0, $fk_entrepot = 0, $objectLines = [], $empty_label = '', $forcecombo = 0, $events = [], $morecss = 'minwidth200')
    {
        ...
        if (!empty($fk_product) && $fk_product > 0) {
            $productIdArray = [(int) $fk_product]; // only show lot stock for product
        } else {
    ...
    private function loadLotStock($productIdArray = [])
    {
        ...
        if ($cacheLoaded) {
            ...
            $parameters = ['productIdList' => $productIdList];
    ...
    public function selectLotDataList($htmlname = 'batch_id', $empty = 0, $fk_product = 0, $fk_entrepot = 0, $objectLines = [])
    {
        ...
        $hookmanager->initHooks(['productdao']);
        $parameters = ['productIdArray' => $productIdArray, 'htmlname' => $htmlname];
