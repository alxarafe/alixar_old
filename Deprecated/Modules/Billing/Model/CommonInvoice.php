 ...
 */
abstract class CommonInvoice extends GenericDocument
{
    use CommonIncoterm;
    ...
    /**
     * @var int     Sub type of invoice (A subtype code coming from llx_invoice_subtype table. May be used by some
        ...
        if ($resql) {
            ...
            if ($obj) {
                if ($multicurrency < 0) {
                    ...
                    return ['alreadypaid' => (float) $obj->amount, 'alreadypaid_multicurrency' => (float) $obj->multicurrency_amount];
     ...
     *  Should always be empty, except if option FACTURE_DEPOSITS_ARE_JUST_PAYMENTS is on for sale invoices (not
     *  recommended), of FACTURE_SUPPLIER_DEPOSITS_ARE_JUST_PAYMENTS is on for purchase invoices (not recommended).
     *
     * @param int $multicurrency Return multicurrency_amount instead of amount
     *
     * @return     float                       Return integer <0 and set ->error if KO, Sum of deposits amount
     *                                         otherwise
     ...
     * @param int $multicurrency Return multicurrency_amount instead of amount
     *
     * @return     float                       Return integer <0 and set ->error if KO, Sum of credit notes and
     *                                         deposits amount otherwise
     ...
     * @param int $multicurrency Return multicurrency_amount instead of amount
     *
     * @return     float                       Return integer <0 if KO, Sum of credit notes and deposits amount
     *                                         otherwise
     */
    ...
    public function getListOfPayments($filtertype = '', $multicurrency = 0)
    {
        ...
        if ($resql) {
            ...
            while ($i < $num) {
                $obj = $this->db->fetch_object($resql);
                $tmp = ['amount' => $obj->amount, 'type' => $obj->code, 'date' => $obj->datep, 'num' => $obj->num, 'ref' => $obj->ref];
                if (!empty($field3)) {
            ...
            if ($sql) {
                ...
                if ($resql) {
                    ...
                    while ($i < $num) {
                        $obj = $this->db->fetch_object($resql);
                        if ($multicurrency) {
                            $retarray[] = ['amount' => $obj->multicurrency_amount, 'type' => $obj->type, 'date' => $obj->date, 'num' => '0', 'ref' => $obj->ref];
                        } else {
                            $retarray[] = ['amount' => $obj->amount, 'type' => $obj->type, 'date' => $obj->date, 'num' => '', 'ref' => $obj->ref];
                        }
                        $i++;
                    }
                } else {
     ...
     * @param integer $alreadypaid 0=No payment already done, >0=Some payments were already done (we recommend to put
     *                             here amount paid if you have it, 1 otherwise)
     *
     ...
     * @param int    $checkduplicateamongall 0=Default (check among open requests only to find if request already
     *                                       exists). 1=Check also among requests completely processed and cancel if at
     *                                       least 1 request exists whatever is its status.
     *
     ...
     *  Send the payment order to the service (for a direct debit order or a credit transfer order) and record an event
     *  in llx_actioncomm.
     *
     * @param User   $fuser       User asking the direct debit transfer
