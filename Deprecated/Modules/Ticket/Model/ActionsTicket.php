...
class ActionsTicket extends CommonHookActions
{
    ...
    public function viewStatusActions(Ticket $object)
    {
        ...
        $exclude_status = [Ticket::STATUS_CLOSED, Ticket::STATUS_CANCELED];
        // Exclude actual status
        $exclude_status = array_merge($exclude_status, [(int) $object->status]);
    ...
    public function emailElementlist($parameters, &$object, &$action, $hookmanager)
    {
        ...
        if (in_array('admin', explode(':', $parameters['context']))) {
            $this->results = ['ticket_send' => $langs->trans('MailToSendTicketMessage')];
        }
