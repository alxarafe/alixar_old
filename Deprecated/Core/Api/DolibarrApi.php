...
class DolibarrApi
{
    ...
    public function __construct($db, $cachedir = '', $refreshCache = false)
    {
        ...
        $dolibarr_main_url_root = BASE_URL;
     ...
     * @param int    $resource_id  Object ID if we want to check a particular record (optional) is linked to a owned
     *                             thirdparty (optional).
     * @param string $dbtablename  'TableName&SharedElement' with Tablename is table where object is stored.
     *                             SharedElement is an optional key to define where to check entity. Not used if
     *                             objectid is null (optional)
     * @param string $feature2     Feature to check, second level of permission (optional). Can be or check with
     *                             'level1|level2'.
     * @param string $dbt_keyfield Field name for socid foreign key if not fk_soc. Not used if objectid is null
     *                             (optional)
     ...
     *                              Example: "t.ref:like:'SO-%'", "t.date_creation:<:'20160101'",
     *                              "t.date_creation:<:'2016-01-01 12:30:00'", "t.nature:is:NULL",
     *                              "t.field2:isnot:NULL"
     *
    ...
    protected function _checkValForAPI($field, $value, $object)
    {
        ...
        if (!is_array($value)) {
            ...
            if (in_array($field, ['note', 'note_private', 'note_public', 'desc', 'description'])) {
