...
abstract class Stats
{
     ...
     * @param int $format 0=Label of abscissa is a translated text, 1=Label of abscissa is month number, 2=Label of
     *                    abscissa is first letter of month
     *
     ...
     * Criteria used to build request are defined into the constructor of parent class into
     * xxx/class/xxxstats.class.php
     * The caller of class can add more filters into sql request by adding criteris into the $stats->where property
     * just after calling constructor.
     *
     * @param int $endyear    Start year
     * @param int $startyear  End year
     * @param int $cachedelay Delay we accept for cache file (0=No read, no save of cache, -1=No read but save)
        ...
        if ($resql) {
            ...
            while ($i < $num) {
                ...
                if ($i < $limit || $num == $limit) {
                    $result[$i] = [$row[0], $row[1]]; // Ref of product, nb
                } else {
                    $other += $row[1];
                }
                $i++;
            ...
            if ($num > $limit) {
                $result[$i] = [$langs->transnoentitiesnoconv("Other"), $other];
            }
            $this->db->free($resql);
        } else {
            dol_print_error($this->db);
