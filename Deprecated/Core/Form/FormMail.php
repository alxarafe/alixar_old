...
class FormMail extends Form
{
    ...
    /**
     * @var string  If you know a MSGID of an email and want to send the email in reply to it. Will be added into
     *      header as In-Reply-To: <...>
     */
    public $inreplyto;
    ...
    /**
     * @var int|string      0=No attaches files, 1=Show attached files, 2=Can add new attached files, 'text'=Show
     *      attached files and the text
     */
    public $withfile;
    ...
    /**
     * @var int 1=Add a checkbox "Attach also main document" for mass actions (checked by default), -1=Add checkbox
    ...
    public function get_attached_files()
    {
        ...
        return ['paths' => $listofpaths, 'names' => $listofnames, 'mimes' => $listofmimes];
    }
    ...
    public function get_form($addfileaction = 'addfile', $removefileaction = 'removefile')
    {
        ...
        $langs->loadLangs(['other', 'mails', 'members']);
        ...
        $hookmanager->initHooks(['formmail']);
        ...
        if (!empty($reshook)) {
            ...
            if (count($modelmail_array) > 0) {
                ...
                !empty($this->param['models']) && in_array($this->param['models'], [
            ...
            foreach ($this->substit as $key => $val) {
                ...
                if (in_array($key, ['__NEWREF__', '__REFCLIENT__', '__REFSUPPLIER__', '__SUPPLIER_ORDER_DATE_DELIVERY__', '__SUPPLIER_ORDER_DELAY_DELIVERY__'])) {
                    continue;
                }
            ...
            if (!empty($this->withfrom)) {
                if (!empty($this->withfromreadonly)) {
                    ...
                    if (
                        !($this->fromtype === 'user' && $this->fromid > 0)
                        ...
                        $liste['user'] = ['label' => $s, 'data-html' => $s];
                        ...
                        if (getDolGlobalString('MAIN_INFO_SOCIETE_MAIL')) {
                            ...
                            $liste['company'] = ['label' => $s, 'data-html' => $s];
                        }
                        ...
                        if (!empty($arraydefaultmessage->email_from)) {
                            ...
                            $liste['from_template_' . GETPOST('modelmailselected')] = ['label' => $templatemailfrom, 'data-html' => $templatemailfrom];
                        }
                        ...
                        if (!empty($this->fromalsorobot)) {
                            if (getDolGlobalString('MAIN_MAIL_EMAIL_FROM') && getDolGlobalString('MAIN_MAIL_EMAIL_FROM') != getDolGlobalString('MAIN_INFO_SOCIETE_MAIL')) {
                                ...
                                $liste['main_from'] = ['label' => $s, 'data-html' => $s];
                            }
                        }
                        ...
                        foreach ($listaliases as $typealias => $listalias) {
                            ...
                            foreach ($listaliasarray as $listaliasval) {
                                ...
                                if ($listaliasval) {
                                    ...
                                    $liste[$typealias . '_' . $posalias] = ['label' => $listaliasval, 'data-html' => $listaliasval];
                                }
                            }
                        }
                        ...
                        foreach ($liste as $key => $val) {
                            if (!empty($liste[$key]['data-html'])) {
                                $liste[$key]['data-html'] = str_replace(['&lt;', '<', '&gt;', '>'], ['__LTCHAR__', '__LTCHAR__', '__GTCHAR__', '__GTCHAR__'], $liste[$key]['data-html']);
                                $liste[$key]['data-html'] = str_replace(['__LTCHAR__', '__GTCHAR__'], ['<span class="opacitymedium">(', ')</span>'], $liste[$key]['data-html']);
                            }
                        }
                ...
                if (is_numeric($this->withfile)) {
                    ...
                    if (count($listofpaths)) {
                        foreach ($listofpaths as $key => $val) {
                            ...
                            $out .= ' ' . $formfile->showPreview([], $formfile_params[2], $formfile_params[4]);
     ...
     *                                      0 for first found whatever is position (priority order depends on lang
     *                                      provided or not) or
        ...
        if ($this->withtoreadonly) {
            ...
            if (!empty($this->withto) && is_array($this->withto)) {
                ...
                foreach ($tmparray as $key => $val) {
                    ...
                    $tmparray[$key]['label'] = str_replace(['<', '>'], ['(', ')'], $tmparray[$key]['label']);
                    // multiselect array convert html entities into options tags, even if we don't want this, so we encode them a second time
                    $tmparray[$key]['label'] = dol_htmlentities($tmparray[$key]['label'], ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8', true);

                    $tmparray[$key]['labelhtml'] = $label;
                    $tmparray[$key]['labelhtml'] = str_replace(['&lt;', '<', '&gt;', '>'], ['__LTCHAR__', '__LTCHAR__', '__GTCHAR__', '__GTCHAR__'], $tmparray[$key]['labelhtml']);
                    $tmparray[$key]['labelhtml'] = str_replace(['__LTCHAR__', '__GTCHAR__'], ['<span class="opacitymedium">(', ')</span>'], $tmparray[$key]['labelhtml']);
                }
    ...
    /**
     * Set ->substit (and ->substit_line) array from object. This is call when suggesting the email template into forms
